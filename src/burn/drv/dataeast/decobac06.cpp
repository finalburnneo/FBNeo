...
void bac06_draw_layer(UINT8* vram, UINT16 control[2][4], UINT8* rsram, UINT8* csram, UINT8* gfx8, INT32 col8,
                      INT32 mask8, UINT8* gfx16, INT32 col16, INT32 mask16, INT32 widetype, INT32 opaque)
{
	constexpr INT32 dims[4][3][2] = {
		{{128, 32}, {64, 64}, {32, 128}},
		{{64, 16}, {32, 32}, {16, 64}},
		{{128, 16}, {64, 32}, {32, 64}},
		{{256, 16}, {128, 32}, {64, 64}}
	...
	auto ram = (UINT16*)vram;

	INT32 scrollx = (control[1][0]) & ((wide * tsize) - 1);
	INT32 scrolly = (control[1][1] + bac06_yadjust) & ((high * tsize) - 1);
	INT32 rsenable = (control[0][0] & 0x04) && (rsram != nullptr);
	INT32 csenable = (control[0][0] & 0x08) && (csram != nullptr);

	if (rsenable || csenable)
	{
		auto scrx = (UINT16*)rsram;
		auto scry = (UINT16*)csram;
		...
		for (INT32 row = 0; row < high; row++)
		{
			...
			for (INT32 col = 0; col < wide; col++)
			{
				...
				else
				{
					if (size)
					{
						Render16x16Tile_Mask_Clip(pTransDraw, code & mask16, sx, sy, color, bac06_depth, 0, col16,
						                          gfx16);
					}
					else
					{
