...
static UINT8 cart_readDummy(     Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeDummy(    Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readLorom(     Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeLorom(    Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readExLorom(   Cart* cart, UINT8 bank, UINT16 adr);
static UINT8 cart_readLoromSeta( Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeLoromSeta(Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readLoromDSP(  Cart* cart, UINT8 bank, UINT16 adr);
static void	 cart_writeLoromDSP( Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readHirom(     Cart* cart, UINT8 bank, UINT16 adr);
static UINT8 cart_readExHirom(   Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeHirom(    Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readHiromDSP(  Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeHiromDSP( Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readCX4(       Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeCX4(      Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readLoromSA1(  Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeLoromSA1( Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readLoromOBC1( Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeLoromOBC1(Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readLoromSDD1( Cart* cart, UINT8 bank, UINT16 adr);
static void  cart_writeLoromSDD1(Cart* cart, UINT8 bank, UINT16 adr, UINT8 val);
static UINT8 cart_readSuperFX(   Cart* cart, UINT8 bank, UINT16 adr);
...
void upd_run() {
	...
	INT32 to_run = (int)((UINT64)upd_sync_to - upd_cycles);
	if (to_run > 0) {
...
void upd_handleState(StateHandler* sh, INT32 cart_type)
{
	void *upd_data = NULL;
	INT32 upd_data_size = upd96050ExportRegs(&upd_data);

	sh_handleByteArray(sh, (UINT8*)upd_data, upd_data_size);							// from upd7725 cpu core
...
{
	const INT32 bios_sizes[2] = { 0x2000, 0x10000 };
	const INT32 data_sizes[2] = { 0x0800, 0x01000 };

	bprintf(0, _T("bios_reform:  seta? %x\n"), is_seta);

	INT32 bios_size = bios_sizes[is_seta];
	INT32 data_size = data_sizes[is_seta];
...
void cart_load(Cart* cart, INT32 type, UINT8* rom, INT32 romSize, UINT8* biosrom, INT32 biosromSize, INT32 ramSize, bool ramFill, bool hasBattery)
{
	cart->type = type;
	cart_mapRwHandlers(cart);
	cart_mapRun(cart);
	...
	if (biosromSize > 0 && (type == CART_LOROMDSP || type == CART_HIROMDSP || type == CART_LOROMSETA)) {
		...
		upd_ram = (UINT8*)BurnMalloc(0x2000); // 0x200 dsp, 0x800 seta
	}
...
bool cart_handleBattery(Cart* cart, bool save, UINT8* data, INT32* size)
{
...
static bool cart_msuPort(UINT8 bank, UINT16 adr)
{
	UINT8 b = bank & 0x7f;                       // fold $80-$ff onto $00-$7f
...
static UINT8 cart_readSFEX(Cart* cart, UINT8 bank, UINT16 adr)
{
	UINT8 value = cart_readLorom(cart, bank, adr);

	if (bank != 0x80 || adr < 0x8100 || adr > 0x81f0) {
		return value;
	}
...
static void cart_writeSFEX(Cart* cart, UINT8 bank, UINT16 adr, UINT8 val)
{
	if (bank != 0x80 || adr < 0x8000 || adr > 0x87ff) {
		cart_writeLorom(cart, bank, adr, val);
		return;
	}
