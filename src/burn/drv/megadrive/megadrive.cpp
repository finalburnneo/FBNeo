...
static INT32 SekCyclesDoneFrameF() { return ((SekCycleCnt - SekCycleCntDELTA) - m68k_ICount); }
// for SN psg stream buffer
...
static UINT8 *AllMem = nullptr, *MemEnd = nullptr;
static UINT8 *RamStart, *RamEnd;

static UINT8* RomMain;
static UINT8* OriginalRom = nullptr;
...
static void MegadriveCheckHardware()
{
	...
	if (MegadriveDIP[0] & 0x01)
	{
		...
		bprintf(PRINT_IMPORTANT, _T("Autodetecting Cartridge (Hardware Code: %02x%02x%02x%02x):\n"), RomMain[0x1f0],
		        RomMain[0x1f1], RomMain[0x1f2], RomMain[0x1f3]);
		Hardware = 0x80;
...
inline static void CalcCol(INT32 index, UINT16 nColour)
{
	...
	g >>= 1;
	b >>= 1;
	MegadriveCurPal[index + 0x40] = MegadriveCurPal[index + 0xc0] = BurnHighCol(
		color_ramp[r], color_ramp[g], color_ramp[b], 0);
...
static INT32 MemIndex()
{
	...
	Next = AllMem;
	RomMain = Next;
	Next += MAX_CARTRIDGE_SIZE; // 68000 ROM, Max enough
	...
	RamSVid = (UINT16*)Next;
	Next += 0x000040 * sizeof(UINT16); // VSRam
	RamVid = (UINT16*)Next;
	Next += 0x010000 * sizeof(UINT16); // Video Ram
	RamVReg = (struct PicoVideo*)Next;
	Next += sizeof(struct PicoVideo);

	JoyPad = (struct MegadriveJoyPad*)Next;
	Next += sizeof(struct MegadriveJoyPad);

	RamEnd = Next;

	SRam = Next;
	Next += MAX_SRAM_SIZE; // SRam
	// Keep RamMisc out of the Ram section to keep from getting cleared on reset.
	RamMisc = (struct PicoMisc*)Next;
	Next += sizeof(struct PicoMisc);

	MegadriveCurPal = (UINT32*)Next;
	Next += 0x000040 * sizeof(UINT32) * 4;

	HighColFull = Next;
	Next += (8 + 320 + 8) * ((240 + 1) * 2);

	LineBuf = (UINT16*)Next;
	Next += 320 * (240 * 2) * sizeof(UINT16); // palete-processed line-buffer (dink / for sonic mode)

	HighCacheA = (INT32*)Next;
	Next += (41 + 1) * sizeof(INT32); // caches for high layers
	HighCacheB = (INT32*)Next;
	Next += (41 + 1) * sizeof(INT32);
	HighCacheS = (INT32*)Next;
	Next += (80 + 1) * sizeof(INT32); // and sprites
	HighPreSpr = (INT32*)Next;
	Next += (80 * 2 + 1) * sizeof(INT32); // slightly preprocessed sprites
	HighSprZ = (INT8*)Next;
	Next += (320 + 8 + 8); // Z-buffer for accurate sprites and shadow/hilight mode
...
static void __fastcall Megadrive68K_Z80WriteByte(UINT32 address, UINT8 data)
{
	...
	{
		// z80 ram: 0000 - 1fff, 2000 - 3fff(mirror)
...
static UINT8 __fastcall MegadriveReadByte(UINT32 address)
{
	...
	case 0xa11101:
		{
			// lsb of busreq status is literally random
			return d;
		}
	...
	case 0xA11100:
		{
			if (byteValue & 1)
			{
				if (Z80HasBus == 1)
				{
					...
					// synch before disconnecting.  fixes hang in Golden Axe III (z80run)
					Z80HasBus = 0;
				}
			}
			else
...
static constexpr INT32 dma_bsycles[] = {
...
static INT32 DMABURN()
{
	// add cycles to the 68k cpu
	if (dma_xfers)
	{
		return CheckDMA();
	}
...
static void DmaSlow(INT32 len)
{
	...
	case 1: // vram
		...
		for (; len; len--)
		{
			...
			if (a & 1) d = (d << 8) | (d >> 8);
			r[a >> 1] = static_cast<UINT16>(d); // will drop the upper bits
			// AutoIncrement
			a = static_cast<UINT16>(a + inc);
			// didn't src overlap?
	...
	RamVReg->addr = static_cast<UINT16>(a);
}

static void DmaCopy(INT32 len)
{
	auto vr = (UINT8*)RamVid;
	UINT8* vrs;
...
static void DmaFill(INT32 data)
{
	...
	auto vr = (UINT8*)RamVid;
	UINT8 high = static_cast<UINT8>(data >> 8);
	UINT16 a = RamVReg->addr;
	UINT8 inc = RamVReg->reg[0xf];
...
static void CommandChange()
{
	...
	RamVReg->type = static_cast<UINT8>(((cmd >> 2) & 0xc) | (cmd >> 30));
	if (RamVReg->type == 1)
	{
		// vram
	...
	addr = (cmd >> 16) & 0x3fff;
	addr |= (cmd << 14) & 0xc000;
	RamVReg->addr = static_cast<UINT16>(addr);
	RamVReg->addr_u = static_cast<UINT8>((cmd >> 2) & 1);
...
static UINT16 __fastcall MegadriveVideoReadWord(UINT32 sekAddress)
{
	...
	case 0x00: // data
		switch (RamVReg->type)
		{
		case 0: res = BURN_ENDIAN_SWAP_INT16(RamVid [(RamVReg->addr >> 1) & 0x7fff]);
			break;
		case 4: res = BURN_ENDIAN_SWAP_INT16(RamSVid[(RamVReg->addr >> 1) & 0x003f]);
			break;
		case 8: res = BURN_ENDIAN_SWAP_INT16(RamPal [(RamVReg->addr >> 1) & 0x003f]);
			break;
		}
	...
	case 0x04: // command
		...
		else
		{
			if ((wordValue & 0xc000) == 0x8000)
			{
				...
				if (num == 11)
				{
					constexpr UINT8 h_msks[4] = {0x00, 0x07, 0xf8, 0xff};
	...
	default:
		{
			...
			UINT8 retval = 0xf & ~(JoyPad->pad[padnum] >> (JoyPad->teamplayer[port].table[JoyPad->teamplayer[port].
				counter - 8] & 0xf));
		...
		case 3:
			{
				...
				if (offset < 3)
				{
					if (TeamPlayerMode)
					{
						...
						{
						case 1: data = (offset == 1) ? teamplayer_read() : 0x7f;
							break; // teamplayer port 1, nothing port 2
						case 2: data = (offset == 2) ? teamplayer_read() : PadRead(0);
							break; // teamplayer port2, gamepad port 1
						}
					}
					if (FourWayPlayMode)
					{
...
static void __fastcall MegadriveIOWriteByte(UINT32 sekAddress, UINT8 byteValue)
{
	...
	else if (TeamPlayerMode)
	{
		...
		case 1:
			if (TeamPlayerMode == 2)
			{
				// teamplayer port 2, gamepad port 1
				PadWrite(offset - 1, byteValue, &RamIO[offset]);
			}
			else
			{
...
{
	return static_cast<INT64>(SekCyclesDoneFrame()) * nSoundRate / TOTAL_68K_CYCLES;
}

inline static INT32 MegadriveSynchroniseStreamPAL(INT32 nSoundRate)
{
	return static_cast<INT64>(SekCyclesDoneFrame()) * nSoundRate / TOTAL_68K_CYCLES_PAL;
}
...
static INT32 MegadriveResetDo()
{
	...
	{
		dma_xfers = BurnRandom() & 0x7fff;
		// random start cycle, so Bonkers has a different boot-up logo each run and possibly affects other games as well.
	}
	else
	{
		dma_xfers = 0;
		// the above is bad for "input recording from power on", so we leave it off for everything except bonkers.
	}
...
static INT32 __fastcall MegadriveIrqCallback(INT32 irq)
{
	switch (irq)
	{
	case 4: RamVReg->pending_ints = 0x00;
		break;
	case 6: RamVReg->pending_ints &= ~0x20;
		break;
	}
	SekSetIRQLine(0, CPU_IRQSTATUS_NONE);
...
static void __fastcall MegadriveZ80ProgWrite(UINT16 a, UINT8 d)
{
	...
	if ((a & 0xff00) == 0x6000)
	{
		// z80 68k-ram accessor bank shift register
	...
	if (bLoad)
	{
		...
		for (i = 0; i < RomNum; i++)
		{
			...
			case SEGA_MD_ROM_LOAD_NORMAL:
				{
					nRet = BurnLoadRom(RomMain + Offset, i, 1);
					if (nRet) return 1;
					break;
				}
			...
			case SEGA_MD_ROM_LOAD16_WORD_SWAP:
				{
					...
					BurnByteswap(RomMain + Offset,
					             (bDoIpsPatch && ri.nLen < nIpsMemExpLen[LOAD_ROM])
						             ? nIpsMemExpLen[LOAD_ROM]
						             : ri.nLen);
					break;
				}
			...
			case SEGA_MD_ROM_LOAD16_BYTE:
				{
					nRet = BurnLoadRom(RomMain + Offset, i, 2);
					if (nRet) return 1;
					break;
				}

			case SEGA_MD_ROM_LOAD_NORMAL_CONTINUE_020000_080000:
				{
					// ghouls[1] (Ghouls 'n Ghosts)
					nRet = BurnLoadRom(RomMain + Offset, i, 1);
					if (nRet) return 1;
...
static UINT16 __fastcall LK3ReadWord(UINT32 address)
{
	if (address < 0x100000)
	{
		// banked rom access (15 - 1 because indexing UINT16 array instead of byte)
		auto Rom = (UINT16*)RomMain;
...
static void __fastcall LK3WriteByte(UINT32 address, UINT8 data)
{
	...
	if (address >= 0x600000 && address <= 0x6fffff)
	{
		...
		{
		case 0: RamMisc->L3Reg[2] = RamMisc->L3Reg[0] << 1;
			break;
		case 1: RamMisc->L3Reg[2] = RamMisc->L3Reg[0] >> 1;
			break;
		case 2: RamMisc->L3Reg[2] = (RamMisc->L3Reg[0] >> 4) | ((RamMisc->L3Reg[0] & 0x0f) << 4);
			break;
		case 3:
		default: RamMisc->L3Reg[2] = BITSWAP08(RamMisc->L3Reg[0], 0, 1, 2, 3, 4, 5, 6, 7);
			break;
		}
	}
	if (address >= 0x700000)
...
static UINT8 __fastcall Chinfi3ProtReadByte(UINT32 /*sekAddress*/)
{
	...
	if (SekGetPC(0) == 0x1c24) // background gfx etc.
	{
		retDat = SekDbgGetRegister(SEK_REG_D3) & 0xff;
		//		retDat <<= 8;
		return retDat;
	}
	if (SekGetPC(0) == 0x10c4a) // unknown
	{
		return BurnRandom() & 0xff; //space->machine().rand();
	}
	if (SekGetPC(0) == 0x10c50) // unknown
	{
		return BurnRandom() & 0xff; //space->machine().rand();
	}
	if (SekGetPC(0) == 0x10c52) // relates to the game speed..
	{
		retDat = SekDbgGetRegister(SEK_REG_D4) & 0xff;
		//		retDat <<= 8;
		return retDat;
	}
	if (SekGetPC(0) == 0x061ae)
	{
		retDat = SekDbgGetRegister(SEK_REG_D3) & 0xff;
		//		retDat <<= 8;
		return retDat;
	}
	if (SekGetPC(0) == 0x061b0)
	{
		retDat = SekDbgGetRegister(SEK_REG_D3) & 0xff;
		//		retDat <<= 8;
		return retDat;
	...
	case 0x400000:
		{
			...
			memcpy(RomMain, RomMain + (RamMisc->RealtecBankAddr * 0x20000) + 0x400000,
			       RamMisc->RealtecBankSize * 0x20000);
			memcpy(RomMain + RamMisc->RealtecBankSize * 0x20000,
			       RomMain + (RamMisc->RealtecBankAddr * 0x20000) + 0x400000, RamMisc->RealtecBankSize * 0x20000);
			return;
		}
...
{
	if (((BurnDrvGetHardwareCode() & 0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_CM_JCART) || ((BurnDrvGetHardwareCode() &
	...
	if ((BurnDrvGetHardwareCode() & 0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_SSF2)
	{
		OriginalRom = BurnMalloc(MAX_CARTRIDGE_SIZE);
	...
	if ((BurnDrvGetHardwareCode() & 0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_REDCL_EN)
	{
		if (RomSize == 0x200005)
		{
			...
			OriginalRom = BurnMalloc(0x200005);
	...
	{
		OriginalRom = BurnMalloc(RomSize);
	...
	{
		OriginalRom = BurnMalloc(RomSize * 2); // add a little buffer on the end so memcpy @ the last bank doesn't crash
		memcpy(OriginalRom, RomMain, RomSize);
...
static void __fastcall MegadriveSRAMToggleWriteByte(UINT32 sekAddress, UINT8 byteValue)
{
	if (sekAddress == 0xa130f1)
	{
		...
		bprintf(0, _T("SRam Status: %S%S\n"), RamMisc->SRamActive ? "Active " : "Disabled ",
		        RamMisc->SRamReadOnly ? "ReadOnly" : "Read/Write");
	}
}
...
static void MegadriveSetupSRAM()
{
	...
	RamMisc->SRamReg = 0;
	MegadriveBackupRam = nullptr;

	if ((BurnDrvGetHardwareCode() & HARDWARE_SEGA_MEGADRIVE_SRAM_00400) || (BurnDrvGetHardwareCode() &
		HARDWARE_SEGA_MEGADRIVE_SRAM_00800) || (BurnDrvGetHardwareCode() & HARDWARE_SEGA_MEGADRIVE_SRAM_01000) || (
		BurnDrvGetHardwareCode() & HARDWARE_SEGA_MEGADRIVE_SRAM_04000) || (BurnDrvGetHardwareCode() &
	...
	if (((BurnDrvGetHardwareCode() & 0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_NBA_JAM_TE) || ((BurnDrvGetHardwareCode() &
		0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_NFL_QB_96) || ((BurnDrvGetHardwareCode() & 0xff) ==
		HARDWARE_SEGA_MEGADRIVE_PCB_C_SLAM))
	{
		RamMisc->SRamHasSerialEEPROM = 1;
	...
	if (((BurnDrvGetHardwareCode() & 0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_CODE_MASTERS) || ((BurnDrvGetHardwareCode() &
		0xff) == HARDWARE_SEGA_MEGADRIVE_PCB_CM_JCART_SEPROM))
	{
		RamMisc->SRamHasSerialEEPROM = 1;
...
INT32 MegadriveInit()
{
	...
	MegadriveLoadRoms(false);
	if (MegadriveLoadRoms(true)) return 1;
	...
	pBurnDrvPalette = MegadriveCurPal;
	...
	if (strstr(BurnDrvGetTextA(DRV_NAME), "forgottn") || strstr(BurnDrvGetTextA(DRV_NAME), "ustrike") || strstr(
		BurnDrvGetTextA(DRV_NAME), "kotm"))
	{
...
static INT32 TileNorm(INT32 sx, INT32 addr, INT32 pal)
{
	...
	{
		t = pack & 0x0000f000;
		if (t) pd[0] = static_cast<UINT8>(pal | (t >> 12));
		t = pack & 0x00000f00;
		if (t) pd[1] = static_cast<UINT8>(pal | (t >> 8));
		t = pack & 0x000000f0;
		if (t) pd[2] = static_cast<UINT8>(pal | (t >> 4));
		t = pack & 0x0000000f;
		if (t) pd[3] = static_cast<UINT8>(pal | (t));
		t = pack & 0xf0000000;
		if (t) pd[4] = static_cast<UINT8>(pal | (t >> 28));
		t = pack & 0x0f000000;
		if (t) pd[5] = static_cast<UINT8>(pal | (t >> 24));
		t = pack & 0x00f00000;
		if (t) pd[6] = static_cast<UINT8>(pal | (t >> 20));
		t = pack & 0x000f0000;
		if (t) pd[7] = static_cast<UINT8>(pal | (t >> 16));
		return 0;
	}
	return 1; // Tile blank
}
...
static INT32 TileFlip(INT32 sx, INT32 addr, INT32 pal)
{
	...
	{
		t = pack & 0x000f0000;
		if (t) pd[0] = static_cast<UINT8>(pal | (t >> 16));
		t = pack & 0x00f00000;
		if (t) pd[1] = static_cast<UINT8>(pal | (t >> 20));
		t = pack & 0x0f000000;
		if (t) pd[2] = static_cast<UINT8>(pal | (t >> 24));
		t = pack & 0xf0000000;
		if (t) pd[3] = static_cast<UINT8>(pal | (t >> 28));
		t = pack & 0x0000000f;
		if (t) pd[4] = static_cast<UINT8>(pal | (t));
		t = pack & 0x000000f0;
		if (t) pd[5] = static_cast<UINT8>(pal | (t >> 4));
		t = pack & 0x00000f00;
		if (t) pd[6] = static_cast<UINT8>(pal | (t >> 8));
		t = pack & 0x0000f000;
		if (t) pd[7] = static_cast<UINT8>(pal | (t >> 12));
		return 0;
	}
	return 1; // Tile blank
}
...
static INT32 TileNormRlim(INT32 sx, INT32 addr, INT32 pal, INT32 rlim)
{
	...
	if (pack)
	{
		switch (rlim)
		{
		case 7: t = pack & 0x00f00000;
			if (t) pd[6] = static_cast<UINT8>(pal | (t >> 20)); // no breaks!
		case 6: t = pack & 0x0f000000;
			if (t) pd[5] = static_cast<UINT8>(pal | (t >> 24));
		case 5: t = pack & 0xf0000000;
			if (t) pd[4] = static_cast<UINT8>(pal | (t >> 28));
		case 4: t = pack & 0x0000000f;
			if (t) pd[3] = static_cast<UINT8>(pal | (t));
		case 3: t = pack & 0x000000f0;
			if (t) pd[2] = static_cast<UINT8>(pal | (t >> 4));
		case 2: t = pack & 0x00000f00;
			if (t) pd[1] = static_cast<UINT8>(pal | (t >> 8));
		case 1: t = pack & 0x0000f000;
			if (t) pd[0] = static_cast<UINT8>(pal | (t >> 12));
		}
		return 0;
	}
...
static INT32 TileFlipRlim(INT32 sx, INT32 addr, INT32 pal, INT32 rlim)
{
	...
	if (pack)
	{
		switch (rlim)
		{
		case 7: t = pack & 0x00000f00;
			if (t) pd[6] = static_cast<UINT8>(pal | (t >> 8)); // no breaks!
		case 6: t = pack & 0x000000f0;
			if (t) pd[5] = static_cast<UINT8>(pal | (t >> 4));
		case 5: t = pack & 0x0000000f;
			if (t) pd[4] = static_cast<UINT8>(pal | (t));
		case 4: t = pack & 0xf0000000;
			if (t) pd[3] = static_cast<UINT8>(pal | (t >> 28));
		case 3: t = pack & 0x0f000000;
			if (t) pd[2] = static_cast<UINT8>(pal | (t >> 24));
		case 2: t = pack & 0x00f00000;
			if (t) pd[1] = static_cast<UINT8>(pal | (t >> 20));
		case 1: t = pack & 0x000f0000;
			if (t) pd[0] = static_cast<UINT8>(pal | (t >> 16));
		}
		return 0;
	}
...
static INT32 TileNormSH(INT32 sx, INT32 addr, INT32 pal)
{
	...
	if (pack)
	{
		...
		t = (pack & 0x0000000f);
		sh_pix(3);
		t = (pack & 0xf0000000) >> 28;
		sh_pix(4);
		t = (pack & 0x0f000000) >> 24;
		sh_pix(5);
		t = (pack & 0x00f00000) >> 20;
		sh_pix(6);
		t = (pack & 0x000f0000) >> 16;
		sh_pix(7);
		return 0;
	}
	return 1; // Tile blank
...
static INT32 TileNormZ(INT32 sx, INT32 addr, INT32 pal, INT32 zval)
{
	...
	if (pack)
	{
		...
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[0] = static_cast<UINT8>(pal | (t >> 12));
				zb[0] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x00000f00;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[1] = static_cast<UINT8>(pal | (t >> 8));
				zb[1] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x000000f0;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[2] = static_cast<UINT8>(pal | (t >> 4));
				zb[2] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x0000000f;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[3] = static_cast<UINT8>(pal | (t));
				zb[3] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0xf0000000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[4] = static_cast<UINT8>(pal | (t >> 28));
				zb[4] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x0f000000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[5] = static_cast<UINT8>(pal | (t >> 24));
				zb[5] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x00f00000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[6] = static_cast<UINT8>(pal | (t >> 20));
				zb[6] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x000f0000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[7] = static_cast<UINT8>(pal | (t >> 16));
				zb[7] = static_cast<INT8>(zval);
			}
		}
...
static INT32 TileFlipZ(INT32 sx, INT32 addr, INT32 pal, INT32 zval)
{
	...
	if (pack)
	{
		...
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[0] = static_cast<UINT8>(pal | (t >> 16));
				zb[0] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x00f00000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[1] = static_cast<UINT8>(pal | (t >> 20));
				zb[1] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x0f000000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[2] = static_cast<UINT8>(pal | (t >> 24));
				zb[2] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0xf0000000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[3] = static_cast<UINT8>(pal | (t >> 28));
				zb[3] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x0000000f;
		...
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[5] = static_cast<UINT8>(pal | (t >> 4));
				zb[5] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x00000f00;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[6] = static_cast<UINT8>(pal | (t >> 8));
				zb[6] = static_cast<INT8>(zval);
			}
		}
		t = pack & 0x0000f000;
		if (t)
		{
			...
			if (zval > zb_s)
			{
				pd[7] = static_cast<UINT8>(pal | (t >> 12));
				zb[7] = static_cast<INT8>(zval);
			}
		}
		if (collision) RamVReg->status |= 0x20;
...
{
	constexpr INT8 shift[4] = {5, 6, 5, 7}; // 32,64 or 128 sized tilemaps (2 is invalid)
	struct TileStrip ts;
...
static void DrawTilesFromCache(INT32* hc, INT32 sh, INT32 rlim)
{
	...
	while ((code = *hc++))
	{
		if (!sh && static_cast<INT16>(code) == blank) continue;

		// Get tile address/2:
		addr = (code & 0x7ff) << 4;
		addr += static_cast<UINT32>(code) >> 25; // y offset into tile
		dx = (code >> 16) & 0x1ff;
		if (sh)
		{
			UINT8* zb = HighCol + dx;
		...
		if (zero) blank = static_cast<INT16>(code);
	}
}
...
static void DrawSprite(INT32* sprite, INT32** hc, INT32 sh)
{
	...
	if (code & 0x8000)
	{
		// high priority - cache it
		*(*hc)++ = (tile << 16) | ((code & 0x0800) << 5) | ((sx << 6) & 0x0000ffc0) | ((code >> 9) & 0x30) | ((sprite[0]
			>> 16) & 0xf);
	}
	else
	{
...
static void DrawSpriteZ(INT32 pack, INT32 pack2, INT32 shpri, INT32 sprio)
{
	...
	{
	default:
	case 0: fTileFunc = TileNormZ;
		break;
	case 1: fTileFunc = TileNormZSH;
		break;
	case 2: fTileFunc = TileFlipZ;
		break;
	case 3: fTileFunc = TileFlipZSH;
		break;
	}
...
static void DrawSpriteInterlace(UINT32* sprite)
{
	...
	height &= 3;
	width++;
	height++; // Width and height in tiles
...
static void DrawSpritesFromCache(INT32* hc, INT32 sh)
{
	...
	while ((code = *hc++))
	{
		...
		delta &= 3;
		width++;
		delta++; // Width and height in tiles
		if (code & 0x10000) delta = -delta; // Flip X
		delta <<= 4;
		tile = (static_cast<UINT32>(code) >> 17) << 1;
...
static void PrepareSprites(INT32 full)
{
	...
	else
	{
		for (; u < 80; u++)
		{
			...
			*pd++ = (width << 28) | (height << 24) | skip | (hv << 16) | static_cast<UINT16>(sy);
			*pd++ = (sx << 16) | static_cast<UINT16>(code2);
...
static void DrawAllSprites(INT32* hcache, INT32 maxwidth, INT32 prio, INT32 sh)
{
	...
	for (i = u = n = 0; (pack = *ps) && n < 20; ps += 2, u++)
	{
		...
		if (rs & 4)
		{
			...
			DrawSpriteZ(pack, pack2, sh | (prio << 1), static_cast<INT8>(0x1f - n));
			continue;
		}
...
static void BackFill(INT32 reg7, INT32 sh)
{
	...
	auto pd = (UINT32*)(HighCol + 8);
	auto end = (UINT32*)(HighCol + 8 + 320);
...
static INT32 DrawDisplay(INT32 sh)
{
	...
	if (!hvwind)
	{
		// we might have a vertical window here
	...
	{
		{
			...
			UINT8* pSrc = HighColFull + (Scanline + offset + ((interlacemode2 & RamVReg->field) * 240)) * (8 + 320 + 8)
				+ 8;
...
constexpr INT32 v_res[2] = {224, 240};

static INT32 res_check()
{
	if (pBurnDraw == nullptr) return 1; // Don't try to change modes if display not active.

	INT32 v_idx = (RamVReg->reg[1] & 8) >> 3;

	if ((RamVReg->reg[12] & (4 | 2)) == (4 | 2))
	{
		// interlace mode 2
	...
	else if ((MegadriveDIP[1] & 3) == 3 && (~RamVReg->reg[12] & 1))
	{
		BurnDrvGetVisibleSize(&screen_width, &screen_height);
...
INT32 MegadriveDraw()
{
	...
	else if ((RamVReg->reg[12] & 1) || ((MegadriveDIP[1] & 0x03) == 0 || (MegadriveDIP[1] & 0x03) == 3))
	{
		// Normal / "Screen Resize"
		for (INT32 j = 0; j < screen_height; j++)
		{
...
INT32 MegadriveFrame()
{
	...
	for (INT32 y = 0; y < lines; y++)
	{
		if (y > lines_vis && nBurnCPUSpeedAdjust > 0x100)
			SekRunM68k(
				static_cast<INT32>(static_cast<INT64>(CYCLES_M68K_LINE) * (nBurnCPUSpeedAdjust - 0x100) / 0x0100));
		...
		// H-Interrupts:
		if ((y <= lines_vis) && (--hint < 0))
		{
			// y <= lines_vis: Comix Zone, Golden Axe
