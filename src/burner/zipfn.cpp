...
{
	if (nFileType == ZIPFN_FILETYPE_ZIP && Zip == nullptr) return 1;
	if (pList == nullptr) return 1;

#ifdef INCLUDE_7Z_SUPPORT
	if (nFileType == ZIPFN_FILETYPE_7ZIP && _7ZipFile == nullptr) return 1;
#endif
	...
	if (nFileType == ZIPFN_FILETYPE_ZIP)
	{
		...
		auto List = static_cast<struct ZipEntry*>(malloc(nListLen * sizeof(struct ZipEntry)));
		if (List == nullptr)
		{
			unzClose(Zip);
			return 1;
		     ...
		     nCurrFile++, nNextRet = unzGoToNextFile(Zip))
		{
			...
			nRet = unzGetCurrentFileInfo(Zip, &FileInfo, nullptr, 0, nullptr, 0, nullptr, 0);
			if (nRet != UNZ_OK) continue;

			// Allocate space for the filename
			auto szName = static_cast<char*>(malloc(FileInfo.size_filename + 1));
			if (szName == nullptr) continue;

			nRet = unzGetCurrentFileInfo(Zip, &FileInfo, szName, FileInfo.size_filename + 1, nullptr, 0, nullptr, 0);
			if (nRet != UNZ_OK) continue;
		...
		*pList = List;
		if (pnListCount != nullptr) *pnListCount = nListLen;
	...
	if (nFileType == ZIPFN_FILETYPE_7ZIP)
	{
		...
		{
			size_t len = SzArEx_GetFileNameUtf16(&_7ZipFile->db, i, nullptr);
			...
			if (len > tempSize)
			{
				...
				tempSize = len;
				temp = static_cast<UInt16*>(SZipAlloc(nullptr, tempSize * sizeof(temp[0])));
				if (temp == nullptr)
				{
					free(List);
					List = nullptr;
			...
			szFileName = static_cast<char*>(malloc(len * 2 * sizeof(char*)));
			if (szFileName == nullptr) continue;
...
INT32 ZipLoadFile(UINT8* Dest, INT32 nLen, INT32* pnWrote, INT32 nEntry)
{
	...
	if (nFileType == ZIPFN_FILETYPE_ZIP)
	{
		...
		if (nRet >= 0 && pnWrote != nullptr) *pnWrote = nRet;
	...
	if (nFileType == ZIPFN_FILETYPE_7ZIP)
	{
		...
		// Return how many bytes were copied
		if (_7zerr == _7ZERR_NONE && pnWrote != nullptr) *pnWrote = static_cast<INT32>(nWrote);
...
INT32 __cdecl ZipLoadOneFile(char* arcName, const char* fileName, void** Dest, INT32* pnWrote)
{
	...
	if (nFileType == ZIPFN_FILETYPE_ZIP)
	{
		...
		if (fileName != nullptr)
		{
			...
			for (nCurrFile = 0, nNextRet = UNZ_OK;
			     ...
			     nCurrFile++, nNextRet = unzGoToNextFile(Zip))
			{
				nRet = unzGetCurrentFileInfo(Zip, &FileInfo, szName, MAX_PATH, nullptr, 0, nullptr, 0);
				if (nRet != UNZ_OK) continue;
		...
		if (*Dest == nullptr)
		{
			*Dest = static_cast<UINT8*>(malloc(FileInfo.uncompressed_size));
			if (!*Dest)
			{
				unzCloseCurrentFile(Zip);
				ZipClose();
	...
	if (nFileType == ZIPFN_FILETYPE_7ZIP)
	{
		...
		nCurrFile = _7z_search_crc_match(_7ZipFile, 0, fileName, strlen(fileName), false, true);
		if (nCurrFile == -1)
		{
			ZipClose();
			return 1;
		...
		if (*Dest == nullptr)
		{
			*Dest = static_cast<UINT8*>(malloc(size));
			if (!*Dest)
			{
				ZipClose();
				return 1;
		...
		// use zlib crc32 module to calc crc of decompressed data, and check against 7z header
		UINT32 nCalcCrc = crc32(0, static_cast<const Bytef*>(*Dest), nWrote);
		if (nCalcCrc != crc)
		{
			ZipClose();
			if (*Dest) free(*Dest);
