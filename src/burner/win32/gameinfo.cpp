...
static void SetPreview(TCHAR* szPreviewDir, int nAspectFlag)
{
	...
	IMAGE img = {0, 0, 0, 0, nullptr, nullptr, 0};
	...
	if (fp)
	{
		...
		{
			double nAspect = static_cast<double>(img.width) / img.height;
			img_width = static_cast<int>(static_cast<double>(IMG_MAX_HEIGHT) * nAspect);

			if (img_width > IMG_MAX_WIDTH)
			{
				img_width = IMG_MAX_WIDTH;
				img_height = static_cast<int>(static_cast<double>(IMG_MAX_WIDTH) / nAspect);
			}
		}
	...
	if (hNewImage)
	{
		...
		if (bPngImageOrientation == 0)
		{
			...
			SendDlgItemMessage(hDlg, IDC_SCREENSHOT_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);
			ShowWindow(GetDlgItem(hDlg, IDC_SCREENSHOT_V), SW_HIDE);
		}
		else
		{
			SendDlgItemMessage(hDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	...
	{
		SendDlgItemMessage(hDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP,
		                   (LPARAM)LoadBitmap(hAppInst, MAKEINTRESOURCE(BMP_SPLASH)));
		SendDlgItemMessage(hDlg, IDC_SCREENSHOT_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);
		ShowWindow(GetDlgItem(hDlg, IDC_SCREENSHOT_V), SW_HIDE);
	}
}
...
static int GameInfoInit()
{
	...
	while ((pszName = BurnDrvGetText(nGetTextFlags | DRV_NEXTNAME | DRV_FULLNAME)) != nullptr)
	{
	...
	_stprintf(szText, _T("%s") _T(SEPERATOR_1) _T("%s"), FBALoadStringEx(hAppInst, IDS_GAMEINFO_DIALOGTITLE, true),
	          szFullName);
	...
	UINT idsString[17] = {
		IDS_GAMEINFO_ROMINFO, IDS_GAMEINFO_HDD, IDS_GAMEINFO_SAMPLES, IDS_GAMEINFO_HISTORY, IDS_GAMEINFO_INGAME,
		IDS_GAMEINFO_TITLE, IDS_GAMEINFO_SELECT, IDS_GAMEINFO_VERSUS, IDS_GAMEINFO_HOWTO, IDS_GAMEINFO_SCORES,
		IDS_GAMEINFO_BOSSES, IDS_GAMEINFO_GAMEOVER, IDS_GAMEINFO_FLYER, IDS_GAMEINFO_CABINET, IDS_GAMEINFO_MARQUEE,
	...
	for (int i = 0; i < 17; i++)
	{
		...
		SendMessage(hTabControl, TCM_INSERTITEM, static_cast<WPARAM>(i), (LPARAM)&TCI);
	}
	...
	SendDlgItemMessage(hGameInfoDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	SendDlgItemMessage(hGameInfoDlg, IDC_SCREENSHOT_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);

	// Favorite game checkbox
	CheckDlgButton(hGameInfoDlg,IDFAVORITESET,
	               (CheckFavorites(BurnDrvGetTextA(DRV_NAME)) != -1) ? BST_CHECKED : BST_UNCHECKED);
	...
	SendMessage(hInfoControl, WM_SETTEXT, 0, (LPARAM)szFullName);
	...
	else
	{
		...
		{
			_stprintf(szItemText + _tcslen(szItemText), _T("%suses ROMs from %s"), bBracket ? _T(", ") : _T(" ("),
	...
	{
		_stprintf(szItemText + _tcslen(szItemText), _T("%suses samples from %s"), bBracket ? _T(", ") : _T(" ("),
		          BurnDrvGetText(DRV_SAMPLENAME));
		bBracket = true;
	}
	if (bBracket)
	{
	...
	SendMessage(hInfoControl, WM_SETTEXT, 0, (LPARAM)szItemText);
	...
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_SEL_BOOTLEG, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_HACK)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_SEL_HACK, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_HOMEBREW)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_SEL_HOMEBREW, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_DEMO)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_SEL_DEMO, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	TCHAR szPlayersMax[100];
	_stprintf(szPlayersMax, FBALoadStringEx(hAppInst, IDS_NUM_PLAYERS_MAX, true));
	_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_NUM_PLAYERS, true),
	          bUseInfo ? _T(", ") : _T(""), BurnDrvGetMaxPlayers(),
	          (BurnDrvGetMaxPlayers() != 1) ? szPlayersMax : _T(""));
	bUseInfo = true;
	if (BurnDrvGetText(DRV_BOARDROM))
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_BOARD_ROMS_FROM, true),
	...
	_stprintf(szItemText, FBALoadStringEx(hAppInst, IDS_HARDWARE_DESC, true),
	          BurnDrvGetTextA(DRV_MANUFACTURER) ? BurnDrvGetText(DRV_MANUFACTURER) : szUnknown,
	          BurnDrvGetText(DRV_DATE),
	          (((BurnDrvGetHardwareCode() & HARDWARE_SNK_MVS) == HARDWARE_SNK_MVS) && ((BurnDrvGetHardwareCode() &
		          HARDWARE_PUBLIC_MASK)) == HARDWARE_SNK_NEOGEO)
		          ? szCartridge
	...
	{
		_stprintf(szItemText + _tcslen(szItemText), _T("%shigh scores supported"),
		          _tcslen(szItemText) ? _T(", ") : _T(""));
	}
	SendMessage(hInfoControl, WM_SETTEXT, 0, (LPARAM)szItemText);
	...
	{
		// assume max 0x100 roms per game
		int nRet;
		struct BurnRomInfo ri;
		char nLen[10] = "";
		char nCrc[10] = "";
		char* szRomName = nullptr;
		char Type[100] = "";
		char FormatType[100] = "";
		...
		LvItem.iItem = RomPos;
		LvItem.iSubItem = 0;
		LvItem.pszText = ANSIToTCHAR(szRomName, nullptr, 0);
		SendMessage(hList, LVM_INSERTITEM, 0, (LPARAM)&LvItem);

		sprintf(nLen, "%d", ri.nLen);
		LvItem.iSubItem = 1;
		LvItem.pszText = ANSIToTCHAR(nLen, nullptr, 0);
		...
		{
			LvItem.iSubItem = 2;
			LvItem.pszText = ANSIToTCHAR(nCrc, nullptr, 0);
			SendMessage(hList, LVM_SETITEM, 0, (LPARAM)&LvItem);
		}
		...
		LvItem.iSubItem = 3;
		LvItem.pszText = ANSIToTCHAR(FormatType, nullptr, 0);
		SendMessage(hList, LVM_SETITEM, 0, (LPARAM)&LvItem);
	...
	if (BurnDrvGetTextA(DRV_BOARDROM))
	{
		...
		for (int j = 0; j < 0x100; j++)
		{
			...
			LvItem.iItem = RomPos;
			LvItem.iSubItem = 0;
			LvItem.pszText = ANSIToTCHAR(szBoardRomName, nullptr, 0);
			SendMessage(hList, LVM_INSERTITEM, 0, (LPARAM)&LvItem);

			sprintf(nLenBoard, "%d", riBoard.nLen);
			LvItem.iSubItem = 1;
			LvItem.pszText = ANSIToTCHAR(nLenBoard, nullptr, 0);
			SendMessage(hList, LVM_SETITEM, 0, (LPARAM)&LvItem);
			...
			{
				LvItem.iSubItem = 2;
				LvItem.pszText = ANSIToTCHAR(nCrcBoard, nullptr, 0);
				SendMessage(hList, LVM_SETITEM, 0, (LPARAM)&LvItem);
			}
			...
			LvItem.iSubItem = 3;
			LvItem.pszText = ANSIToTCHAR(BoardFormatType, nullptr, 0);
			SendMessage(hList, LVM_SETITEM, 0, (LPARAM)&LvItem);
	...
	LvItem.cchTextMax = 256;
	int SamplePos = 0;
	if (BurnDrvGetTextA(DRV_SAMPLENAME) != nullptr)
	{
		for (int i = 0; i < 0x100; i++)
		{
			// assume max 0x100 samples per game
			int nRet;
			struct BurnSampleInfo si;
			char* szSampleName = nullptr;
			...
			LvItem.iSubItem = 0;
			LvItem.pszText = ANSIToTCHAR(szSampleName, nullptr, 0);
			SendMessage(hList, LVM_INSERTITEM, 0, (LPARAM)&LvItem);
	...
	for (int i = 0; i < 0x100; i++)
	{
		// assume max 0x100 hdds per game
		int nRet;
		struct BurnHDDInfo hddi;
		char nLen[10] = "";
		char nCrc[10] = "";
		char* szHDDName = nullptr;
		...
		LvItem.iItem = HDDPos;
		LvItem.iSubItem = 0;
		LvItem.pszText = ANSIToTCHAR(szHDDName, nullptr, 0);
		SendMessage(hList, LVM_INSERTITEM, 0, (LPARAM)&LvItem);
	...
	sprintf(szFileName, "%shistory.dat", TCHARToANSI(szAppHistoryPath, nullptr, 0));
	...
	TCHAR szBuffer[50000] = _T(
		"{\\rtf1\\ansi{\\fonttbl(\\f0\\fnil\\fcharset0 Verdana;)}{\\colortbl;\\red220\\green0\\blue0;\\red0\\green0\\blue0;}");

	GetHistoryDatHardwareToken(&rom_token[0]);
	strcpy(DRIVER_NAME, BurnDrvGetTextA(DRV_NAME));

	if (strncmp("$info=", rom_token, 6))
	{
		// non-arcade game detected. (token not "$info=" !)
...
static INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Msg == WM_COMMAND)
	{
		...
		if (Id == IDRESCANSET && Notify == BN_CLICKED)
		{
			...
			case 1:
				{
					...
					BzipClose();
					BzipOpen(false); // this time, get the missing roms/error message.
					{
						HWND hScrnWndtmp = hScrnWnd;
						// Crash preventer: Make the gameinfo dialog (this one) the parent modal for the popup message
			...
			if (gameAv[nGiDriverSelected] > 0)
			{
				...
				if (TabPage != 0)
				{
					MessageBox(hGameInfoDlg, FBALoadStringEx(hAppInst, IDS_ERR_LOAD_OK, true),
					           FBALoadStringEx(hAppInst, IDS_ERR_INFORMATION, true), MB_OK);
				}
				if (TabPage == 0)
				{
	...
	{
		if ((HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_LABELCOMMENT) || (HWND)lParam ==
			GetDlgItem(hGameInfoDlg, IDC_LABELROMNAME) || (HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_LABELROMINFO) ||
			(HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_LABELSYSTEM) || (HWND)lParam ==
			GetDlgItem(hGameInfoDlg, IDC_LABELNOTES) || (HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_LABELGENRE) || (
				HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_TEXTCOMMENT) || (HWND)lParam ==
			GetDlgItem(hGameInfoDlg, IDC_TEXTROMNAME) || (HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_TEXTROMINFO) || (
				HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_TEXTSYSTEM) || (HWND)lParam ==
			GetDlgItem(hGameInfoDlg, IDC_TEXTNOTES) || (HWND)lParam == GetDlgItem(hGameInfoDlg, IDC_TEXTGENRE))
		{
