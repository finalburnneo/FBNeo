...
HBITMAP hPrevBmp = nullptr;
HBITMAP hTitleBmp = nullptr;

HWND hSelDlg = nullptr;
static HWND hSelList = nullptr;
static HWND hParent = nullptr;
static HWND hInfoLabel[6] = {nullptr, nullptr, nullptr, nullptr, nullptr}; // 4 things in our Info-box
static HWND hInfoText[6] = {nullptr, nullptr, nullptr, nullptr, nullptr}; // 4 things in our Info-box
...
int nIconsSizeXY = 16;
bool bEnableIcons = false;
bool bIconsLoaded = false;
bool bIconsOnlyParents = true;
int nIconsXDiff;
int nIconsYDiff;
...
// Filter TreeView
HWND hFilterList = nullptr;
HTREEITEM hFilterCapcomMisc = nullptr;
HTREEITEM hFilterCave = nullptr;
HTREEITEM hFilterCps1 = nullptr;
HTREEITEM hFilterCps2 = nullptr;
HTREEITEM hFilterCps3 = nullptr;
HTREEITEM hFilterDataeast = nullptr;
HTREEITEM hFilterGalaxian = nullptr;
HTREEITEM hFilterIrem = nullptr;
HTREEITEM hFilterKaneko = nullptr;
HTREEITEM hFilterKonami = nullptr;
HTREEITEM hFilterNeogeo = nullptr;
HTREEITEM hFilterPacman = nullptr;
HTREEITEM hFilterPgm = nullptr;
HTREEITEM hFilterPsikyo = nullptr;
HTREEITEM hFilterSega = nullptr;
HTREEITEM hFilterSeta = nullptr;
HTREEITEM hFilterTaito = nullptr;
HTREEITEM hFilterTechnos = nullptr;
HTREEITEM hFilterToaplan = nullptr;
HTREEITEM hFilterMiscPre90s = nullptr;
HTREEITEM hFilterMiscPost90s = nullptr;
HTREEITEM hFilterMegadrive = nullptr;
HTREEITEM hFilterPce = nullptr;
HTREEITEM hFilterMsx = nullptr;
HTREEITEM hFilterNes = nullptr;
HTREEITEM hFilterFds = nullptr;
HTREEITEM hFilterNgp = nullptr;
HTREEITEM hFilterChannelF = nullptr;
HTREEITEM hFilterSms = nullptr;
HTREEITEM hFilterGg = nullptr;
HTREEITEM hFilterSg1000 = nullptr;
HTREEITEM hFilterColeco = nullptr;
HTREEITEM hFilterSpectrum = nullptr;
HTREEITEM hFilterMidway = nullptr;
HTREEITEM hFilterBootleg = nullptr;
HTREEITEM hFilterDemo = nullptr;
HTREEITEM hFilterHack = nullptr;
HTREEITEM hFilterHomebrew = nullptr;
HTREEITEM hFilterPrototype = nullptr;
HTREEITEM hFilterGenuine = nullptr;
HTREEITEM hFilterHorshoot = nullptr;
HTREEITEM hFilterVershoot = nullptr;
HTREEITEM hFilterScrfight = nullptr;
HTREEITEM hFilterVsfight = nullptr;
HTREEITEM hFilterBios = nullptr;
HTREEITEM hFilterBreakout = nullptr;
HTREEITEM hFilterCasino = nullptr;
HTREEITEM hFilterBallpaddle = nullptr;
HTREEITEM hFilterMaze = nullptr;
HTREEITEM hFilterMinigames = nullptr;
HTREEITEM hFilterPinball = nullptr;
HTREEITEM hFilterPlatform = nullptr;
HTREEITEM hFilterPuzzle = nullptr;
HTREEITEM hFilterQuiz = nullptr;
HTREEITEM hFilterSportsmisc = nullptr;
HTREEITEM hFilterSportsfootball = nullptr;
HTREEITEM hFilterMisc = nullptr;
HTREEITEM hFilterMahjong = nullptr;
HTREEITEM hFilterRacing = nullptr;
HTREEITEM hFilterShoot = nullptr;
HTREEITEM hFilterRungun = nullptr;
HTREEITEM hFilterAction = nullptr;
HTREEITEM hFilterStrategy = nullptr;
HTREEITEM hFilterRpg = nullptr;
HTREEITEM hFilterSim = nullptr;
HTREEITEM hFilterAdv = nullptr;
HTREEITEM hFilterCard = nullptr;
HTREEITEM hFilterBoard = nullptr;
HTREEITEM hFilterOtherFamily = nullptr;
HTREEITEM hFilterMslug = nullptr;
HTREEITEM hFilterSf = nullptr;
HTREEITEM hFilterKof = nullptr;
HTREEITEM hFilterDstlk = nullptr;
HTREEITEM hFilterFatfury = nullptr;
HTREEITEM hFilterSamsho = nullptr;
HTREEITEM hFilter19xx = nullptr;
HTREEITEM hFilterSonicwi = nullptr;
HTREEITEM hFilterPwrinst = nullptr;
HTREEITEM hFilterSonic = nullptr;
HTREEITEM hFilterDonpachi = nullptr;
HTREEITEM hFilterMahou = nullptr;
HTREEITEM hFilterCapcomGrp = nullptr;
HTREEITEM hFilterSegaGrp = nullptr;

HTREEITEM hRoot = nullptr;
HTREEITEM hBoardType = nullptr;
HTREEITEM hFamily = nullptr;
HTREEITEM hGenre = nullptr;
HTREEITEM hFavorites = nullptr;
HTREEITEM hHardware = nullptr;
...
static UINT64 FdsValue = static_cast<UINT64>(HARDWARE_PREFIX_FDS) >> 24;
static UINT64 MASKFDS = static_cast<UINT64>(1) << FdsValue; // 1 << 0x1f - needs casting or.. bonkers!

// this is where things start going above the 32bit-zone. *solved w/64bit UINT?*
static UINT64 NgpValue = static_cast<UINT64>(HARDWARE_PREFIX_NGP) >> 24;
static UINT64 MASKNGP = static_cast<UINT64>(1) << NgpValue;

static UINT64 ChannelFValue = static_cast<UINT64>(HARDWARE_PREFIX_CHANNELF) >> 24;
static UINT64 MASKCHANNELF = static_cast<UINT64>(1) << ChannelFValue;

static UINT64 MASKALL = (MASKCAPMISC | MASKCAVE | MASKCPS | MASKCPS2 | MASKCPS3 | MASKDATAEAST | MASKGALAXIAN | MASKIREM
	| MASKKANEKO | MASKKONAMI | MASKNEOGEO | MASKPACMAN | MASKPGM | MASKPSIKYO | MASKSEGA | MASKSETA | MASKTAITO |
	MASKTECHNOS | MASKTOAPLAN | MASKMISCPRE90S | MASKMISCPOST90S | MASKMEGADRIVE | MASKPCENGINE | MASKSMS | MASKGG |
	MASKSG1000 | MASKCOLECO | MASKMSX | MASKSPECTRUM | MASKMIDWAY | MASKNES | MASKFDS | MASKNGP | MASKCHANNELF);
...
static NODEINFO* nBurnDrv = nullptr;
static NODEINFO* nBurnZipListDrv = nullptr;
...
static int DoExtraFilters()
{
	...
	if ((nLoadMenuFamilyFilter & MASKFAMILYOTHER) && (!(BurnDrvGetFamilyFlags() & FBF_MSLUG))
		...
		&& (!(BurnDrvGetFamilyFlags() & FBF_MAHOU)))
		return 1;
...
{
	auto ni0 = (struct NODEINFO*)p0;
	auto ni1 = (struct NODEINFO*)p1;
...
static int SelListMake()
{
	...
	nBurnDrv = static_cast<NODEINFO*>(malloc(nBurnDrvCount * sizeof(NODEINFO)));
	...
	if (nLoadMenuShowY & SHOWSHORT)
	{
		...
		if (nBurnZipListDrv)
		{
			...
			nBurnZipListDrv = nullptr;
		}
		nBurnZipListDrv = static_cast<NODEINFO*>(malloc(nBurnDrvCount * sizeof(NODEINFO)));
	...
	for (i = nBurnDrvCount - 1; i >= 0; i--)
	{
		...
		if (BurnDrvGetText(DRV_PARENT) != nullptr && (BurnDrvGetFlags() & BDF_CLONE))
		{
			// Skip clones
			continue;
		}
		...
		UINT64 nHardware = static_cast<UINT64>(1) << ((static_cast<UINT32>(BurnDrvGetHardwareCode()) >> 24) & 0x3f);
		if ((nHardware & MASKALL) && ((nHardware & nLoadMenuShowX) || (nHardware & MASKALL) == 0))
		{
			continue;
		}

		if (avOk && (!(nLoadMenuShowY & UNAVAILABLE)) && !gameAv[nBurnDrvActive])
		{
			// Skip non-available games if needed
			continue;
		}

		if (avOk && (!(nLoadMenuShowY & AVAILABLE)) && gameAv[nBurnDrvActive])
		{
			// Skip available games if needed
			continue;
		}
		...
		if (szSearchString[0])
		{
			TCHAR* StringFound = nullptr;
			TCHAR* StringFound2 = nullptr;
			TCHAR* StringFound3 = nullptr;
		...
		TvItem.item.pszText = (nLoadMenuShowY & SHOWSHORT)
			                      ? BurnDrvGetText(DRV_NAME)
			                      : RemoveSpace(BurnDrvGetText(DRV_ASCIIONLY | DRV_FULLNAME));
	...
	for (i = nBurnDrvCount - 1; i >= 0; i--)
	{
		...
		if (BurnDrvGetTextA(DRV_PARENT) == nullptr || !(BurnDrvGetFlags() & BDF_CLONE))
		{
			// Skip parents
			continue;
		}
		...
		if (!TvItem.hParent)
		{
			...
			for (j = 0; j < nBurnDrvCount; j++)
			{
				...
				if (!strcmp(szTempName, BurnDrvGetTextA(DRV_NAME)))
				{
					...
					TempTvItem.item.pszText = (nLoadMenuShowY & SHOWSHORT)
						                          ? BurnDrvGetText(DRV_NAME)
						                          : RemoveSpace(BurnDrvGetText(DRV_ASCIIONLY | DRV_FULLNAME));
					TempTvItem.item.lParam = (LPARAM)&nBurnDrv[nTmpDrvCount];
					nBurnDrv[nTmpDrvCount].hTreeHandle = (HTREEITEM)SendMessage(
						hSelList, TVM_INSERTITEM, 0, (LPARAM)&TempTvItem);
	...
	for (i = 0; i < nTmpDrvCount; i++)
	{
		...
		if (nBurnDrv[i].bIsParent && ((nLoadMenuShowY & AUTOEXPAND) || !gameAv[nBurnDrv[i].nBurnDrvNo] || !
			CheckWorkingStatus(nBurnDrv[i].nBurnDrvNo)))
		{
			for (j = 0; j < nTmpDrvCount; j++)
			{
	...
	_stprintf(szRomsAvailableInfo, FBALoadStringEx(hAppInst, IDS_SEL_SETSTATUS, true), nTmpDrvCount,
	          nBurnDrvCount - REDUCE_TOTAL_SETS_BIOS, nMissingDrvCount);
	SendDlgItemMessage(hSelDlg, IDC_DRVCOUNT, WM_SETTEXT, 0, (LPARAM)static_cast<LPCTSTR>(szRomsAvailableInfo));
...
static void MyEndDialog()
{
	...
	SendDlgItemMessage(hSelDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	SendDlgItemMessage(hSelDlg, IDC_SCREENSHOT_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);

	SendDlgItemMessage(hSelDlg, IDC_SCREENSHOT2_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	SendDlgItemMessage(hSelDlg, IDC_SCREENSHOT2_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	...
	if (hNotFoundEss)
	{
		...
		hNotFoundEss = nullptr;
	}
	if (hNotFoundNonEss)
	{
		DestroyIcon(hNotFoundNonEss);
		hNotFoundNonEss = nullptr;
	}
	if (hDrvIconMiss)
	{
		DestroyIcon(hDrvIconMiss);
		hDrvIconMiss = nullptr;
	}
...
static void SelOkay()
{
	...
	auto hSelectHandle = (HTREEITEM)SendMessage(hSelList, TVM_GETNEXTITEM, TVGN_CARET, ~0U);

	if (!hSelectHandle)
	{
		// Nothing is selected, return without closing the window
		return;
	}
	...
	if (!CheckWorkingStatus(nSelect))
	{
		...
		if (MessageBox(hSelDlg, FBALoadStringEx(hAppInst, IDS_ERR_NON_WORKING, true), szWarningText,
...
static void RefreshPanel()
{
	...
	hPrevBmp = PNGLoadBitmap(hSelDlg, nullptr, 213, 160, 2);
	hTitleBmp = PNGLoadBitmap(hSelDlg, nullptr, 213, 160, 2);
	...
	for (int i = 0; i < 6; i++)
	{
		SendMessage(hInfoText[i], WM_SETTEXT, 0, (LPARAM)_T(""));
...
FILE* OpenPreview(int nIndex, TCHAR* szPath)
{
	...
	if (!fp && BurnDrvGetText(DRV_PARENT) && bTryParent)
	{
		// Try the parent
...
static int UpdatePreview(bool bReset, TCHAR* szPath, int HorCtrl, int VerCtrl)
{
	...
	else
	{
		...
		bImageOrientation = FALSE;
		hNewImage = PNGLoadBitmap(hSelDlg, nullptr, 213, 160, 2);
	}
	...
	if (bImageOrientation == 0)
	{
		...
		SendDlgItemMessage(hSelDlg, VerCtrl, STM_SETIMAGE, IMAGE_BITMAP, NULL);
		ShowWindow(GetDlgItem(hSelDlg, HorCtrl), SW_SHOW);
		ShowWindow(GetDlgItem(hSelDlg, VerCtrl), SW_HIDE);
	}
	else
	{
		SendDlgItemMessage(hSelDlg, HorCtrl, STM_SETIMAGE, IMAGE_BITMAP, NULL);
...
{
	ShellExecute(nullptr, _T("open"), static_cast<TCHAR*>(arg), nullptr, nullptr, SW_SHOWNORMAL);
...
static void ViewEmma()
{
	...
	_stprintf(szShellExURL, _T("http://adb.arcadeitalia.net/dettaglio_mame.php?game_name=%s"),
	          fbn_to_mame(BurnDrvGetText(DRV_NAME)));

	hThread = (HANDLE)_beginthreadex(nullptr, 0, DoShellExThread, szShellExURL, 0, &ThreadID);
}
...
static void RebuildEverything()
{
	...
	TreeBuilding = 1;
	SendMessage(hSelList, WM_SETREDRAW, FALSE, (LPARAM)TVI_ROOT); // disable redraw
	SendMessage(hSelList, TVM_DELETEITEM, 0, (LPARAM)TVI_ROOT); // Destory all nodes
	SelListMake();
	SendMessage(hSelList, WM_SETREDRAW, TRUE, (LPARAM)TVI_ROOT); // enable redraw
...
static void CreateFilters()
{
	...
	_TVCreateFiltersA(hGenre, IDS_GENRE_SPORTSFOOTBALL, hFilterSportsfootball,
...
void LoadDrvIcons()
{
	...
	hDrvIcon = static_cast<HICON*>(malloc((nBurnDrvCount + 256) * sizeof(HICON)));
...

	{
		...
		hConsDrvIcon[ICON_MEGADRIVE] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                            nIconsSizeXY, LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%spce_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_PCEFAM] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                         nIconsSizeXY, LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%ssg1000_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SG1000] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                         nIconsSizeXY, LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%scolecovision_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_COLECO] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                         nIconsSizeXY, LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%ssms_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SMS] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                      LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%sgamegear_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_GG] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                     LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%smsx_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_MSX] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                      LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%sspectrum_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SPECTRUM] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                           nIconsSizeXY, LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%snes_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_NES] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                      LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%sfds_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_FDS] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                      LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%sngp_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_NGP] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                      LR_LOADFROMFILE));

		_stprintf(szIcon, _T("%schannelf_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_CHANNELF] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY,
		                                                           nIconsSizeXY, LR_LOADFROMFILE));
	}
	...
	for (unsigned int nDrvIndex = 0; nDrvIndex < nBurnDrvCount; nDrvIndex++)
	{
		...
		if (bIconsOnlyParents && BurnDrvGetText(DRV_PARENT) != nullptr && (BurnDrvGetFlags() & BDF_CLONE))
		{
			// Skip clones
			continue;
		}
		...
		hDrvIcon[nDrvIndex] = static_cast<HICON>(LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                   LR_LOADFROMFILE));
...
static INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		SendDlgItemMessage(hDlg, IDC_SCREENSHOT2_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
		SendDlgItemMessage(hDlg, IDC_SCREENSHOT2_V, STM_SETIMAGE, IMAGE_BITMAP, NULL);

		hWhiteBGBrush = CreateSolidBrush(RGB(0xFF, 0xFF, 0xFF));

		hExpand = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_PLUS), IMAGE_ICON, 16, 16,
		                                       LR_DEFAULTCOLOR));
		hCollapse = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_MINUS), IMAGE_ICON, 16, 16,
		                                         LR_DEFAULTCOLOR));

		hNotWorking = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTWORKING), IMAGE_ICON,
		                                           nIconsSizeXY, nIconsSizeXY,
		                                           LR_DEFAULTCOLOR));
		hNotFoundEss = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTFOUND_ESS), IMAGE_ICON,
		                                            nIconsSizeXY, nIconsSizeXY,
		                                            LR_DEFAULTCOLOR));
		hNotFoundNonEss = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTFOUND_NON), IMAGE_ICON,
		                                               nIconsSizeXY, nIconsSizeXY,
		                                               LR_DEFAULTCOLOR));

		hDrvIconMiss = static_cast<HICON>(LoadImage(hAppInst, MAKEINTRESOURCE(IDI_APP), IMAGE_ICON, nIconsSizeXY,
		                                            nIconsSizeXY,
		                                            LR_DEFAULTCOLOR));
		...
		SetWindowPos(hDlg, nullptr, 0, 0, nSelDlgWidth, nSelDlgHeight, SWP_NOZORDER);
	...
	{
		auto hItemChanged = (HTREEITEM)lParam;
	...
	if (Msg == WM_COMMAND)
	{
		if (HIWORD(wParam) == EN_CHANGE && LOWORD(wParam) == IDC_SEL_SEARCH)
		{
			...
			SetTimer(hDlg, IDC_SEL_SEARCHTIMER, 500, nullptr);
		}
		...
		if (HIWORD(wParam) == BN_CLICKED)
		{
			...
			case IDGAMEINFO:
				...
				{
					MessageBox(hSelDlg, FBALoadStringEx(hAppInst, IDS_ERR_NO_DRIVER_SELECTED, true),
			...
			case IDC_SEL_IPSMANAGER:
				if (bDrvSelected)
				{
					...
					nBurnDrvActive = nOldnBurnDrvActive;
					// due to some weird bug in sel.cpp, nBurnDrvActive can sometimes change when clicking in new dialogs.
					LoadIpsActivePatches();
	...
	if (Msg == WM_WINDOWPOSCHANGED)
	{
		...
		if (bIsWindowsXP && nTmpDrvCount < 12)
		{
			// Fix an issue on WinXP where the scrollbar overwrites past the tree size when less then 12 items are in the list
		...
		InvalidateRect(hSelDlg, nullptr, true);
		UpdateWindow(hSelDlg);
	...
	if (Msg == WM_NOTIFY)
	{
		...
		if (!TreeBuilding && pnmtv->hdr.code == NM_DBLCLK && pnmtv->hdr.idFrom == IDC_TREE1)
		{
			...
			if (hSelectHandle == nullptr) return 1;
			...
			case CDDS_ITEMPREPAINT:
				{
...

					{
						...
						if (((NODEINFO*)TvItem.lParam)->bIsParent)
						{
							...
							{
								DrawIconEx(lplvcd->nmcd.hdc, rect.left + 4, rect.top + nIconsYDiff, hCollapse, 16, 16,
								           0, nullptr, DI_NORMAL);
							}
							else
							{
								if (TvItem.cChildren)
								{
									DrawIconEx(lplvcd->nmcd.hdc, rect.left + 4, rect.top + nIconsYDiff, hExpand, 16, 16,
									           0, nullptr, DI_NORMAL);
								}
							}
						}
...

					{
						...
						if (nLoadMenuShowY & SHOWSHORT)
						{
							DrawText(lplvcd->nmcd.hdc, BurnDrvGetText(DRV_NAME), -1, &rect,
...

						{
							...
							if (!CheckWorkingStatus(((NODEINFO*)TvItem.lParam)->nBurnDrvNo))
							{
								DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hNotWorking, nIconsSizeXY,
								           nIconsSizeXY, 0, nullptr, DI_NORMAL);
							...
							else
							{
								...
								{
									DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hNotFoundEss, nIconsSizeXY,
									           nIconsSizeXY, 0, nullptr, DI_NORMAL);
									rect.left += nIconsSizeXY + 4;
								}
								else
								{
									if (!(nLoadMenuShowY & AVAILABLE) && !(gameAv[((NODEINFO*)TvItem.lParam)->
										nBurnDrvNo] & 2))
									{
										DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hNotFoundNonEss, nIconsSizeXY,
										           nIconsSizeXY, 0, nullptr, DI_NORMAL);
										rect.left += nIconsSizeXY + 4;
									}
								}
						...
						if (bEnableIcons && bIconsLoaded)
						{
							...
							{
								DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hDrvIcon[nBurnDrvActive],
								           nIconsSizeXY, nIconsSizeXY, 0, nullptr, DI_NORMAL);
							}

							if (!hDrvIcon[nBurnDrvActive])
							{
								DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hDrvIconMiss, nIconsSizeXY,
								           nIconsSizeXY, 0, nullptr, DI_NORMAL);
							}
							rect.left += nIconsSizeXY + 4;
						}
						...
						DrawText(lplvcd->nmcd.hdc, szText, -1, &rect,
						         DT_NOPREFIX | DT_SINGLELINE | DT_LEFT | DT_VCENTER);
		...
		if (pNmHdr->code == TVN_SELCHANGED && !TreeBuilding && LOWORD(wParam) == IDC_TREE1)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				case 0:
					{
						...
						if ((BurnDrvGetFlags() & BDF_CLONE) && BurnDrvGetTextA(DRV_PARENT))
						{
							...
							_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_CLONE_OF, true),
							...
							if (nBurnDrvActive < nBurnDrvCount)
							{
								...
								{
									_stprintf(szItemText + _tcslen(szItemText),
									          FBALoadStringEx(hAppInst, IDS_ROMS_FROM_1, true),
									          BurnDrvGetText(DRV_PARENT));
								}
							}
						...
						else
						{
							...
							{
								_stprintf(szItemText + _tcslen(szItemText),
								          FBALoadStringEx(hAppInst, IDS_ROMS_FROM_2, true),
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_SAMPLES_FROM, true), bBracket ? _T(", ") : _T(" ("),
				...
				case 1:
					{
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_SEL_PROTOTYPE, true));
							bUseInfo = true;
						}
						if (BurnDrvGetFlags() & BDF_BOOTLEG)
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_SEL_BOOTLEG, true), bUseInfo ? _T(", ") : _T(""));
							bUseInfo = true;
						}
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_SEL_HOMEBREW, true), bUseInfo ? _T(", ") : _T(""));
							bUseInfo = true;
						}
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_BOARD_ROMS_FROM, true),
				...
				case 2:
					{
						...
						_stprintf(szItemText, FBALoadStringEx(hAppInst, IDS_HARDWARE_DESC, true),
						          BurnDrvGetTextA(DRV_MANUFACTURER)
							          ? BurnDrvGetText(nGetTextFlags | DRV_MANUFACTURER)
							          : szUnknown, BurnDrvGetText(DRV_DATE),
						          (((BurnDrvGetHardwareCode() & HARDWARE_SNK_MVS) == HARDWARE_SNK_MVS) && ((
				...
				case 3:
					{
						...
						SendMessage(hInfoText[i], WM_SETTEXT, 0, (LPARAM)szText);
						if (szText[0])
						{
							EnableWindow(hInfoLabel[i], TRUE);
						}
				...
				case 4:
					{
						_stprintf(szItemText, _T("%s"),
						          BurnDrvGetTextA(DRV_COMMENT) ? BurnDrvGetText(nGetTextFlags | DRV_COMMENT) : _T(""));
						if (BurnDrvGetFlags() & BDF_HISCORE_SUPPORTED)
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBALoadStringEx(hAppInst, IDS_HISCORES_SUPPORTED, true),
		...
		if (!TreeBuilding && pnmtv->hdr.code == NM_RCLICK && pnmtv->hdr.idFrom == IDC_TREE1)
		{
			...
			CheckMenuItem(hMenuX, GAMESEL_MENU_FAVORITE,
			              (CheckFavorites(BurnDrvGetTextA(DRV_NAME)) != -1) ? MF_CHECKED : MF_UNCHECKED);

			TrackPopupMenu(hMenuX, TPM_LEFTALIGN | TPM_RIGHTBUTTON, oPoint.x, oPoint.y, 0, hSelDlg, nullptr);
...
static void UpdateInfoROMInfo()
{
	...
	if ((BurnDrvGetFlags() & BDF_CLONE) && BurnDrvGetTextA(DRV_PARENT))
	{
		...
		if (nBurnDrvActive < nBurnDrvCount)
		{
			if (BurnDrvGetText(DRV_PARENT))
			{
				_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_ROMS_FROM_1, true),
				          BurnDrvGetText(DRV_PARENT));
			}
		}
		nBurnDrvActive = nOldDrvSelect;
	...
	else
	{
		if (BurnDrvGetTextA(DRV_PARENT))
		{
			_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_ROMS_FROM_2, true),
			          bBracket ? _T(", ") : _T(" ("), BurnDrvGetText(DRV_PARENT));
			bBracket = true;
		}
	}
	if (BurnDrvGetText(DRV_BOARDROM))
	{
		_stprintf(szItemText + _tcslen(szItemText), FBALoadStringEx(hAppInst, IDS_BOARD_ROMS_FROM, true),
		          bBracket ? _T(", ") : _T(" ("), BurnDrvGetText(DRV_BOARDROM));
		bBracket = true;
	}
	if (bBracket)
	{
	...
	if (hInfoText[0])
	{
		SendMessage(hInfoText[0], WM_SETTEXT, 0, (LPARAM)szItemText);
	}
	if (hInfoLabel[0])
	{
...
static void UpdateInfoRelease()
{
	...
	_stprintf(szItemText, _T("%s (%s, %s %s)"),
	          BurnDrvGetTextA(DRV_MANUFACTURER) ? BurnDrvGetText(nGetTextFlags | DRV_MANUFACTURER) : szUnknown,
	          BurnDrvGetText(DRV_DATE),
	          BurnDrvGetText(nGetTextFlags | DRV_SYSTEM),
	          (BurnDrvGetHardwareCode() & HARDWARE_PREFIX_CARTRIDGE) ? szCartridge : szHardware);

	if (hInfoText[2])
	{
		SendMessage(hInfoText[2], WM_SETTEXT, 0, (LPARAM)szItemText);
	}
	if (hInfoLabel[2])
	{
...
static void UpdateInfoGameInfo()
{
	...
	{
		pszPosition += _sntprintf(pszPosition, szText + 1024 - pszPosition, _T(SEPERATOR_1) _T("%s"),
		                          BurnDrvGetText(nGetTextFlags | DRV_COMMENT));
	}

	if (hInfoText[3])
	{
		SendMessage(hInfoText[3], WM_SETTEXT, 0, (LPARAM)szText);
	}
	if (hInfoLabel[3])
	{
		if (szText[0])
...
static int MVSpreviewUpdateSlot(int nSlot, HWND hDlg)
{
	...
	if (nSlot >= 0 && nSlot < 6)
	{
		hInfoLabel[0] = nullptr;
		hInfoLabel[1] = nullptr;
		hInfoLabel[2] = nullptr;
		hInfoLabel[3] = nullptr;
		...
		for (int j = 0; j < 4; j++)
		{
			if (hInfoText[j])
			{
				SendMessage(hInfoText[j], WM_SETTEXT, 0, (LPARAM)_T(""));
			}
			if (hInfoLabel[j])
			{
		...
		if (nBurnDrvActive < nBurnDrvCount)
		{
			...
			{
				hMVSpreview[nSlot] = PNGLoadBitmap(hDlg, nullptr, 72, 54, 4);
			}
		...
		{
			hMVSpreview[nSlot] = PNGLoadBitmap(hDlg, nullptr, 72, 54, 4);
			SendMessage(hInfoText[0], WM_SETTEXT, 0, (LPARAM)FBALoadStringEx(hAppInst, IDS_EMPTY, true));
		}
...
static int MVSpreviewEndDialog()
{
	for (int i = 0; i < 6; i++)
	{
		DeleteObject(hMVSpreview[i]);
		hMVSpreview[i] = nullptr;
	}
	...
	case WM_COMMAND:
		...
		if (HIWORD(wParam) == BN_CLICKED)
		{
			if (LOWORD(wParam) == IDOK)
			{
				...
				{
					MessageBox(hSelDlg, FBALoadStringEx(hAppInst, IDS_ERR_NO_DRIVER_SEL_SLOT1, true),
					           FBALoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
					break;
				}
				MVSpreviewEndDialog();
...
int SelMVSDialog()
{
	return FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_MVS_SELECT_CARTS), hScrnWnd, MVSpreviewProc);
}
