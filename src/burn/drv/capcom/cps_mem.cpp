...
static UINT8 *CpsMem = nullptr, *CpsMemEnd = nullptr;
UINT8* CpsRam90 = nullptr;
UINT8 *CpsZRamC0 = nullptr, *CpsZRamF0 = nullptr, *CpsEncZRom = nullptr;
UINT8* CpsSavePal = nullptr;
UINT8* CpsSaveReg[MAX_RASTER + 1];
UINT8* CpsSaveFrg[MAX_RASTER + 1];
static UINT8* CpsSaveRegData = nullptr;
static UINT8* CpsSaveFrgData = nullptr;
UINT8 *CpsRam660 = nullptr, *CpsRam708 = nullptr, *CpsReg = nullptr, *CpsFrg = nullptr;
UINT8* CpsRamFF = nullptr;

CpsMemScanCallback CpsMemScanCallbackFunction = nullptr;
...
static INT32 CpsMemIndex()
{
	...
	CpsRam90 = Next;
	Next += 0x030000; // Video Ram
	CpsRamFF = Next;
	Next += 0x010000; // Work Ram
	CpsReg = Next;
	Next += 0x000100; // Registers

	CpsSavePal = Next;
	Next += 0x002000; // Draw Copy of Correct Palette

	if (((Cps == 2) && !Cps2DisableQSnd) || Cps1Qs == 1)
	{
		CpsZRamC0 = Next;
		Next += 0x001000; // Z80 c000-cfff
		CpsZRamF0 = Next;
		Next += 0x001000; // Z80 f000-ffff
	}

	if (Cps == 2)
	{
		CpsRam660 = Next;
		Next += 0x004000; // Extra Memory
		CpsRam708 = Next;
		Next += 0x010000; // Obj Ram
		CpsFrg = Next;
		Next += 0x000010; // 'Four' Registers (Registers at 0x400000)

		ZBuf = (UINT16*)Next;
		Next += 384 * 224 * 2; // Sprite Masking Z buffer

		CpsSaveRegData = Next;
		Next += 0x0100 * (MAX_RASTER + 1); // Draw Copy of registers
		CpsSaveFrgData = Next;
		Next += 0x0010 * (MAX_RASTER + 1); // Draw Copy of 'Four' Registers
	...
	{
		CpsSaveRegData = Next;
		Next += 0x0100; // Draw Copy of registers
		CpsSaveFrgData = Next;
		Next += 0x0010; // Draw Copy of 'Four' Registers
...
static INT32 AllocateMemory()
{
	...
	CpsMem = nullptr; // Find out how much memory is needed
	CpsMemIndex();
	nLen = CpsMemEnd - static_cast<UINT8*>(nullptr);

	if ((CpsMem = BurnMalloc(nLen)) == nullptr)
	{
		return 1;
	}
...
INT32 CpsMemExit()
{
	...
	CpsMemScanCallbackFunction = nullptr;
...
INT32 CpsAreaScan(INT32 nAction, INT32* pnMin)
{
	...
	if (Cps == 2 || Cps1Qs == 1 || PangEEP == 1 || CpsBootlegEEPROM == 1)
	{
		// Scan EEPROM
