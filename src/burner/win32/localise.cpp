...
static constexpr unsigned int nMaxResources = 2000;
...
static FBAResourceInfo_t* FBAResourceInfo = nullptr;

using DLGTEMPLATEEX = struct
{
	WORD dlgVer;
	WORD signature;
	DWORD helpID;
...
using DLGITEMTEMPLATEEX = struct
{
	DWORD helpID;
	DWORD exStyle;
	DWORD style;
...
int BuildTemplateDlgTemplateEx(const DLGTEMPLATEEX* pTemplate, LPTSTR lpszName, FILE* fp)
{
	...
	auto pTemplateDataIn = (char*)(((INT_PTR)pTemplate + 26 + sizeof(WORD) - 1) & ~(sizeof(WORD) - 1));
...
static BOOL CALLBACK FBALocaliseEnumResourceNamesDialog(HMODULE /* hModule */, LPCTSTR /* lpszType */, LPTSTR lpszName,
                                                        LONG_PTR lParam)
{
	auto pTemplate = static_cast<LPCDLGTEMPLATE>(LoadResource(hAppInst, FindResource(hAppInst, lpszName, RT_DIALOG)));

	if (LockResource((HGLOBAL)pTemplate))
	{
		if (((DLGTEMPLATEEX*)pTemplate)->signature == 0xFFFF && ((DLGTEMPLATEEX*)pTemplate)->dlgVer == 1)
		{
			if (BuildTemplateDlgTemplateEx((DLGTEMPLATEEX*)pTemplate, lpszName, nullptr))
			{
...
static BOOL CALLBACK FBALocaliseEnumResourceNamesMenu(HMODULE /* hModule */, LPCTSTR /* lpszType */, LPTSTR lpszName,
                                                      LONG_PTR lParam)
{
	auto pTemplate = LoadResource(hAppInst, FindResource(hAppInst, lpszName, RT_MENU));

	if (LockResource(pTemplate))
	{
		if (static_cast<MENUITEMTEMPLATEHEADER*>(pTemplate)->versionNumber == 0)
		{
			BuildTemplateMenuTemplate(pTemplate, lpszName, (FILE*)lParam);
		}
	}
...
static BOOL CALLBACK FBALocaliseEnumResourceNamesString(HMODULE /* hModule */, LPCTSTR /* lpszType */, LPTSTR lpszName,
                                                        LONG_PTR lParam)
{
	auto pwsz = static_cast<wchar_t*>(LoadResource(hAppInst, FindResource(hAppInst, lpszName, RT_STRING)));
...
static BOOL CALLBACK FBALocaliseEnumResTypeProc(HMODULE /* hModule */, LPTSTR lpszType, LONG_PTR lParam)
{
	...
	{
		EnumResourceNames(nullptr, lpszType, &FBALocaliseEnumResourceNamesDialog, lParam);
	}
	if (lpszType == RT_MENU)
	{
		EnumResourceNames(nullptr, lpszType, &FBALocaliseEnumResourceNamesMenu, lParam);
	}
	if (lpszType == RT_STRING)
	{
		EnumResourceNames(nullptr, lpszType, &FBALocaliseEnumResourceNamesString, lParam);
	}
...
static int FBALocaliseWriteTemplate(TCHAR* pszTemplate)
{
	...
	EnumResourceTypes(nullptr, FBALocaliseEnumResTypeProc, (LONG_PTR)fp);
...
DLGTEMPLATE* TranslateDlgTemplateEx(const DLGTEMPLATEEX* pTemplate, const LocaliseResourceInfo* pLocaliseInfo)
{
	// 26 is sizeof(DLGTEMPLATEEX)
	auto pTemplateDataIn = (char*)(((INT_PTR)pTemplate + 26 + sizeof(WORD) - 1) & ~(sizeof(WORD) - 1));
	char* pTemplateDataOut = nullptr;
	char* pTemplateDataInSync = nullptr;
	char* pTemplateDataOutSync = nullptr;

#ifdef PRINT_TRANSLATION_INFO
	dprintf(_T("Translating dialogex template:\n"));
#endif

	pTemplateDataOut = static_cast<char*>(malloc(26));
	memcpy(pTemplateDataOut, pTemplate, 26);
	pTemplateDataInSync = pTemplateDataIn;
...
INT_PTR FBADialogBox(HINSTANCE hInstance, LPTSTR lpTemplate, HWND hWndParent, DLGPROC lpDialogFunc)
{
		...
		UINT_PTR)lpTemplate].pResourceTranslation)
	{
		return DialogBoxIndirect(
			hInstance, static_cast<LPCDLGTEMPLATE>(FBAResourceInfo[(INT_PTR)lpTemplate].pResourceTranslation),
...
MENUTEMPLATE* TranslateMenuTemplate(const MENUTEMPLATE* pTemplate, const LocaliseResourceInfo* pLocaliseInfo)
{
	...
	pTemplateDataOut = static_cast<char*>(malloc(sizeof(MENUITEMTEMPLATEHEADER)));
	memcpy(pTemplateDataOut, pTemplate, sizeof(MENUITEMTEMPLATEHEADER) + ((MENUITEMTEMPLATEHEADER*)pTemplate)->offset);
	pTemplateDataInSync = pTemplateDataIn;
...
HMENU FBALoadMenu(HINSTANCE hInstance, LPTSTR lpMenuName)
{
	...
	{
		return LoadMenuIndirect(FBAResourceInfo[(INT_PTR)lpMenuName].pResourceTranslation);
	}
...
TCHAR* FBALoadStringEx(HINSTANCE hInstance, UINT uID, bool bTranslate)
{
	if (bLocalisationActive && bTranslate && uID < nMaxResources)
	{
		if (FBAResourceInfo[uID].pResourceTranslation)
		{
			...
			return static_cast<TCHAR*>(FBAResourceInfo[uID].pResourceTranslation);
		}
	}

	{
		// Convert the string ID into a bundle number
		auto pwsz = static_cast<wchar_t*>(LoadResource(
			hInstance, FindResource(hInstance, MAKEINTRESOURCE(uID / 16 + 1), RT_STRING)));

		if ((pwsz = static_cast<wchar_t*>(LockResource(pwsz))) != nullptr)
		{
			// Locate the string in the bundle
...
int FBALoadString(HINSTANCE hInstance, UINT uID, LPTSTR lpBuffer, int nBufferMax)
{
	if (bLocalisationActive && uID < nMaxResources)
	{
		if (FBAResourceInfo[uID].pResourceTranslation)
		{
			int nLen = _tcslen(static_cast<TCHAR*>(FBAResourceInfo[uID].pResourceTranslation));

#if defined (UNICODE)
			_tcsncpy(lpBuffer, static_cast<TCHAR*>(FBAResourceInfo[uID].pResourceTranslation), nBufferMax - 1);
#else
...
static int FBALocaliseParseFile(TCHAR* pszFilename)
{
	...
	while (true)
	{
		...
		{
#endif
			if (fgets(szTemp, sizeof(szTemp), h) == nullptr)
			{
				break;
			}
		...
		{
			if (_fgetts(szLine, 5120, h) == nullptr)
			{
				break;
			}
		}
		...
		if ((t = LabelCheckW(s, L"version")) != nullptr)
		{
			// Version
			s = t;
		...
		if ((t = LabelCheckW(s, L"codepage")) != nullptr)
		{
			// Set codepage
			s = t;
		...
		if ((t = LabelCheckW(s, L"menu")) != nullptr)
		{
			s = t;
			...
			if (nInside)
			{
				FBALocaliseError(pszFilename, nLine, _T("missing closing bracket"), nullptr);
				break;
			}
			if (*s != _T('{'))
			{
				FBALocaliseError(pszFilename, nLine, _T("missing opening bracket"), nullptr);
				break;
			}
			nInside = 1;
		...
		if ((t = LabelCheckW(s, L"dialog")) != nullptr)
		{
			// Add new resource
			s = t;
		...
		if ((t = LabelCheckW(s, L"string")) != nullptr)
		{
			s = t;
			...
			if (nID < nMaxResources)
			{
				...
				if (szEnd - szQuote > 0)
				{
					...
					wcscpy(static_cast<wchar_t*>(FBAResourceInfo[nID].pResourceTranslation), wszBuffer);
#else
					{
		...
		if (t && t != s)
		{
			...
			if (n < 512)
			{
				...
				if (wcslen(szQuote))
				{
					if (CurrentResource.pControlInfo[n] == nullptr)
					{
						CurrentResource.pControlInfo[n] = static_cast<LocaliseControlInfo*>(malloc(
							sizeof(LocaliseControlInfo)));
					}
		...
		if (*s == L'}')
		{
			if (nInside == 0)
			{
				FBALocaliseError(pszFilename, nLine, _T("rogue closing bracket"), nullptr);
				break;
			}
			...
			if (nInside == 0)
			{
				if (CurrentResource.nID < nMaxResources)
				{
					...
					if (nResourceType == RT_DIALOG)
					{
						...
						pTemplate = static_cast<LPCDLGTEMPLATE>(LoadResource(
				...
				for (int i = 0; i < 1024; i++)
				{
					if (CurrentResource.pControlInfo[i])
					{
						...
						CurrentResource.pControlInfo[i] = nullptr;
					}
				}
				memset(&CurrentResource, 0, sizeof(LocaliseResourceInfo));
			}
		...
		for (unsigned int i = 0; i < nMaxResources; i++)
		{
			if (FBAResourceInfo[i].nResourceFlags & RES_DEALLOCATE)
			{
				if (FBAResourceInfo[i].pResourceTranslation)
				{
					...
					FBAResourceInfo[i].pResourceTranslation = nullptr;
				}
			}
		}
...
int FBALocaliseInit(TCHAR* pszTemplate)
{
	...
	if (pszTemplate == nullptr || _tcslen(pszTemplate) == 0)
	{
	...
	FBAResourceInfo = static_cast<FBAResourceInfo_t*>(malloc(nMaxResources * sizeof(FBAResourceInfo_t)));
	if (FBAResourceInfo == nullptr)
	{
		return 1;
	}
