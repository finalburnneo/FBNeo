	...
	case WM_INITDIALOG:
		{
			...
			SendMessage(GetDlgItem(hDlg, IDC_WAIT_LABEL_A), WM_SETTEXT, (WPARAM)0,
			            (LPARAM)FBNLoadStringEx(hAppInst, IDS_SCANNING_ROMS, true));
			...
			if (hParent == NULL)
			{
				...
				SetWindowPos(hDlg, HWND_TOPMOST, (rect.right - rect.left) / 2 - x / 2,
				             (rect.bottom - rect.top) / 2 - y / 2, x, y, 0);
