...
void run(cpu_state *cpu, int cycles, bool stepping)
{
	...
	void write_ioreg(cpu_state* cpu, dword addr, word value)
	{
		...
		switch (reg)
		{
		case PSIZE: cpu->pshift = log2((double)value);
			break;
		case CONVDP: cpu->convdp = 1 << (~value & 0x1F);
			break;
		case CONVSP: cpu->convsp = 1 << (~value & 0x1F);
			break;
		case INTPEND:
	...
	rdfield_handler rdfield_table[64] = {
		...
		&rdfield_32, &rdfield_1_sx, &rdfield_2_sx, &rdfield_3_sx, &rdfield_4_sx, &rdfield_5_sx, &rdfield_6_sx,
		&rdfield_7_sx,
		&rdfield_8_sx, &rdfield_9_sx, &rdfield_10_sx, &rdfield_11_sx, &rdfield_12_sx, &rdfield_13_sx, &rdfield_14_sx,
		&rdfield_15_sx,
		&rdfield_16_sx, &rdfield_17_sx, &rdfield_18_sx, &rdfield_19_sx, &rdfield_20_sx, &rdfield_21_sx, &rdfield_22_sx,
		&rdfield_23_sx,
		&rdfield_24_sx, &rdfield_25_sx, &rdfield_26_sx, &rdfield_27_sx, &rdfield_28_sx, &rdfield_29_sx, &rdfield_30_sx,
		&rdfield_31_sx,
	};
}
