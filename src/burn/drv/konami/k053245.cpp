...
void K053245Init(INT32 chip, UINT8* gfx, UINT8* gfxexp, INT32 mask,
                 void (*callback)(INT32* code, INT32* color, INT32* priority))
{
	K053245Ram[chip] = BurnMalloc(0x800); // enough
	K053245Buf[chip] = BurnMalloc(0x800); // enough
...
{
	auto buf = (UINT16*)K053245Buf[chip];
...
{
	auto ret = (UINT16*)K053245Ram[chip];
...
#define NUM_SPRITES 128
	...
	auto sprbuf = (UINT16*)K053245Buf[chip];
        ...
        */
		...
		for (y = 0; y < h; y++)
		{
			...
			for (x = 0; x < w; x++)
			{
				...
				if (shadow)
				{
					konami_render_zoom_shadow_sprite(gfxdata, c, nBpp[chip], color, sx >> 12, sy >> 12, fx, fy, 16, 16,
					                                 zw, zh, pri, 0);
					continue;
				}
				...
				{
					konami_draw_16x16_priozoom_sprite(gfxdata, c, nBpp[chip], color, 0, sx >> 12, sy >> 12, fx, fy, 16,
					                                  16, zw, zh, pri);
				}
			}
		}
