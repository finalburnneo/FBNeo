
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <stdbool.h>

#include "snes.h"
#include "cart.h"
#include "ppu.h"
#include "dsp.h"
#include "apu.h"
#include "msu1.h"
#include "msu1_backend.h"
#include "statehandler.h"
#include "gsu.h"

static const INT32 stateVersion = 1;

typedef struct CartHeader {
	// normal header
	UINT8  headerVersion;		// 1, 2, 3
	char   name[22];			// $ffc0-$ffd4 (max 21 bytes + \0), $ffd4=$00: header V2
	UINT8  speed;				// $ffd5.7-4 (always 2 or 3)
	UINT8  type;				// $ffd5.3-0
	UINT8  coprocessor;			// $ffd6.7-4
	UINT8  chips;				// $ffd6.3-0
	UINT32 romSize;				// $ffd7 (0x400 << x)
	UINT32 ramSize;				// $ffd8 (0x400 << x)
	UINT8  region;				// $ffd9 (also NTSC/PAL)
	UINT8  maker;				// $ffda ($33: header V3)
	UINT8  version;				// $ffdb
	UINT16 checksumComplement;	// $ffdc,$ffdd
	UINT16 checksum;			// $ffde,$ffdf
	// v2/v3 (v2 only exCoprocessor)
	char  makerCode[3];			// $ffb0,$ffb1: (2 chars + \0)
	char  gameCode[5];			// $ffb2-$ffb5: (4 chars + \0)
	UINT32 flashSize;			// $ffbc (0x400 << x)
	UINT32 exRamSize;			// $ffbd (0x400 << x) (used for GSU?)
	UINT8 specialVersion;		// $ffbe
	UINT8 exCoprocessor;		// $ffbf (if coprocessor = $f)
	// calculated stuff
	INT16 score;				// score for header, to see which mapping is most likely
	bool  pal;					// if this is a rom for PAL regions instead of NTSC
	UINT8 cartType;				// calculated type
	bool  hasBattery;			// battery
} CartHeader;

static void readHeader(const UINT8* data, INT32 length, INT32 location, CartHeader* header, INT32 position);

static UINT32 CalculateCRC32(const UINT8* data, INT32 length)
{
	UINT32 crc = 0xffffffff;

	for (INT32 i = 0; i < length; i++) {
		crc ^= data[i];
		for (INT32 j = 0; j < 8; j++) {
			crc = (crc >> 1) ^ ((crc & 1) ? 0xedb88320 : 0);
		}
	}

	return ~crc;
}

bool snes_loadRom(Snes* snes, const UINT8* data, INT32 length, UINT8* biosdata, INT32 bioslength)
{
	// if smaller than smallest possible, don't load
	if (length < 0x8000) {
		printf("Failed to load rom: rom to small (%d bytes)\n", length);
		return false;
	}
	// check headers
	const INT32 max_headers = 8;
	CartHeader headers[max_headers];
	memset(headers, 0, sizeof(headers));
	for (INT32 i = 0; i < max_headers; i++) {
		headers[i].score = -50;
	}
	if (length >= 0x8000  ) readHeader(data, length, 0x7fc0,   &headers[0], 0); // lorom
	if (length >= 0x8200  ) readHeader(data, length, 0x81c0,   &headers[1], 1); // lorom + header
	if (length >= 0x10000 ) readHeader(data, length, 0xffc0,   &headers[2], 2); // hirom
	if (length >= 0x10200 ) readHeader(data, length, 0x101c0,  &headers[3], 3); // hirom + header
	if (length >= 0x410000) readHeader(data, length, 0x407fc0, &headers[4], 4); // exlorom
	if (length >= 0x410200) readHeader(data, length, 0x4081c0, &headers[5], 5); // exlorom + header
	if (length >= 0x410000) readHeader(data, length, 0x40ffc0, &headers[6], 6); // exhirom
	if (length >= 0x410200) readHeader(data, length, 0x4101c0, &headers[7], 7); // exhirom + header
	// see which it is, go backwards to allow picking ExHiROM over HiROM for roms with headers in both spots
	INT32 max  = 0;
	INT32 used = 0;
	for (INT32 i = max_headers - 1; i >= 0; i--) {
		if (headers[i].score > max) {
			max = headers[i].score;
			used = i;
		}
	}
	bprintf(0, _T("header used %d\n"), used);
	if (used & 1) {
		// odd-numbered ones are for headered roms
		data   += 0x200; // move pointer past header
		length -= 0x200; // and subtract from size
	}
	UINT8 isSFEX = length == 0x200000 && CalculateCRC32(data, length) == 0xdad59b9f;
	// check if we can load it
	if (headers[used].cartType > 4) {
		bprintf(0, _T("Failed to load rom: unsupported type (%d)\n"), headers[used].cartType);
		return false;
	}
	// expand to a power of 2
	INT32 origLength = length;   // true image size before power-of-2 mirror padding (below)
	INT32 newLength  = 0x8000;
	while (true) {
		if (length <= newLength) {
			break;
		}
		newLength *= 2;
	}
	UINT8* newData = BurnMalloc(newLength);
	memcpy(newData, data, length);
	INT32 test = 1;
	while (length != newLength) {
		if (length & test) {
			memcpy(newData + length, newData + length - test, test);
			length += test;
		}
		test *= 2;
	}

	// coprocessor check
	if (headers[used].exCoprocessor == 0x10) {
		headers[used].cartType = CART_CX4; // cx4
	}

	// -- cart specific config --
	snes->ramFill = 0x00;						// default, 00-fill
	UINT8 cartRamFill = 0x00;					// cart-ram or save ram default-fill

	if (!strcmp(headers[used].name, "DEATH BRADE") || !strcmp(headers[used].name, "POWERDRIVE")) {
		snes->ramFill = 0xff;					// games prefer 0xff fill
	}
	if (!strcmp(headers[used].name, "ASHITANO JOE") || !strcmp(headers[used].name, "SUCCESS JOE")) {
		snes->ramFill = 0x3f;					// game prefers 0x3f fill
	}
	if (!strcmp(headers[used].name, "PGA TOUR GOLF")) {
		snes->ramFill = 0x3f;
	}
	if (!strcmp(headers[used].name, "KEN GRIFFEY JR BASEBA")) {
		cartRamFill = 0xff;
	}
	if (!strcmp(headers[used].name, "UNIRACERS")) {
		snes->vramhack = 2;
	}
	if (!strcmp(headers[used].name, "SUPER MARIO KART")) {
		snes->ramFill = 0x3f;					// start always select 2p if 00-fill
	}
	if (!strcmp(headers[used].name, "BATMAN--REVENGE JOKER")) {
		headers[used].cartType = CART_LOROM;	// it's detected as HiROM but actually LoROM (prototype)
	}
	if (!strcmp(headers[used].name, "AS AVEN DE BATMAN&ROB")) {
		headers[used].chips = 1;				// advbatmantp has 8k sram (non-battery backed)
	}

	UINT8 romType = (headers[used].coprocessor << 4) | headers[used].chips;
	switch (headers[used].coprocessor) {
		case 1:
			headers[used].cartType = (romType == 0x17 || romType == 0x18) ? CART_SUPERFX3 : CART_SUPERFX;
			break;
		case 2:
			headers[used].cartType = CART_LOROMOBC1;
			break;
		case 3:
			headers[used].cartType = CART_LOROMSA1;
			break;
		case 4:
			headers[used].cartType = CART_LOROMSDD1;
	}

	bool isSPC7110 = false;
	if (headers[used].headerVersion >= 2
		&& headers[used].coprocessor == 0x0f && headers[used].exCoprocessor == 0x00
		&& headers[used].cartType != CART_CX4) {
		isSPC7110 = true;
	}
	if (isSPC7110) {
		headers[used].cartType = CART_SPC7110;
	}

	UINT32 superfx_ramSize = 0;
	UINT8  superfx_gsuType = SNES_GSU_1;
	if (headers[used].cartType == CART_SUPERFX || headers[used].cartType == CART_SUPERFX3) {
		const char* sfxname = headers[used].name;
		if (headers[used].cartType == CART_SUPERFX3) {
			superfx_ramSize = 0x20000;
			superfx_gsuType = SNES_GSU_3;
			headers[used].hasBattery = romType == 0x18;
		} else {
			if (!strcmp(sfxname, "YOSSY'S ISLAND")   ||			// Yoshi's Island (Japan)
				!strcmp(sfxname, "YOSHI'S ISLAND")   ||			// Yoshi's Island (USA/EUR)
				!strcmp(sfxname, "STAR FOX 2")       ||			// Star Fox 2 (Classic Mini, Switch Online)
				!strcmp(sfxname, "DOOM")             ||
				!strcmp(sfxname, "FX SKIING NINTENDO 96")) {	// Winter Gold (PAL, GSU-2)
				superfx_ramSize = 0x20000;						// GSU-2: 128KB
			} else if (!strcmp(sfxname, "STAR WING") ||			// Star Fox (PAL)
				!strcmp(sfxname, "STAR FOX")         ||			// Star Fox (NTSC)
				!strcmp(sfxname, "STUNT RACE FX")    ||
				!strcmp(sfxname, "VORTEX")           ||
				!strcmp(sfxname, "DIRT TRAX FX")     ||
				!strcmp(sfxname, "DIRT RACER")       ||			// Dirt Racer (PAL, GSU-1)
				!strcmp(sfxname, "POWERSLIDE")) {				// Power Slide (PAL proto, GSU-1)
				superfx_ramSize = 0x10000;						// GSU-1: 64KB
			}
			if (superfx_ramSize == 0) {
				superfx_ramSize = headers[used].exRamSize;
			}
			if (superfx_ramSize < 0x10000 || superfx_ramSize > 0x20000) {
				superfx_ramSize = (newLength > 0x200000) ? 0x20000 : 0x10000;
			}
			superfx_gsuType = (superfx_ramSize > 0x10000) ? SNES_GSU_2 : SNES_GSU_1;
			headers[used].hasBattery = true;
		}
		bprintf(0, _T("superfx: gsu-%d ram size %x\n"), superfx_gsuType, superfx_ramSize);
	}

	UINT32 spc7110_promSize = 0;
	UINT32 spc7110_eromSize = 0;
	UINT32 spc7110_trueSize = 0;
	UINT32 spc7110_ramSize  = 0;
	bool   spc7110_hasRTC   = false;
	if (headers[used].cartType == CART_SPC7110) {
		spc7110_promSize = (newLength > 0x100000) ? 0x100000 : 0;	// 1MB PROM
		if ((UINT32)origLength >= 0x700000) {
			spc7110_eromSize = 0x100000;							// fixed 1MB expansion @ 0x600000
			spc7110_trueSize = 0x700000;							// fixed logical board size (ignore any tail)
		} else {
			spc7110_trueSize = (UINT32)origLength;					// commercial cart: prom + drom, no expansion
		}
		spc7110_ramSize = (headers[used].chips > 0 && headers[used].ramSize >= 0x2000)
			? headers[used].ramSize : 0x2000;						// >= 8KB save SRAM
		headers[used].hasBattery = true;
		spc7110_hasRTC = (headers[used].chips == 0x9);
		bprintf(0, _T("spc7110: prom %x drom %x erom %x ram %x rtc %d\n"), spc7110_promSize, spc7110_trueSize - spc7110_promSize - spc7110_eromSize, spc7110_eromSize, spc7110_ramSize, spc7110_hasRTC);
	}

	UINT8 msu1_base = 0;
	bool  msu1Enable = false;
	// Enable if this romset has MSU-1 media, or (for a clone) its parent does.
	const TCHAR* msu1Game = BurnDrvGetText(DRV_NAME);
	if (msu1Game != NULL && snes_msu1_backend_detect(msu1Game)) {
		msu1Enable = true;
	} else {
		const TCHAR* p = BurnDrvGetText(DRV_PARENT);
		if (p != NULL && p[0] && snes_msu1_backend_detect(p)) msu1Enable = true;
	}
	if (msu1Enable) {
		msu1_base = (headers[used].cartType == CART_HIROM
			|| headers[used].cartType == CART_EXHIROM) ? CART_HIROM : CART_LOROM;
		headers[used].cartType = CART_MSU1;
		snes_msu1_backend_setGame(msu1Game, BurnDrvGetText(DRV_PARENT));
		snes_msu1_backend_install();
		bprintf(0, _T("msu1: overlay on %S base (media: support/snesmsu1/%s/)\n"), cart_gettype(msu1_base), msu1Game);
	}

	switch (bioslength) {
		case 0x2800:	// DSP1-4
			bprintf(0, _T("-we have dsp bios, lets go!\n"));
			switch (headers[used].cartType) {
				case CART_LOROM:
					headers[used].cartType = CART_LOROMDSP;
					break;
				case CART_HIROM:
					headers[used].cartType = CART_HIROMDSP;
					break;
			}
			break;
		case 0x11000:	// st010/st011
			bprintf(0, _T("-we have st010/st011 bios, lets go!\n"));
			headers[used].cartType = CART_LOROMSETA;;
			break;
		case 0x28000:	// st018 (Seta ARMv3): 128KB program ROM + 32KB data ROM
			bprintf(0, _T("-we have st018 bios, lets go!\n"));
			headers[used].cartType = CART_ST018;
			break;
	}

	if (isSFEX) {
		headers[used].cartType = CART_SFEX;
	}

	// load it
	bprintf(0, _T("Loaded %S rom (%S)\n"), cart_gettype(headers[used].cartType), headers[used].pal ? "PAL" : "NTSC");
	bprintf(0, _T("\"%S\"\n"), headers[used].name);

	INT32 bankSize = 0;
	switch (headers[used].cartType) {
		case CART_HIROM:
		case CART_EXHIROM:
		case CART_HIROMDSP:
			bankSize = 0x10000;
			break;
		default:
			bankSize = 0x8000;
			break;
	}

	bprintf(
		0, _T("%dK banks: %d, ramsize: %d%S, coprocessor: %x\n"),
		bankSize / 1024, newLength / bankSize, headers[used].chips > 0 ? headers[used].ramSize : 0, (headers[used].hasBattery) ? " (battery-backed)" : "", headers[used].exCoprocessor
	);

	INT32 loadRamSize;
	if (headers[used].cartType == CART_SUPERFX || headers[used].cartType == CART_SUPERFX3) {
		loadRamSize = superfx_ramSize;
	} else if (headers[used].cartType == CART_SPC7110) {
		loadRamSize = spc7110_ramSize;
	} else {
		loadRamSize = (headers[used].chips > 0) ? headers[used].ramSize : 0;
	}

	cart_load(
		snes->cart, headers[used].cartType,
		newData, newLength, biosdata, bioslength,
		loadRamSize,
		cartRamFill,
		headers[used].hasBattery
	);

	if (headers[used].cartType == CART_SUPERFX || headers[used].cartType == CART_SUPERFX3) {
		snes->cart->gsuType = superfx_gsuType;
		snes->cart->oscillator = (superfx_gsuType == SNES_GSU_3) ? 85760000u : 21440000u;
	}

	if (headers[used].cartType == CART_SPC7110) {
		snes->cart->promSize    = spc7110_promSize;
		snes->cart->eromSize    = spc7110_eromSize;
		snes->cart->romTrueSize = spc7110_trueSize;	// pre-padding size drives the drom/erom split
		snes->cart->hasRTC      = spc7110_hasRTC;
	}

	if (headers[used].cartType == CART_MSU1) {
		snes->cart->msuBase = msu1_base;			// LoROM/HiROM base the $2000-$2007 overlay sits on
	}

	snes->palTiming = headers[used].pal;			// set region before reset, so co-processors see correct timing
	snes_reset(snes, true);							// reset after loading
	BurnFree(newData);
	return true;
}

bool snes_isPal(Snes* snes)
{
	return snes->palTiming;
}

void snes_setButtonState(Snes* snes, INT32 player, INT32 button, INT32 pressed, INT32 device)
{
	// set key in controller
	Input* input = (player == 1) ? snes->input1 : snes->input2;
	uint32_t* c_state = &input->currentState;

	input_setType(input, device);

	if (pressed) {
		*c_state |= 1 << button;
	} else {
		*c_state &= ~(1 << button);
	}
}

#define is_gun_offscreen(x,y) (!(x == 0 || x == 255 || y == 0 || y == 255) )

void snes_setGunState(Snes* snes, INT32 x1, INT32 y1, INT32 x2, INT32 y2)
{
	// set gun coords
	Input* input = snes->input2;	// gun always in input 2
	uint32_t* c_state = &input->currentState;

	switch (input->type) {
		case DEVICE_SUPERSCOPE:
			*c_state |= 0xff00;
			if (*c_state & SCOPE_FIRE || *c_state & SCOPE_CURSOR) {
				ppu_latchScope(x1, y1);
			}
			break;
		case DEVICE_JUSTIFIER:
			*c_state |= 0xe5500;	// Justifier serial bit stream "header"

			switch (input->devParam & 1) {
				case 0:
					if (is_gun_offscreen(x2, y2)) {
						ppu_latchScope(x2, y2);
					}
					break;
				case 1:
					if (is_gun_offscreen(x1, y1)) {
						ppu_latchScope(x1, y1);
					}
					break;
			}
			break;
	}
}

void snes_setMouseState(Snes* snes, INT32 player, INT16 x, INT16 y, UINT8 buttonA, UINT8 buttonB)
{
	Input* input = (player == 1) ? snes->input1 : snes->input2;
	input_setType(input, DEVICE_MOUSE);
	input_setMouse(input, x, y, buttonA, buttonB);
}

void snes_setPixels(Snes* snes, UINT8* pixelData, INT32 height)
{
	// size is 4 (rgba) * 512 (w) * 480 (h)
	ppu_putPixels(pixelData, height);
}

void snes_setSamples(Snes* snes, INT16* sampleData, INT32 samplesPerFrame)
{
	// size is 2 (int16) * 2 (stereo) * samplesPerFrame
	// sets samples in the sampleData
	dsp_getSamples(snes->apu->dsp, sampleData, samplesPerFrame);
	if (snes->cart != NULL && snes->cart->type == CART_MSU1 && bBurnRunAheadFrame == 0) {
		INT32 outRate = samplesPerFrame * (snes->palTiming ? 50 : 60);
		snes_msu1_mixSamples(sampleData, samplesPerFrame, outRate, snes->apu->dsp->mute ? 1 : 0);
	}
}

INT32 snes_saveBattery(Snes* snes, UINT8* data)
{
	INT32 size = 0;
	cart_handleBattery(snes->cart, true, data, &size);
	return size;
}

bool snes_loadBattery(Snes* snes, UINT8* data, INT32 size)
{
	return cart_handleBattery(snes->cart, false, data, &size);
}

INT32 snes_saveState(Snes* snes, UINT8* data)
{
	StateHandler* sh = sh_init(true, NULL, 0);
	uint32_t id = 0x4653534c; // 'LSSF' LakeSnes State File
	uint32_t version = stateVersion;
	sh_handleInts(sh, &id, &version, &version, NULL); // second version to be overridden by length
	cart_handleTypeState(snes->cart, sh);
	// save data
	snes_handleState(snes, sh);
	// store
	sh_placeInt(sh, 8, sh->offset);
	if (data != NULL) memcpy(data, sh->data, sh->offset);
	INT32 size = sh->offset;
	sh_free(sh);
	return size;
}

bool snes_loadState(Snes* snes, UINT8* data, INT32 size)
{
	StateHandler* sh = sh_init(false, data, size);
	uint32_t id = 0, version = 0, length = 0;
	sh_handleInts(sh, &id, &version, &length, NULL);
	bool cartMatch = cart_handleTypeState(snes->cart, sh);
	if (id != 0x4653534c || version != stateVersion || length != size || !cartMatch) {
		sh_free(sh);
		return false;
	}
	// load data
	snes_handleState(snes, sh);
	// finish
	sh_free(sh);
	return true;
}

static void readHeader(const UINT8* data, INT32 length, INT32 location, CartHeader* header, INT32 position)
{
	// read name, TODO: non-ASCII names?
	for (INT32 i = 0; i < 21; i++) {
		UINT8 ch = data[location + i];
		if (ch >= 0x20 && ch < 0x7f) {
			header->name[i] = ch;
		} else {
			header->name[i] = '.';
		}
	}
	header->name[21] = 0;
	// clean name (strip end space)
	INT32 slen = strlen(header->name);
	while (slen > 0 && header->name[slen - 1] == ' ') {
		header->name[slen - 1] = '\0';
		slen--;
	}
	// read rest
	header->speed       = data[location + 0x15] >> 4;
	header->type        = data[location + 0x15] & 0xf;
	header->coprocessor = data[location + 0x16] >> 4;
	header->chips       = data[location + 0x16] & 0xf;
	bprintf(0, _T("cart type, copro, chips: %x, %x, %x\n"), header->type, header->coprocessor, header->chips);
	header->hasBattery  = (header->chips == 0x02 || header->chips == 0x05 || header->chips == 0x06);
	header->romSize     = 0x400 << data[location + 0x17];
	header->ramSize     = 0x400 << data[location + 0x18];
	header->region      = data[location + 0x19];
	header->maker       = data[location + 0x1a];
	header->version     = data[location + 0x1b];
	header->checksumComplement = (data[location + 0x1d] << 8) + data[location + 0x1c];
	header->checksum    = (data[location + 0x1f] << 8) + data[location + 0x1e];
	// read v3 and/or v2
	header->headerVersion = 1;
	if (header->maker == 0x33) {
		header->headerVersion = 3;
		// maker code
		for (INT32 i = 0; i < 2; i++) {
			UINT8 ch = data[location - 0x10 + i];
			if (ch >= 0x20 && ch < 0x7f) {
				header->makerCode[i] = ch;
			} else {
				header->makerCode[i] = '.';
			}
		}
		header->makerCode[2] = 0;
		// game code
		for (INT32 i = 0; i < 4; i++) {
			UINT8 ch = data[location - 0xe + i];
			if (ch >= 0x20 && ch < 0x7f) {
				header->gameCode[i] = ch;
			} else {
				header->gameCode[i] = '.';
			}
		}
		header->gameCode[4] = 0;
		header->flashSize = 0x400 << data[location - 4];
		header->exRamSize = 0x400 << data[location - 3];
		header->specialVersion = data[location - 2];
		header->exCoprocessor  = data[location - 1];
	} else if (data[location + 0x14] == 0) {
		header->headerVersion = 2;
		header->exCoprocessor = data[location - 1];
	}
	//bprintf(0, _T("ramsize %x  exramsize %x\n"),header->ramSize, header->exRamSize);
	//bprintf(0, _T("exCoprocessor %x\n"), header->exCoprocessor);
	// get region
	header->pal = (header->region >= 0x2 && header->region <= 0xc) || header->region == 0x11;
	header->cartType = location < 0x9000 ? CART_LOROM : CART_HIROM;
	if (location > 0x400000) {
		// Ex...
		if (location == 0x407fc0 || location == 0x4081c0) {
			header->cartType = CART_EXLOROM;	// ExLoROM
		} else {
			header->cartType = CART_EXHIROM;	// ExHiROM
		}
	}
	// get score
	// TODO: check name, maker/game-codes (if V3) for ASCII, more vectors,
	//   more first opcode, rom-sizes (matches?), type (matches header location?)
	INT32 score = 0;
	score += (header->speed == 2 || header->speed == 3) ? 5 : -4;
	score += (header->type  <= 3 || header->type  == 5) ? 5 : -2;
	score += (header->coprocessor <= 5 || header->coprocessor >= 0xe) ? 5 : -2;
	score += (header->chips <= 6 || header->chips == 9 || header->chips == 0xa) ? 5 : -2;
	score += (header->region <= 0x14) ? 5 : -2;
	score += (header->checksum != 0 && header->checksumComplement != 0 && header->checksum + header->checksumComplement == 0xffff) ? 8 : -6;
	//bprintf(0, _T("header checksum/compl: %x %x\n"), header->checksum, header->checksumComplement);
	UINT16 resetVector = data[location + 0x3c] | (data[location + 0x3d] << 8);
	score += (resetVector >= 0x8000) ? 8 : -20;
	// check first opcode after reset
	INT32 opcodeLoc = location + 0x40 - 0x8000 + (resetVector & 0x7fff);
	UINT8 opcode = 0xff;
	if (opcodeLoc < length) {
		opcode = data[opcodeLoc];
	} else {
		score -= 14;
	}
	if (opcode == 0x78 || opcode == 0x18) {
		// sei, clc (for clc:xce)
		score += 6;
	}
	if (opcode == 0x4c || opcode == 0x5c || opcode == 0x9c) {
		// jmp abs, jml abl, stz abs
		score += 3;
	}
	if (opcode == 0x00 || opcode == 0xff || opcode == 0xdb) {
		// brk, sbc alx, stp
		score -= 6;
	}
	score += (score > 0x20) ? position : 0;	// pick "ex..ROM" over "..ROM"
	bprintf(0, _T("opcode, score: %x  %x\n"), opcode, score);
	header->score = score;
}
