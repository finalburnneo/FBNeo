...
static uint8_t cpu_read(uint32_t adr) {
  intDelay = false;
  return sa1_cpuRead(adr);
}

static void cpu_write(uint32_t adr, uint8_t val) {
  intDelay = false;
  sa1_cpuWrite(adr, val);
}
...
static inline void idle_jump() {
	if (sa1_isrom_address((k << 16) | pc)) {
		cpu_idle(); // sa1 only
...
static inline void idle_branch() {
	if (pc & 0x01 && sa1_isrom_address((k << 16) | pc)) {
		cpu_idle(); // sa1 only
	}
}
