...
static UINT8* pSShot = nullptr;
static UINT8* pFreeMe[3] = {nullptr, nullptr, nullptr};
...
INT32 MakeScreenShot()
{
	...
	if (nVidImageBPP < 4)
	{
		auto pTemp = static_cast<UINT8*>(malloc(w * h * sizeof(INT32)));
		...
		if (nVidImageBPP == 2)
		{
			for (INT32 i = 0; i < h * w; i++)
			{
				...
				// Red
				*(pTemp + i * 4 + 0) = static_cast<UINT8>((nColour & 0x1F) << 3);
				*(pTemp + i * 4 + 0) |= *(pTemp + 4 * i + 0) >> 5;

				if (nVidImageDepth == 15)
				{
					// Green
					*(pTemp + i * 4 + 1) = static_cast<UINT8>(((nColour >> 5) & 0x1F) << 3);
					*(pTemp + i * 4 + 1) |= *(pTemp + i * 4 + 1) >> 5;
					// Blue
					*(pTemp + i * 4 + 2) = static_cast<UINT8>(((nColour >> 10) & 0x1F) << 3);
				...
				{
					// Green
					*(pTemp + i * 4 + 1) = static_cast<UINT8>(((nColour >> 5) & 0x3F) << 2);
					*(pTemp + i * 4 + 1) |= *(pTemp + i * 4 + 1) >> 6;
					// Blue
					*(pTemp + i * 4 + 2) = static_cast<UINT8>(((nColour >> 11) & 0x1F) << 3);
					*(pTemp + i * 4 + 2) |= *(pTemp + i * 4 + 2) >> 5;
				}
			}
		}
	...
	{
		auto pTemp = static_cast<UINT8*>(malloc(w * h * 3)); // bgrbgrbgr...
	...
	sprintf(szSShotName, "%s%s-%.2d-%.2d-%.2d%.2d%.2d.png", SSHOT_DIRECTORY, BurnDrvGetTextA(DRV_NAME),
...
#ifdef _UNICODE
	sprintf(szSoftware, APP_TITLE " v%.20ls using LibSPNG v%d.%d.%d", szAppBurnVer, SPNG_VERSION_MAJOR,
	        SPNG_VERSION_MINOR, SPNG_VERSION_PATCH);
#else
	...
	strcpy(text_ptr[0].keyword, "Title");
	text_ptr[0].text = BurnDrvGetTextA(DRV_FULLNAME);
	strcpy(text_ptr[1].keyword, "Author");
	text_ptr[1].text = szAuthor;
	strcpy(text_ptr[2].keyword, "Description");
	text_ptr[2].text = szDescription;
	strcpy(text_ptr[3].keyword, "Copyright");
	text_ptr[3].text = szCopyright;
	strcpy(text_ptr[4].keyword, "Creation Time");
	text_ptr[4].text = szTime;
	strcpy(text_ptr[5].keyword, "Software");
	text_ptr[5].text = szSoftware;
	strcpy(text_ptr[6].keyword, "Source");
	text_ptr[6].text = szSource;
	strcpy(text_ptr[7].keyword, "Comment");
	text_ptr[7].text =
		"This screenshot was created by running the game in an emulator; it might not accurately reflect the actual hardware the game was designed to run on.";
