...
static INT32 decrypt(const INT32 param1, const INT32 param2, const INT32 enc_prev_word, const INT32 dec_prev_word,
                     const INT32 enc_word)
{
	const INT32 swap = (BIT(dec_prev_word, 8) << 1) | BIT(dec_prev_word, 7);
	const INT32 type = (BIT(dec_prev_word, 12) << 1) | BIT(dec_prev_word, 2);
	INT32 res = 0;
	INT32 k = 0;

	switch (swap)
	{
	case 0: res = BITSWAP16(enc_word, 1, 2, 0, 14, 12, 15, 4, 8, 13, 7, 3, 6, 11, 5, 10, 9);
		break;
	case 1: res = BITSWAP16(enc_word, 14, 10, 4, 15, 1, 6, 12, 11, 8, 0, 9, 13, 7, 3, 5, 2);
		break;
	case 2: res = BITSWAP16(enc_word, 2, 13, 15, 1, 12, 8, 14, 4, 6, 0, 9, 5, 10, 7, 3, 11);
		break;
	case 3: res = BITSWAP16(enc_word, 3, 8, 1, 13, 14, 4, 15, 0, 10, 2, 7, 12, 6, 11, 9, 5);
		break;
	}
	...
	res = ((res & 0x003f) | ((res + (k << 6)) & 0x07c0) | ((res + (k << 11)) & 0xf800)) ^ ((param1 << 6) | (param1 <<
		11));
