...
static void GameInpInitMacros()
{
	...
	for (UINT32 i = 0; i < nGameInpCount; i++)
	{
		...
		bool bPlayerInInfo = (toupper(bii.szInfo[0]) == 'P' && bii.szInfo[1] >= '1' && bii.szInfo[1] <= '6');
	...
	INT32 nRealButtons = ((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_CAPCOM_CPS2)
		                     ? nFireButtons - 2
		                     : nFireButtons;
	INT32 nIndex = 0;
	...
	{
		UINT8* pArrow[4] = {nullptr, nullptr, nullptr, nullptr}; // { up, down, left, right }
		...
		for (INT32 i = nIndex; i < nGameInpCount; i++, nIndex++)
		{
			...
			if (static_cast<UINT32>(bii.szName[1] - '0') == (nPlayer + 1))
			{
				...
				if (_stricmp(" Right", bii.szName + 2) == 0)
				{
					// The last key value is ...
					nIndex++;
					break;
				}
			}
		...
		{
			// Create a Weak Punch + Weak Kick macro, Combination keys in the sfa3 series
			pgi->nInput = GIT_MACRO_AUTO;
			pgi->nType = BIT_DIGITAL;
			pgi->Macro.nMode = 0;
		...
		if (nPunchx3[nPlayer] == 7 && nKickx3[nPlayer] == 7)
		{
			// Create a Medium Punch + Medium Kick macro, Combination keys in the sfa3 series
			pgi->nInput = GIT_MACRO_AUTO;
			pgi->nType = BIT_DIGITAL;
			pgi->Macro.nMode = 0;
		...
		if (nPunchx3[nPlayer] == 7 && nKickx3[nPlayer] == 7)
		{
			// Create a Strong Punch + Strong Kick macro, Combination keys in the sfa3 series
			pgi->nInput = GIT_MACRO_AUTO;
			pgi->nType = BIT_DIGITAL;
			pgi->Macro.nMode = 0;
		...
		if (nPunchx3[nPlayer] == 7 && nKickx3[nPlayer] == 7)
		{
			// Create a Strong Punch + Weak Kick macro, Quick cancel technique in sf2ce series
			pgi->nInput = GIT_MACRO_AUTO;
			pgi->nType = BIT_DIGITAL;
			pgi->Macro.nMode = 0;
		...
		if (nRealButtons == 4 && HW_MISC)
		{
			// PGM & Other 4 key games;
			pgi->nInput = GIT_MACRO_AUTO;
			pgi->nType = BIT_DIGITAL;
			pgi->Macro.nMode = 0;
	...
	// LUA Hotkeys
	for (int hotkey_num = 0; lua_hotkeys[hotkey_num] != nullptr; hotkey_num++)
	{
		char hotkey_name[40];
...
INT32 GameInpInit()
{
	...
	GameInp = static_cast<struct GameInp*>(malloc(nSize));
	if (GameInp == nullptr)
	{
		return 1;
	}
...
static TCHAR* SliderInfo(struct GameInp* pgi, TCHAR* s)
{
	...
	pgi->Input.Slider.nSliderSpeed = static_cast<INT16>(_tcstol(s, &szRet, 0));
	s = szRet;
	if (s == nullptr)
	{
		return s;
	...
	pgi->Input.Slider.nSliderCenter = static_cast<INT16>(_tcstol(s, &szRet, 0));
	s = szRet;
	if (s == nullptr)
	{
		return s;
...
static INT32 StringToJoyAxis(struct GameInp* pgi, TCHAR* s)
{
	...
	pgi->Input.JoyAxis.nJoy = static_cast<UINT8>(_tcstol(s, &szRet, 0));
	if (szRet == nullptr)
	{
		return 1;
	}
	s = szRet;
	pgi->Input.JoyAxis.nAxis = static_cast<UINT8>(_tcstol(s, &szRet, 0));
	if (szRet == nullptr)
	{
		return 1;
	}
...
static INT32 StringToMouseAxis(struct GameInp* pgi, TCHAR* s)
{
	...
	pgi->Input.MouseAxis.nAxis = static_cast<UINT8>(_tcstol(s, &szRet, 0));
	if (szRet == nullptr)
	{
		return 1;
	}
...
static INT32 StringToMacro(struct GameInp* pgi, TCHAR* s)
{
	...
	if (szRet)
	{
		...
		pgi->Macro.Switch.nCode = static_cast<UINT16>(_tcstol(s, &szRet, 0));
		return 0;
	}
...
static INT32 StringToInp(struct GameInp* pgi, TCHAR* s)
{
	...
	if (szRet)
	{
		pgi->nInput = GIT_CONSTANT;
		s = szRet;
		pgi->Input.Constant.nConst = static_cast<UINT8>(_tcstol(s, &szRet, 0));
	...
	if (szRet)
	{
		...
		s = szRet;
		pgi->Input.Switch.nCode = static_cast<UINT16>(_tcstol(s, &szRet, 0));
		return 0;
	}
	...
	if (szRet)
	{
		...
		pgi->Input.Slider.SliderAxis.nSlider[0] = static_cast<UINT16>(_tcstol(s, &szRet, 0));
		s = szRet;
		if (s == nullptr)
		{
			return 1;
		}
		pgi->Input.Slider.SliderAxis.nSlider[1] = static_cast<UINT16>(_tcstol(s, &szRet, 0));
		s = szRet;
		if (s == nullptr)
		{
			return 1;
	...
	if (szRet)
	{
		...
		pgi->Input.Slider.JoyAxis.nJoy = static_cast<UINT8>(_tcstol(s, &szRet, 0));
		s = szRet;
		if (s == nullptr)
		{
			return 1;
		}
		pgi->Input.Slider.JoyAxis.nAxis = static_cast<UINT8>(_tcstol(s, &szRet, 0));
		s = szRet;
		if (s == nullptr)
		{
			return 1;
...
static UINT32 InputInfoToNum(TCHAR* szName)
{
	for (UINT32 i = 0; i < nGameInpCount; i++)
	{
		...
		if (_tcsicmp(szName, ANSIToTCHAR(bii.szInfo, nullptr, 0)) == 0)
		{
			return i;
		}
	}
...
static UINT32 InputNameToNum(TCHAR* szName)
{
	for (UINT32 i = 0; i < nGameInpCount; i++)
	{
		...
		if (_tcsicmp(szName, ANSIToTCHAR(bii.szName, nullptr, 0)) == 0)
		{
			return i;
		}
	}
...
TCHAR* InputNumToName(UINT32 i)
{
	...
	return ANSIToTCHAR(bii.szName, nullptr, 0);
}
...
static UINT32 MacroNameToNum(TCHAR* szName)
{
	...
	for (UINT32 i = 0; i < nMacroCount; i++, pgi++)
	{
		if (pgi->nInput & GIT_GROUP_MACRO)
		{
			if (_tcsicmp(szName, ANSIToTCHAR(pgi->Macro.szName, nullptr, 0)) == 0)
			{
				return i;
			}
		}
...
static INT32 AddCustomMacro(TCHAR* szValue, bool bOverWrite)
{
	...
	for (UINT32 j = nGameInpCount; j < nGameInpCount + nMacroCount; j++)
	{
		...
		{
			if (LabelCheck(szQuote, ANSIToTCHAR(GameInp[j].Macro.szName, nullptr, 0)))
			{
				nInput = j;
				break;
			}
	...
	_tcscpy(szQuote, ANSIToTCHAR(GameInp[nInput].Macro.szName, nullptr, 0));

	if ((szValue = LabelCheck(szEnd, _T("undefined"))) != nullptr)
	{
		nMode = 0;
	}
	else
	{
		if ((szValue = LabelCheck(szEnd, _T("switch"))) != nullptr)
		{
			if (bOverWrite || GameInp[nInput].Macro.nMode == 0)
			{
				GameInp[nInput].Macro.Switch.nCode = static_cast<UINT16>(_tcstol(szValue, &szValue, 0));
			}
	...
	if (nMode >= 0)
	{
		...
		for (INT32 i = 0; i < 4; i++)
		{
			GameInp[nInput].Macro.pVal[i] = nullptr;
			...
			for (UINT32 j = 0; j < nGameInpCount; j++)
			{
				...
				TCHAR* szString = LabelCheck(szQuote, ANSIToTCHAR(bii.szName, nullptr, 0));
				if (szString && szEnd)
				{
					GameInp[nInput].Macro.pVal[i] = bii.pVal;
					GameInp[nInput].Macro.nInput[i] = j;

					GameInp[nInput].Macro.nVal[i] = static_cast<UINT8>(_tcstol(szEnd, &szValue, 0));
...
tIniStruct gamehw_cfg[] = {
	{
		_T("CPS-1/CPS-2/CPS-3 hardware"), _T("config/presets/cps.ini"),
		{
			HARDWARE_CAPCOM_CPS1, HARDWARE_CAPCOM_CPS1_QSOUND, HARDWARE_CAPCOM_CPS1_GENERIC, HARDWARE_CAPCOM_CPSCHANGER,
			HARDWARE_CAPCOM_CPS2, HARDWARE_CAPCOM_CPS3, 0
		},
		"$info="
	},
