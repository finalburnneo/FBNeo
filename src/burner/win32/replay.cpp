...
bool bReplayFrameCounterDisplay = true;
TCHAR szFilter[1024];
INT32 movieFlags = 0;
...
UINT8* ReplayExternalData = nullptr;

#define MOVIE_FLAG_FROM_POWERON			(1<<1)
#define MOVIE_FLAG_WITH_NVRAM			(1<<2) // to be used w/FROM_POWERON (if selected)

constexpr UINT32 nMovieVersion = 0x0404;
constexpr UINT32 nMinimumMovieVersion = 0x0404; // anything lower will not work w/this version of FB Neo
...
static char* ReplayDecodeDateTime()
{
	...
	sprintf(ttime, "%02d/%02d/%04d @ %02d:%02d:%02d%s", MovieInfo.month + 1, MovieInfo.day,
	        2000 + (MovieInfo.year % 100), fixhour, MovieInfo.minute, MovieInfo.second,
	        (MovieInfo.hour > 12) ? "pm" : "am");
...
static void PrintInputs()
{
		...
		if (bii.pVal && bii.szInfo && bii.szInfo[0])
		{
			...
			if ((bii.nType & BIT_GROUP_ANALOG) && bii.pShortVal && *bii.pShortVal)
			{
				if (stricmp(bii.szInfo + 2, " x-axis") == 0 && bii.nType == BIT_GROUP_ANALOG)
				{
					...
					{
						if (static_cast<INT16>(*bii.pShortVal) > 0x80)
	...
	VidSNewJoystickMsg(nullptr); // Clear surface.

	// Draw shadows
	int got_shadow = 0;
	for (INT32 player = 0; player < 2; player++)
	{
		if (GetCurrentFrame() < nPrintInputsActive[player] + (60 * 5))
		{
			// time out np2active after 300 frames or so...
			INT32 nLen = (player == 1) ? _tcslen(lines[0]) : 0;
			swprintf(lines[0] + nLen, L"  ^  %c%c%c  ", OFFBUTTONS[player][0], OFFBUTTONS[player][1],
			         OFFBUTTONS[player][2]);
			swprintf(lines[1] + nLen, L" < >      ");
			swprintf(lines[2] + nLen, L"  v  %c%c%c  ", OFFBUTTONS[player][3], OFFBUTTONS[player][4],
			         OFFBUTTONS[player][5]);
	...
	for (INT32 player = 0; player < 2; player++)
	{
		...
		swprintf(lines[0] + nLen, L"  %c  %c%c%c  ", UDLR[player][0] ? '^' : ' ', BUTTONS[player][0],
		         BUTTONS[player][1], BUTTONS[player][2]);
		swprintf(lines[1] + nLen, L" %c %c      ", UDLR[player][2] ? '<' : ' ', UDLR[player][3] ? '>' : ' ');
		swprintf(lines[2] + nLen, L"  %c  %c%c%c  ", UDLR[player][1] ? 'v' : ' ', BUTTONS[player][3],
...
INT32 StartRecord()
{
...

	{
		...
		time_t nLocalTime = time(nullptr);
	...
	if (bStartFromReset)
	{
		...
		if (bWithNVRAM)
		{
			// only applies w/FROM_POWERON
			movieFlags |= MOVIE_FLAG_WITH_NVRAM;
		}

		if (!StartFromReset(nullptr, bWithNVRAM))
		{
	...
	{
		constexpr char szFileHeader[] = "FB1 "; // File identifier
		...
		else
		{
			...
			if (nRet >= 0)
			{
				constexpr char szChunkHeader[] = "FR1 "; // Chunk identifier
				INT32 nZero = 0;
...
INT32 StartReplay(const TCHAR* szFileName) // const char* szFileName = NULL
{
	...
	if (szFileName)
	{
		...
		if (!bReplayDontClose)
		{
			...
			DisplayReplayProperties(nullptr, false);
		}
	}
	else
	{
	...
	wszMetadata[0] = L'\0';
	{
		...
		if (memcmp(ReadHeader, szFileHeader, 4))
		{
		...
		else
		{
			...
			if (movieFlags & MOVIE_FLAG_FROM_POWERON)
			{
				// Starts from reset
			...
			{
				// Load the savestate associated with the recording
			...
			if (nRet == 0)
			{
				constexpr char szChunkHeader[] = "FR1 "; // Chunk identifier
				fread(ReadHeader, 1, 4, fp); // Read identifier
				if (memcmp(ReadHeader, szChunkHeader, 4))
				{
					// Not the right file type
					fclose(fp);
					fp = nullptr;
				...
				else
				{
					...
					nStartFrame = GetCurrentFrame();
					bReplayDontClose = false; // we don't need it anymore from this point
					...
					// Read metadata
					constexpr char szMetadataHeader[] = "FRM1";
...
static void CloseRecord()
{
	...
	if (nMetaLen > 0)
	{
		...
		constexpr char szChunkHeader[] = "FRM1";
		fwrite(szChunkHeader, 1, 4, fp);
		INT32 nMetaSize = nMetaLen * 2;
		fwrite(&nMetaSize, 1, 4, fp);
		auto metabuf = static_cast<UINT8*>(malloc(nMetaSize));
		INT32 i;
		for (i = 0; i < nMetaLen; ++i)
		{
...
{
	*ptr++ = static_cast<UINT8>(v & 0xff);
	*ptr++ = static_cast<UINT8>((v >> 8) & 0xff);
	*ptr++ = static_cast<UINT8>((v >> 16) & 0xff);
	*ptr++ = static_cast<UINT8>((v >> 24) & 0xff);
}

static inline UINT32 Read32(const UINT8*& ptr)
{
	UINT32 v;
	v = static_cast<UINT32>(*ptr++);
	v |= static_cast<UINT32>((*ptr++) << 8);
	v |= static_cast<UINT32>((*ptr++) << 16);
	v |= static_cast<UINT32>((*ptr++) << 24);
	return v;
}
...
{
	UINT16 v;
	v = static_cast<UINT16>(*ptr++);
	v |= static_cast<UINT16>((*ptr++) << 8);
	return v;
}
...
INT32 FreezeInput(UINT8** buf, INT32* size)
{
	...
	*buf = static_cast<UINT8*>(malloc(*size));
	if (!*buf)
	{
...
{
	UINT32 n = Read32(buf);
	if (n > 0x100 || static_cast<unsigned>(size) < (4 + 2 * n))
	{
...
static void DisplayPropertiesError(HWND hDlg, INT32 nErrType)
{
	if (hDlg != nullptr)
	{
		...
		case 0:
			...
			break;
		case 1:
			SetDlgItemTextW(hDlg, IDC_METADATA,
			                _T("ERROR: Incompatible file-type.  Try playback with an earlier version of FBNeo.\0"));
			break;
		case 2:
...
void DisplayReplayProperties(HWND hDlg, bool bClear)
{
	if (hDlg != nullptr)
	{
		...
		long lStringLength = SendDlgItemMessage(hDlg, IDC_CHOOSE_LIST, CB_GETLBTEXTLEN, static_cast<WPARAM>(lIndex), 0);
		if (lStringLength + 1 > MAX_PATH)
		{
			return;
		}

		SendDlgItemMessage(hDlg, IDC_CHOOSE_LIST, CB_GETLBTEXT, static_cast<WPARAM>(lIndex), (LPARAM)szChoice);
	...
	constexpr char szFileHeader[] = "FB1 "; // File identifier
	constexpr char szSavestateHeader[] = "FS1 "; // Chunk identifier
	constexpr char szRecordingHeader[] = "FR1 "; // Chunk identifier
	constexpr char szMetadataHeader[] = "FRM1"; // Chunk identifier
	char ReadHeader[4];
	INT32 nChunkSize = 0;
	...
	INT32 nFrames = 0;
	INT32 nUndoCount = 0;
	wchar_t* local_metadata = nullptr;
	...
	if (hDlg != nullptr)
	{
		...
		else
		{
			...
			SendDlgItemMessage(hDlg, IDC_READONLY, BM_SETCHECK, (bReplayReadOnly) ? BST_CHECKED : BST_UNCHECKED, 0);
	...
	if (!bStartFromReset || (bStartFromReset && movieFlagsTemp & MOVIE_FLAG_WITH_NVRAM))
	{
		memset(ReadHeader, 0, 4);
		fread(ReadHeader, 1, 4, fd); // Read identifier
		if (memcmp(ReadHeader, szSavestateHeader, 4))
		{
			// Not the chunk type
			fclose(fd);
	...
	memset(ReadHeader, 0, 4);
	fread(ReadHeader, 1, 4, fd); // Read identifier
	if (memcmp(ReadHeader, szRecordingHeader, 4))
	{
		// Not the chunk type
		fclose(fd);
		DisplayPropertiesError(hDlg, 1 /* most likely recorded w/ an earlier version */);
	...
	if (nThisMovieVersion < nMinimumMovieVersion)
	{
		// Uhoh, wrong format!
		fclose(fd);
		DisplayPropertiesError(hDlg, 1 /* most likely recorded w/ an earlier version */);
	...
	if (memcmp(ReadHeader, szMetadataHeader, 4) == 0)
	{
		...
		local_metadata = static_cast<wchar_t*>(malloc((nMetaLen + 1) * sizeof(wchar_t)));
	...
	if (hDlg != nullptr)
	{
		...
		sprintf(szStartType, "%s %s", (bStartFromReset) ? "Power-On" : "Savestate",
		        (bStartFromReset && movieFlagsTemp & MOVIE_FLAG_WITH_NVRAM) ? "w/NVRAM" : "");

		if (nFileVer)
			sprintf(szRecordedFrom, "%s, v%x.%x.%x.%02x", szStartType, nFileVer >> 20, (nFileVer >> 16) & 0x0F,
			        (nFileVer >> 8) & 0xFF, nFileVer & 0xFF);
		else
			...
			case IDOK:
				{
					...
					if (lIndex != CB_ERR)
					{
						if (lIndex == lCount - 1)
						{
							...
							if (nRet != 0)
							{
								LONG lOtherIndex = SendDlgItemMessage(hDlg, IDC_CHOOSE_LIST, CB_FINDSTRING,
								                                      static_cast<WPARAM>(-1), (LPARAM)szChoice);
								...
								{
									SendDlgItemMessage(hDlg, IDC_CHOOSE_LIST, CB_INSERTSTRING, lIndex,
									                   (LPARAM)szChoice);
