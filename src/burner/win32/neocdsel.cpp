...
static void NeoCDList_iso9660_CheckDirRecord(HWND hList, TCHAR* pszFile, FILE* fp, int nSector)
{
	...
	while (1)
	{
		...
		if (!(Flags & (1 << 1)))
		{
			...
			char szData[32];
			sprintf(szData, "%c%c%c%c%c%c%c", Data[0x100], Data[0x101], Data[0x102], Data[0x103], Data[0x104],
			        Data[0x105], Data[0x106]);
	...
	if (IsFileExt(pszFile, _T(".img")) || IsFileExt(pszFile, _T(".bin")))
	{
		...
		if (fp)
		{
			...
			if (lSize > (nSectorLength * 16))
			{
				...
				iso9660_ReadOffset(&IsoCheck[0], fp, nSectorLength * 16 + 1, 1, 5);
				// get Standard Identifier Field from PVD
...
static INT_PTR CALLBACK NeoCDList_WndProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		_stprintf(szDialogTitle, FBNLoadStringEx(hAppInst, IDS_NGCD_DIAG_TITLE, true), _T(APP_TITLE), _T(SEPERATOR_1),
		          _T(SEPERATOR_1));
