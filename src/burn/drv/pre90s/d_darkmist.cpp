...
static INT32 MemIndex()
{
	...
	DrvColPROM = Next;
	Next += 0x000400;
	DrvColTable = (UINT16*)Next;
	Next += 0x0400 * sizeof(UINT16);
	...
	layer_enable = Next;
	Next += 0x000004; // round up to 4 for alignment
	z80_bank = Next;
	Next += 0x000004;
	sprite_bank = Next;
	Next += 0x000004;
...
static void decrypt_gfx()
{
	...
	{
		rom[i] = buf[BITSWAP24(i, 23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 13, 12, 3, 2, 1, 11, 10, 9, 8, 0, 7, 6, 5,
		                       4)];
	}
	...
	for (INT32 i = 0; i < size; i++)
	{
		rom[i] = buf[BITSWAP24(i, 23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 13, 5, 4, 3, 2, 12, 11, 10, 9, 8, 1, 0, 7,
		                       6)];
	}
	...
	for (INT32 i = 0; i < size; i++)
	{
		rom[i] = buf[BITSWAP24(i, 23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 12, 11, 10, 9, 8, 5, 4, 3, 13, 7, 6, 1, 0,
		                       2)];
	}
	...
	for (INT32 i = 0; i < size; i++)
	{
		rom[i] = buf[BITSWAP24(i, 23, 22, 21, 20, 19, 18, 17, 16, 15, 6, 5, 4, 3, 2, 14, 13, 12, 11, 8, 7, 1, 0, 10,
		                       9)];
	}
	...
	for (INT32 i = 0; i < size; i++)
	{
		rom[i] = buf[BITSWAP24(i, 23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 5, 4, 3, 2, 11, 10, 9, 8, 13, 12, 1, 0, 7,
		                       6)];
	}
...
static void DrvPaletteInit()
{
	for (INT32 i = 0; i < 0x400; i++)
	{
		...
		else
		{
			...
			switch (i & 0x300)
			{
			case 0x000: ctabentry = ctabentry | 0x80;
				break;
			case 0x100: ctabentry = ctabentry | 0x00;
				break;
			case 0x200: ctabentry = ctabentry | 0x40;
				break;
			case 0x300: ctabentry = ctabentry | 0xc0;
				break;
			}
		}
...
{
	INT32 scrollx = ((DrvScrollRAM[0x02] << 1) & 0xfe) + (DrvScrollRAM[0x02] >> 7) + ((DrvScrollRAM[0x01] & 0xf0) << 4)
		+ ((DrvScrollRAM[0x01] & 0x01) << 12);
...
static void draw_fg_layer()
{
	...
	for (INT32 y = 0; y < 14 + scry2; y++)
	{
		...
		for (INT32 x = 0; x < 16 + scrx2; x++)
		{
			...
			if (y == 0 || y >= 14 || x == 0 || x >= 16)
				Render16x16Tile_Mask_Clip(pTransDraw, code, (x * 16) - scrx1, (y * 16) - scry1, color, 4, 0, 0x100,
				                          DrvGfxROM1);
			else
				Render16x16Tile_Mask(pTransDraw, code, (x * 16) - scrx1, (y * 16) - scry1, color, 4, 0, 0x100,
				                     DrvGfxROM1);
		}
	}
}
...
static void draw_txt_layer()
{
	for (INT32 offs = 32 * 2; offs < (32 * 32) - (32 * 2); offs++)
	{
		...
		RenderTileTranstab(pTransDraw, DrvGfxROM0, code, (color << 4) + 0x300, 0x4f, sx, sy - 16, 0, 0, 8, 8,
		                   DrvColPROM);
	}
}
...
static void draw_sprites()
{
	for (INT32 i = 0; i < 0x1000; i += 0x20)
	{
		...
		RenderTileTranstab(pTransDraw, DrvGfxROM2, code, (color << 4) + 0x200, 0x4f, sx, sy - 16, flipx, flipy, 16, 16,
		                   DrvColPROM);
	}
}
...
struct BurnDriver BurnDrvDarkmist = {
	"darkmist", nullptr, nullptr, nullptr, "1986",
	"The Lost Castle In Darkmist\0", nullptr, "Seibu Kaihatsu (Taito license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_TAITO_MISC, GBF_VERSHOOT, 0,
	nullptr, darkmistRomInfo, darkmistRomName, nullptr, nullptr, nullptr, nullptr, DarkmistInputInfo, DarkmistDIPInfo,
