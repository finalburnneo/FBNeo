...
int PrintGlobalMemoryInfo()
{
	...
	AddLine(_T("Physical RAM: %7i KB (%4i MB) total, %7i KB (%4i MB) avail"), stat.dwTotalPhys / 1024,
	        stat.dwTotalPhys / (1024 * 1024), stat.dwAvailPhys / 1024, stat.dwAvailPhys / (1024 * 1024));
	AddLine(_T("Total RAM:    %7i KB (%4i MB) total, %7i KB (%4i MB) avail"), stat.dwTotalPageFile / 1024,
	        stat.dwTotalPageFile / (1024 * 1024), stat.dwAvailPageFile / 1024, stat.dwAvailPageFile / (1024 * 1024));
	...
	if (hPsapiDLL)
	{
		...
		if (pGetProcessMemoryInfo)
		{
			...
			if (pGetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc)))
			{
				...
				AddLine(_T("%s%7i KB in use (%i KB peak, %i KB virtual)"), szLine, pmc.WorkingSetSize / 1024,
				        pmc.PeakWorkingSetSize / 1024, pmc.PagefileUsage / 1024);
			}
		}
...
HKEY FindMonitor(TCHAR* szHardwareID, TCHAR* szDriver)
{
	...
	for (int i = 0; !bFound && RegEnumKeyEx(hKey, i, szName, &nNameSize, NULL, szClass, &nClassSize, &ftLastWriteTime)
	     != ERROR_NO_MORE_ITEMS; i++)
	{
		...
		for (int j = 0; !bFound && RegEnumKeyEx(hSubKey, j, szSubName, &nSubNameSize, NULL, szSubClass, &nSubClassSize,
		                                        &ftSubLastWriteTime) != ERROR_NO_MORE_ITEMS; j++)
		{
...
int ProcessEDID(HKEY hMonitorKey)
{
	...
	if (RegQueryValueEx(hMonitorInfoKey, _T("EDID"), NULL, &nType, EDIDData, &nEDIDSize) == 0)
	{
		...
		AddLine(_T("        Display size ~%dx%dcm, Gamma %1.2lf"), EDIDData[0x15], EDIDData[0x16],
		        ((double)EDIDData[0x17] + 100.0) / 100.0);
					...
					for (int j = 0; pEnumDisplayDevices(ddAdapter.DeviceName, j, &ddDisplay, 0); j++)
					{
						...
						if (ddDisplay.StateFlags & DISPLAY_DEVICE_ACTIVE)
						{
							if (!_tcsnicmp(ddDisplay.DeviceKey, _T("\\Registry\\Machine\\"), 18))
							{
								...
								RegQueryValueEx(hMonitorKey, _T("DeviceDesc"), NULL, &nType, (BYTE*)szDeviceDesc,
								                &nDeviceDescSize);
		...
		for (int i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &did); i++)
		{
			...
			for (int j = 0; list[j]; j++)
			{
				if (!_tcsicmp(list[j], szClass))
				{
					...
					SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_LOWERFILTERS, NULL, (BYTE*)szService, 1024,
					                                 NULL);
					if (szService[0] == _T('\0'))
					{
						// If we haven't got any lower filter drivers, just use the service key
						SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_SERVICE, NULL, (BYTE*)szService, 1024,
						                                 NULL);
					}
			...
			if (HIWORD(wParam) == BN_CLICKED)
			{
				...
				if (LOWORD(wParam) == IDC_SYSINFO_LOG_SAVE)
				{
					...
					if (fp)
					{
						...
						if (nSize && szText)
						{
							SendDlgItemMessage(hDlg, IDC_SYSINFO_EDIT, WM_GETTEXT, (WPARAM)(nSize + 1) * sizeof(TCHAR),
							                   (LPARAM)szText);
