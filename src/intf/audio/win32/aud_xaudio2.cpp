...
static int XAudio2InitVoices()
{
	...
	if (FAILED(
		hr = pXAudio2->CreateMasteringVoice(&pMasterVoice, XAUDIO2_DEFAULT_CHANNELS, nAudSampleRate[1], 0, 0, NULL)))
	{
		return 1;
	}
	...
	if (FAILED(
			hr = pXAudio2->CreateSourceVoice(&pSourceVoice, &wfx, 0, XAUDIO2_DEFAULT_FREQ_RATIO, &voiceContext, NULL,
				NULL))
	)
	{
		return 1;
...
static int XAudio2GetSettings(InterfaceInfo* pInfo)
{
	...
	_sntprintf(szString, MAX_PATH, _T("Audio is delayed by approx. %ims"),
	           int(100000.0 / (nXAudio2Fps / (nAudSegCount - 1.0))));
	IntInfoAddStringModule(pInfo, szString);
	return 0;
}

struct AudOut AudOutXAudio2 = {
	XAudio2BlankSound, XAudio2Check, XAudio2Init, XAudio2SetCallback, XAudio2Play, XAudio2Stop, XAudio2Exit,
	XAudio2SetVolume, XAudio2GetSettings, _T("XAudio2 audio output")
};
//#else
