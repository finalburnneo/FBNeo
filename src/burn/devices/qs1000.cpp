...
static constexpr INT8 s_index_shift[8] = {-1, -1, -1, -1, 2, 4, 6, 8};
...
{
	// nibble to bit map
	static constexpr INT8 nbl2bit[16][4] =
	{
	...
	for (int step = 0; step <= 48; step++)
	{
		...
		int stepval = floor(16.0 * pow(11.0 / 10.0, step));
...
static UINT8* banked_rom = nullptr;
...
{
	return static_cast<INT32>(static_cast<float>(nBurnSoundLen * (pCPUTotalCycles() / (nDACCPUMHZ / (nBurnFPS /
		100.0000)))));
}
...
void qs1000_update(INT16* outputs, INT32 samples_len)
{
	...
	for (INT32 i = (nFractionalPosition & 0xFFFF0000) >> 15; i < (samples_len << 1); i += 2, nFractionalPosition +=
	     nSampleSize)
	{
		...
		nLeftSample[0] += static_cast<INT32>(pBufL[(nFractionalPosition >> 16) - 3]);
		nLeftSample[1] += static_cast<INT32>(pBufL[(nFractionalPosition >> 16) - 2]);
		nLeftSample[2] += static_cast<INT32>(pBufL[(nFractionalPosition >> 16) - 1]);
		nLeftSample[3] += static_cast<INT32>(pBufL[(nFractionalPosition >> 16) - 0]);

		nRightSample[0] += static_cast<INT32>(pBufR[(nFractionalPosition >> 16) - 3]);
		nRightSample[1] += static_cast<INT32>(pBufR[(nFractionalPosition >> 16) - 2]);
		nRightSample[2] += static_cast<INT32>(pBufR[(nFractionalPosition >> 16) - 1]);
		nRightSample[3] += static_cast<INT32>(pBufR[(nFractionalPosition >> 16) - 0]);

		nTotalLeftSample = INTERPOLATE4PS_16BIT((nFractionalPosition >> 4) & 0x0fff, nLeftSample[0], nLeftSample[1],
		                                        nLeftSample[2], nLeftSample[3]);
		nTotalRightSample = INTERPOLATE4PS_16BIT((nFractionalPosition >> 4) & 0x0fff, nRightSample[0], nRightSample[1],
		                                         nRightSample[2], nRightSample[3]);
...
{
	UINT32 table_addr = (m_channels[ch].m_regs[0x01] << 16) | (m_channels[ch].m_regs[0x02] << 8) | m_channels[ch].m_regs
		[0x03];
...
void qs1000_init(UINT8* program_rom, UINT8* samples, INT32 samplesize)
{
	...
	qs1000_p1_out = nullptr;
	qs1000_p2_out = nullptr;
	qs1000_p3_out = nullptr;
	qs1000_p1_in = nullptr;
	qs1000_p2_in = nullptr;
	qs1000_p3_in = nullptr;
	...
	// for resampling
	nSampleSize = static_cast<UINT64>(qs1000_rate) * (1 << 16) / nBurnSoundRate;
...
void qs1000_exit()
{
	...
	if (mixer_buffer_left)
	{
		...
		mixer_buffer_left = mixer_buffer_right = nullptr;
	}
}
...
void qs1000_set_write_handler(INT32 port, void (*handler)(UINT8))
{
	switch (port)
	{
	case 1: qs1000_p1_out = handler;
		return;
	case 2: qs1000_p2_out = handler;
		return;
	case 3: qs1000_p3_out = handler;
	}
}
...
void qs1000_set_read_handler(INT32 port, UINT8 (*handler)())
{
	switch (port)
	{
	case 1: qs1000_p1_in = handler;
		return;
	case 2: qs1000_p2_in = handler;
		return;
	case 3: qs1000_p3_in = handler;
	}
}
