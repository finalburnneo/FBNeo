...
using CpsObjDrawDoFn = INT32(*)(INT32, INT32);
using CpsScrXDrawDoFn = INT32(*)(UINT8*, INT32, INT32);
using CpsLayersDoFn = void(*)();
using CpsrPrepareDoFn = INT32(*)();
using CpsrRenderDoFn = INT32(*)();
...
static INT32 DrawStar(INT32 nLayer)
{
	...
	for (nStar = 0; nStar < 0x1000; nStar++)
	{
		...
		if (nStarColour != 0x0F)
		{
			nStarXPos = (((nStar >> 8) << 5) - *((INT16*)(CpsSaveReg[0] + 0x18 + (nLayer << 2))) + (nStarColour & 0x1F)
				- 64) & 0x01FF;
			nStarYPos = ((nStar & 0xFF) - *((INT16*)(CpsSaveReg[0] + 0x1A + (nLayer << 2))) - 16) & 0xFF;

			if (nStarXPos < 384 && nStarYPos < 224)
			{
				nStarColour = ((nStarColour & 0xE0) >> 1) + ((GetCurrentFrame() >> 4) % (
					(nStarColour & 0x80) ? 0xe : 0xf));
				PutPix(pBurnDraw + (nBurnPitch * nStarYPos) + (nBurnBpp * nStarXPos),
				       CpsPal[0x0800 + (nLayer << 9) + nStarColour]);
			}
		}
	}
...
static void Cps2Layers()
{
	...
	do
	{
		...
		INT32 nLayPri = (CpsSaveFrg[nSlice][4] << 8) | CpsSaveFrg[nSlice][5];
		// Layer priority register at word (400004)
