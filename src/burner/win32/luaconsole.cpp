...
struct ControlLayoutInfo
{
	...
	{
		NONE,
		// leave the control where it was
		RESIZE_END,
		// resize the control
...
static constexpr int numControlLayoutInfos = sizeof(controlLayoutInfos) / sizeof(*controlLayoutInfos);
...
void WinLuaOnStart(INT64 hDlgAsInt)
{
	...
	//LuaPerWindowInfo& info = LuaWindowInfo[hDlg];
	//info.started = true;
	EnableWindow(GetDlgItem(hDlg, IDC_BUTTON_LUABROWSE), false);
	// disable browse while running because it misbehaves if clicked in a frameadvance loop
...
void WinLuaOnStop(INT64 hDlgAsInt)
{
	...
	// bring to front among other script/secondary windows, since a stopped script will have some message for the user that would be easier to miss otherwise
	...
	case WM_INITDIALOG:
		{
			...
			int windowIndex = 0;
			//std::find(LuaScriptHWnds.begin(), LuaScriptHWnds.end(), hDlg) - LuaScriptHWnds.begin();
			...
			else if (static_cast<int>(r.left) - static_cast<int>(width2) > 0)
			{
				r.right -= width2;
				r.left -= width2;
			}
			...
			SystemParametersInfo(SPI_GETICONTITLELOGFONT, sizeof(LOGFONT), &LuaConsoleLogFont, 0);
			// reset with an acceptable font
			return true;
		}
		break;
	...
	case WM_SIZE:
		{
			...
			for (int i = 0; i < numControlLayoutInfos; i++)
			{
				...
				{
				case ControlLayoutInfo::RESIZE_END: width += deltaWidth;
					break;
				case ControlLayoutInfo::MOVE_START: x += deltaWidth;
					break;
				default: break;
				}
				switch (layoutInfo.verticalLayout)
				{
				case ControlLayoutInfo::RESIZE_END: height += deltaHeight;
					break;
				case ControlLayoutInfo::MOVE_START: y += deltaHeight;
					break;
				default: break;
				}

				SetWindowPos(hCtrl, nullptr, x, y, width, height, 0);
			...
			RedrawWindow(hDlg, nullptr, nullptr, RDW_INVALIDATE);
		}
		break;
		...
		case IDC_BUTTON_LUAEDIT:
			{
				...
				SendDlgItemMessage(hDlg,IDC_EDIT_LUAPATH,WM_GETTEXT, 512, (LPARAM)temp);
				// tell the OS to open the file with its associated editor,
				// without blocking on it or leaving a command window open.
				if ((INT_PTR)ShellExecuteA(nullptr, "edit", _TtoA(temp), nullptr, nullptr, SW_SHOWNORMAL) ==
					SE_ERR_NOASSOC)
					if ((INT_PTR)ShellExecuteA(nullptr, "open", _TtoA(temp), nullptr, nullptr, SW_SHOWNORMAL) ==
						SE_ERR_NOASSOC)
						ShellExecuteA(nullptr, nullptr, "notepad", _TtoA(temp), nullptr, SW_SHOWNORMAL);
			}
			break;
		...
		case IDC_BUTTON_LUABROWSE:
			{
				...
				ZeroMemory(&luaofn, sizeof(OPENFILENAME));
				...
				luaofn.Flags = OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_EXPLORER;
				// hide previously-ignored read-only checkbox (the real read-only box is in the open-movie dialog itself)
		...
		case IDC_EDIT_LUAPATH:
			{
				...
				EnableWindow(GetDlgItem(hDlg, IDOK), file != nullptr);
				if (file)
					fclose(file);
			}
			break;
