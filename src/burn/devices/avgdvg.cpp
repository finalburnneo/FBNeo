	...
	for (i = 0; i < nvect; i++)
	{
		if (vectbuf[i].status == VGVECTOR)
		{
			...
			if (has_clip)
			{
				...
				if (x0 < cx0)
				{
					y0 += static_cast<INT64>(cx0 - x0) * static_cast<INT64>(y1 - y0) / (x1 - x0);
					x0 = cx0;
				}
				else if (x0 > cx1)
				{
					y0 += static_cast<INT64>(cx1 - x0) * static_cast<INT64>(y1 - y0) / (x1 - x0);
					x0 = cx1;
				}
				if (x1 < cx0)
				{
					y1 += static_cast<INT64>(cx0 - x1) * static_cast<INT64>(y1 - y0) / (x1 - x0);
					x1 = cx0;
				}
				else if (x1 > cx1)
				{
					y1 += static_cast<INT64>(cx1 - x1) * static_cast<INT64>(y1 - y0) / (x1 - x0);
					x1 = cx1;
				}
				...
				if (y0 < cy0)
				{
					x0 += static_cast<INT64>(cy0 - y0) * static_cast<INT64>(x1 - x0) / (y1 - y0);
					y0 = cy0;
				}
				else if (y0 > cy1)
				{
					x0 += static_cast<INT64>(cy1 - y0) * static_cast<INT64>(x1 - x0) / (y1 - y0);
					y0 = cy1;
				}
				if (y1 < cy0)
				{
					x1 += static_cast<INT64>(cy0 - y1) * static_cast<INT64>(x1 - x0) / (y1 - y0);
					y1 = cy0;
				}
				else if (y1 > cy1)
				{
					x1 += static_cast<INT64>(cy1 - y1) * static_cast<INT64>(x1 - x0) / (y1 - y0);
					y1 = cy1;
				}
...
void avgdvg_go()
{
	...
	if (vector_engine == USE_DVG)
	{
		...
		last_cyc = static_cast<INT32>(static_cast<float>(1512000) / 1000000000 * 4500 * total_length);
	}
	...
	else
	{
		...
		{
			avgdvg_halt_next = pCPUTotalCycles();
			last_cyc = static_cast<INT32>(static_cast<float>(1512000) / 1000000000 * 2000 * total_length);
