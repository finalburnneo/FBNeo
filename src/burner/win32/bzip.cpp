...
static TCHAR* szBzipName[BZIP_MAX] = {nullptr,}; // Zip files to search through
...
static struct RomFind* RomFind = nullptr;
static int nRomCount = 0;
static int nTotalSize = 0;
static struct ZipEntry* List = nullptr;
...
static int FindRom(int i)
{
	...
	for (int nAka = 0; nAka < 0x10000; nAka++)
	{
		...
		nRet = FindRomByName(ANSIToTCHAR(szPossibleName, nullptr, 0));
		if (nRet >= 0)
		{
			return nRet;
		}
...
static int CheckRoms()
{
	for (int i = 0; i < nRomCount; i++)
	{
		...
		if (/*ri.nCrc &&*/ (ri.nType & BRF_OPT) == 0 && (ri.nType & BRF_NODUMP) == 0)
		{
			...
			if (ri.nType & BRF_ESS)
			{
				// essential rom - without it the game may not run at all
				nBzipError |= nError << 0;
			}
			if (ri.nType & BRF_PRG)
			{
				// rom which contains program information
				nBzipError |= nError << 1;
			}
			if (ri.nType & BRF_GRA)
			{
				// rom which contains graphics information
				nBzipError |= nError << 2;
			}
			if (ri.nType & BRF_SND)
			{
				// rom which contains sound information
				nBzipError |= nError << 3;
			}
		}
	}
...
static int __cdecl BzipBurnLoadRom(unsigned char* Dest, int* pnWrote, int i)
{
	...
	ProgressUpdateBurner(ri.nLen ? 1.0 / (static_cast<double>(nTotalSize) / ri.nLen) : 0, szText, false);
	...
	{
		// If we haven't got the right zip file currently open
		ZipClose();
		nCurrentZip = -1;
		if (ZipOpen(TCHARToANSI(szBzipName[nWantZip], nullptr, 0)))
		{
			return 1;
		}
	...
	if (ZipLoadFile(Dest, ri.nLen, pnWrote, RomFind[i].nPos))
	{
		...
		FBAPopupAddText(PUF_TEXT_DEFAULT, MAKEINTRESOURCE(nRet == 2 ? IDS_ERR_LOAD_DISK_CRC : IDS_ERR_LOAD_DISK),
		                pszRomName, GetFilenameW(szBzipName[nCurrentZip]));
...
int BzipOpen(bool bootApp)
{
	...
	{
		if (BurnDrvGetRomInfo(nullptr, nRomCount))
		{
			break;
		}
	}
	...
	RomFind = static_cast<struct RomFind*>(malloc(nMemLen));
	if (RomFind == nullptr)
	{
		return 1;
	}
	...
	for (int y = 0, z = 0; y < BZIP_MAX && z < BZIP_MAX; y++)
	{
		...
		{
			// Traverse the user-configured rom paths
			...
			{
				// Check existence of the rom zip/7z archive file

				bFound = true;

				szBzipName[z] = static_cast<TCHAR*>(malloc(MAX_PATH * sizeof(TCHAR)));
	...
	{
		if (szBzipName[z] == nullptr)
		{
			continue;
		}

		if (ZipOpen(TCHARToANSI(szBzipName[z], nullptr, 0)) == 0)
		{
			...
			for (int i = 0; i < nRomCount; i++)
			{
				...
				{
					// Already found this and it's okay
					continue;
				}
				...
				if (List[nFind].nLen == ri.nLen)
				{
					...
					{
						// If we know the CRC
						if (List[nFind].nCrc != ri.nCrc)
						{
							// Length okay, but CRC wrong
							RomFind[i].nState = 2;
						}
					}
				}
...
int BzipClose()
{
	...
	BurnExtLoadRom = nullptr; // Can't call our function to load each rom anymore
