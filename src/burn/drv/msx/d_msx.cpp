...
//
// TODO:
// 1: DONE.  Make a raw-input mode for keyboard input w/shifted chars? possibly?
// 2: Clean-up!
...
static UINT8 *RAM[8];                      /* Main RAM (8x8kB pages) */
static UINT8 *EmptyRAM = NULL;             /* Empty RAM page (8kB)   */
static UINT8 *MemMap[4][4][8];   /* Memory maps [PPage][SPage][Addr] */

static UINT8 *RAMData;                     /* RAM Mapper contents    */
static UINT8 RAMMapper[4];                 /* RAM Mapper state       */
static UINT8 RAMMask;                      /* RAM Mapper mask        */
static INT32 RAMPages    = 4;              /* Number of RAM pages    */

static UINT8 *ROMData[MAXSLOTS];           /* ROM Mapper contents    */
...
static UINT8 EnWrite[4];                   /* 1 if write enabled     */
static UINT8 PSL[4],SSL[4];                /* Lists of current slots */
static UINT8 PSLReg,SSLReg[4];   /* Storage for A8h port and (FFFFh) */
static UINT8 SCCOn[2];                     /* 1 = SCC page active    */

static UINT8 Kana, KanaByte;
...
static const UINT8 CartMap[4][4] =
{ { 255,3,4,5 },{ 0,0,0,0 },{ 1,1,1,1 },{ 2,255,255,255 } };

/** MegaROM Mapper Names *************************************/
static const char *ROMNames[MAXMAPPERS+1] = 
{ 
  "GENERIC/8kB\0","GENERIC/16kB\0","KONAMI5/8kB\0",
  "KONAMI4/8kB\0","ASCII/8kB\0","ASCII/16kB\0",
  "GMASTER2/SRAM\0","FMPAC/SRAM\0","UNKNOWN\0"
};

int LoadCart(UINT8 *cartbuf, int cartsize, int Slot, int Type);
void SetMegaROM(int Slot,UINT8 P0,UINT8 P1,UINT8 P2,UINT8 P3);
...
/*************************************************************/
void MapROM_w(UINT16 A, UINT8 V)
{
  UINT8 I,J,PS,SS,*P;
  ...
  if(((A&0xFF00)==0x9800)&&SCCOn[I])
  {
    ...
    /* as a scratch pad.                             */
    if(!ROMData[I]&&(offset<0x80)) EmptyRAM[0x1800+offset]=V;
  ...
  switch(ROMType[I])
  {
    case MAP_GEN8: /* Generic 8kB cartridges (Konami, etc.) */
      ...
      if(Verbose&0x08)
        bprintf(0, _T("ROM-MAPPER %c: 8kB ROM page #%d at %d:%d:%04Xh\n"),I+'A',V,PS,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_GEN16: /* Generic 16kB cartridges (MSXDOS2, HoleInOneSpecial) */
      ...
      if(Verbose&0x08)
        bprintf(0, _T("ROM-MAPPER %c: 16kB ROM page #%d at %d:%d:%04Xh\n"),I+'A',V>>1,PS,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_ASCII8: /* ASCII 8kB cartridges */
      ...
      if((A>=0x6000)&&(A<0x8000))
      {
        ...
        if(V&(ROMMask[I]+1))
        {
          ...
          P=SRAMData[I];
          if(Verbose&0x08)
			  bprintf(0, _T("ROM-MAPPER %c: 8kB SRAM at %d:%d:%04Xh\n"),I+'A',PS,SS,J*0x2000+0x4000);
#endif
		  P=0; // NO SRAM -DINK
        }
        else
...
/*************************************************************/
int GuessROM(const UINT8 *Buf,int Size)
{
...
int LoadCart(UINT8 *cartbuf, int cartsize, int Slot, int Type)
{
  ...
  if((Len>=2)&&((C1!='A')||(C2!='B')))
    {
      C1=cartbuf[0+0x2000*(Len-2)];
      C2=cartbuf[1+0x2000*(Len-2)];
    }

  /* If we can't find "AB" signature, drop out */     
  if((C1!='A')||(C2!='B'))
  {
	bprintf(0, _T("  Not a valid cartridge ROM\n"));
    return(0);
  }

  bprintf(0, _T("  Cartridge %c: %dkB "),'A'+Slot,Len*8);
...
static INT32 DrvInit()
{
	...
	ppi8255_init(1);
	//PPI0PortReadA	= msx_ppi8255_portA_read;
	PPI0PortReadB	= msx_ppi8255_portB_read;
	//PPI0PortReadC	= msx_ppi8255_portC_read;
...
static struct BurnRomInfo msx_msxRomDesc[] = {
    { "msx.rom",     0x8000, 0x94ee12f3, BRF_BIOS }, // 0x80 - standard bios
...
static struct BurnRomInfo MSX_msxbasicRomDesc[] = {
	{ "msx.rom",	0x08000, 0x94ee12f3, BRF_PRG | BRF_ESS },
};
...
// MSX-FAN Fandom Library 1 (Jpn)

static struct BurnRomInfo MSX_fandom1RomDesc[] = {
	{ "lh532046",	0x40000, 0x519be53e, BRF_PRG | BRF_ESS },
};

STDROMPICKEXT(MSX_fandom1, MSX_fandom1, msx_msx)
STD_ROM_FN(MSX_fandom1)

struct BurnDriver BurnDrvMSX_fandom1 = {
	"MSX_fandom1", NULL, "msx_msx", NULL, "1988",
	"MSX-FAN Fandom Library 1 (Jpn)\0", NULL, "Tecnopolis", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_ASCII8, GBF_MISC, 0,
	MSXGetZipName, MSX_fandom1RomInfo, MSX_fandom1RomName, NULL, NULL, MSXInputInfo, MSXDIPInfo,
	DrvInit, DrvExit, DrvFrame, TMS9928ADraw, DrvScan, NULL, 0x10,
	272, 228, 4, 3
};


// MSX-FAN Fandom Library 2 (Jpn)

static struct BurnRomInfo MSX_fandom2RomDesc[] = {
	{ "lh532061",	0x40000, 0x53c25b6b, BRF_PRG | BRF_ESS },
};

STDROMPICKEXT(MSX_fandom2, MSX_fandom2, msx_msx)
STD_ROM_FN(MSX_fandom2)

struct BurnDriver BurnDrvMSX_fandom2 = {
	"MSX_fandom2", NULL, "msx_msx", NULL, "1988",
	"MSX-FAN Fandom Library 2 (Jpn)\0", NULL, "Tecnopolis", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_ASCII8, GBF_MISC, 0,
	MSXGetZipName, MSX_fandom2RomInfo, MSX_fandom2RomName, NULL, NULL, MSXInputInfo, MSXDIPInfo,
	DrvInit, DrvExit, DrvFrame, TMS9928ADraw, DrvScan, NULL, 0x10,
	272, 228, 4, 3
};


// MSX-FAN Fandom Library 3 (Jpn)

static struct BurnRomInfo MSX_fandom3RomDesc[] = {
	{ "lh532075",	0x40000, 0x790d5335, BRF_PRG | BRF_ESS },
};

STDROMPICKEXT(MSX_fandom3, MSX_fandom3, msx_msx)
STD_ROM_FN(MSX_fandom3)

struct BurnDriver BurnDrvMSX_fandom3 = {
	"MSX_fandom3", NULL, "msx_msx", NULL, "1988",
	"MSX-FAN Fandom Library 3 (Jpn)\0", NULL, "Tecnopolis", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_ASCII8, GBF_MISC, 0,
	MSXGetZipName, MSX_fandom3RomInfo, MSX_fandom3RomName, NULL, NULL, MSXInputInfo, MSXDIPInfo,
	DrvInit, DrvExit, DrvFrame, TMS9928ADraw, DrvScan, NULL, 0x10,
	272, 228, 4, 3
};
...
static struct BurnRomInfo MSX_rtypeRomDesc[] = {
	{ "lh532163",	0x40000, 0xc4c12176, BRF_PRG | BRF_ESS },
};
...
struct BurnDriver BurnDrvMSX_rtype = {
	...
	NULL, NULL, NULL, NULL,
	0, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_RTYPE, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_rtypea = {
	...
	NULL, NULL, NULL, NULL,
	0 | BDF_CLONE, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_RTYPE, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_amc = {
	...
	"Astro Marine Corps (Spa)\0", NULL, "Dinamic Software?", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX | HARDWARE_MSX_MAPPER_ASCII16, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_bcquest = {
	...
	"B.C.'s Quest (Jpn)\0", NULL, "Toshiba EMI", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_bcquesta = {
	...
	"B.C.'s Quest (Jpn, Alt)\0", NULL, "Toshiba EMI", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_butampan = {
	...
	"Butam Pants (Jpn)\0", NULL, "HAL Kenkyuujo", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_gokiburi = {
	...
	"Gokiburi Daisakusen (Jpn)\0", NULL, "Magicsoft", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_grogrev = {
	...
	"Grog's Revenge (Jpn)\0", NULL, "Comptiq", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_grogreva = {
	...
	"Grog's Revenge (Jpn, Alt)\0", NULL, "Comptiq", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MSX, GBF_MISC, 0,
...
struct BurnDriver BurnDrvMSX_gabalin = {
	...
	"Mashou no Yakata - Gabalin (Jpn)\0", NULL, "Pony Canyon", "MSX",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MSX, GBF_MISC, 0,
