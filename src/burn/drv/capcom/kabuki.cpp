                   ...
                   INT32 base_addr, INT32 length, INT32 swap_key1, INT32 swap_key2, INT32 addr_key, INT32 xor_key)
{
	...
	for (A = 0; A < length; A++)
	{
		...
		dest_op[A] = static_cast<UINT8>(bytedecode(src[A], swap_key1, swap_key2, xor_key, select));

		/* decode data */
		select = ((A + base_addr) ^ 0x1fc0) + addr_key + 1;
		dest_data[A] = static_cast<UINT8>(bytedecode(src[A], swap_key1, swap_key2, xor_key, select));
	}
}
