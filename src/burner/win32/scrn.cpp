...
HWND hScrnWnd = nullptr; // Handle to the screen window
HWND hRebar = nullptr; // Handle to the Rebar control containing the menu
...
static char* CreateKailleraList()
{
	...
	int nSize = 256 * 1024;
	auto pList = static_cast<char*>(malloc(nSize));
	char* pName = pList;
		...
		for (nBurnDrvActive = 0; nBurnDrvActive < nBurnDrvCount; nBurnDrvActive++)
		{
			if (CheckFavorites(BurnDrvGetTextA(DRV_NAME)) != -1)
			{
				...
				if (pName + strlen(szDecoratedName) >= pList + nSize)
				{
					char* pNewList;
					nSize <<= 1;
					pNewList = static_cast<char*>(realloc(pList, nSize));
					if (pNewList == nullptr)
					{
						return nullptr;
					}
...
static int WINAPI gameCallback(char* game, int player, int numplayers)
{
	...
	TCHAR szTemp1[256];
	TCHAR szTemp2[256];
	VidSAddChatMsg(FBALoadStringEx(hAppInst, IDS_NETPLAY_START, true), 0xFFFFFF, BurnDrvGetText(DRV_FULLNAME),
	               0xFFBFBF);
...
static void WINAPI kChatCallback(char* nick, char* text)
{
	...
	VidSAddChatMsg(szTemp, 0xBFBFFF, ANSIToTCHAR(text, nullptr, 0), 0x7F7FFF);
}
...
static void WINAPI kDropCallback(char* nick, int playernb)
{
	...
	VidSAddChatMsg(szTemp, 0xFFFFFF, nullptr, 0);
}
...
static void DoNetGame()
{
	...
	ki.moreInfosCallback = nullptr;

	Kaillera_Set_Infos(&ki);

	Kaillera_Select_Server_Dialog(nullptr);
...
int CreateDatfileWindows(int bType)
{
	...
	if (bType == DAT_NEOGEO_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Neo Geo only"));
	if (bType == DAT_MEGADRIVE_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Megadrive only"));
	if (bType == DAT_PCENGINE_ONLY)
		_sntprintf(szConsoleString, 64, _T(", PC-Engine only"));
	if (bType == DAT_TG16_ONLY)
		_sntprintf(szConsoleString, 64, _T(", TurboGrafx16 only"));
	if (bType == DAT_SGX_ONLY)
		_sntprintf(szConsoleString, 64, _T(", SuprGrafx only"));
	if (bType == DAT_SG1000_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Sega SG-1000 only"));
	if (bType == DAT_COLECO_ONLY)
		_sntprintf(szConsoleString, 64, _T(", ColecoVision only"));
	if (bType == DAT_MASTERSYSTEM_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Master System only"));
	if (bType == DAT_GAMEGEAR_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Game Gear only"));
	if (bType == DAT_MSX_ONLY)
		_sntprintf(szConsoleString, 64, _T(", MSX 1 Games only"));
	if (bType == DAT_SPECTRUM_ONLY)
		_sntprintf(szConsoleString, 64, _T(", ZX Spectrum Games only"));
	if (bType == DAT_NES_ONLY)
		_sntprintf(szConsoleString, 64, _T(", NES Games only"));
	if (bType == DAT_FDS_ONLY)
		_sntprintf(szConsoleString, 64, _T(", FDS Games only"));
	if (bType == DAT_NGP_ONLY)
		_sntprintf(szConsoleString, 64, _T(", NeoGeo Pocket Games only"));
	if (bType == DAT_CHANNELF_ONLY)
		_sntprintf(szConsoleString, 64, _T(", Fairchild Channel F Games only"));

	TCHAR szProgramString[25];
	_sntprintf(szProgramString, 25, _T("ClrMame Pro XML"));

	_sntprintf(szChoice, MAX_PATH, _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), szAppBurnVer, szProgramString,
	           szConsoleString);
...
int CreateAllDatfilesWindows()
{
	...
	{
		// Browse dialog returned non-filesystem path
	...
	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(""));
	create_datfile(szFilename, DAT_ARCADE_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Neo Geo only"));
	create_datfile(szFilename, DAT_NEOGEO_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Megadrive only"));
	create_datfile(szFilename, DAT_MEGADRIVE_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", PC-Engine only"));
	create_datfile(szFilename, DAT_PCENGINE_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", TurboGrafx16 only"));
	create_datfile(szFilename, DAT_TG16_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", SuprGrafx only"));
	create_datfile(szFilename, DAT_SGX_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Sega SG-1000 only"));
	create_datfile(szFilename, DAT_SG1000_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", ColecoVision only"));
	create_datfile(szFilename, DAT_COLECO_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Master System only"));
	create_datfile(szFilename, DAT_MASTERSYSTEM_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Game Gear only"));
	create_datfile(szFilename, DAT_GAMEGEAR_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", MSX 1 Games only"));
	create_datfile(szFilename, DAT_MSX_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", ZX Spectrum Games only"));
	create_datfile(szFilename, DAT_SPECTRUM_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", NES Games only"));
	create_datfile(szFilename, DAT_NES_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", FDS Games only"));
	create_datfile(szFilename, DAT_FDS_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Neo Geo Pocket Games only"));
	create_datfile(szFilename, DAT_NGP_ONLY);

	_sntprintf(szFilename, MAX_PATH, _T("%s") _T(APP_TITLE) _T(" v%.20s (%s%s).dat"), buffer, szAppBurnVer,
	           szProgramString, _T(", Fairchild Channel F Games only"));
...
static LRESULT CALLBACK ScrnProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSCOMMAND:
		{
			if (OnSysCommand(hWnd, static_cast<UINT>(wParam), static_cast<short>(LOWORD(lParam)),
...
static int OnMouseMove(HWND hwnd, int x, int y, UINT keyIndicators)
{
	if (bDrag && hwnd == hScrnWnd && keyIndicators == MK_LBUTTON && !nVidFullscreen && !bMenuEnabled)
	{
		...
		if ((nLeftButtonX - (clientRect.left + x)) < nDragX && (nLeftButtonX - (clientRect.left + x)) > -nDragX && (
			nLeftButtonY - (clientRect.top + y)) < nDragY && (nLeftButtonY - (clientRect.top + y)) > -nDragY)
		{
			SetWindowPos(hScrnWnd, nullptr, nOldWindowX, nOldWindowY, 0, 0, SWP_NOREPOSITION | SWP_NOSIZE);
		}
		else
		{
			...
			SetWindowPos(hScrnWnd, nullptr, nWindowPosX, nWindowPosY, 0, 0, SWP_NOREPOSITION | SWP_NOSIZE);
		}
...
static int OnLButtonUp(HWND hwnd, int x, int y, UINT)
{
	...
	if (nVidFullscreen)
	{
		...
		if (UseDialogs())
		{
			...
			TrackPopupMenuEx(hMenuPopup, TPM_LEFTALIGN | TPM_TOPALIGN, clientRect.left + x, clientRect.top + y,
			                 hScrnWnd, nullptr);
			return 0;
		}
	}
...
{
	bHasFocus = fActivate;
	if (!kNetGame && bAutoPause && !bAltPause && hInpdDlg == nullptr && hInpCheatDlg == nullptr && hInpDIPSWDlg ==
		nullptr)
	{
...
{
	if (bVidOkay && bRunPause && bDrvOkay && (hSelDlg == nullptr))
	{
		// Redraw the screen to show certain messages while paused. - dink
...
static void OnPaint(HWND hWnd)
{
	if (hWnd == hScrnWnd)
	{
		...
		if (!nVidFullscreen)
		{
			RedrawWindow(hRebar, nullptr, nullptr, RDW_FRAME /*| RDW_UPDATENOW*/ | RDW_ALLCHILDREN);
		}
	}
}
...
static void OnDestroy(HWND)
{
	...
	hScrnWnd = nullptr; // Make sure handle is not used again
}
...
void ScrnInitLua()
{
	if (UseDialogs())
	{
		...
		{
			InputSetCooperativeLevel(false, bAlwaysProcessKeyboardInput);
			LuaConsoleHWnd = CreateDialog(hAppInst, MAKEINTRESOURCE(IDD_LUA), NULL, DlgLuaScriptDialog);
		}
		else
		{
...
static void OnCommand(HWND /*hDlg*/, int id, HWND /*hwndCtl*/, UINT codeNotify)
{
	...
	case MENU_STARTNET:
		if (Init_Network())
		{
			MessageBox(hScrnWnd, FBALoadStringEx(hAppInst, IDS_ERR_NO_NETPLAYDLL, true),
			           FBALoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
			break;
		}
#ifdef BUILD_A68K
	...
	case MENU_MEMCARD_CREATE:
		if (bDrvOkay && UseDialogs() && !kNetGame && (BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) ==
			HARDWARE_SNK_NEOGEO)
		{
			InputSetCooperativeLevel(false, bAlwaysProcessKeyboardInput);
			AudBlankSound();
	...
	case MENU_ASPECTNORMALVERL:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 4;
		nVidVerScrnAspectY = 3;
		POST_INITIALISE_MESSAGE;
		break;
	case MENU_ASPECTLCDVERL:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 5;
		nVidVerScrnAspectY = 4;
		POST_INITIALISE_MESSAGE;
		break;
	...
	case MENU_ASPECTWIDELCDVERL:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 16;
		nVidVerScrnAspectY = 10;
		POST_INITIALISE_MESSAGE;
		break;
	case MENU_ASPECTNORMALVERP:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 3;
		nVidVerScrnAspectY = 4;
		POST_INITIALISE_MESSAGE;
		break;
	case MENU_ASPECTLCDVERP:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 4;
		nVidVerScrnAspectY = 5;
		POST_INITIALISE_MESSAGE;
		break;
	case MENU_ASPECTWIDEVERP:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 9;
		nVidVerScrnAspectY = 16;
		POST_INITIALISE_MESSAGE;
		break;
	case MENU_ASPECTWIDELCDVERP:
		bMonitorAutoCheck = false;
		nVidVerScrnAspectX = 10;
		nVidVerScrnAspectY = 16;
		POST_INITIALISE_MESSAGE;
		break;
	...
	case MENU_INPUT_AUTOFIRE_RATE_1: nAutoFireRate = 24;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_2: nAutoFireRate = 12;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_3: nAutoFireRate = 8;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_4: nAutoFireRate = 6;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_5: nAutoFireRate = 4;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_6: nAutoFireRate = 2;
		break;
	...
	case MENU_INPUT_REWIND_ENABLED:
		...
		StateRewindReInit();
		break;
	case MENU_INPUT_REWIND_128MB: nRewindMemory = 128;
		StateRewindReInit();
		break;
	case MENU_INPUT_REWIND_256MB: nRewindMemory = 256;
		StateRewindReInit();
		break;
	case MENU_INPUT_REWIND_512MB: nRewindMemory = 512;
		StateRewindReInit();
		break;
	case MENU_INPUT_REWIND_768MB: nRewindMemory = 768;
		StateRewindReInit();
		break;
	case MENU_INPUT_REWIND_1GB: nRewindMemory = 1024;
		StateRewindReInit();
		break;
	...
	case MENU_CHEATSEARCH_START:
		{
			if (CheatSearchStart() == 0)
			{
				...
				VidSAddChatMsg(nullptr, 0xFFFFFF, szText, 0xFFBFBF);
	...
	case MENU_CHEATSEARCH_NOCHANGE:
		{
			...
			VidSAddChatMsg(nullptr, 0xFFFFFF, tmpmsg, 0xFFBFBF);

			if (nValues <= CHEATSEARCH_SHOWRESULTS)
			{
				for (unsigned int i = 0; i < nValues; i++)
				{
					_stprintf(tmpmsg, FBALoadStringEx(hAppInst, IDS_CHEAT_SEARCH_RESULTS, true),
					          CheatSearchShowResultAddresses[i], CheatSearchShowResultValues[i]);
					VidSAddChatMsg(nullptr, 0xFFFFFF, tmpmsg, 0xFFBFBF);
				}
			}
			break;
	...
	case MENU_CONTENTS:
		{
			if (!nVidFullscreen)
			{
				...
				{
					fclose(fp);
					ShellExecute(nullptr, _T("open"), _T("fbneo.chm"), nullptr, nullptr, SW_SHOWNORMAL);
				}
			}
			break;
		}
	...
	case MENU_WHATSNEW:
		{
			if (!nVidFullscreen)
			{
				...
				{
					fclose(fp);
					ShellExecute(nullptr, _T("open"), _T("whatsnew.html"), nullptr, nullptr, SW_SHOWNORMAL);
				}
			}
			break;
		}
	...
	case MENU_WWW_HOME:
		if (!nVidFullscreen)
		{
			ShellExecute(nullptr, _T("open"), _T("https://neo-source.com/"), nullptr, nullptr, SW_SHOWNORMAL);
		}
		break;
	...
	case MENU_WWW_GITHUB:
		if (!nVidFullscreen)
		{
			ShellExecute(nullptr, _T("open"), _T("https://github.com/finalburnneo/FBNeo"), nullptr, nullptr,
			             SW_SHOWNORMAL);
		}
		break;
			...
			case MENU_ENHANCED_SOFT_CRTx44:
				{
					...
					nVidBlitterOpt[nVidSelect] |= 0x03000000 + (static_cast<long long>(id - MENU_ENHANCED_SOFT_STRETCH)
						<< 32);
					POST_INITIALISE_MESSAGE;
					break;
				}
	...
	if (hwndChat)
	{
		...
		case EN_CHANGE:
			{
				bEditTextChanged = true;
				SendMessage(hwndChat, WM_GETTEXT, static_cast<WPARAM>(MAX_CHAT_SIZE) + 1, (LPARAM)EditText);
				break;
			}
		case EN_KILLFOCUS:
			{
...
static void OnEnterIdle(HWND /*hwnd*/, UINT /*source*/, HWND /*hwndSource*/)
{
	...
	while (kNetGame && !PeekMessage(&Message, nullptr, 0, 0, PM_NOREMOVE))
	{
		RunIdle();
	}
}
...
int ScrnSize()
{
	...
	if (hScrnWnd == nullptr || nVidFullscreen)
	{
		return 1;
	}
	...
	if (nVidSelect == 2 && nVidBlitterOpt[2] & 0x0100)
	{
		// The Software effects blitter uses a fixed size
		nMaxSize = 9;
	}
	else
	{
	...
	if ((bVidCorrectAspect || bVidFullStretch) && !(nVidSelect == 2 && (nVidBlitterOpt[2] & 0x0100) == 0))
	{
		...
		do
		{
			if (nBmapWidth < nBmapHeight && bVidScanRotate)
			{
				...
				if (hh > static_cast<INT64>(ww) * nVidScrnAspectX * nGameAspectY * nScrnHeight / (nScrnWidth *
					nVidScrnAspectY * nGameAspectX))
				{
					hh = static_cast<INT64>(ww) * nVidScrnAspectX * nGameAspectY * nScrnHeight / (nScrnWidth *
						nVidScrnAspectY * nGameAspectX);
				}
			}
			else
			{
				...
				if (ww > static_cast<INT64>(hh) * nVidScrnAspectY * nGameAspectX * nScrnWidth / (nScrnHeight *
					nVidScrnAspectX * nGameAspectY))
				{
					ww = static_cast<INT64>(hh) * nVidScrnAspectY * nGameAspectX * nScrnWidth / (nScrnHeight *
						nVidScrnAspectX * nGameAspectY);
				}
			}
		}
...
int ScrnTitle()
{
	...
	if (bDrvOkay)
	{
		...
		pszPosition += _sntprintf(szText, 1024, _T(APP_TITLE) _T(" v%.20s") _T(SEPERATOR_1) _T("%s"), szAppBurnVer,
		                          pszName);
		while ((pszName = BurnDrvGetText(DRV_NEXTNAME | DRV_FULLNAME)) != nullptr)
		{
	...
	{
		_stprintf(szText, _T(APP_TITLE) _T(" v%.20s") _T(SEPERATOR_1) _T("[%s]"), szAppBurnVer,
		          FBALoadStringEx(hAppInst, IDS_SCRN_NOGAME, true));
	}
...
int ScrnInit()
{
	...
	hScrnWnd = CreateWindowEx(nWindowExStyles, szClass, _T(APP_TITLE), nWindowStyles,
	                          ...
	                          nullptr, nullptr, hAppInst, nullptr);
	...
	if (!nVidFullscreen)
	{
		...
		if (bMenuEnabled)
		{
			...
			hRebar = CreateWindowEx(WS_EX_TOOLWINDOW,
			                        ...
			                        RBS_BANDBORDERS | CCS_NOPARENTALIGN | CCS_NODIVIDER | WS_BORDER | WS_CHILD |
			                        WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE,
			                        0, 0, 0, 0,
			                        hScrnWnd, nullptr, hAppInst, nullptr);

			rebarInfo.cbSize = sizeof(REBARINFO);
			rebarInfo.fMask = 0;
			rebarInfo.himl = nullptr;
			...
			SendMessage(hRebar, RB_INSERTBAND, static_cast<WPARAM>(-1), (LPARAM)&rebarBandInfo);
