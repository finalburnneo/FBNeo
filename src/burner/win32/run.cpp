...
static bool bAppDoStep = false;
static bool bAppDoFast = false;
static bool bAppDoFasttoggled = false;
static bool bAppDoRewind = false;
...
static void CheckSystemMacros() // These are the Pause / FFWD macros added to the input dialog
{
	...
	if (!kNetGame)
	{
		...
		// SlowMo
		int slow = macroSystemSlowMo[0] * 1 + macroSystemSlowMo[1] * 2 + macroSystemSlowMo[2] * 3 + macroSystemSlowMo[3]
			* 4 + macroSystemSlowMo[4] * 5;
		if (slow)
		{
			slow -= 1;
			if (slow >= 0 && slow <= 4)
		...
		for (int hotkey_num = 0; hotkey_debounces[hotkey_num].macroSystemLuaHotkey_ref != nullptr; hotkey_num++)
		{
			...
			hotkey_debounces[hotkey_num].macroSystemLuaHotkey_val = *hotkey_debounces[hotkey_num].
				macroSystemLuaHotkey_ref;
			if (
			...
			{
				CallRegisteredLuaFunctions(static_cast<LuaCallID>(LUACALL_HOTKEY_1 + hotkey_num));
			}
...
static void DisplayFPS()
{
	...
	double fps = static_cast<double>(nFPSTotal - nPreviousFrames) * CLOCKS_PER_SEC / (temptime - fpstimer);

	_sntprintf(fpsstring, 7, _T("%2.2lf"), fps);
	if (fpstimer && temptime - fpstimer > 0)
	{
		// avoid strange fps values
...
int RunFrame(int bDraw, int bPause)
{
	...
	else
	{
		...
		if (kNetGame)
		{
			...
			if (KailleraGetInput())
			{
				// Synchronize input with Kaillera
				return 0;
			}
		}
		else
		{
			if (nReplayStatus == 2)
			{
				...
				if (ReplayInput())
				{
					...
					SetPauseMode(true); // Replay has finished
					bAppDoFast = false;
					bAppDoFasttoggled = false; // Disable FFWD
					MenuEnableItems();
		...
		if (bDraw)
		{
			...
			if (!bRunAhead || (BurnDrvGetFlags() & BDF_RUNAHEAD_DISABLED) || bAppDoFast)
			{
				if (VidFrame())
				{
					// Do one normal frame (w/o RunAhead)
					...
					pBurnDraw = nullptr; // Make sure no image is drawn
					BurnDrvFrame();
			...
			{
				pBurnDraw = (BurnDrvGetFlags() & BDF_RUNAHEAD_DRAWSYNC) ? pVidImage : nullptr;
				BurnDrvFrame();
				StateRunAheadSave();
				...
				if (VidFrame())
				{
					...
					pBurnDraw = nullptr; // Make sure no image is drawn, since video failed this time 'round.
					BurnDrvFrame();
				}
		...
		{
			// frame skipping / ffwd-frame (without avi writing)
			pBurnDraw = nullptr; // Make sure no image is drawn
			BurnDrvFrame();
		}
...
static int RunGetNextSound(int bDraw)
{
	...
	if (bRunPause)
	{
		if (bAppDoStep)
		{
			bAppDoStep = false; // done one step
			RunFrame(bDraw, 0);
	...
	if (WaveLog != nullptr && pBurnSoundOut != nullptr)
	{
		// log to the file
...
void (*cBurnerKeyCallback)(UINT8 code, UINT8 KeyType, UINT8 down) = nullptr;
...
int RunMessageLoop()
{
	...
	do
	{
		...
		while (PeekMessage(&Msg, nullptr, WM_APP + 0, WM_APP + 0, PM_NOREMOVE))
		{
			if (Msg.message != WM_QUIT)
			{
				PeekMessage(&Msg, nullptr, WM_APP + 0, WM_APP + 0, PM_REMOVE);
			}
		}
						...
						case 'S':
							{
								...
								if (bMute)
								{
									...
									_stprintf(buffer, FBALoadStringEx(hAppInst, IDS_SOUND_MUTE, true),
									          nAudVolume / 100);
								}
								else
								{
									nAudVolume = nOldAudVolume; // restore volume
									_stprintf(buffer, FBALoadStringEx(hAppInst, IDS_SOUND_MUTE_OFF, true),
									          nAudVolume / 100);
								}
						...
						case VK_OEM_PLUS:
							{
								...
								{
									_stprintf(buffer, FBALoadStringEx(hAppInst, IDS_SOUND_VOLUMESET, true),
									          nAudVolume / 100);
									VidSNewShortMsg(buffer);
								}
								MenuUpdateVolume();
						...
						case VK_RETURN:
							{
								if (hwndChat)
								{
									...
									if (i)
									{
										Kaillera_Chat_Send(TCHARToANSI(EditText, nullptr, 0));
