...
static HWND hLocalDownDlg = nullptr;
...
{
	HINTERNET connect = InternetOpen(_T("MyBrowser"), INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);

	if (!connect)
	{
		MessageBox(hLocalDownDlg, FBALoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_CONNECT, true),
		           FBALoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
		return 1;
	}

	HINTERNET OpenAddress = InternetOpenUrl(
		connect, _T("https://finalburnneo.github.io/FBNeo-translations/localisationinfo/"), nullptr, 0,
		INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, 0);
	if (!OpenAddress)
	{
		MessageBox(hLocalDownDlg, FBALoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_OPEN_URL, true),
		           FBALoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
	...
	while (Tokens != nullptr)
	{
		if (i & 1)
		{
			SendDlgItemMessage(hLocalDownDlg, IDC_CHOOSE_LIST, CB_ADDSTRING, 0,
			                   (LPARAM)ANSIToTCHAR(Tokens, nullptr, 0));
		}
		else
		{
	...
	SendDlgItemMessage(hLocalDownDlg, IDC_CHOOSE_LIST, CB_SETCURSEL, 0, 0);
...
static int LocaliseDownloadOkay()
{
	...
	SendMessage(GetDlgItem(hLocalDownDlg, IDC_CHOOSE_LIST), CB_GETLBTEXT, static_cast<WPARAM>(nIndex),
	            (LPARAM)szChoice);
	...
	if ((fFlt = _tfopen(szChoice, _T("wb"))) == nullptr) return 1;
	...
	HINTERNET OpenAddress = InternetOpenUrl(connect, url, nullptr, 0,
	                                        INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, 0);
	if (!OpenAddress)
	{
		MessageBox(hLocalDownDlg, FBALoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_OPEN_URL, true),
...
int LocaliseDownloadCreate(HWND hParentWND)
{
	...
	FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_DOWNLOAD_LOCAL), hParent, DefInpProc);
	return 1;
}
