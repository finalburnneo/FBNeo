...
INT32 write_datfile(INT32 bType, FILE* fDat)
{
	...
	if (bType == DAT_NGP_ONLY) fprintf(fDat, "\t\t<name>" APP_TITLE " - NGP|NGPC Games</name>\n");
	...
	if (bType == DAT_NGP_ONLY) _ftprintf(fDat, _T("\t\t<description>") _T(APP_TITLE) _T(" v%s") _T(" NGP|NGPC Games</description>\n"), szAppBurnVer);
	...
	for (nGameSelect=0;nGameSelect<nBurnDrvCount;nGameSelect++)
	{
		...
		if ((((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_SEGA_MEGADRIVE)
			...
			|| ((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_SNK_NGPC)
		...
		remove_driver_leader(HARDWARE_SNK_NGPC, 5, 1)
	...
	for (nBurnDrvActive=0; nBurnDrvActive<nBurnDrvCount; nBurnDrvActive++)
	{
		...
		remove_driver_leader(HARDWARE_SNK_NGPC, 5, 0)
