...
static INT32 AviCreateFile()
{
	...
	if (nAviSplit == 0)
	{
		// Create the filename @ the first file in our set
		sprintf(szAviFileName, "%s%s-%.2d-%.2d-%.2d%.2d%.2d", TAVI_DIRECTORY, BurnDrvGetTextA(DRV_NAME),
...
static INT32 AviCreateVidStream()
{
	...
	*/
	...
	if (nAviSplit > 0)
	{
		// next 2gig chunk, don't ask user for video type.
	...
	else
	{
		...
		nRet = ICCompressorChoose(
			...
			&FBAvi.bih, // uncompressed data input format
			nullptr, // no preview window
	...
	nRet = SetRect(
		...
		static_cast<INT32>(FBAvi.bih.biWidth),
		static_cast<INT32>(FBAvi.bih.biHeight));
	if (nRet == 0)
	{
#ifdef AVI_DEBUG
...
INT32 AviRecordFrame(INT32 bDraw)
{
	...
	*/
	if (bDraw)
	{
		...
		case 2: MakeBitmap2x();
			break;
		// Double the pixels, this must always happen otherwise the compression size and video quality - especially when uploaded to yt - will suck. -dink
		case 3: MakeBitmap3x();
			break; // Triple the pixels, for 720p/60hz* yt videos (yay!) *) if * 3 >= 720 && hz >= 60
		}
...
static void FreeBMP()
{
	if (FBAvi.pBitmapBuf1)
	{
		...
		FBAvi.pBitmapBuf1 = nullptr;
	}
	if (FBAvi.pBitmapBuf2)
	{
		free(FBAvi.pBitmapBuf2);
		FBAvi.pBitmapBuf2 = nullptr;
	}
	FBAvi.pBitmap = nullptr;
}
...
INT32 AviStart_INT()
{
	...
	FBAvi.pBitmapBuf1 = static_cast<UINT8*>(malloc(FBAvi.bih.biSizeImage));
	if (FBAvi.pBitmapBuf1 == nullptr)
	{
		return 1; // not enough memory to create allocate bitmap
	}
	FBAvi.pBitmapBuf2 = static_cast<UINT8*>(malloc(FBAvi.bih.biSizeImage));
	if (FBAvi.pBitmapBuf2 == nullptr)
	{
		free(FBAvi.pBitmapBuf1);
		return 1; // not enough memory to create allocate bitmap
	}
