...
static void pce_refresh_sprites(INT32 which, INT32 line, UINT8* drawn, UINT16* line_buffer)
{
	...
	for (i = 0; i < 64; i++)
	{
		static constexpr INT32 cgy_table[] = {16, 32, 64, 64};
								...
								if (priority || drawn[pixel_x] == 0)
								{
									...
									if (vdc_width[which] != 512)
									{
										...
										while (pixel_x + dp < (((obj_x + x + 1) * main_width) / vdc_width[which]))
										{
											...
											line_buffer[pixel_x + dp] = color_base + vce_data[0x100 + (palette << 4) +
												buf[x]];
											dp++;
										}
									}
...
static void do_vblank(INT32 which)
{
	...
	if ((vdc_data[which][DCR] & DCR_DSR) || vdc_dvssr_write[which])
	{
		...
		{
			vdc_sprite_ram[which][i] = (vdc_vidram[which][(vdc_data[which][DVSSR] << 1) + i * 2 + 1] << 8) | vdc_vidram[
				which][(vdc_data[which][DVSSR] << 1) + i * 2];
		}
...
{
	static constexpr INT32 width_table[4] = {5, 6, 7, 7};
	...
	case 0x03:
		{
			...
			case CR:
				{
					static constexpr UINT8 inctab[] = {1, 32, 64, 128};
