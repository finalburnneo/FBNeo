...
UINT8* CpsrBase = nullptr; // Tile data base
INT32 nCpsrScrX = 0, nCpsrScrY = 0; // Basic scroll info
UINT16* CpsrRows = nullptr; // Row scroll table, 0x400 words long
...
static void GetRowsRange(INT32* pnStart, INT32* pnWidth, INT32 nRowFrom, INT32 nRowTo)
{
	...
	nStart = BURN_ENDIAN_SWAP_INT16(CpsrRows[nRowFrom&0x3ff]);
	nStart &= 0x3ff;
	nWidth = 0;
	for (i = nRowFrom; i < nRowTo; i++)
	{
		INT32 nViz;
		INT32 nDiff;
		nViz = BURN_ENDIAN_SWAP_INT16(CpsrRows[i&0x3ff]);
		nViz &= 0x3ff;
