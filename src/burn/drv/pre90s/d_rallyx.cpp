...
static UINT8* DrvZ80Ram1_weird = nullptr;
static UINT8* DrvZ80Ram2 = nullptr;
static UINT8* DrvVideoRam = nullptr;
static UINT8* DrvRadarAttrRam = nullptr;
static UINT8* DrvPromPalette = nullptr;
static UINT8* DrvPromLookup = nullptr;
static UINT8* DrvPromVidLayout = nullptr;
static UINT8* DrvPromVidTiming = nullptr;
static UINT8* DrvChars = nullptr;
static UINT8* DrvSprites = nullptr;
static UINT8* DrvDots = nullptr;
...
{
	// Default Values
	{0x10, 0xff, 0xff, 0x01, nullptr},
	{0x11, 0xff, 0xff, 0xcb, nullptr},
...
static INT32 MemIndex()
{
	...
	DrvPromLookup = Next;
	Next += 0x00100;
	DrvPromVidLayout = Next;
	Next += 0x00020;
	DrvPromVidTiming = Next;
	Next += 0x00020;
	NamcoSoundProm = Next;
	Next += 0x00100;
...
static INT32 DrvInit()
{
	...
	DrvTempRom = BurnMalloc(0x01000);

	// Load Z80 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x01000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x02000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x03000, 3, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom, 5, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x08, 2, 4, 4, DotPlaneOffsets, DotXOffsets, DotYOffsets, 0x80, DrvTempRom, DrvDots);

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromPalette, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromLookup, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidLayout, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidTiming, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(NamcoSoundProm, 10, 1);
	if (nRet != 0) return 1;
...
static INT32 DrvaInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x00800, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x01000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x01800, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x02000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x02800, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x03000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x03800, 7, 1);
	if (nRet != 0) return 1;

	// Load and decode the chars and sprites
	nRet = BurnLoadRom(DrvTempRom + 0x0000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x0800, 9, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x100, 2, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
	GfxDecode(0x40, 2, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
	...
	// Load and decode the dots
	memset(DrvTempRom, 0, 0x1000);
	nRet = BurnLoadRom(DrvTempRom, 10, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x08, 2, 4, 4, DotPlaneOffsets, DotXOffsets, DotYOffsets, 0x80, DrvTempRom, DrvDots);

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromPalette, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromLookup, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidLayout, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidTiming, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(NamcoSoundProm, 15, 1);
	if (nRet != 0) return 1;
...
static INT32 NrallyxInit()
{
	...
	// Load and decode the chars and sprites
	memset(DrvTempRom, 0, 0x1000);
	nRet = BurnLoadRom(DrvTempRom + 0x0000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x0800, 5, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x100, 2, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
	GfxDecode(0x40, 2, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
	...
	// Load and decode the dots
	memset(DrvTempRom, 0, 0x1000);
	nRet = BurnLoadRom(DrvTempRom, 6, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x08, 2, 4, 4, DotPlaneOffsets, DotXOffsets, DotYOffsets, 0x80, DrvTempRom, DrvDots);

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromPalette, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromLookup, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidLayout, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidTiming, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(NamcoSoundProm, 11, 1);
	if (nRet != 0) return 1;
...
static INT32 JunglerInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom2 + 0x00000, 4, 1);
	if (nRet != 0) return 1;

	// Load and decode the chars and sprites
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00800, 6, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x100, 2, 8, 8, JunglerCharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
	GfxDecode(0x40, 2, 16, 16, JunglerSpritePlaneOffsets, JunglerSpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom,
	          DrvSprites);
	...
	// Load the PROMs
	nRet = BurnLoadRom(DrvPromPalette, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromLookup, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidLayout, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidTiming, 11, 1);
	if (nRet != 0) return 1;
...
static INT32 LococommonDrvInit(INT32 prgroms, INT32 soundroms)
{
	...
	DrvTempRom = BurnMalloc(0x04000);

	// Load Z80 Program Roms
	for (INT32 i = 0; i < prgroms; i++)
	{
		nRet = BurnLoadRom(DrvZ80Rom1 + (i * 0x01000), i, 1);
		if (nRet != 0) return 1;
	}
	...
	{
		nRet = BurnLoadRom(DrvZ80Rom2 + (i * 0x01000), i + prgroms, 1);
		if (nRet != 0) return 1;
	}
	...
	// Load and decode the chars and sprites
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 0 + rompos, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x01000, 1 + rompos, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x200, 2, 8, 8, JunglerCharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
	GfxDecode(0x80, 2, 16, 16, JunglerSpritePlaneOffsets, JunglerSpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom,
	          DrvSprites);

	// Load and decode the dots
	memset(DrvTempRom, 0, 0x1000);
	nRet = BurnLoadRom(DrvTempRom, 2 + rompos, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x08, 2, 4, 4, DotPlaneOffsets, DotXOffsets, DotYOffsets, 0x80, DrvTempRom, DrvDots);

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromPalette, 3 + rompos, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromLookup, 4 + rompos, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromVidLayout, 5 + rompos, 1);
	if (nRet != 0) return 1;
...
{
	static constexpr INT32 ResistancesRG[3] = {1000, 470, 220};
	static constexpr INT32 ResistancesB[2] = {470, 220};
	double rWeights[3], gWeights[3], bWeights[2];
	UINT32 Palette[32];
	UINT32 i;

	compute_resistor_weights(0, 255, -1.0, 3, &ResistancesRG[0], rWeights, 0, 0, 3, &ResistancesRG[0], gWeights, 0, 0,
...
static void DrvCalcPaletteJungler()
{
	static constexpr INT32 ResistancesRG[3] = {1000, 470, 220};
	static constexpr INT32 ResistancesB[2] = {470, 220};
	static constexpr INT32 ResistancesSTAR[2] = {150, 100};
	double rWeights[3], gWeights[3], bWeights[2];
...
struct BurnDriver BurnDrvRallyx = {
	"rallyx", nullptr, nullptr, "rallyx", "1980",
	"Rally X (32k Ver.?)\0", nullptr, "Namco", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, RallyxRomInfo, RallyxRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 260, 288, 224, 4, 3
};

struct BurnDriver BurnDrvRallyxa = {
	"rallyxa", "rallyx", nullptr, "rallyx", "1980",
	"Rally X\0", nullptr, "Namco", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, RallyxaRomInfo, RallyxaRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DrvDIPInfo,
...
struct BurnDriver BurnDrvRallyxm = {
	"rallyxm", "rallyx", nullptr, "rallyx", "1980",
	"Rally X (Midway)\0", nullptr, "Namco (Midway license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, RallyxmRomInfo, RallyxmRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 260, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvRallyxmr = {
	"rallyxmr", "rallyx", nullptr, "rallyx", "1980",
	"Rally X (Model Racing bootleg)\0", nullptr, "bootleg (Model Racing)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, RallyxmrRomInfo, RallyxmrRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DrvDIPInfo,
	DrvaInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 260, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvDngrtrck = {
	"dngrtrck", "rallyx", nullptr, "rallyx", "1980",
	"Danger Track (bootleg of Rally X)\0", nullptr, "bootleg (Petaco)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, DngrtrckRomInfo, DngrtrckRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DngrtrckDIPInfo,
	DrvaInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 260, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvNrallyx = {
	"nrallyx", nullptr, nullptr, "rallyx", "1981",
	"New Rally X\0", nullptr, "Namco", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, NrallyxRomInfo, NrallyxRomName, nullptr, nullptr, RallyxSampleInfo, RallyxSampleName, DrvInputInfo,
	DrvDIPInfo,
...
struct BurnDriver BurnDrvJungler = {
	"jungler", nullptr, nullptr, nullptr, "1981",
	"Jungler\0", nullptr, "Konami", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAZE, 0,
	nullptr, JunglerRomInfo, JunglerRomName, nullptr, nullptr, nullptr, nullptr, JunglerInputInfo, JunglerDIPInfo,
	JunglerInit, DrvExit, JunglerFrame, DrvDrawJungler, DrvScan,
	nullptr, 324, 224, 288, 3, 4
};
...
struct BurnDriver BurnDrvTactcian = {
	"tactcian", nullptr, nullptr, nullptr, "1982",
	"Tactician (set 1)\0", nullptr, "Konami (Sega license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	...
	nullptr, tactcianRomInfo, tactcianRomName, nullptr, nullptr, nullptr, nullptr, TactcianInputInfo, TactcianDIPInfo,
...
struct BurnDriver BurnDrvTactcian2 = {
	"tactcian2", "tactcian", nullptr, nullptr, "1981",
	"Tactician (set 2)\0", nullptr, "Konami (Sega license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, tactcian2RomInfo, tactcian2RomName, nullptr, nullptr, nullptr, nullptr, TactcianInputInfo, TactcianDIPInfo,
	TactcianDrvInit, DrvExit, JunglerFrame, DrvDrawJungler, DrvScan,
	nullptr, 324, 224, 288, 3, 4
};
...
struct BurnDriver BurnDrvLocomotn = {
	"locomotn", nullptr, nullptr, nullptr, "1982",
	"Loco-Motion\0", nullptr, "Konami (Centuri license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, locomotnRomInfo, locomotnRomName, nullptr, nullptr, nullptr, nullptr, LocomotnInputInfo, LocomotnDIPInfo,
	LocomotnDrvInit, DrvExit, JunglerFrame, DrvDrawJungler, DrvScan,
	nullptr, 324, 224, 256, 3, 4
};
...
struct BurnDriver BurnDrvGutangtn = {
	"gutangtn", "locomotn", nullptr, nullptr, "1982",
	"Guttang Gottong\0", nullptr, "Konami (Sega license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S,
	GBF_PUZZLE, 0,
	nullptr, gutangtnRomInfo, gutangtnRomName, nullptr, nullptr, nullptr, nullptr, LocomotnInputInfo, LocomotnDIPInfo,
...
struct BurnDriver BurnDrvCottong = {
	"cottong", "locomotn", nullptr, nullptr, "1982",
	"Cotocoto Cottong\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, cottongRomInfo, cottongRomName, nullptr, nullptr, nullptr, nullptr, LocomotnInputInfo, LocomotnDIPInfo,
...
struct BurnDriver BurnDrvLocoboot = {
	"locoboot", "locomotn", nullptr, nullptr, "1982",
	"Loco-Motion (bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, locobootRomInfo, locobootRomName, nullptr, nullptr, nullptr, nullptr, LocomotnInputInfo, LocomotnDIPInfo,
...
struct BurnDriver BurnDrvCommsega = {
	"commsega", nullptr, nullptr, nullptr, "1983",
	"Commando (Sega)\0", nullptr, "Sega", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, commsegaRomInfo, commsegaRomName, nullptr, nullptr, nullptr, nullptr, CommsegaInputInfo, CommsegaDIPInfo,
