...
// Digital Processing
void ProcessJoystick(UINT8* input, INT8 playernum, INT8 up_bit, INT8 down_bit, INT8 left_bit, INT8 right_bit,
                     UINT8 flags)
{
	...
	if (flags & INPUT_4WAY)
	{
		...
		if (*input != DrvInputPrev[playernum])
		{
			...
			else
			{
				...
				if ((fourway[playernum] & rl) && (fourway[playernum] & ud))
					fourway[playernum] &= ud;
				// when frame starts on diagonal, mask it out (super rare, but not impossible)
...
void CompileInput(UINT8** input, void* output, INT32 num, INT32 bits, UINT32* init)
{
	...
	{
		if (bits > 16) static_cast<UINT32*>(output)[j] = init[j];
		if (bits > 8 && bits < 17) static_cast<UINT16*>(output)[j] = init[j];
		if (bits < 9) static_cast<UINT8*>(output)[j] = init[j];

		for (INT32 i = 0; i < bits; i++)
		{
			if (bits > 16) static_cast<UINT32*>(output)[j] ^= (input[j][i] & 1) << i;
			if (bits > 8 && bits < 17) static_cast<UINT16*>(output)[j] ^= (input[j][i] & 1) << i;
			if (bits < 9) static_cast<UINT8*>(output)[j] ^= (input[j][i] & 1) << i;
		}
	}
}
...
UINT8 ProcessAnalog(INT16 anaval, INT32 reversed, INT32 flags, UINT8 scalemin, UINT8 scalemax, UINT8 centerval)
{
	...
	if (flags & INPUT_MIGHTBEDIGITAL && static_cast<UINT16>(anaval) == 0xffff)
	{
