...
static HWND hNeoCDWnd = nullptr;
static HWND hListView = nullptr;
static HWND hProcParent = nullptr;
...
// CD image stuff
constexpr int nSectorLength = 2352;
...
static int NeoCDList_AddGame(TCHAR* pszFile, unsigned int nGameID)
{
	...
	{
		game = static_cast<NGCDGAME*>(malloc(sizeof(NGCDGAME)));
...
static int CALLBACK ListViewCompareFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	TCHAR buf1[MAX_PATH];
	TCHAR buf2[MAX_PATH];
	auto lpsd = (struct LVCOMPAREINFO*)lParamSort;

	ListView_GetItemText(lpsd->hWnd, static_cast<int>(lParam1), lpsd->nColumn, buf1, sizeof(buf1));
	ListView_GetItemText(lpsd->hWnd, static_cast<int>(lParam2), lpsd->nColumn, buf2, sizeof(buf2));
...
static TCHAR* NeoCDList_ParseCUE(TCHAR* pszFile)
{
	...
	TCHAR* szISO = nullptr;
	szISO = static_cast<TCHAR*>(malloc(sizeof(TCHAR) * 2048));
	if (!szISO) return nullptr;
	...
	while (!feof(fp))
	{
		...
		// track info
		if ((t = LabelCheck(szBuffer, _T("TRACK"))) != nullptr)
		{
			s = t;
			...
			if ((t = LabelCheck(s, _T("MODE1/2352"))) != nullptr)
			{
				//bprintf(0, _T(".cue: Track #%d, data.\n"), track);
				continue;
			}
			if ((t = LabelCheck(s, _T("AUDIO"))) != nullptr)
			{
...
static void NeoCDList_ShowPreview(HWND hDlg, TCHAR* szFile, int nCtrID, int nFrameCtrID, float maxw, float maxh)
{
	...
	{
		HRSRC hrsrc = FindResource(nullptr, MAKEINTRESOURCE(BMP_SPLASH), RT_BITMAP);
		HGLOBAL hglobal = LoadResource(nullptr, hrsrc);

		auto pbmih = static_cast<BITMAPINFOHEADER*>(LockResource(hglobal));
	...
	float w = static_cast<float>(PNGRes.nWidth);
	float h = static_cast<float>(PNGRes.nHeight);
	...
	HBITMAP hCoverBmp = PNGLoadBitmap(hDlg, fp, static_cast<int>(w), static_cast<int>(h), 0);

	SetWindowPos(GetDlgItem(hDlg, nCtrID), nullptr, static_cast<int>(pt.x + x), static_cast<int>(pt.y + y), 0, 0,
...
static HWND hNeoCDList_CoverDlg = nullptr;
...
{
	FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_NCD_COVER_DLG), hNeoCDWnd, NeoCDList_CoverWndProc);
}
...
static INT_PTR CALLBACK NeoCDList_WndProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		_stprintf(szDialogTitle, FBALoadStringEx(hAppInst, IDS_NGCD_DIAG_TITLE, true), _T(APP_TITLE), _T(SEPERATOR_1),
		          _T(SEPERATOR_1));
		SetWindowText(hDlg, szDialogTitle);

		hProcessThread = (HANDLE)_beginthreadex(nullptr, 0, NeoCDList_DoProc, nullptr, 0, &ProcessThreadID);
	...
	if (Msg == WM_NOTIFY)
	{
		...
		if (pNMLV->hdr.code == LVN_ITEMCHANGED && pNMLV->hdr.idFrom == IDC_NCD_LIST)
		{
			...
			SendMessage(hListView, LVM_GETITEMTEXT, static_cast<WPARAM>(iItem), (LPARAM)&LvItem);
...
int NeoCDList_Init()
{
	return FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_NCD_DLG), hScrnWnd, NeoCDList_WndProc);
}
