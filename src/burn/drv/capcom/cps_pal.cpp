...
static UINT8* CpsPalSrc = nullptr; // Copy of current input palette
UINT32* CpsPal = nullptr; // Hicolor version of palette
...
INT32 CpsPalInit()
{
	...
	CpsPalSrc = BurnMalloc(nLen);
	if (CpsPalSrc == nullptr)
	{
		return 1;
	}
...
INT32 CpsPalUpdate(UINT8* pNewPal)
{
	...
	auto PaletteRAM = (UINT16*)CpsPalSrc;
