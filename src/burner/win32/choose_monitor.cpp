	...
	case WM_INITDIALOG:
		{
			...
			{
				MonitorIDs[i] = static_cast<TCHAR*>(malloc(32 * sizeof(TCHAR)));
			...
			for (i = 0; EnumDisplayDevices(nullptr, i, &dd, 0); i++)
			{
				if (dd.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP)
				{
					...
					if (dd.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE)
						_stprintf(szTemp, _T("%s (default)"), szTemp);
	...
	case WM_CLOSE:
		{
			if (bOK)
			{
				...
				int nItem = static_cast<int>(SendDlgItemMessage(hDlg, IDC_CHOOSE_MONITOR_HOR_LIST, CB_GETCURSEL, 0, 0));
				if (MonitorIDs[nItem])
					_stprintf(HorScreen, _T("%s"), MonitorIDs[nItem]);

				nItem = static_cast<int>(SendDlgItemMessage(hDlg, IDC_CHOOSE_MONITOR_VER_LIST, CB_GETCURSEL, 0, 0));
				if (MonitorIDs[nItem])
					_stprintf(VerScreen, _T("%s"), MonitorIDs[nItem]);
			}
			EndDialog(hDlg, 0);
			break;
		}
...
{
	FBADialogBox(hAppInst,MAKEINTRESOURCE(IDD_CHOOSEMONITOR), hScrnWnd, ResProc);
	return 0;
}
