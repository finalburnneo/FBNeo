...
UINT8* CpsBootlegSpriteRam = nullptr;

INT32 Cps1LockSpriteList910000 = 0;
INT32 Cps1DetectEndSpriteList8000 = 0;

Cps1ObjGetCallback Cps1ObjGetCallbackFunction = nullptr;
Cps1ObjDrawCallback Cps1ObjDrawCallbackFunction = nullptr;

// Our copy of the sprite table
static UINT8* ObjMem = nullptr;
...
static void CpsBlendInit()
{
	...
	blendtable = BurnMalloc(0x40000); // maximum number of sprites
	...
	while (true)
	{
		if (fgets(szLine, 64, fa) == nullptr) break;
...
INT32 CpsObjInit()
{
	...
	ObjMem = BurnMalloc((nMax << 3) * nFrameCount);
	if (ObjMem == nullptr)
	{
		return 1;
	}
...
INT32 CpsObjExit()
{
	...
	Cps1ObjGetCallbackFunction = nullptr;
	Cps1ObjDrawCallbackFunction = nullptr;
...
INT32 Cps2ObjDraw(INT32 nLevelFrom, INT32 nLevelTo)
{
	...
	// Go through all the Objs
	for (ZValue = static_cast<UINT16>(nMaxZValue); ZValue <= nCount; ZValue++, ps += nPsAdd)
	{
		INT32 x, y, n, a, bx, by, dx, dy;
		INT32 nFlip;
		...
		if (a & 0x80)
		{
			// marvel vs capcom ending sprite off-set
			x += CpsSaveFrg[0][0x9];
		}
		...
		for (dy = 0; dy < by; dy++)
		{
			for (dx = 0; dx < bx; dx++)
			{
				...
				if (Sfa2ObjHack)
				{
					...
					if (nCpstTile == 0x1a410 && (a & 0x1f) == 0 && (nCpstX == 0x40 || nCpstX == 0x100) && nCpstY == 0x50
						&& v == 0 && nLevelFrom == 0 && nLevelTo == 0)
						continue;
					if (nCpstTile == 0x1a411 && (a & 0x1f) == 0 && (nCpstX == 0x50 || nCpstX == 0x110) && nCpstY == 0x50
						&& v == 0 && nLevelFrom == 0 && nLevelTo == 0)
						continue;
