...
static INT32 MemIndex()
{
	...
	Next += 0x301 * sizeof(UINT32);

	DrvTempDraw = (UINT16*)Next;
	Next += 224 * 256 * sizeof(UINT16);
...
static UINT8 __fastcall DrvZ80Read1(UINT16 a)
{
	...
	{
		auto PagedRAM = DrvPagedRam;
		if (DrvVRamPage == 1) PagedRAM = DrvPagedRam + 0x2000;
...
static void __fastcall DrvZ80Write1(UINT16 a, UINT8 d)
{
	if (a >= 0xc000 && a <= 0xdfff)
	{
		...
		{
			DrvBgScrollX = (DrvPagedRam[0x2000 + BG_SCROLLX_LSB] + ((DrvPagedRam[0x2000 + BG_SCROLLX_MSB] & 0x03) << 8))
				& 0x3ff;
		}

		if (offset == BG_SCROLLY_LSB || offset == BG_SCROLLY_MSB)
		{
			DrvBgScrollY = (DrvPagedRam[0x2000 + BG_SCROLLY_LSB] + ((DrvPagedRam[0x2000 + BG_SCROLLY_MSB] & 0x01) << 8))
				& 0x1ff;
		}
...
static INT32 DrvInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom2 + 0x00000, 2, 1);
	if (nRet != 0) return 1;

	// Load and decode the sprites
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 4, 1);
	if (nRet != 0) return 1;
	...
	memset(DrvTempRom, 0, 0x20000);
	nRet = BurnLoadRom(DrvTempRom, 7, 1);
	if (nRet != 0) return 1;
...
static void DrvEnableBgClipMode()
{
	...
	else
	{
		...
		{
		case 0:
		case 4: if (sy1_old != DrvBgSy1) DrvBgClipMode++;
			break;
		case 2:
		case 6: if (sy2_old != DrvBgSy2) DrvBgClipMode++;
			break;
		case 8:
		case 10:
		case 12:
		case 14: if (sx1_old != DrvBgSx1) DrvBgClipMode++;
			break;
		case 1:
		case 5: if (DrvBgSy1 == 0xf0) DrvBgClipMode++;
			break;
		case 3:
		case 7: if (DrvBgSy2 == 0xf0) DrvBgClipMode++;
			break;
		case 9:
		case 11: if (DrvBgSx1 == 0xf0) DrvBgClipMode++;
			break;
		case 13:
		case 15: if (sx1_old == 0xf0) DrvBgClipMode++;
		case 16: if (DrvBgSy1 != 0x00) DrvBgClipMode = 0;
			break;
		}
		...
		case 1: DrvBgClipMinY = DrvBgSy1;
			break;
		case 3: DrvBgClipMaxY = DrvBgSy2;
			break;
		case 5: DrvBgClipMaxY = DrvBgSy1;
			break;
		case 7: DrvBgClipMinY = DrvBgSy2;
			break;
		case 9:
		case 15: DrvBgClipMinX = DrvBgSx1;
			break;
		case 11:
		case 13: DrvBgClipMaxX = DrvBgSx1;
			break;
		}
...
static void DrvDrawBlendGfx(UINT8* gfxbase, UINT32 code, UINT32 color, INT32 flipx, INT32 flipy, INT32 offsx,
                            INT32 offsy, INT32 transparent_color)
{
	color *= 16;
...
static INT32 DrvDraw()
{
		...
		else
		{
			...
			for (INT32 y = 0; y < nScreenHeight; y++, pSrc += nScreenWidth, pAlp += nScreenWidth, pDest += nBurnPitch)
			{
				for (INT32 x = 0; x < nScreenWidth; x++)
				{
					if (pSrc[x] & 0x8000)
					{
						((UINT32*)pDest)[x] = DrvBlendFunction(DrvPalette[pSrc[x] & 0x3ff], DrvPalette[pAlp[x] & 0x3ff],
						                                       pAlp[x] / 0x0400);
					}
					else
					{
	...
	{
		// skip alpha for now on anything less than 24-bit
...
struct BurnDriver BurnDrvPsychic5 = {
	"psychic5", nullptr, nullptr, nullptr, "1987",
	"Psychic 5 (World)\0", nullptr, "Jaleco / NMK", "Miscellaneous",
...
struct BurnDriver BurnDrvPsychic5j = {
	"psychic5j", "psychic5", nullptr, nullptr, "1987",
	"Psychic 5 (Japan)\0", nullptr, "Jaleco / NMK", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S,
	GBF_PLATFORM, 0,
	nullptr, Psychic5jRomInfo, Psychic5jRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x300, 224, 256, 3, 4
};
