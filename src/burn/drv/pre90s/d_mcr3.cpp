...
static INT32 (*port_write_handler)(UINT8 address, UINT8 data) = nullptr;
static INT32 (*port_read_handler)(UINT8 address) = nullptr;
...
static class LowPass2* LP1 = nullptr;
...
static struct BurnInputInfo StargrdsInputList[] = {
	{"P1 Coin", BIT_DIGITAL, DrvJoy1 + 0, "p1 coin"},
	{"P1 Start / Weapon", BIT_DIGITAL, DrvJoy1 + 2, "p1 start"},
	...
	{"P2 Coin", BIT_DIGITAL, DrvJoy1 + 1, "p2 coin"},
	{"P2 Start / Weapon", BIT_DIGITAL, DrvJoy1 + 3, "p2 start"},
	...
	{"P3 Start / Weapon", BIT_DIGITAL, DrvJoy6 + 3, "p3 start"},
...
{
	{0x21, 0xff, 0xff, 0xff, nullptr},
	{0x22, 0xff, 0xff, 0xff, nullptr},
	{0x23, 0xff, 0xff, 0xff, nullptr},
	{0x24, 0xff, 0xff, 0xff, nullptr},
	{0x25, 0xff, 0xff, 0xff, nullptr},
	{0x26, 0xff, 0xff, 0x80, nullptr},
...
static struct BurnDIPInfo RampageDIPList[] =
{
	...
	{0x1a, 0xff, 0xff, 0xff, nullptr},
	{0x1b, 0xff, 0xff, 0xff, nullptr},
	{0x1c, 0xff, 0xff, 0x20, nullptr},
...
static INT32 DrvGfxDecode()
{
	...
	INT32 XOffs1[32] = {
		...
		L * 0 + 0 + 8, L * 0 + 4 + 8, L * 1 + 0 + 8, L * 1 + 4 + 8, L * 2 + 0 + 8, L * 2 + 4 + 8, L * 3 + 0 + 8,
		L * 3 + 4 + 8,
		L * 0 + 0 + 16, L * 0 + 4 + 16, L * 1 + 0 + 16, L * 1 + 4 + 16, L * 2 + 0 + 16, L * 2 + 4 + 16, L * 3 + 0 + 16,
		L * 3 + 4 + 16,
		L * 0 + 0 + 24, L * 0 + 4 + 24, L * 1 + 0 + 24, L * 1 + 4 + 24, L * 2 + 0 + 24, L * 2 + 4 + 24, L * 3 + 0 + 24,
		L * 3 + 4 + 24
	};
	...
	for (INT32 i = 0; i < nGraphicsLen[0] * 2; i += 0x40)
	{
		// 2x size and invert pixel
...
static void sound_system_init(INT32 sound_system)
{
	...
	switch (sound_system)
	{
	case 0: soundsgood_init(0, 0, Drv68KROM, Drv68KRAM);
		break;
	case 1: tcs_init(0, 0, 0, DrvSndROM, DrvSndRAM);
		break;
	...
	case 4: csd_init(0, 0, Drv68KROM, Drv68KRAM);
		break;
	}
...
static INT32 DrvInit(INT32 sound_system)
{
	...
	z80ctc_init(5000000, 0, ctc_interrupt, ctc_trigger, nullptr, nullptr);
	ZetClose();
...
static INT32 DrvExit()
{
	...
	sound_input_bank = 0;
	port_write_handler = nullptr;
	port_read_handler = nullptr;
	sprite_color_mask = 0;
	flip_screen_x = 0;
...
static void draw_sprites(INT32 color_mask, INT32 dx, INT32 dy)
{
	...
	for (INT32 offs = 0x200 - 4; offs >= 0; offs -= 4)
	{
		...
		if (!flipscreen)
		{
			if (nSpriteEnable & 2) RenderPrioMaskTranstabSprite(pTransDraw, DrvGfxROM1, code % codemask, (color * 16),
			                                                    0xff, sx, sy, flipx, flipy, 32, 32, DrvTransTab[0], 0);
			if (nSpriteEnable & 4) RenderPrioMaskTranstabSprite(pTransDraw, DrvGfxROM1, code % codemask, (color * 16),
			                                                    0xff, sx, sy, flipx, flipy, 32, 32, DrvTransTab[1], 2);
		}
		else
		{
			RenderPrioMaskTranstabSprite(pTransDraw, DrvGfxROM1, code % codemask, (color * 16), 0xff, 480 - sx,
			                             452 - sy, !flipx, !flipy, 32, 32, DrvTransTab[0], 0);
			RenderPrioMaskTranstabSprite(pTransDraw, DrvGfxROM1, code % codemask, (color * 16), 0xff, 480 - sx,
			                             452 - sy, !flipx, !flipy, 32, 32, DrvTransTab[1], 2);
		}
	}
}
...
static INT32 SpyhuntDraw()
{
	...
	if (is_spyhunt)
	{
		if (lamp & 0x04)
		{
			...
			RenderZoomedTile(pTransDraw, DrvGfxROM1, 0x200, 0, 0, nScreenWidth - 16, 32, 0, 0, 32, 32, 0x10000 / 2,
			                 0x10000 / 2);
			RenderZoomedTile(pTransDraw, DrvGfxROM1, 0x201, 0, 0, nScreenWidth - 16, 48, 0, 0, 32, 32, 0x10000 / 2,
			                 0x10000 / 2);
		}
		if (lamp & 0x01)
		{
			// oil
			RenderZoomedTile(pTransDraw, DrvGfxROM1, 0x016, 0, 0, nScreenWidth - 16, 64, 0, 0, 32, 32, 0x10000 / 2,
			                 0x10000 / 2);
		}
		if (lamp & 0x08)
		{
			// smoke
			RenderZoomedTile(pTransDraw, DrvGfxROM1, 0x019, 0, 0, nScreenWidth - 16, 80, 0, 0, 32, 32, 0x10000 / 2,
			                 0x10000 / 2);
		}
		if (lamp & 0x02)
		{
			// missile (pre-rotated)
			RenderZoomedTile(pTransDraw, DrvGfxROM1, 0x202, 0, 0, nScreenWidth - 24, 96, 0, 0, 32, 32, 0x10000,
			                 0x10000);
		}
		if (lamp & 0x10)
		{
...
static INT32 CSDSSIOFrame()
{
	...
	if (pBurnSoundOut)
	{
		if (is_spyhunt)
		{
			...
			if (LP1)
			{
				// this filter actually does the filtering
...
struct BurnDriver BurnDrvDemoderm = {
	"demoderm", "demoderb", nullptr, nullptr, "1984",
	"Demolition Derby (MCR-3 Mono Board Version)\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE, 4, HARDWARE_MISC_PRE90S, GBF_ACTION, 0,
	nullptr, demodermRomInfo, demodermRomName, nullptr, nullptr, nullptr, nullptr, DemodermInputInfo, DemodermDIPInfo,
...
struct BurnDriver BurnDrvSarge = {
	"sarge", nullptr, nullptr, nullptr, "1985",
	"Sarge\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 4, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, sargeRomInfo, sargeRomName, nullptr, nullptr, nullptr, nullptr, SargeInputInfo, SargeDIPInfo,
	...
	case 0x06:
		{
			...
			if (~data & 0x80)
			{
				if (maxrpm_adc_select < 2)
				{
					// p2 gas, p1 gas
					latched_input = ProcessAnalog(analogs[maxrpm_adc_select], 1,
					                              INPUT_DEADZONE | INPUT_LINEAR | INPUT_MIGHTBEDIGITAL, 0x30, 0xff);
				...
				{
					// p2 wheel, p1 wheel
					latched_input = ProcessAnalog(analogs[maxrpm_adc_select], (maxrpm_adc_select == 2) ? 0 : 1,
					                              INPUT_DEADZONE, 0x40, 0xb4);
				}
			}
...
struct BurnDriver BurnDrvMaxrpm = {
	"maxrpm", nullptr, nullptr, nullptr, "1986",
	"Max RPM (ver 2)\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 2, HARDWARE_MISC_PRE90S, GBF_RACING, 0,
	nullptr, maxrpmRomInfo, maxrpmRomName, nullptr, nullptr, nullptr, nullptr, MaxrpmInputInfo, MaxrpmDIPInfo,
...
struct BurnDriver BurnDrvRampage = {
	"rampage", nullptr, nullptr, nullptr, "1986",
	"Rampage (Rev 3, 8/27/86)\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 3, HARDWARE_MISC_PRE90S, GBF_ACTION, 0,
	nullptr, rampageRomInfo, rampageRomName, nullptr, nullptr, nullptr, nullptr, RampageInputInfo, RampageDIPInfo,
...
struct BurnDriver BurnDrvRampage2 = {
	"rampage2", "rampage", nullptr, nullptr, "1986",
	"Rampage (Rev 2, 8/4/86)\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_PRE90S, GBF_ACTION, 0,
	nullptr, rampage2RomInfo, rampage2RomName, nullptr, nullptr, nullptr, nullptr, RampageInputInfo, RampageDIPInfo,
	RampageInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc, 0x40,
	512, 480, 4, 3
};
...
struct BurnDriver BurnDrvPowerdrv = {
	"powerdrv", nullptr, nullptr, nullptr, "1986",
	"Power Drive\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 3, HARDWARE_MISC_PRE90S, GBF_RACING, 0,
	nullptr, powerdrvRomInfo, powerdrvRomName, nullptr, nullptr, nullptr, nullptr, PowerdrvInputInfo, PowerdrvDIPInfo,
...
struct BurnDriver BurnDrvStargrds = {
	"stargrds", nullptr, nullptr, nullptr, "1987",
	"Star Guards\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 4, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, stargrdsRomInfo, stargrdsRomName, nullptr, nullptr, nullptr, nullptr, StargrdsInputInfo, StargrdsDIPInfo,
...
static void spyhunt_rom_swap()
{
	...
	{
		// swap halves of last program rom
...
struct BurnDriver BurnDrvSpyhunt = {
	"spyhunt", nullptr, "midssio", nullptr, "1983",
	"Spy Hunter\0", nullptr, "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_MISC_PRE90S,
	GBF_ACTION | GBF_SHOOT, 0,
	nullptr, spyhuntRomInfo, spyhuntRomName, nullptr, nullptr, nullptr, nullptr, SpyhuntInputInfo, SpyhuntDIPInfo,
...
struct BurnDriver BurnDrvSpyhuntp = {
	"spyhuntp", "spyhunt", "midssio", nullptr, "1983",
	"Spy Hunter (Playtronic license)\0", nullptr, "Bally Midway (Playtronic license)", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_MISC_PRE90S,
	GBF_ACTION | GBF_SHOOT, 0,
	nullptr, spyhuntpRomInfo, spyhuntpRomName, nullptr, nullptr, nullptr, nullptr, SpyhuntInputInfo, SpyhuntDIPInfo,
	SpyhuntInit, DrvExit, CSDSSIOFrame, SpyhuntDraw, DrvScan, &DrvRecalc, 0x40,
	480, 480, 3, 4
};
...
struct BurnDriverD BurnDrvCrater = {
	"crater", nullptr, "midssio", nullptr, "1984",
	"Crater Raider\0", "Graphics Issues", "Bally Midway", "MCR3",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_NOT_WORKING, 2, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, craterRomInfo, craterRomName, nullptr, nullptr, nullptr, nullptr, CraterInputInfo, CraterDIPInfo,
