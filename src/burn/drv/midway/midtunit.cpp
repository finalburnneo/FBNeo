...
static const UINT8* JdreddpProtTable = nullptr;
...
static INT32 MemIndex()
{
	...
	DrvBootROM = Next;
	Next += 0x800000 * sizeof(UINT8);
	DrvSoundROM = Next;
	Next += 0x1000000 * sizeof(UINT8);
	DrvGfxROM = Next;
	Next += 0x2000000 * sizeof(UINT8);
	DrvSoundProgROM = Next;
	Next += 0x0040000 * sizeof(UINT8);

	DrvNVRAM = Next;
	Next += 0x002000 * sizeof(UINT16);

	AllRam = Next;
	DrvRAM = Next;
	Next += TOBYTE(0x400000) * sizeof(UINT16);
	DrvSoundProgRAM = Next;
	Next += 0x002000 * sizeof(UINT8);
	DrvSoundProgRAMProt = Next;
	Next += 0x000100 * sizeof(UINT8);
	DrvPalette = Next;
	Next += 0x20000 * sizeof(UINT8);
	DrvPaletteB = (UINT32*)Next;
...
{
	INT32 cyc = (static_cast<double>(TMS34010TotalCycles()) / 63 * 100) - Dcs2kTotalCycles();
	if (cyc > 0)
	{
		Dcs2kRun(cyc);
	}
...
static void MKsound_msm6295bankswitch(INT32 bank)
{
	...
	{
	case 0:
	case 1: bank_offs = 0x40000;
		break;
	case 2: bank_offs = 0x20000;
		break;
	case 3: bank_offs = 0x00000;
		break;
	case 4: bank_offs = 0xe0000;
		break;
	case 5: bank_offs = 0xc0000;
		break;
	case 6: bank_offs = 0xa0000;
		break;
	case 7: bank_offs = 0x80000;
		break;
	}
...
static UINT8 MKSoundRead(UINT16 address)
{
	...
	{
		if (address >= SoundProtAddressStart && address <= SoundProtAddressEnd)
		{
			// MK sound protection ram.
...
static UINT16 TUnitVramRead(UINT32 address)
{
	...
	if (nVideoBank)
		return (BURN_ENDIAN_SWAP_INT16(DrvVRAM16[offset]) & 0x00ff) | (BURN_ENDIAN_SWAP_INT16(DrvVRAM16[offset + 1]) <<
			8);
...
static void TUnitVramWrite(UINT32 address, UINT16 data)
{
	...
	{
		DrvVRAM16[offset] = BURN_ENDIAN_SWAP_INT16(
			(BURN_ENDIAN_SWAP_INT16(DrvVRAM16[offset]) & 0xff) | ((data & 0xff) << 8));
		DrvVRAM16[offset + 1] = BURN_ENDIAN_SWAP_INT16(
			(BURN_ENDIAN_SWAP_INT16(DrvVRAM16[offset + 1]) & 0xff) | (data & 0xff00));
	}
}
...
static constexpr UINT8 jdredd_prot_values_13240[] =
{
	0x28
};

static constexpr UINT8 jdredd_prot_values_76540[] =
{
	0x04, 0x08
};
...
INT32 TUnitInit()
{
	...
	if ((AllMem = BurnMalloc(nLen)) == nullptr)
		return 1;
	...
	TMS34010SetCpuCyclesPerFrame(static_cast<INT32>(midtunit_cpurate / 54.71));
	...
	if (TUnitIsMK || TUnitIsNbajam || TUnitIsNbajamTe || TUnitIsJdreddp)
	{
		...
		MSM6295Init(0, /*1000000 / MSM6295_PIN7_HIGH*/8000, true);
...
INT32 TUnitFrame()
{
	...
	INT32 nCyclesTotal[2] = {(50000000 / 8 / (nBurnFPS / 100)), (2000000 / (nBurnFPS / 100))};
	INT32 nCyclesDone[2] = {nExtraCycles, 0};

	if (nSoundType == SOUND_DCS)
	{
		nCyclesTotal[1] = 10000000 / (nBurnFPS / 100);
		Dcs2kNewFrame();
	}
