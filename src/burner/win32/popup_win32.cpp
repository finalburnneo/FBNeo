...
static TCHAR* pszBufferEnglish = nullptr;
static TCHAR* pszBufferLocal = nullptr;
	...
	case WM_DRAWITEM:
		{
			...
			{
				DrawIconEx(((LPDRAWITEMSTRUCT)lParam)->hDC, 0, 0,
				           static_cast<HICON>(LoadImage(nullptr, MAKEINTRESOURCE(nIcon), IMAGE_ICON, 0, 0,
				                                        LR_LOADTRANSPARENT | LR_SHARED)), 32, 32, 0, nullptr,
	...
	case WM_INITDIALOG:
		{
			...
			// Set the icon
			SendDlgItemMessage(hDlg, IDC_MESSAGE_ICON, STM_SETIMAGE, IMAGE_ICON,
			                   (LPARAM)LoadImage(nullptr, MAKEINTRESOURCE(nIcon), IMAGE_ICON, 32, 32, LR_SHARED));
...

			{
...

				{
					...
					HDC hDC = GetDC(hWnd);
					auto hFont = static_cast<HFONT>(SelectObject(hDC, hEditFont));
				...
				if (bLocalisationActive && pszBufferLocal)
				{
					...
					MoveWindow(hDlg, rect.left, rect.top, rect.right - rect.left + sizee.cx,
					           rect.bottom - rect.top + sizee.cy + sizel.cy + 12, FALSE);

					hWnd = GetDlgItem(hDlg, IDC_MESSAGE_EDIT_LOCAL);
					GetWindowRect(hWnd, &rect);
					SetWindowPos(hWnd, nullptr, 0, 0, rect.right - rect.left + sizel.cx,
					             rect.bottom - rect.top + sizel.cy, SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW);
					...
					SetWindowPos(hWnd, nullptr, 0, 0, 9999, rect.bottom - p.y + sizel.cy + 6,
				...
				else
				{
					...
					MoveWindow(hDlg, rect.left, rect.top, rect.right - rect.left + sizee.cx,
					           rect.bottom - rect.top + sizee.cy, FALSE);
				}

				hWnd = GetDlgItem(hDlg, IDC_MESSAGE_EDIT_ENG);
				SendMessage(hWnd, EM_SETBKGNDCOLOR, 0, GetSysColor(COLOR_3DFACE));
				GetWindowRect(hWnd, &rect);
				SetWindowPos(hWnd, nullptr, rect.left - p.x, rect.top - p.y, rect.right - rect.left + sizee.cx,
				             rect.bottom - rect.top + sizee.cy,
				             (pszBufferLocal ? 0 : SWP_NOMOVE) | SWP_NOZORDER | SWP_NOREDRAW);
				...
				hWnd = GetDlgItem(hDlg, IDOK);
				GetWindowRect(hWnd, &rect);
				SetWindowPos(hWnd, nullptr, p.x - (rect.right - rect.left) / 2, rect.top - p.y + sizee.cy, 0, 0,
...
int FBAPopupAddText(int nFlags, TCHAR* pszFormat, ...)
{
	...
	if (nFlags & PUF_TEXT_TRANSLATE)
	{
		...
		if (bLocalisationActive && (nFlags & PUF_TEXT_TRANSLATE) && pszStringLocal && *pszStringLocal)
		{
			...
			if (nLen > 0)
			{
				auto pszNewBuffer = static_cast<TCHAR*>(realloc(pszBufferLocal,
				                                                (nLen + nBufferLocalSize + 1) * sizeof(TCHAR)));
				if (pszNewBuffer)
				{
	...
	if (pszStringEnglish && *pszStringEnglish)
	{
		...
		if (nLen > 0)
		{
			auto pszNewBuffer = static_cast<TCHAR*>(realloc(pszBufferEnglish,
			                                                (nLen + nBufferEnglishSize + 1) * sizeof(TCHAR)));
			if (pszNewBuffer)
			{
