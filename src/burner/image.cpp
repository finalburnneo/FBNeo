...
INT32 img_alloc(IMAGE* img)
{
	...
	img->rowbytes = (static_cast<DWORD>(img->width) * 24 + 31) / 32 * 4;
	img->imgbytes = img->rowbytes * img->height;
	img->rowptr = static_cast<BYTE**>(malloc(static_cast<size_t>(img->height) * 4 * sizeof(BYTE*)));

	if (img->bmpbits == nullptr)
	{
		img->flags |= IMG_FREE;
		img->bmpbits = static_cast<BYTE*>(malloc(img->imgbytes));
	}

	if (img->rowptr == nullptr || img->bmpbits == nullptr)
	{
		img_free(img);
		return 1;
	}
...
static void interpolateRowH(const IMAGE* img, const INT32 y, double* row, const INT32 width)
{
	...
	{
		double f = static_cast<double>(x) * img->width / width;
		x2 = static_cast<INT32>(f);
		f -= x2;
...
static inline void interpolateRowV(const double f, const INT32 y, double** row, const IMAGE* img)
{
	...
	for (UINT32 x = 0; x < img->width * 3; x++)
	{
		...
		if (c[4] < 0.0) c[4] = 0.0;
		else if (c[4] > 255.0) c[4] = 255.0;

		img->rowptr[y][x] = static_cast<UINT8>(c[4]);
	}
}
...
static INT32 img_process(IMAGE* img, UINT32 width, UINT32 height, INT32 /*preset*/, bool /*swapRB*/)
{
	...
	double ratio = static_cast<double>(height * width) / (img->height * img->width);
	...
	if (ratio >= 1.0)
	{
		...
		for (INT32 i = 0; i < 4; i++)
		{
			row[i] = static_cast<double*>(malloc(width * (3 + 1) * sizeof(double)));
			// added +1 to prevent crash with corrupt png
		}
		...
		{
			double f = static_cast<double>(y) * img->height / height;

			y2 = static_cast<UINT32>(f);
			f -= y2;
	...
	for (UINT32 y = 0; y < sized_img.height; y++)
	{
		for (UINT32 x = 0; x < sized_img.width; x++)
		{
			...
			yf = static_cast<double>(img->height) * y / sized_img.height - y1;

			x1 = img->width * x / sized_img.width;
			xf = static_cast<double>(img->width) * x / sized_img.width - x1;
			r1 = static_cast<double>(img->rowptr[y1][x1 * 3 + 0]) * (1.0 - xf);
			g1 = static_cast<double>(img->rowptr[y1][x1 * 3 + 1]) * (1.0 - xf);
			b1 = static_cast<double>(img->rowptr[y1][x1 * 3 + 2]) * (1.0 - xf);

			for (x1 = x1 + 1; x1 < img->width * (x + 1) / sized_img.width; x1++)
			{
				r1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 0]);
				g1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 1]);
				b1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 2]);
			}

			if (x1 < img->width)
			{
				xf = static_cast<double>(img->width) * (x + 1) / sized_img.width - x1;
				r1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 0]) * xf;
				g1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 1]) * xf;
				b1 += static_cast<double>(img->rowptr[y1][x1 * 3 + 2]) * xf;
			}
			...
			if (y1 < img->height)
			{
				yf = static_cast<double>(img->height) * (y + 1) / sized_img.height - y1;
			...
			sized_img.rowptr[y][x * 3 + 0] = static_cast<UINT8>(r0 * ratio); // + 0.5;
			sized_img.rowptr[y][x * 3 + 1] = static_cast<UINT8>(g0 * ratio); // + 0.5;
			sized_img.rowptr[y][x * 3 + 2] = static_cast<UINT8>(b0 * ratio); // + 0.5;
		}
	}
...
INT32 PNGLoad(IMAGE* img, FILE* fp, INT32 nPreset)
{
	...
	else
	{
		...
		auto pResourceData = static_cast<BYTE*>(LockResource(hglobal));
	...
	if (img_process(&temp_img, img->width ? img->width : temp_img.width, img->height ? img->height : temp_img.height,
	                nPreset, false))
	{
		img_free(&temp_img);
		return 1;
...
{
	IMAGE temp_img = {0, 0, 0, 0, nullptr, nullptr, 0};
