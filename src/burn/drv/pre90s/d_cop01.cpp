...
{
	{0x12, 0xff, 0xff, 0x67, nullptr},
	{0x13, 0xff, 0xff, 0x3f, nullptr},
	{0x14, 0xff, 0xff, 0x07, nullptr},
...
static void dac_recalc_freq()
{
	INT32 LastIdx = -1;
	INT32 Idx = 0;
	constexpr INT32 interleave = 256;

	for (INT32 i = 0; i < interleave; i++)
	{
		Idx = static_cast<INT32>(round(((double)prot_dac_freq / 60 / (double)interleave) * (double)i));
...
static void mightguy_prot_data_write(UINT8 data)
{
	...
	case 0x33: prot_rom_address &= 0x00ff;
		prot_rom_address |= data << 8;
		break;
	case 0x34: prot_rom_address &= 0xff00;
		prot_rom_address |= data;
		break;
	case 0x35: prot_adj_address &= 0x00ff;
		prot_adj_address |= data << 8;
		break;
	case 0x36: prot_adj_address &= 0xff00;
		prot_adj_address |= data;
		break;
	case 0x40: prot_mgtimer = 9350 / prot_timer_rate;
		prot_mgtimer_count = 0;
		break;
	case 0x41: prot_mgtimer = 0;
		prot_timer_reg = 0;
		break;
	case 0x51: prot_dac_start_address &= 0x00ff;
		prot_dac_start_address |= data << 8;
		break;
	case 0x52: prot_dac_start_address &= 0xff00;
		prot_dac_start_address |= data;
		break;
	case 0x11:
		if (data == 0)
		{
		...
		break;
	case 0x18: prot_timer_rate = (data >> 4) - 11;
		prot_dac_freq = (prot_timer_rate * 71) * 18;
		dac_recalc_freq();
		break;
	case 0x90: prot_const90 = data;
		break;
#if 0
		case 0x42: break;
		case 0x43: break;
		case 0x92: break;
#endif
	default: DrvProtRAM[protection_command] = data;
		break; //bprintf(0, _T("%02X %02X\n"), protection_command, data); break;
	}
}
...
static void DrvPaletteInit()
{
	...
	for (INT32 i = 0; i < 0x100; i++)
	{
		DrvPalette[0x100 + i] = DrvPalette[0xc0 | (i & 0x30) | (DrvColPROM[0x300 | ((i & 0x40) >> 2) | (i & 0x0f)] &
			0x0f)];
...
static INT32 MightguyFrame()
{
	...
	for (INT32 i = 0; i < nInterleave; i++)
	{
		...
		if (dac_intrl_table[i])
		{
			// clock-in the dac @ prot_dac_freq (see dac_recalc_freq())
...
struct BurnDriver BurnDrvCop01 = {
	"cop01", nullptr, nullptr, nullptr, "1985",
	"Cop 01 (set 1)\0", nullptr, "Nichibutsu", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PLATFORM, 0,
	nullptr, cop01RomInfo, cop01RomName, nullptr, nullptr, nullptr, nullptr, Cop01InputInfo, Cop01DIPInfo,
...
struct BurnDriver BurnDrvCop01a = {
	"cop01a", "cop01", nullptr, nullptr, "1985",
	"Cop 01 (set 2)\0", nullptr, "Nichibutsu", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PLATFORM, 0,
	nullptr, cop01aRomInfo, cop01aRomName, nullptr, nullptr, nullptr, nullptr, Cop01InputInfo, Cop01DIPInfo,
	Cop01Init, DrvExit, Cop01Frame, DrvDraw, DrvScan, &DrvRecalc, 0x190,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvMightguy = {
	"mightguy", nullptr, nullptr, nullptr, "1986",
	"Mighty Guy\0", nullptr, "Nichibutsu", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, mightguyRomInfo, mightguyRomName, nullptr, nullptr, nullptr, nullptr, Cop01InputInfo, MightguyDIPInfo,
