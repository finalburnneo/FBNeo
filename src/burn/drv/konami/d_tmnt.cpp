...
static UINT8* DrvSoundRom = nullptr;
static UINT8* DrvUPD7759CRom = nullptr;
static UINT8* DrvTileRom = nullptr;
static UINT8* DrvSpriteRom = nullptr;
static UINT8* DrvSpriteRam = nullptr;
static UINT8* DrvPaletteRam = nullptr;
static UINT8* DrvNvRam = nullptr;
...
static INT16* DrvTitleSample = nullptr;
...
static constexpr eeprom_interface BlswhstlEEPROMInterface =
{
	7,
	8,
	"011000",
...
static constexpr eeprom_interface thndrx2_eeprom_interface =
{
	7,
	8,
	"011000",
...
static struct BurnDIPInfo SsridershsDIPList[] =
{
	...
	// Fake DIPs
	{0x00, 0xff, 0xff, 0x00, nullptr}, // 1v4
...
{
	{0x21, 0xff, 0xff, 0xff, nullptr},
	{0x22, 0xff, 0xff, 0x7f, nullptr},
	{0x23, 0xff, 0xff, 0xf0, nullptr},
...
static INT32 TmntMemIndex()
{
	...
	DrvTitleSample = (INT16*)Next;
	Next += 0x40000 * sizeof(INT16);
	DrvTiles = Next;
	Next += 0x008000 * 8 * 8;
	DrvSprites = Next;
	Next += 0x004000 * 16 * 16;
...
{
	static const UINT8 thndrx2_eeprom_data[128] = {
		// Enable stereo sound (Dink)
	...
	static const UINT8 thndrx2j_eeprom_data[128] = {
		// Enable stereo sound (Dink)
	...
	static const UINT8 thndrx2a_eeprom_data[128] = {
		// Enable stereo sound (Dink)
	...
	else
	{
		...
		else if (strncmp(BurnDrvGetTextA(DRV_NAME), "thndrx2a", 8) == 0)
		{
			EEPROMFill(thndrx2a_eeprom_data, 0, 128);
		}
		else if (strncmp(BurnDrvGetTextA(DRV_NAME), "thndrx2", 7) == 0)
		{
...
static UINT16 __fastcall Blswhstl68KReadWord(UINT32 a)
{
	if (a >= 0x300000 && a <= 0x303fff)
	{
		auto SpriteRam = (UINT16*)DrvSpriteRam;
...
static void tmnt2_protection_write()
{
	...
	xoffs = static_cast<INT16>(src[2]); // local x
	yoffs = static_cast<INT16>(src[3]); // local y
	...
	xmod = static_cast<INT16>(mod[6]); // global x
	ymod = static_cast<INT16>(mod[7]); // global y
	zmod = static_cast<INT16>(mod[8]); // global z
	xzoom = mod[0x1c / 2];
	...
	if (!xlock)
	{
		...
		{
			i >>= 8;
			xoffs += static_cast<INT32>(pow(i, /*1.891292*/1.841292) * xoffs / 599.250121);
		}
		else if (i < 0)
		{
	...
	if (!ylock)
	{
		...
		{
			i >>= 8;
			yoffs += static_cast<INT32>(pow(i, /*1.891292*/1.841292) * yoffs / 599.250121);
		}
		else if (i < 0)
		{
	...
	SekWriteWord((dst_addr + 4) * 2, static_cast<UINT32>(yoffs));
	SekWriteWord((dst_addr + 6) * 2, static_cast<UINT32>(xoffs));
...
static void ssriders_protection_w(INT32 offset)
{
	...
	if (offset == 1)
	{
		...
		for (logical_pri = 1; logical_pri < 0x100; logical_pri <<= 1)
		{
			...
			for (i = 0; i < 128; i++)
			{
				if ((SekReadWord(0x180006 + 128 * i) >> 8) == static_cast<UINT16>(logical_pri))
				{
...
{
	*Code |= ((*Colour & 0x01) << 8) | ((*Colour & 0x10) << 5) | ((*Colour & 0x0c) << 8) | (Bank << 12) | (
		BlswhstlTileRomBank << 14);
	*Colour = LayerColourBase[Layer] + ((*Colour & 0xe0) >> 5);
}
...
static INT32 TmntInit()
{
	...
	nRet = BurnLoadRom(Drv68KRom + 0x000001, 0, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x000000, 1, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x040001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x040000, 3, 2);
	if (nRet != 0) return 1;

	// Load Z80 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom, 4, 1);
	if (nRet != 0) return 1;

	// Load and decode the tiles
	nRet = BurnLoadRom(DrvTileRom + 0x000000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x080000, 6, 1);
	if (nRet != 0) return 1;
	...
	// Load the sprites
	DrvTempRom = BurnMalloc(0x200100);
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x080000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x100000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x180000, 10, 1);
	if (nRet != 0) return 1;
	...
	// Decode the sprites
	GfxDecode(0x200000 / 128, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x400, DrvSpriteRom,
	          DrvSprites);

	// Load K007232 Sample Roms
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 13, 1);
	if (nRet != 0) return 1;

	// Load UPD7759C Sample Roms
	nRet = BurnLoadRom(DrvUPD7759CRom + 0x00000, 14, 1);
	if (nRet != 0) return 1;
...
static INT32 MiaInit()
{
	...
	// Load Z80 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom, 2, 1);
	if (nRet != 0) return 1;

	// Load and decode the tiles
	nRet = BurnLoadRom(DrvTileRom + 0x000000, 3, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x000001, 4, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x020000, 5, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x020001, 6, 2);
	if (nRet != 0) return 1;
	...
	// Load the sprites
	DrvTempRom = BurnMalloc(0x100000);
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x080000, 8, 1);
	if (nRet != 0) return 1;
	...
	// Load K007232 Sample Roms
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 10, 1);
	if (nRet != 0) return 1;
...
static INT32 CuebrickInit()
{
	...
	// Load and decode the tiles
	nRet = BurnLoadRom(DrvTileRom + 0x000000, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x000001, 3, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x020000, 4, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x020001, 5, 2);
	if (nRet != 0) return 1;
	byte_shuffle(DrvTileRom, 0x040000);
	GfxDecode(0x2000, 4, 8, 8, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTileRom, DrvTiles);

	// Load the sprites
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 6, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x000001, 7, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x020000, 8, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x020001, 9, 2);
	if (nRet != 0) return 1;
...
static INT32 BlswhstlInit()
{
	...
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 9, 1);
	if (nRet != 0) return 1;
...
static INT32 SsridersInit()
{
	...
	nRet = BurnLoadRom(Drv68KRom + 0x000000, 1, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x080001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x080000, 3, 2);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x100000, 8, 1);
	if (nRet != 0) return 1;
...
static INT32 Thndrx2Init()
{
	...
	// Load and decode the tiles
	nRet = BurnLoadRom(DrvTileRom + 0x000000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x080000, 4, 1);
	if (nRet != 0) return 1;
	konami_rom_deinterleave_2(DrvTileRom, 0x100000);
	GfxDecode(0x8000, 4, 8, 8, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTileRom, DrvTiles);

	// Load the sprites
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x080000, 6, 1);
	if (nRet != 0) return 1;
	konami_rom_deinterleave_2(DrvSpriteRom, 0x100000);
	GfxDecode(0x2000, 4, 16, 16, SpritePlaneOffsets2, SpriteXOffsets, SpriteYOffsets, 0x400, DrvSpriteRom, DrvSprites);

	// Load K053260 Sample Roms
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 7, 1);
	if (nRet != 0) return 1;
...
static INT32 Tmnt2Init()
{
	...
	nRet = BurnLoadRom(DrvSpriteRom + 0x100000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x200000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x300000, 10, 1);
	if (nRet != 0) return 1;
	konami_rom_deinterleave_2(DrvSpriteRom, 0x400000);
	K053245GfxDecode(DrvSpriteRom, DrvSprites, 0x400000);

	// Load K053260 Sample Roms
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 11, 1);
	if (nRet != 0) return 1;
...
static INT32 QgakumonInit()
{
	...
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x200000, 8, 1);
	if (nRet != 0) return 1;
	konami_rom_deinterleave_2(DrvSpriteRom, 0x400000);
	K053245GfxDecode(DrvSpriteRom, DrvSprites, 0x400000);
...
static INT32 PunkshotInit()
{
	...
	nRet = BurnLoadRom(DrvTileRom + 0x000000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTileRom + 0x040000, 4, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvSpriteRom + 0x000000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSpriteRom + 0x100000, 6, 1);
	if (nRet != 0) return 1;
	konami_rom_deinterleave_2(DrvSpriteRom, 0x200000);
...
static void DrvCalcPalette()
{
	...
	{
		INT32 Offset = i & ~1;
		UINT32 Data = (BURN_ENDIAN_SWAP_INT16(PaletteRam[Offset]) << 8) |
			BURN_ENDIAN_SWAP_INT16(PaletteRam[Offset + 1]);
...
struct BurnDriver BurnDrvTmnt = {
	"tmnt", nullptr, nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (World 4 Players, version X)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntRomInfo, TmntRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmntu = {
	"tmntu", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (US 4 Players, version R)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntuRomInfo, TmntuRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmntua = {
	"tmntua", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (US 4 Players, version N)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntuaRomInfo, TmntuaRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmntub = {
	"tmntub", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (US 4 Players, version J)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntubRomInfo, TmntubRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmntuc = {
	"tmntuc", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (US 4 Players, version H)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntucRomInfo, TmntucRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmht = {
	"tmht", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Hero Turtles (UK 4 Players, version F)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmhtRomInfo, TmhtRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmhta = {
	"tmhta", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Hero Turtles (UK 4 Players, version S)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmhtaRomInfo, TmhtaRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmhtb = {
	"tmhtb", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Hero Turtles (UK 4 Players, version ?)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmhtbRomInfo, TmhtbRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmntj = {
	"tmntj", "tmnt", nullptr, nullptr, "1990",
	"Teenage Mutant Ninja Turtles (Japan 4 Players, version 2)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntjRomInfo, TmntjRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmnta = {
	"tmnta", "tmnt", nullptr, nullptr, "1990",
	"Teenage Mutant Ninja Turtles (Asia 4 Players, version ?)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, TmntaRomInfo, TmntaRomName, nullptr, nullptr, nullptr, nullptr, TmntInputInfo, TmntDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmht2p = {
	"tmht2p", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Hero Turtles (UK 2 Players, version U)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, Tmht2pRomInfo, Tmht2pRomName, nullptr, nullptr, nullptr, nullptr, Tmnt2pInputInfo, Tmnt2pDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmht2pa = {
	"tmht2pa", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Hero Turtles (UK 2 Players, version ?)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, Tmht2paRomInfo, Tmht2paRomName, nullptr, nullptr, nullptr, nullptr, Tmnt2pInputInfo, Tmnt2pDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmht2pj = {
	"tmnt2pj", "tmnt", nullptr, nullptr, "1990",
	"Teenage Mutant Ninja Turtles (Japan 2 Players, version 1)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, Tmnt2pjRomInfo, Tmnt2pjRomName, nullptr, nullptr, nullptr, nullptr, Tmnt2pInputInfo, Tmnt2pDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvTmht2po = {
	"tmnt2po", "tmnt", nullptr, nullptr, "1989",
	"Teenage Mutant Ninja Turtles (Oceania 2 Players, version ?)\0", nullptr, "Konami", "GX963",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, Tmnt2poRomInfo, Tmnt2poRomName, nullptr, nullptr, nullptr, nullptr, Tmnt2pInputInfo, Tmnt2pDIPInfo,
	TmntInit, TmntExit, TmntFrame, TmntDraw, TmntScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvMia = {
	"mia", nullptr, nullptr, nullptr, "1989",
	"M.I.A. - Missing in Action (version T)\0", nullptr, "Konami", "GX808",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, MiaRomInfo, MiaRomName, nullptr, nullptr, nullptr, nullptr, MiaInputInfo, MiaDIPInfo,
	MiaInit, MiaExit, MiaFrame, TmntDraw, MiaScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvMia2 = {
	"mia2", "mia", nullptr, nullptr, "1989",
	"M.I.A. - Missing in Action (version S)\0", nullptr, "Konami", "GX808",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, Mia2RomInfo, Mia2RomName, nullptr, nullptr, nullptr, nullptr, MiaInputInfo, MiaDIPInfo,
	MiaInit, MiaExit, MiaFrame, TmntDraw, MiaScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvCuebrick = {
	"cuebrick", nullptr, nullptr, nullptr, "1989",
	"Cue Brick (World, version D)\0", nullptr, "Konami", "GX903",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_PUZZLE, 0,
	nullptr, CuebrickRomInfo, CuebrickRomName, nullptr, nullptr, nullptr, nullptr, MiaInputInfo, MiaDIPInfo,
	CuebrickInit, CommonExit, CuebrickFrame, TmntDraw, CuebrickScan,
	nullptr, 0x400, 304, 224, 4, 3
};

struct BurnDriver BurnDrvBlswhstl = {
	"blswhstl", nullptr, nullptr, nullptr, "1991",
	"Bells & Whistles (World, version L)\0", nullptr, "Konami", "GX060",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, BlswhstlRomInfo, BlswhstlRomName, nullptr, nullptr, nullptr, nullptr, BlswhstlInputInfo, nullptr,
	BlswhstlInit, BlswhstlExit, BlswhstlFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 224, 280, 3, 4
};

struct BurnDriver BurnDrvBlswhstla = {
	"blswhstla", "blswhstl", nullptr, nullptr, "1991",
	"Bells & Whistles (Asia, version M)\0", nullptr, "Konami", "GX060",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, BlswhstlaRomInfo, BlswhstlaRomName, nullptr, nullptr, nullptr, nullptr, BlswhstlInputInfo, nullptr,
	BlswhstlInit, BlswhstlExit, BlswhstlFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 224, 280, 3, 4
};

struct BurnDriver BurnDrvDetatwin = {
	"detatwin", "blswhstl", nullptr, nullptr, "1991",
	"Detana!! Twin Bee (Japan ver. J)\0", nullptr, "Konami", "GX060",
	L"\u51FA\u305F\u306A!! Twin Bee \u30C4\u30A4\u30F3\u30D3\u30FC (Japan, version J)\0", nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, DetatwinRomInfo, DetatwinRomName, nullptr, nullptr, nullptr, nullptr, BlswhstlInputInfo, nullptr,
	BlswhstlInit, BlswhstlExit, BlswhstlFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 224, 280, 3, 4
};

struct BurnDriver BurnDrvSsriders = {
	"ssriders", nullptr, nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. EAC)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersRomInfo, SsridersRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersebd = {
	"ssridersebd", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (2 Players ver. EBD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersebdRomInfo, SsridersebdRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersebc = {
	"ssridersebc", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (2 Players ver. EBC)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersebcRomInfo, SsridersebcRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersuda = {
	"ssridersuda", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. UDA)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersudaRomInfo, SsridersudaRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4psInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsriderseaa = {
	"ssriderseaa", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. EAA)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsriderseaaRomInfo, SsriderseaaRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4psInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersuab = {
	"ssridersuab", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. UAB)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersuabRomInfo, SsridersuabRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersuac = {
	"ssridersuac", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. UAC)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersuacRomInfo, SsridersuacRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersubc = {
	"ssridersubc", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (2 Players ver. UBC)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersubcRomInfo, SsridersubcRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersabd = {
	"ssridersabd", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (2 Players ver. ABD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersabdRomInfo, SsridersabdRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersadd = {
	"ssridersadd", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. ADD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersaddRomInfo, SsridersaddRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4psInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersjac = {
	"ssridersjac", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. JAC)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersjacRomInfo, SsridersjacRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersjad = {
	"ssridersjad", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (4 Players ver. JAD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersjadRomInfo, SsridersjadRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvSsridersjbd = {
	"ssridersjbd", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (2 Players ver. JBD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, SsridersjbdRomInfo, SsridersjbdRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	SsridersInit, BlswhstlExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};
...
struct BurnDriverD BurnDrvSsridersb = {
	"ssridersb", "ssriders", nullptr, nullptr, "1991",
	"Sunset Riders (bootleg 4 Players ver. ADD)\0", nullptr, "Konami", "GX064",
	nullptr, nullptr, nullptr, nullptr,
	BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_RUNGUN, 0,
	nullptr, ssridersbRomInfo, ssridersbRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4psInputInfo, nullptr,
	SsridersbInit, CommonExit, SsridersFrame, BlswhstlDraw, SsridersScan,
	nullptr, 0x810, 288, 224, 4, 3
};

struct BurnDriver BurnDrvThndrx2 = {
	"thndrx2", nullptr, nullptr, nullptr, "1991",
	"Thunder Cross II (World)\0", nullptr, "Konami", "GX073",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_HORSHOOT, 0,
	nullptr, thndrx2RomInfo, thndrx2RomName, nullptr, nullptr, nullptr, nullptr, Thndrx2InputInfo, nullptr,
	Thndrx2Init, BlswhstlExit, Thndrx2Frame, Thndrx2Draw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvThndrx2a = {
	"thndrx2a", "thndrx2", nullptr, nullptr, "1991",
	"Thunder Cross II (Asia)\0", nullptr, "Konami", "GX073",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_HORSHOOT, 0,
	nullptr, thndrx2aRomInfo, thndrx2aRomName, nullptr, nullptr, nullptr, nullptr, Thndrx2InputInfo, nullptr,
	Thndrx2Init, BlswhstlExit, Thndrx2Frame, Thndrx2Draw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvThndrx2j = {
	"thndrx2j", "thndrx2", nullptr, nullptr, "1991",
	"Thunder Cross II (Japan)\0", nullptr, "Konami", "GX073",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_HORSHOOT, 0,
	nullptr, thndrx2jRomInfo, thndrx2jRomName, nullptr, nullptr, nullptr, nullptr, Thndrx2InputInfo, nullptr,
	Thndrx2Init, BlswhstlExit, Thndrx2Frame, Thndrx2Draw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvLgtnfght = {
	"lgtnfght", nullptr, nullptr, nullptr, "1990",
	"Lightning Fighters (World)\0", nullptr, "Konami", "GX939",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, lgtnfghtRomInfo, lgtnfghtRomName, nullptr, nullptr, nullptr, nullptr, LgtnfghtInputInfo, LgtnfghtDIPInfo,
	LgtnfghtInit, BlswhstlExit, LgtnfghtFrame, BlswhstlDraw, Thndrx2aScan,
	nullptr, 0x800, 224, 288, 3, 4
};

struct BurnDriver BurnDrvLgtnfghtu = {
	"lgtnfghtu", "lgtnfght", nullptr, nullptr, "1990",
	"Lightning Fighters (US)\0", nullptr, "Konami", "GX939",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, lgtnfghtuRomInfo, lgtnfghtuRomName, nullptr, nullptr, nullptr, nullptr, LgtnfghtInputInfo, LgtnfghtDIPInfo,
	LgtnfghtInit, BlswhstlExit, LgtnfghtFrame, BlswhstlDraw, Thndrx2aScan,
	nullptr, 0x800, 224, 288, 3, 4
};

struct BurnDriver BurnDrvLgtnfghta = {
	"lgtnfghta", "lgtnfght", nullptr, nullptr, "1990",
	"Lightning Fighters (Asia)\0", nullptr, "Konami", "GX939",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, lgtnfghtaRomInfo, lgtnfghtaRomName, nullptr, nullptr, nullptr, nullptr, LgtnfghtInputInfo, LgtnfghtDIPInfo,
	LgtnfghtInit, BlswhstlExit, LgtnfghtFrame, BlswhstlDraw, Thndrx2aScan,
	nullptr, 0x800, 224, 288, 3, 4
};

struct BurnDriver BurnDrvTrigon = {
	"trigon", "lgtnfght", nullptr, nullptr, "1990",
	"Trigon (Japan)\0", nullptr, "Konami", "GX939",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_KONAMI_68K_Z80, GBF_VERSHOOT, 0,
	nullptr, trigonRomInfo, trigonRomName, nullptr, nullptr, nullptr, nullptr, LgtnfghtInputInfo, LgtnfghtDIPInfo,
	LgtnfghtInit, BlswhstlExit, LgtnfghtFrame, BlswhstlDraw, Thndrx2aScan,
	nullptr, 0x800, 224, 288, 3, 4
};

struct BurnDriver BurnDrvTmnt2 = {
	"tmnt2", nullptr, nullptr, nullptr, "1991",
	"Teenage Mutant Ninja Turtles - Turtles in Time (4 Players ver. UAA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmnt2RomInfo, tmnt2RomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvTmnt22pu = {
	"tmnt22pu", "tmnt2", nullptr, nullptr, "1991",
	"Teenage Mutant Ninja Turtles - Turtles in Time (2 Players ver. UDA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmnt22puRomInfo, tmnt22puRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvTmnt24pu = {
	"tmnt24pu", "tmnt2", nullptr, nullptr, "1991",
	"Teenage Mutant Ninja Turtles - Turtles in Time (4 Players ver. UEA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmnt24puRomInfo, tmnt24puRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvTmht22pe = {
	"tmht22pe", "tmnt2", nullptr, nullptr, "1991",
	"Teenage Mutant Hero Turtles - Turtles in Time (2 Players ver. EBA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmht22peRomInfo, tmht22peRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvTmht24pe = {
	"tmht24pe", "tmnt2", nullptr, nullptr, "1991",
	"Teenage Mutant Hero Turtles - Turtles in Time (4 Players ver. EAA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmht24peRomInfo, tmht24peRomName, nullptr, nullptr, nullptr, nullptr, SsridersInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvTmnt2a = {
	"tmnt2a", "tmnt2", nullptr, nullptr, "1991",
	"Teenage Mutant Ninja Turtles - Turtles in Time (4 Players ver. ADA)\0", nullptr, "Konami", "GX063",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SCRFIGHT, 0,
	nullptr, tmnt2aRomInfo, tmnt2aRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4psInputInfo, nullptr,
	Tmnt2Init, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvQgakumon = {
	"qgakumon", nullptr, nullptr, nullptr, "1993",
	"Quiz Gakumon no Susume (Japan ver. JA2 Type L)\0", nullptr, "Konami", "GX248",
	L"\u30AF\u30A4\u30BA \u5B66\u554F\u30CE\u30B9\u30B9\u30E1 Gakumon no Susume (Japan ver. JA2 Type L)\0", nullptr,
	nullptr, nullptr,
	BDF_GAME_WORKING, 2, HARDWARE_KONAMI_68K_Z80, GBF_QUIZ, 0,
	nullptr, qgakumonRomInfo, qgakumonRomName, nullptr, nullptr, nullptr, nullptr, QgakumonInputInfo, nullptr,
	QgakumonInit, BlswhstlExit, Tmnt2Frame, BlswhstlDraw, SsridersScan,
	nullptr, 0x800, 304, 224, 4, 3
};

struct BurnDriver BurnDrvPunkshot = {
	"punkshot", nullptr, nullptr, nullptr, "1990",
	"Punk Shot (US 4 Players)\0", nullptr, "Konami", "GX907",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 4, HARDWARE_KONAMI_68K_Z80, GBF_SPORTSMISC, 0,
	nullptr, punkshotRomInfo, punkshotRomName, nullptr, nullptr, nullptr, nullptr, PunkshotInputInfo, PunkshotDIPInfo,
	PunkshotInit, PunkshotExit, PunkshotFrame, PunkshotDraw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};

struct BurnDriver BurnDrvPunkshot2 = {
	"punkshot2", "punkshot", nullptr, nullptr, "1990",
	"Punk Shot (US 2 Players)\0", nullptr, "Konami", "GX907",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SPORTSMISC, 0,
	nullptr, punkshot2RomInfo, punkshot2RomName, nullptr, nullptr, nullptr, nullptr, Punkshot2InputInfo,
	Punkshot2DIPInfo,
	PunkshotInit, PunkshotExit, PunkshotFrame, PunkshotDraw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvPunkshot2e = {
	"punkshot2e", "punkshot", nullptr, nullptr, "1990",
	"Punk Shot (World 2 Players)\0", nullptr, "Konami", "GX907",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SPORTSMISC, 0,
	nullptr, punkshot2eRomInfo, punkshot2eRomName, nullptr, nullptr, nullptr, nullptr, Punkshot2InputInfo,
	Punkshot2DIPInfo,
	PunkshotInit, PunkshotExit, PunkshotFrame, PunkshotDraw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvPunkshotj = {
	"punkshotj", "punkshot", nullptr, nullptr, "1990",
	"Punk Shot (Japan 2 Players)\0", nullptr, "Konami", "GX907",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_KONAMI_68K_Z80, GBF_SPORTSMISC, 0,
	nullptr, punkshotjRomInfo, punkshotjRomName, nullptr, nullptr, nullptr, nullptr, Punkshot2InputInfo,
	PunkshotjDIPInfo,
	PunkshotInit, PunkshotExit, PunkshotFrame, PunkshotDraw, Thndrx2aScan,
	nullptr, 0x800, 288, 224, 4, 3
};
...
struct BurnDriver BurnDrvSsridershs = {
	"ssridershs", "ssriders", nullptr, nullptr, "2019-2020",
	...
	nullptr, SsridershsRomInfo, SsridershsRomName, nullptr, nullptr, nullptr, nullptr, Ssriders4pVerSwInputInfo,
	SsridershsDIPInfo,
