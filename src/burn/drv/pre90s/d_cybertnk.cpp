...
{
	{0x0e, 0xff, 0xff, 0x08, nullptr},
	{0x0f, 0xff, 0xff, 0xe4, nullptr},
...
static INT32 MemIndex()
{
	...
	DrvScroll0 = (UINT16*)Next;
	Next += 0x0004 * sizeof(UINT16);
	DrvScroll1 = (UINT16*)Next;
	Next += 0x0004 * sizeof(UINT16);
	DrvScroll2 = (UINT16*)Next;
	Next += 0x0004 * sizeof(UINT16);
...
{
	auto spr = (UINT32*)DrvGfxROM4;

	for (INT32 x = 0; x < 0x200000 / 4; x++)
	{
		spr[x] = BURN_ENDIAN_SWAP_INT32(
			BITSWAP32(BURN_ENDIAN_SWAP_INT32(spr[x]), 27,26,25,24, 19,18,17,16, 11,10,9,8, 3,2,1,0, 31,30,29,28, 23,22,
				21,20, 15,14,13,12, 7,6,5,4 ));
	}
}
...
static INT32 DrvInit()
{
	...
	BurnY8950Init(2, 3579545, DrvSndROM0, 0x40000, DrvSndROM1, 0x80000, nullptr, &DrvSynchroniseStream, 0);
...
static void draw_layer(UINT8* ram_base, UINT8* gfx_base, UINT16* scroll_base, UINT8* transtab, INT32 color_base)
{
	...
	for (INT32 offs = 0; offs < 128 * 32; offs++)
	{
		...
		case 0: break;
		case 4: break;
		case 1: Render8x8Tile_Mask_Clip(pTransDraw, code, sx, sy, color, 4, 0, color_base, gfx_base);
			break;
		case 5: Render8x8Tile_Mask(pTransDraw, code, sx, sy, color, 4, 0, color_base, gfx_base);
			break;
		case 3: Render8x8Tile_Clip(pTransDraw, code, sx, sy, color, 4, color_base, gfx_base);
			break;
		case 7: Render8x8Tile(pTransDraw, code, sx, sy, color, 4, color_base, gfx_base);
			break;
		}
	}
}
...
{
	auto spr_ram = (UINT16*)DrvSprRAM;
	auto sprrom = (UINT32*)DrvGfxROM4;
	...
	for (INT32 offs = 0; offs < 0x1000 / 2; offs += 8)
	{
		...
		INT32 spr_offs = ((BURN_ENDIAN_SWAP_INT16(spr_ram[offs+0]) & 0x0007) << 16) + BURN_ENDIAN_SWAP_INT16(
			spr_ram[offs+1]);
...
static INT32 DrvDraw()
{
	...
	DrvPalette[0x2000] = ((nSpriteEnable & 0x3f) == 0x3f)
		                     ? BurnHighCol(0x00, 0x00, 0x00, 0)
		                     : BurnHighCol(0xff, 0x00, 0xff, 0); // black / magenta
...
static INT32 DrvFrame()
{
	...
	nCyclesTotal[0] = static_cast<INT32>(static_cast<INT64>(10000000) * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[1] = static_cast<INT32>(static_cast<INT64>(10000000) * nBurnCPUSpeedAdjust / (0x0100 * 60));
...
struct BurnDriver BurnDrvCybertnk = {
	"cybertnk", nullptr, nullptr, nullptr, "1988",
	"Cyber Tank (v1.4)\0", nullptr, "Coreland", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 2, HARDWARE_MISC_PRE90S, GBF_SHOOT, 0,
	nullptr, cybertnkRomInfo, cybertnkRomName, nullptr, nullptr, nullptr, nullptr, CybertnkInputInfo, CybertnkDIPInfo,
