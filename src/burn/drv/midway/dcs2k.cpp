...
{
	pSoundROM = static_cast<UINT8*>(ptr);
	nSoundSize = size;
...
void Dcs2kBoot()
{
	auto buffer = BurnMalloc(0x2000); // +0x1000 for adsp paging
...
void Dcs2kReset()
{
	...
	samples_from = static_cast<INT32>((double)((sample_rate * 100) / nBurnFPS) + 0.5);
}

static UINT16 ReadRAMBank(UINT32 address)
{
	auto p = (UINT16*)pSoundROM;
