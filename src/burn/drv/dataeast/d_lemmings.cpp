...
static inline void palette_update_one(INT32 offset)
{
	...
	DrvPalette[offset / 2] = BurnHighCol(
		BURN_ENDIAN_SWAP_INT16(p[offset+1]), BURN_ENDIAN_SWAP_INT16(p[offset+1]) >> 8,
		BURN_ENDIAN_SWAP_INT16(p[offset]), 0);
}
...
static INT32 MemIndex()
{
	...
	DrvGfxROM2 = Next;
	Next += 0x020000; // expanded char tiles

	pTempDraw = (UINT16*)Next;
	Next += 0x800 * 0x200 * sizeof(INT16);
...
static INT32 DrvGfxDecode()
{
	...
	INT32 XOffs[16] = {
		7, 6, 5, 4, 3, 2, 1, 0, 16 * 8 + 7, 16 * 8 + 6, 16 * 8 + 5, 16 * 8 + 4, 16 * 8 + 3, 16 * 8 + 2, 16 * 8 + 1,
		16 * 8 + 0
	};
	INT32 YOffs[16] = {
	...
	for (INT32 offs = 0; offs < 0x400; offs += 4)
	{
		...
		while (multi >= 0)
		{
			if (flipy)
			{
				if (flipx)
				{
					Render16x16Tile_Mask_FlipXY_Clip(pTransDraw, (code - multi * inc) & 0x7ff, sx, sy + -16 * multi,
					                                 color, 4, 0, color_offset, rom);
				}
				else
				{
					Render16x16Tile_Mask_FlipY_Clip(pTransDraw, (code - multi * inc) & 0x7ff, sx, sy + -16 * multi,
					                                color, 4, 0, color_offset, rom);
				}
			}
			else
			{
				if (flipx)
				{
					Render16x16Tile_Mask_FlipX_Clip(pTransDraw, (code - multi * inc) & 0x7ff, sx, sy + -16 * multi,
					                                color, 4, 0, color_offset, rom);
				}
				else
				{
					Render16x16Tile_Mask_Clip(pTransDraw, (code - multi * inc) & 0x7ff, sx, sy + -16 * multi, color, 4,
					                          0, color_offset, rom);
				}
			}
...
struct BurnDriver BurnDrvLemmings = {
	"lemmings", nullptr, nullptr, nullptr, "1991",
	"Lemmings (US prototype)\0", nullptr, "Data East USA", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_PROTOTYPE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_PUZZLE, 0,
	nullptr, lemmingsRomInfo, lemmingsRomName, nullptr, nullptr, nullptr, nullptr, LemmingsInputInfo, LemmingsDIPInfo,
