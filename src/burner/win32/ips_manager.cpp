...
static HWND hIpsDlg = nullptr;
static HWND hParent = nullptr;
static HWND hIpsList = nullptr;
		...
		if (fgets(s, sizeof(s), fp) != nullptr)
		{
			...
			while (fgets(s, sizeof(s), fp) != nullptr)
			{
				...
				if (desc)
				{
					...
					len += strlen(s) + 2;
					p1 = static_cast<char*>(malloc(len + 1));
					sprintf(p1, "%s\r\n%s", desc, s);
					if (desc)
					{
						free(desc);
				...
				{
					desc = static_cast<char*>(malloc(strlen(s) + 1));
					if (desc != nullptr)
						strcpy(desc, s);
				}
			}
	...
	if (hSearch != INVALID_HANDLE_VALUE)
	{
		...
		while (!Done)
		{
			...
			if (fp)
			{
				...
				if (PatchDesc == nullptr) PatchDesc = GetPatchDescByLangcode(fp, 0);
				// Simplified Chinese is the reference language (should always be available!!)
				if (PatchDesc == nullptr) PatchDesc = GetPatchDescByLangcode(fp, 1);

				bprintf(0, _T("PatchDesc [%s]\n"), PatchDesc);

				if (PatchDesc == nullptr)
				{
					PatchDesc = static_cast<TCHAR*>(malloc(1024));
				...
				while (Tokens != nullptr)
				{
					if (nNumTokens == 0)
					{
						...
						nNumNodes = SendMessage(hIpsList, TVM_GETCOUNT, 0, 0);
						...
						if (bAddItem)
						{
							...
							hItemHandles[nHandlePos] = (HTREEITEM)SendMessage(
								hIpsList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
							nHandlePos++;
						}
...
static INT32 IpsManagerInit()
{
	...
	_stprintf(szText, _T("%s") _T(SEPERATOR_1) _T("%s"), FBALoadStringEx(hAppInst, IDS_IPSMANAGER_TITLE, true),
	          szFullName);
	...
	SendDlgItemMessage(hIpsDlg, IDC_CHOOSE_LIST, CB_SETCURSEL, static_cast<WPARAM>(nIpsSelectedLanguage), 0);
...
static void RefreshPatch()
{
	...
	SendMessage(GetDlgItem(hIpsDlg, IDC_TEXTCOMMENT), WM_SETTEXT, 0, NULL);
	SendDlgItemMessage(hIpsDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hPreview);

	auto hSelectHandle = (HTREEITEM)SendMessage(hIpsList, TVM_GETNEXTITEM, TVGN_CARET, ~0U);
	...
	for (INT32 i = 0; i < nNumPatches; i++)
	{
		if (hSelectHandle == hPatchHandlesIndex[i])
		{
			...
			if (fp)
			{
				...
				SendMessage(GetDlgItem(hIpsDlg, IDC_TEXTCOMMENT), WM_SETTEXT, 0, (LPARAM)PatchDesc);
...
{
	SendDlgItemMessage(hIpsDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, NULL);
	...
	case WM_INITDIALOG:
		{
			...
			hPreview = PNGLoadBitmap(hIpsDlg, nullptr, 304, 228, 2);
			SendDlgItemMessage(hIpsDlg, IDC_SCREENSHOT_H, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hPreview);
...
INT32 IpsManagerCreate(HWND hParentWND)
{
	...
	FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_IPS_MANAGER), hParent, DefInpProc);
	return 1;
}
...
static void PatchFile(const char* ips_path, UINT8* base, bool readonly)
{
	...
	if (nullptr == (f = fopen(ips_path, "rb")))
	{
...
static UINT32 hexto32(const char* s)
{
	...
	while ((c = *s++))
	{
		...
		val = (val << 4) | static_cast<UINT32>(v);
	}
...
char* strqtoken(char* s, const char* delims)
{
	...
	else
	{
		...
		prev_str = static_cast<char*>(memchr(token, '\0', MAX_PATH));
	}
...
static void DoPatchGame(const char* patch_name, char* game_name, UINT32 crc, UINT8* base, bool readonly)
{
	char s[MAX_PATH];
	char* p = nullptr;
	char* rom_name = nullptr;
	char* ips_name = nullptr;
	char* ips_offs = nullptr;
	char* ips_crc = nullptr;
	UINT32 nIps_crc = 0;
	FILE* fp = nullptr;
	...
	if ((fp = fopen(patch_name, "rb")) != nullptr)
	{
		bool bTarget = false;
...
#define DELIM_TOKENS " \t\r\n()"
				...
				ips_name = strqtoken(nullptr, DELIM_TOKENS_NAME);
				if (!ips_name)
				{
					continue;
				}
				...
				if (nullptr != (ips_offs = strqtoken(nullptr, DELIM_TOKENS)))
				{
					...
					if (nRomOffset != 0)
					{
						// better get next token (crc)
						ips_offs = strqtoken(nullptr, DELIM_TOKENS);
					}
				}

				if (ips_offs != nullptr && stristr_int(ips_offs, "crc"))
				{
					ips_crc = strqtoken(nullptr, DELIM_TOKENS);
					if (ips_crc)
					{
				...
				{
					sprintf(ips_path, "%s%s\\%s%s", ips_dir, BurnDrvGetTextA(DRV_NAME), ips_name,
					        (has_ext) ? "" : IPS_EXT);
				}
...
{
	if (nullptr == (szTmp = strtok(nullptr, " \t\r\n")))
		return 0U;
...
void GetIpsDrvDefine()
{
	...
	for (INT32 i = 0; i < nActivePatches; i++)
	{
		...
		char str[MAX_PATH] = {0}, *ptr = nullptr, *tmp = nullptr;
		FILE* fp = nullptr;

		if (nullptr != (fp = fopen(ips_data, "rb")))
		{
			while (!feof(fp))
			{
				if (nullptr != fgets(str, sizeof(str), fp))
				{
					ptr = str;
					...
					if (nullptr == (tmp = strtok(ptr, " \t\r\n")))
						continue;
					if (0 != strcmp(tmp, "#define"))
						break;
					if (nullptr == (tmp = strtok(nullptr, " \t\r\n")))
						break;
