...
struct tms5220
{
    ...
    */
	...
	INT32 RDB_flag;					/* whether we should read data register or status register */
	INT32 schedule_dummy_read;			/* set after each load address, so that next read operation
...
static INT32 extract_bits(struct tms5220 *tms, INT32 count);
static INT32 parse_frame(struct tms5220 *tms, INT32 the_first_frame);
static void check_buffer_low(struct tms5220 *tms);
static void set_interrupt_state(struct tms5220 *tms, INT32 state);
...
static void *tms5220_create(void)
{
	...
	tms = (tms5220 *)malloc(sizeof(*tms));
	memset(tms, 0, sizeof(*tms));
	return tms;
}


static void tms5220_destroy(void *chip)
{
	free(chip);
}
...
void tms5220_scan(INT32 nAction, INT32 *pnMin)
{
	...
	if (nAction & ACB_DRIVER_DATA) {
		...
		ba.Data		= our_chip;
		ba.nLen		= STRUCT_SIZE_HELPER(tms5220, schedule_dummy_read);
...
static void tms5220_reset_chip(void *chip)
{
	struct tms5220 *tms = (tms5220 *)chip;
...
void tms5220_set_read(void *chip, INT32 (*func)(int))
{
	struct tms5220 *tms = (tms5220 *)chip;
...
static void tms5220_data_write(void *chip, INT32 data)
{
	struct tms5220 *tms = (tms5220 *)chip;
...
static UINT8 tms5220_status_read(void *chip)
{
	struct tms5220 *tms = (tms5220 *)chip;
...
static UINT8 tms5220_ready_read(void *chip)
{
	struct tms5220 *tms = (tms5220 *)chip;
...
static INT32 tms5220_cycles_to_ready(void *chip)
{
	struct tms5220 *tms = (tms5220 *)chip;
	INT32 answer;
...
UINT8 tms5220_cycles()
{
	return tms5220_cycles_to_ready(our_chip);
}
...
static INT32 tms5220_int_read(void *chip)
{
	struct tms5220 *tms = (tms5220 *)chip;
    return tms->irq_pin;
}
...
UINT8 tms5220_irq()
{
	return tms5220_int_read(our_chip);
}
...
static INT32 parse_frame(struct tms5220 *tms, INT32 the_first_frame)
{
	INT32 bits = 0;	/* number of bits in FIFO (speak external only) */
