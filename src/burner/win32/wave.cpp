...
FILE* WaveLog = nullptr; // wave log file
...
static void MakeOfn(TCHAR* pszFilter)
{
	...
	ofn.lpstrInitialDir = nullptr; // default "Documents" folder.. _T(".\\wav");
...
static int WaveLogHeaderFillIn(FILE* Hand)
{
	...
	nLen = ftell(Hand);
	fseek(Hand, 0x04,SEEK_SET);
	nLen -= 8;
	fwrite(&nLen, 1, sizeof(nLen), Hand); // RIFF value
	fseek(Hand, 0x28,SEEK_SET);
	nLen -= 40;
	fwrite(&nLen, 1, sizeof(nLen), Hand); // data value
	fseek(Hand, 0,SEEK_END);
	return 0;
}
