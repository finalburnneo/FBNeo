...
struct Downsampler {
	...
	void resample(INT16 *in_buffer, INT16 *out_buffer, INT32 samples, double gain) {
		INT32 last_pos = 0;
#if DOWNSAMPLE_DEBUG
		extern int counter;
#endif
...
#endif
			// average + mixdown
			sample = (sample / samples_sourced) * gain;

			if (bAddStream) {
				out_buffer[0] = BURN_SND_CLIP(out_buffer[0] + sample);
				out_buffer[1] = BURN_SND_CLIP(out_buffer[1] + sample);
			} else {
				out_buffer[0] = BURN_SND_CLIP(sample);
				out_buffer[1] = BURN_SND_CLIP(sample);
			}
			out_buffer += 2;
