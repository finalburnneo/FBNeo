...
void xa_cpu::set_bit_8_helper(u16 bit, u8 val)
{
	...
	u8 sfr_val = sfr_read_byte(sfr) & mask;
	sfr_val |= (val << position);
	sfr_write_byte(sfr, sfr_val);
}
...
void xa_cpu::movc_word_rd_indrsinc(u8 rd, u8 rs) { u16 address = get_addr(rs); u16 data = program_read_word(address); address+=2; set_addr(rs, address); do_nz_flags_16(data); sr16(rd, data); cy(4); }
void xa_cpu::movc_byte_rd_indrsinc(u8 rd, u8 rs) { u16 address = get_addr(rs); u8 data = program_read_byte(address); address++; set_addr(rs, address); do_nz_flags_8(data); sr8(rd, data); cy(4); }
