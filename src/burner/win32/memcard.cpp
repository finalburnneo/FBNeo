...
{
	auto szHeader = "FB1 FC1 "; // File + chunk identifier
	char szReadHeader[8] = "";
	...
	else
	{
		...
		auto pTemp = static_cast<unsigned char*>(malloc(nSize >> 1));
...
static int MemCardWrite(TCHAR* szFilename, unsigned char* pData, int nSize)
{
	...
	if (bMemCardFC1Format)
	{
		...
		auto szFileHeader = "FB1 "; // File identifier
		auto szChunkHeader = "FC1 "; // Chunk identifier
		constexpr int nZero = 0;
...
int MemCardCreate()
{
...

	{
		...
		pCard = static_cast<unsigned char*>(malloc(nMemoryCardSize));
...
{
	if (MemCardRead(szMemoryCardFile, static_cast<unsigned char*>(pba->Data), pba->nLen))
	{
		return 1;
	}
...
{
	if (MemCardWrite(szMemoryCardFile, static_cast<unsigned char*>(pba->Data), pba->nLen) == 0)
	{
		nMemoryCardStatus &= ~2;
		MenuEnableItems();
