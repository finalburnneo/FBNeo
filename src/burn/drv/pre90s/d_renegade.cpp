...
static UINT8* DrvM6809Rom = nullptr;
static UINT8* DrvM68705Rom = nullptr;
static UINT8* DrvM68705Ram = nullptr;
static UINT8* DrvADPCMRom = nullptr;
static UINT8* DrvM6502Ram = nullptr;
static UINT8* DrvM6809Ram = nullptr;
static UINT8* DrvVideoRam1 = nullptr;
static UINT8* DrvVideoRam2 = nullptr;
static UINT8* DrvSpriteRam = nullptr;
static UINT8* DrvPaletteRam1 = nullptr;
static UINT8* DrvPaletteRam2 = nullptr;
static UINT8* DrvChars = nullptr;
static UINT8* DrvTiles = nullptr;
	...
	case 0x2000:
		{
			switch (Data & 0x1c)
			{
			case 0x18: DrvADPCMPos = 0 * 0x8000 * 2;
				break;
			case 0x14: DrvADPCMPos = 1 * 0x8000 * 2;
				break;
			case 0x0c: DrvADPCMPos = 2 * 0x8000 * 2;
				break;
			default: DrvADPCMPos = DrvADPCMEnd = 0;
				return;
			}
...
{
	return static_cast<INT64>(M6809TotalCycles()) * nSoundRate / 1500000;
}
...
static void DrvMSM5205Int()
{
	...
	{
		const UINT8 data = DrvADPCMRom[DrvADPCMPos / 2];
...
static INT32 DrvInit(INT32 nMcuType)
{
	...
	DrvTempRom = BurnMalloc(0x60000);

	nRet = BurnLoadRom(DrvM6502Rom + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvM6502Rom + 0x08000, 1, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvM6809Rom + 0x00000, 2, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom, 3, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x400, 3, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x100, DrvTempRom, DrvChars);

	memset(DrvTempRom, 0, 0x60000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 9, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvTiles + (0x000 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvTiles + (0x100 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvTiles + (0x200 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvTiles + (0x300 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvTiles + (0x400 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvTiles + (0x500 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvTiles + (0x600 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvTiles + (0x700 * 16 * 16));
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 12, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x38000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x48000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x58000, 21, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvSprites + (0x000 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvSprites + (0x100 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvSprites + (0x200 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x00000,
	          DrvSprites + (0x300 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvSprites + (0x400 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvSprites + (0x500 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvSprites + (0x600 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x18000,
	          DrvSprites + (0x700 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x30000,
	          DrvSprites + (0x800 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x30000,
	          DrvSprites + (0x900 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x30000,
	          DrvSprites + (0xa00 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x30000,
	          DrvSprites + (0xb00 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x48000,
	          DrvSprites + (0xc00 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x48000,
	          DrvSprites + (0xd00 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile3PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x48000,
	          DrvSprites + (0xe00 * 16 * 16));
	GfxDecode(0x100, 3, 16, 16, Tile4PlaneOffsets, TileXOffsets, TileYOffsets, 0x200, DrvTempRom + 0x48000,
	          DrvSprites + (0xf00 * 16 * 16));

	nRet = BurnLoadRom(DrvADPCMRom + 0x00000, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvADPCMRom + 0x08000, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvADPCMRom + 0x10000, 24, 1);
	if (nRet != 0) return 1;
	...
	{
		nRet = BurnLoadRom(DrvM68705Rom, 25, 1);
		if (nRet != 0) return 1;
...
static void DrvRenderBgLayer()
{
	...
	for (my = 0; my < 16; my++)
	{
		for (mx = 0; mx < 64; mx++)
		{
			...
			{
				Render16x16Tile_Clip(pTransDraw, Code, x, y, Colour, 3, 192,
				                     DrvTiles + ((Attr & 0x07) * 0x100 * 16 * 16));
			}
		...
		if (sy >= 16)
		{
			...
			if (Attr & 0x80)
			{
				...
				if (sx > 16 && sx < 224 && (sy + 16) > 0 && (sy + 16) < 224)
				{
					if (xFlip)
					{
						Render16x16Tile_Mask_FlipX(pTransDraw, Code + 1, sx, sy + 16, Colour, 3, 0, 128,
						                           DrvSprites + (SpriteBank * 0x100 * 16 * 16));
					}
					else
					{
						Render16x16Tile_Mask(pTransDraw, Code + 1, sx, sy + 16, Colour, 3, 0, 128,
						                     DrvSprites + (SpriteBank * 0x100 * 16 * 16));
					}
				}
				else
				{
					if (xFlip)
					{
						Render16x16Tile_Mask_FlipX_Clip(pTransDraw, Code + 1, sx, sy + 16, Colour, 3, 0, 128,
						                                DrvSprites + (SpriteBank * 0x100 * 16 * 16));
					}
					else
					{
						Render16x16Tile_Mask_Clip(pTransDraw, Code + 1, sx, sy + 16, Colour, 3, 0, 128,
						                          DrvSprites + (SpriteBank * 0x100 * 16 * 16));
					}
				}
			}
			...
			if (sx > 16 && sx < 224 && sy > 0 && sy < 224)
			{
				if (xFlip)
				{
					Render16x16Tile_Mask_FlipX(pTransDraw, Code, sx, sy, Colour, 3, 0, 128,
					                           DrvSprites + (SpriteBank * 0x100 * 16 * 16));
				}
				else
				{
					Render16x16Tile_Mask(pTransDraw, Code, sx, sy, Colour, 3, 0, 128,
					                     DrvSprites + (SpriteBank * 0x100 * 16 * 16));
				}
			}
			else
			{
				if (xFlip)
				{
					Render16x16Tile_Mask_FlipX_Clip(pTransDraw, Code, sx, sy, Colour, 3, 0, 128,
					                                DrvSprites + (SpriteBank * 0x100 * 16 * 16));
				}
				else
				{
					Render16x16Tile_Mask_Clip(pTransDraw, Code, sx, sy, Colour, 3, 0, 128,
					                          DrvSprites + (SpriteBank * 0x100 * 16 * 16));
				}
			}
		}
...
struct BurnDriver BurnDrvRenegade = {
	"renegade", nullptr, nullptr, nullptr, "1986",
	"Renegade (US)\0", nullptr, "Technos Japan (Taito America license)", "Miscellaneous",
	...
	nullptr, 0x100, 240, 240, 4, 3
};

struct BurnDriver BurnDrvKuniokun = {
	"kuniokun", "renegade", nullptr, nullptr, "1986",
	"Nekketsu Kouha Kunio-kun (Japan)\0", nullptr, "Technos Japan", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
...
struct BurnDriver BurnDrvRenegadeb = {
	"renegadeb", "renegade", nullptr, nullptr, "1986",
	"Renegade (US bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_TECHNOS, GBF_SCRFIGHT, 0,
	nullptr, DrvubRomInfo, DrvubRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	KuniokunbInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x100, 240, 240, 4, 3
};

struct BurnDriver BurnDrvKuniokunb = {
	"kuniokunb", "renegade", nullptr, nullptr, "1986",
	"Nekketsu Kouha Kunio-kun (Japan bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_TECHNOS, GBF_SCRFIGHT, 0,
