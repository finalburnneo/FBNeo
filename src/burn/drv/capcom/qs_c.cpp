...
static constexpr INT32 nQscClock = 60000000;
static constexpr INT32 nQscClockDivider = 2496;
...
static constexpr INT16 qsound_dry_mix_table[33] = {
...
static constexpr INT16 qsound_wet_mix_table[33] = {
...
static constexpr INT16 qsound_linear_mix_table[33] = {
...
static constexpr INT16 qsound_filter_data[5][95] = {
	{
		// d53 - 0
...
static constexpr INT16 qsound_filter_data2[209] = {
...
static void init_register_map()
{
	...
	// unused registers
	for (i = 0; i < 256; i++)
		register_map[i] = nullptr;

	// PCM registers
	for (i = 0; i < 16; i++) // PCM voices
	{
		register_map[(i << 3) + 0] = &chip.voice[(i + 1) % 16].bank; // Bank applies to the next channel
		register_map[(i << 3) + 1] = (UINT16*)&chip.voice[i].addr; // Current sample position and start position.
		register_map[(i << 3) + 2] = &chip.voice[i].rate; // 4.12 fixed point decimal.
		register_map[(i << 3) + 3] = &chip.voice[i].phase;
		...
		register_map[(i << 3) + 7] = nullptr; // unused
		register_map[i + 0x80] = &chip.voice_pan[i];
	...
	for (i = 0; i < 3; i++) // ADPCM voices
	{
		...
		register_map[(i << 2) + 0xca] = &chip.adpcm[i].start_addr;
		register_map[(i << 2) + 0xcb] = &chip.adpcm[i].end_addr;
		register_map[(i << 2) + 0xcc] = &chip.adpcm[i].bank;
		register_map[(i << 2) + 0xcd] = (UINT16*)&chip.adpcm[i].volume;
		register_map[i + 0xd6] = &chip.adpcm[i].flag; // non-zero to start ADPCM playback
		register_map[i + 0x90] = &chip.voice_pan[16 + i];
	}

	// QSound registers
	register_map[0x93] = (UINT16*)&chip.echo.feedback;
	register_map[0xd9] = &chip.echo.end_pos;
	register_map[0xe2] = &chip.delay_update; // non-zero to update delays
	register_map[0xe3] = &chip.next_state;
...
inline INT16 get_sample(UINT16 bank, UINT16 address)
{
	...
	return static_cast<INT16>((sample_data << 8) & 0xff00);
}
...
static INT64 CalcAdvance()
{
	if (nQscRate)
	{
		return static_cast<INT64>(0x1000) * nQscClock / nQscClockDivider / nQscRate;
	}
	return 0;
}
...
INT32 QscUpdate(INT32 nEnd)
{
	...
	if (nInterpolation < 3)
	{
		...
		for (INT32 i = 0; i < nLen; i++)
		{
			...
			{
				nLeftSample += static_cast<INT32>(nLeftOut * QsndGain[BURN_SND_QSND_OUTPUT_1]);
			}
			if ((QsndOutputDir[BURN_SND_QSND_OUTPUT_1] & BURN_SND_ROUTE_RIGHT) == BURN_SND_ROUTE_RIGHT)
			{
				nRightSample += static_cast<INT32>(nLeftOut * QsndGain[BURN_SND_QSND_OUTPUT_1]);
			}

			if ((QsndOutputDir[BURN_SND_QSND_OUTPUT_2] & BURN_SND_ROUTE_LEFT) == BURN_SND_ROUTE_LEFT)
			{
				nLeftSample += static_cast<INT32>(nRightOut * QsndGain[BURN_SND_QSND_OUTPUT_2]);
			}
			if ((QsndOutputDir[BURN_SND_QSND_OUTPUT_2] & BURN_SND_ROUTE_RIGHT) == BURN_SND_ROUTE_RIGHT)
			{
				nRightSample += static_cast<INT32>(nRightOut * QsndGain[BURN_SND_QSND_OUTPUT_2]);
			}
