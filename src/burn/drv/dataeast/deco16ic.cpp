...
static INT32 (*deco16_bank_callback[4])(INT32 bank);

static INT32 deco16_pf_colorbank[4];
static INT32 deco16_pf_colormask[4];

static UINT8* deco16_graphics_transtab[4] = {nullptr, nullptr, nullptr, nullptr};
...
UINT8* deco16_pf_ram[4] = {nullptr, nullptr, nullptr, nullptr};
...
void deco16_draw_prio_sprite(UINT16* dest, UINT8* gfx, INT32 code, INT32 color, INT32 sx, INT32 sy, INT32 flipx,
                             INT32 flipy, INT32 pri, INT32 spri)
{
	gfx += code * 0x100;
...
void deco16_draw_prio_sprite_nitrobal(UINT16* dest, UINT8* gfx, INT32 code, INT32 color, INT32 sx, INT32 sy,
                                      INT32 flipx, INT32 flipy, INT32 pri, INT32 spri)
{
	gfx += code * 0x100;
...
void deco16_draw_prio_sprite_dumb(UINT16* dest, UINT8* gfx, INT32 code, INT32 color, INT32 sx, INT32 sy, INT32 flipx,
                                  INT32 flipy, INT32 pri, INT32 spri)
{
	gfx += code * 0x100;
...
// normal
void deco16_draw_alphaprio_sprite(UINT32* palette, UINT8* gfx, INT32 code, INT32 color, INT32 sx, INT32 sy, INT32 flipx,
                                  INT32 flipy, INT32 pri, INT32 spri, INT32 alpha)
{
...
void deco16_draw_alphaprio_sprite(UINT32* palette, UINT8* gfx, INT32 code, INT32 color, INT32 sx, INT32 sy, INT32 flipx,
                                  INT32 flipy, INT32 pri, INT32 spri, INT32 alpha, INT32 dumb_mode)
{
	if (alpha == 0) return;
...
void deco16_draw_layer_by_line(INT32 draw_start, INT32 draw_end, INT32 tmap, UINT16* dest, INT32 flags)
{
	...
	auto vram = (UINT16*)deco16_pf_ram[tmap];
	...
	{
		INT32 xoff = deco16_scroll_x[tmap][((y + deco16_yscroll[tmap] + deco16_global_y_offset) & hmask) /
			deco16_scroll_rows[tmap]] & wmask;
...
void deco16_set_bank_callback(INT32 tmap, INT32 (*callback)(INT32 bank))
{
...
{
	if (deco16_graphics_transtab[select] != nullptr) return;

	INT32 size = (select ? 16 : 8);
	INT32 entries = deco16_graphics_mask[select] + 1;
	deco16_graphics_transtab[select] = BurnMalloc(entries);
...
void deco16Init(INT32 no_pf34, INT32 split, INT32 full_width)
{
	...
	deco16_pf_ram[0] = BurnMalloc(0x2000);
	deco16_pf_ram[1] = BurnMalloc(0x2000);

	deco16_pf_rowscroll[0] = BurnMalloc(0x2000);
	deco16_pf_rowscroll[1] = BurnMalloc(0x2000);
	...
	if (no_pf34 == 0)
	{
		deco16_pf_ram[2] = BurnMalloc(0x2000);
		deco16_pf_ram[3] = BurnMalloc(0x2000);

		deco16_pf_rowscroll[2] = BurnMalloc(0x2000);
		deco16_pf_rowscroll[3] = BurnMalloc(0x2000);
	}

	deco16_prio_map = BurnMalloc(512 * 256); // priority map
	deco16_sprite_prio_map = BurnMalloc(512 * 256);

	deco16_bank_callback[0] = nullptr;
	deco16_bank_callback[1] = nullptr;
	deco16_bank_callback[2] = nullptr;
	deco16_bank_callback[3] = nullptr;
...
void deco16Exit()
{
	...
	for (INT32 i = 0; i < 4; i++)
	{
		...
		deco16_pf_rowscroll[i] = nullptr;
		BurnFree(deco16_pf_ram[i]);
		deco16_pf_ram[i] = nullptr;
	}
	...
	if (deco16_graphics_transtab[0])
		BurnFree(deco16_graphics_transtab[0]);
	if (deco16_graphics_transtab[1])
		BurnFree(deco16_graphics_transtab[1]);
	if (deco16_graphics_transtab[2])
		BurnFree(deco16_graphics_transtab[2]);
	if (deco16_graphics_transtab[3])
		BurnFree(deco16_graphics_transtab[3]);
}
...
static void pf_update(INT32 tmap, INT32 scrollx, INT32 scrolly, UINT16* rowscroll, INT32 control0, INT32 control1)
{
	...
	if ((control1 & 0x40) == 0x40 && rowscroll != nullptr) // row scroll
	{
		...
		if (deco16_dragngun_kludge && tmap == 2 && rsize == 1)
		{
			...
			if (BURN_ENDIAN_SWAP_INT16(vram[2]) == 0x1076 && BURN_ENDIAN_SWAP_INT16(vram[3]) == 0x1076)
			{
				// this is our scene!
				roffset = 0x20;
			}
	...
	if ((control1 & 0x20) == 0x20 && rowscroll != nullptr) // column scroll
	{
		INT32 size = deco16_layer_size_select[tmap] ? 16 : 8;
		...
		{
			deco16_scroll_y[tmap][r] = (scrolly + BURN_ENDIAN_SWAP_INT16(rowscroll[(r & mask) + 0x200]) +
				deco16_global_y_offset);
		}
...
{
	if (deco16_bank_callback[0]) deco16_pf_bank[0] = deco16_bank_callback[0](
		BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][7]) & 0xff);
	if (deco16_bank_callback[1]) deco16_pf_bank[1] = deco16_bank_callback[1](
		BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][7]) >> 8);

	pf_update(0, BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][1]), BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][2]),
	          (UINT16*)deco16_pf_rowscroll[0], BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][5]) & 0xff,
	          BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][6]) & 0xff);
	pf_update(1, BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][3]), BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][4]),
	          (UINT16*)deco16_pf_rowscroll[1], BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][5]) >> 8,
	          BURN_ENDIAN_SWAP_INT16(deco16_pf_control[0][6]) >> 8);
}

void deco16_pf34_update()
{
	if (deco16_bank_callback[2]) deco16_pf_bank[2] = deco16_bank_callback[2](
		BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][7]) & 0xff);
	if (deco16_bank_callback[3]) deco16_pf_bank[3] = deco16_bank_callback[3](
		BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][7]) >> 8);

	pf_update(2, BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][1]), BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][2]),
	          (UINT16*)deco16_pf_rowscroll[2], BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][5]) & 0xff,
	          BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][6]) & 0xff);
	pf_update(3, BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][3]), BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][4]),
	          (UINT16*)deco16_pf_rowscroll[3], BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][5]) >> 8,
	          BURN_ENDIAN_SWAP_INT16(deco16_pf_control[1][6]) >> 8);
}
...
void deco16Scan()
{
...

	{
		...
		for (INT32 i = 0; i < 4; i++)
		{
			if (deco16_pf_ram[i] == nullptr) continue;
...
void deco16SoundReset()
{
	...
	if (deco16_sound_enable[2] ||
		deco16_sound_enable[3])
		MSM6295Reset();

	deco16_soundlatch = 0;
}

void deco16SoundInit(UINT8* rom, UINT8* ram, INT32 huc_clock, INT32 ym2203, void (ym2151_port)(UINT32, UINT32),
                     double ym2151vol, INT32 msmclk0, double msmvol0, INT32 msmclk1, double msmvol1)
{
#ifdef ENABLE_HUC6280
	h6280Init(0);
	h6280Open(0);
	...
	if (ym2203)
	{
		BurnYM2203Init(1, 4027500, nullptr, 1);
	}

	BurnYM2151InitBuffered(3580000, 1, nullptr, 0);
	BurnYM2151SetAllRoutes(ym2151vol, BURN_SND_ROUTE_BOTH);
	BurnYM2151SetIrqHandler(&deco16YM2151IrqHandler);
	if (ym2151_port != nullptr)
	{
	...
	MSM6295Init(0, msmclk0 / 132, true);
	MSM6295SetRoute(0, msmvol0, BURN_SND_ROUTE_BOTH);

	if (msmclk1)
	{
		MSM6295Init(1, msmclk1 / 132, true);
...
static void deco_decrypt(UINT8* src, INT32 len, const UINT8* xor_table, const UINT16* address_table,
                         const UINT8* swap_table, INT32 remap_only)
{
	auto rom = (UINT16*)src;
	len /= 2;
	auto buffer = (UINT16*)BurnMalloc(len * sizeof(INT16));
	INT32 i;
...
static UINT16 decrypt(UINT16 data, INT32 address, INT32 select_xor)
{
	...
	{
		0xb52c, 0x2458, 0x139a, 0xc998, 0xce8e, 0x5144, 0x0429, 0xaad4, 0xa331, 0x3645, 0x69a3, 0xac64, 0x1a53, 0x5083,
		0x4dea, 0xd237
	};
...
void deco102_decrypt_cpu(UINT8* data, UINT8* ops, INT32 size, INT32 address_xor, INT32 data_select_xor,
                         INT32 opcode_select_xor)
{
	auto rom = (UINT16*)data;
	auto opcodes = (UINT16*)ops;
	auto buf = (UINT16*)BurnMalloc(size);
...
{
	auto rom = (UINT32*)src;
	auto buf = (UINT32*)BurnMalloc(len);
