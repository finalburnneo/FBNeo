...
HINSTANCE hAppInst = nullptr; // Application Instance
HANDLE hMainThread;
...
char* TCHARToANSI(const TCHAR* pszInString, char* pszOutString, int nOutSize)
{
	...
	if (WideCharToMultiByte(CP_ACP, 0, pszInString, -1, pszBuffer, nBufferSize, nullptr, nullptr))
	{
		return pszBuffer;
	}
...
CHAR* astring_from_utf8(const char* utf8string)
{
	...
	wstring = static_cast<WCHAR*>(malloc(char_count * sizeof(*wstring)));
	...
	result = static_cast<CHAR*>(malloc(char_count * sizeof(*result)));
	if (result != nullptr)
...
WCHAR* wstring_from_utf8(const char* utf8string)
{
	...
	// convert MAME string (UTF-8) to UTF-16
	char_count = MultiByteToWideChar(CP_UTF8, 0, utf8string, -1, nullptr, 0);
	result = static_cast<WCHAR*>(malloc(char_count * sizeof(*result)));
	if (result != nullptr)
...
{
	if (pszSRBuffer == nullptr || pszFind == nullptr || pszReplace == nullptr)
		return;
...
{
	float aspect_ratio = static_cast<float>(x) / static_cast<float>(y);
...
void MonitorAutoCheck()
{
	...
	{
		EnumDisplayMonitors(nullptr, nullptr, MonInfoProc, 0);
	}
}
...
bool SetNumLock(bool bState)
{
	...
	if (bNoChangeNumLock) return false;
...
int ProcessCmdLine()
{
	...
	if (_tcslen(szName))
	{
		...
		{
			// Command: lua file
			FBA_LoadLuaCode(TCHARToANSI(szName, nullptr, 0));
			//bVidAutoSwitchFullDisable = true;
		}
		else if (_tcscmp(szName, _T("-romdata")) == 0)
		{
			// cmdline for romdata
			TCHAR* szPoint = nullptr;
			if (nullptr != (szPoint = _tcsstr(szCmdLine, _T("-romdata"))))
			{
...
static void CreateSupportFolders()
{
	...
	{
		CreateDirectory(szSupportDirs[x], nullptr);
	}
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nShowCmd)
{
	...
	{
		CreateDirectory(szDirs[x], nullptr);
	}
	...
	if (lpCmdLine)
	{
		_tcscpy(szCmdLine, ANSIToTCHAR(lpCmdLine, nullptr, 0));
	}
