...
static constexpr INT32 index_shift[8] = {-1, -1, -1, -1, 2, 4, 6, 8};
...
void seibu_sound_init(INT32 type, INT32 len, INT32 freq0 /*cpu*/, INT32 freq1 /*ym*/, INT32 freq2 /*oki*/)
{
	...
	case 1:
		BurnYM2151InitBuffered(freq1, 1, nullptr, 0);
	...
	if ((seibu_snd_type & 8) == 0)
	{
		MSM6295Init(0, freq2, true);
		MSM6295SetRoute(0, 0.40, BURN_SND_ROUTE_BOTH);

		if (seibu_snd_type & 4)
		{
			MSM6295Init(1, freq2, true);
...
{
	/* nibble to bit map */
	static constexpr INT32 nbl2bit[16][4] =
	{
	...
	for (step = 0; step <= 48; step++)
	{
		...
		INT32 stepval = static_cast<INT32>(floor(16.0 * pow(11.0 / 10.0, step)));
	...
	samples_from = static_cast<INT32>(static_cast<double>((8000 * 100) / nBurnFPS) + 0.5);
...
static void adpcm_update(INT32 chip, INT16* pbuf, INT32 samples)
{
	...
	while ((adpcmplaying[chip] || adpcmending[chip]) && samples > 0)
	{
		if (adpcmplaying[chip])
		{
			...
			adpcmcurrsampl[chip] = static_cast<INT32>(static_cast<double>(adpcm_clock(chip, val) << 4) * 0.40);
		}
		else if (adpcmending[chip])
		{
			// ramp down, to get rid of clicks -dink
			adpcmcurrsampl[chip] *= 0.997;
			// coefficient created with "exp(- log(2) * (1.0/8000) / 0.01)" and slightly tweaked.
