...
#define NUM_SPRITES 128
	...
	for (PriCode = 0; PriCode < NUM_SPRITES; PriCode++) {
		...
		// zoom y is not linear, it can't be expressed as an exponential function
		static const UINT8 zoomy_table[0x40] =
		{
		...
		yZoom = (K051960Ram[Offset + 4] & 0xfc) >> 2;
		yZoom = 128 - zoomy_table[yZoom];

		// rounded up for each sprite height
		for (int i = 0; i < 3; i++)
			if (h <= (1 << i)) yZoom = (yZoom + 1) / 2;

		yZoom *= 8 / h;
		yZoom = 0x10000 / 128 * yZoom;
