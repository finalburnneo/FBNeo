...
static UINT16 IO_read(UINT32 address) { return read_ioreg(&tms34010, address); }
static void IO_write(UINT32 address, UINT16 value) { write_ioreg(&tms34010, address, value); }
...
{
	return run(&tms34010, cycles);
}

void TMS34010TimerCB(INT64 cycles, void (*timer_cb)())
{
	timer_arm(&tms34010, cycles, timer_cb);
}

INT64 TMS34010TotalCycles()
{
	return total_cycles(&tms34010);
}
...
{
	scan(&tms34010, nAction);
}

UINT32 TMS34010GetPC()
{
	return get_pc(&tms34010);
}

UINT32 TMS34010GetPPC()
{
	return get_ppc(&tms34010);
}
...
{
	generate_irq(&tms34010, line);
}

void TMS34010ClearIRQ(UINT32 line)
{
	clear_irq(&tms34010, line);
}
...
{
	return generate_scanline(&tms34010, line, scanlineRenderCallback);
}
