...
static UINT16 (*irq_cause_read_cb)() = nullptr;
static void (*irq_cause_write_cb)(UINT16 data) = nullptr;
static void (*soundlatch_write_cb)(UINT16 data) = nullptr;
static void (*additional_video_chips_cb)() = nullptr;
...
{
	auto p = (UINT16*)(BurnPalRAM + 0x2000);

	for (INT32 i = 0; i < 0x2000 / 2; i++)
	{
		BurnPalette[i] = BurnHighCol(
			pal5bit(BURN_ENDIAN_SWAP_INT16(p[i]) >> 6), pal5bit(BURN_ENDIAN_SWAP_INT16(p[i]) >> 11),
			pal5bit(BURN_ENDIAN_SWAP_INT16(p[i]) >> 1), 0);
	}
}
...
{
	auto m_spriteram = (UINT16*)SpriteRAM;
	auto m_videoregs = (UINT16*)VideoRegs;
	auto m_spriteregs = (UINT16*)SpriteRegs;

	int max_x = (BURN_ENDIAN_SWAP_INT16(m_spriteregs[1]) + 1) * 2;
	int max_y = (BURN_ENDIAN_SWAP_INT16(m_spriteregs[0]) + 1) * 2;

	INT32 m_sprite_xoffs = BURN_ENDIAN_SWAP_INT16(m_videoregs[0x06 / 2]) - (BURN_ENDIAN_SWAP_INT16(m_spriteregs[1]) +
		1);
	INT32 m_sprite_yoffs = BURN_ENDIAN_SWAP_INT16(m_videoregs[0x04 / 2]) - (BURN_ENDIAN_SWAP_INT16(m_spriteregs[0]) +
		1);
	...
	INT32 i, j, pri;
	static constexpr INT32 primask[4] = {0x0000, 0xff00, 0xff00 | 0xf0f0, 0xff00 | 0xf0f0 | 0xcccc};
	...
	for (i = 0; i < 0x20; i++)
	{
		...
		for (j = 0; j < sprites; j++)
		{
			...
			static const INT32 zoomtable[0x40] = {
				0xAAC, 0x800, 0x668, 0x554, 0x494, 0x400, 0x390, 0x334, 0x2E8, 0x2AC, 0x278, 0x248, 0x224, 0x200, 0x1E0,
				0x1C8,
				0x1B0, 0x198, 0x188, 0x174, 0x164, 0x154, 0x148, 0x13C, 0x130, 0x124, 0x11C, 0x110, 0x108, 0x100, 0x0F8,
				0x0F0,
				0x0EC, 0x0E4, 0x0DC, 0x0D8, 0x0D4, 0x0CC, 0x0C8, 0x0C4, 0x0C0, 0x0BC, 0x0B8, 0x0B4, 0x0B0, 0x0AC, 0x0A8,
				0x0A4,
				0x0A0, 0x09C, 0x098, 0x094, 0x090, 0x08C, 0x088, 0x080, 0x078, 0x070, 0x068, 0x060, 0x058, 0x050, 0x048,
				0x040
			};
			...
			if (color == 0xf) /* 8bpp */
			{
				...
				RenderZoomedPrioSprite(pTransDraw, gfx8x8x8 + gfxstart, 0, (color_start >> 4) << 8, 0xff, x, y, flipx,
				                       flipy, width, height, zoom, zoom, primask[pri]);
			}
			else
			{
				...
				RenderZoomedPrioSprite(pTransDraw, gfx4x8x8 + 2 * gfxstart, 0, (color + color_start) << 4, 0xf, x, y,
				                       flipx, flipy, width, height, zoom, zoom, primask[pri]);
			}
...
{
	auto tiletable = (UINT16*)TileRAM;

	INT32 table_index = (code & 0x1ff0) >> 3;
	UINT32 tile = (BURN_ENDIAN_SWAP_INT16(tiletable[table_index + 0]) << 16) + BURN_ENDIAN_SWAP_INT16(
		tiletable[table_index + 1]);
	...
	if (((tile & 0x00f00000) == 0x00f00000) && (support_8bpp))
	{
		...
		switch (code & 0x6000)
		{
		case 0x0000: *pix = data[(y * (big ? 16 : 8)) + x];
			break;
		case 0x2000: *pix = data[(((big ? 15 : 7) - y) * (big ? 16 : 8)) + x];
			break;
		case 0x4000: *pix = data[(y * (big ? 16 : 8)) + ((big ? 15 : 7) - x)];
			break;
		case 0x6000: *pix = data[(((big ? 15 : 7) - y) * (big ? 16 : 8)) + ((big ? 15 : 7) - x)];
			break;
		}
	...
	UINT32 tile2 = big
		               ? ((((tile & 0xfffff) + 4 * (code & 0xf))) * 0x40)
		               : ((((tile & 0xfffff) + (code & 0xf))) * 0x40);
	...
	for (INT32 y = clip_min_y; y < clip_max_y; y++)
	{
		...
		if (!flipscreen)
		{
			...
			for (x = 0; x < scrwidth; x++)
			{
				...
				UINT8 draw = get_tile_pix(tile, big ? (srccol & 0xf) : (srccol & 0x7),
				                          big ? (srcline & 0xf) : (srcline & 0x7), big, &dat);
...
{
	auto m_videoregs = (UINT16*)VideoRegs;
	auto m_scroll = (UINT16*)ScrollRegs;
	auto m_window = (UINT16*)WindowRegs;
	...
	for (layer = 2; layer >= 0; layer--)
	{
		if (pri == ((layers_pri >> (layer * 2)) & 3))
		{
			...
			auto tilemapram = (UINT16*)(VideoRAM[layer]);
...
{
	{
		auto m_blitter_regs = (UINT16*)BlitRegs;
		UINT8* ramdst[4] = {nullptr, VideoRAM[0], VideoRAM[1], VideoRAM[2]};

		UINT8* src = gfx8x8x8;
		UINT32 src_len = graphics_length;

		UINT32 tmap = (BURN_ENDIAN_SWAP_INT16(m_blitter_regs[0x00 / 2]) << 16) + BURN_ENDIAN_SWAP_INT16(
			m_blitter_regs[0x02 / 2]);
		UINT32 src_offs = (BURN_ENDIAN_SWAP_INT16(m_blitter_regs[0x04 / 2]) << 16) + BURN_ENDIAN_SWAP_INT16(
			m_blitter_regs[0x06 / 2]);
		UINT32 dst_offs = (BURN_ENDIAN_SWAP_INT16(m_blitter_regs[0x08 / 2]) << 16) + BURN_ENDIAN_SWAP_INT16(
			m_blitter_regs[0x0a / 2]);
...
void i4x00_init(INT32 cpu_speed, UINT32 address, UINT8* gfx8, UINT8* gfx4, UINT32 gfxlen, void (*irqcausewrite)(UINT16),
                UINT16 (*irqcauseread)(), void (*soundlatch)(UINT16), INT32 has_8bpp, INT32 has_16bpp)
{
	AllRam = nullptr;
	MemIndex();
	INT32 nLen = RamEnd - static_cast<UINT8*>(nullptr);
	if ((AllRam = BurnMalloc(nLen)) == nullptr) return;
...
void i4x00_exit()
{
	...
	irq_cause_read_cb = nullptr;
	irq_cause_write_cb = nullptr;
	soundlatch_write_cb = nullptr;
	additional_video_chips_cb = nullptr;
