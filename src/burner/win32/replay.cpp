...
static void PrintInputs()
{
	...
	if (GetCurrentFrame() < nPrintInputsActive[1] + (60 * 5))
	{
		// time out np2active after 200 frames or so...
	...
	for (INT32 i = 0; i < 2; i++)
	{
		...
		swprintf(lines[1] + nLen, L" %c %c  %c%c  ", UDLR[i][2] ? '<' : ' ', UDLR[i][3] ? '>' : ' ', BUTTONS[i][2],
		         BUTTONS[i][3]);
			...
			if (nRet == 0)
			{
				...
				else
				{
					...
					if (nThisMovieVersion >= 0x0401)
					{
						...
						bprintf(0, _T("Ext Info: %d:%d:%d %d/%d/%d\n"), MovieInfo.hour, MovieInfo.minute,
						        MovieInfo.second, MovieInfo.year, MovieInfo.month, MovieInfo.day);
					}
...
void DisplayReplayProperties(HWND hDlg, bool bClear)
{
	...
	if (nThisMovieVersion >= 0x0401)
	{
		...
		bprintf(0, _T("Ext Info: %d:%d:%d %d/%d/%d\n"), MovieInfo.hour, MovieInfo.minute, MovieInfo.second,
		        MovieInfo.year, MovieInfo.month, MovieInfo.day);
	}
	...
	if (hDlg != 0)
	{
		...
		if (nFileVer)
			sprintf(szRecordedFrom, "%s, v%x.%x.%x.%02x", (bStartFromReset) ? "Power-On" : "Savestate", nFileVer >> 20,
			        (nFileVer >> 16) & 0x0F, (nFileVer >> 8) & 0xFF, nFileVer & 0xFF);
		else
			sprintf(szRecordedFrom, "%s", (bStartFromReset) ? "Power-On" : "Savestate");

		if (nThisMovieVersion >= 0x0401)
		{
			sprintf(szRecordedTime, "%02d/%02d/%04d @ %02d:%02d:%02d%s", MovieInfo.month + 1, MovieInfo.day,
			        2000 + (MovieInfo.year % 100), (MovieInfo.hour > 12) ? MovieInfo.hour - 12 : MovieInfo.hour,
			        MovieInfo.minute, MovieInfo.second, (MovieInfo.hour > 12) ? "pm" : "am");
		}
			...
			case IDOK:
				{
					...
					if (lIndex != CB_ERR)
					{
						if (lIndex == lCount - 1)
						{
							...
							if (nRet != 0)
							{
								LONG lOtherIndex = SendDlgItemMessage(hDlg, IDC_CHOOSE_LIST, CB_FINDSTRING, (WPARAM)-1,
								                                      (LPARAM)szChoice);
