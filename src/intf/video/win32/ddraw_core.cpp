...
static INT32 DDCore_GetFunctions()
{
	...
	_LOADFN(HRESULT, _DirectDrawCreateEx, Empty_DirectDrawCreateEx, (GUID FAR*, LPVOID*, REFIID, IUnknown FAR*),
	        hDDCore, "DirectDrawCreateEx");
	_LOADFN(HRESULT, _DirectDrawCreateClipper, Empty_DirectDrawCreateClipper,
	        (DWORD, LPDIRECTDRAWCLIPPER FAR*, IUnknown FAR*), hDDCore, "DirectDrawCreateClipper");
#ifdef UNICODE
	_LOADFN(HRESULT, _DirectDrawEnumerateEx, Empty_DirectDrawEnumerateEx, (LPDDENUMCALLBACKEXW, LPVOID, DWORD), hDDCore,
...
INT32 DDCore_Init()
{
	...
	{
		MessageBox(NULL, _T("There was a problem while loading functions from DDRAW.DLL"), _T("Error"),
		           MB_OK | MB_ICONERROR);
		return 0;
	}
