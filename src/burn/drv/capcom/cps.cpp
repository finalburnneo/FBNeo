...
UINT8* CpsGfx = nullptr;
UINT32 nCpsGfxLen = 0; // All the graphics
UINT8* CpsRom = nullptr;
UINT32 nCpsRomLen = 0; // Program Rom (as in rom)
UINT8* CpsCode = nullptr;
UINT32 nCpsCodeLen = 0; // Program Rom (decrypted)
UINT8* CpsZRom = nullptr;
UINT32 nCpsZRomLen = 0; // Z80 Roms
INT8* CpsQSam = nullptr;
UINT32 nCpsQSamLen = 0; // QSound Sample Roms
UINT8* CpsAd = nullptr;
UINT32 nCpsAdLen = 0; // ADPCM Data
UINT8* CpsStar = nullptr;
UINT8* CpsKey = nullptr;
UINT32 nCpsKeyLen = 0;
...
{
	INT32 i = 0, j = 0;
	UINT8 *Rom1 = nullptr, *Rom2 = nullptr;
...
{
	auto Rom = BurnMalloc(0x400000 * sizeof(UINT8));
	UINT8 *pt = nullptr, *pr = nullptr;
	INT32 i, j;
...
{
	auto Rom = BurnMalloc(0x200000 * sizeof(UINT8));
	auto Temp = BurnMalloc(0x200000 * sizeof(UINT8));
	UINT8 *pt = nullptr, *pr = nullptr;
	INT32 i;
...
INT32 CpsLoadTilesPang3b5(INT32 nStart)
{
	...
	CpsLoadOne(CpsGfx + 0x000000, nStart + 1, 1, 2);
	CpsLoadOne(CpsGfx + 0x000004, nStart + 2, 1, 0);
	CpsLoadOne(CpsGfx + 0x000004, nStart + 3, 1, 2);
	CpsLoadOne(CpsGfx + 0x200000, nStart + 4, 1, 0);
	CpsLoadOne(CpsGfx + 0x200000, nStart + 5, 1, 2);
	CpsLoadOne(CpsGfx + 0x200004, nStart + 6, 1, 0);
	CpsLoadOne(CpsGfx + 0x200004, nStart + 7, 1, 2);
...
static INT32 Cps2LoadOne(UINT8* Tile, INT32 nNum, INT32 nWord, INT32 nShift)
{
	...
	{
		Cps2Load100000(pt, pr, nShift);
		pt += 0x100000;
		Cps2Load100000(pt, pr + 2, nShift);
		pt += 0x100000;
		pr += 0x80000;
	}
...
INT32 Cps2LoadTilesGigaman2(UINT8* Tile, UINT8* pSrc)
{
	...
	{
		Cps2Load100000(pt, pr, 0);
		pt += 0x100000;
		Cps2Load100000(pt, pr + 2, 0);
		pt += 0x100000;
		pr += 0x80000;
	}
	...
	for (INT32 b = 0; b < 0x200000 >> 19; b++)
	{
		Cps2Load100000(pt, pr, 2);
		pt += 0x100000;
		Cps2Load100000(pt, pr + 2, 2);
		pt += 0x100000;
		pr += 0x80000;
	}
...
static INT32 CpsGetROMs(bool bLoad)
{
	...
	auto CpsQSamLoad = (UINT8*)CpsQSam;
...
INT32 CpsInit()
{
	...
	CpsKey = CpsAd + nCpsAdLen;

	// Create Gfx addr mask
	for (i = 0; i < 31; i++)
	{
		if ((1 << i) >= static_cast<INT32>(nCpsGfxLen))
		{
			break;
		}
	}
	...
	CpsReset = 0;
	Cpi01A = Cpi01C = Cpi01E = 0; // blank other inputs
...
INT32 CpsExit()
{
	...
	CpsRom = CpsZRom = CpsAd = CpsStar = nullptr;
	CpsQSam = nullptr;
	CpsKey = nullptr;
