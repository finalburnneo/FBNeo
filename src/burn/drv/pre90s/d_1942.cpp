...
static UINT8* DrvFgVideoRam = nullptr;
static UINT8* DrvBgVideoRam = nullptr;
...
static UINT8* DrvPromTileLookup = nullptr;
...
static INT32 DrvInit()
{
	...
	DrvTempRom = BurnMalloc(0x10000);

	// Load Z80 #1 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x04000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x10000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x14000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x18000, 4, 1);
	if (nRet != 0) return 1;

	// Load Z80 #2 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom2 + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x02000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x04000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x06000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x0a000, 12, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x04000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x0c000, 16, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x200, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromRed, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromGreen, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBlue, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromCharLookup, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromTileLookup, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpriteLookup, 22, 1);
	if (nRet != 0) return 1;
...
static INT32 DrvablInit()
{
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x14000, 2, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x04000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 7, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x200, 3, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles);
	...
	// Load the PROMs
	nRet = BurnLoadRom(DrvPromRed, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromGreen, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBlue, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromCharLookup, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromTileLookup, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpriteLookup, 15, 1);
	if (nRet != 0) return 1;
...
static void DrvRenderSpriteLayer()
{
	for (int y = 16; y <= nScreenHeight + 16; y++)
	{
		...
		for (int h = 496; h >= 128; h -= 16)
		{
			...
			UINT8 valpha = static_cast<UINT8>(sy);
			UINT8 v2c = static_cast<UINT8>(~v) + 0xff;
			...
			if (!vinzone)
			{
				do
				{
					Render16x16Tile_Mask_Clip(pTransDraw, code + i, sx, (sy + (16 * i) * dir) - 16, col, 4, 15, 1280,
					                          DrvSprites);
				}
				while (i-- > 0);
			}
...
struct BurnDriver BurnDrvNineteen42 = {
	"1942", nullptr, nullptr, nullptr, "1984",
	"1942 (Revision B)\0", nullptr, "Capcom", "Miscellaneous",
...
struct BurnDriver BurnDrvNineteen42a = {
	"1942a", "1942", nullptr, nullptr, "1984",
	"1942 (Revision A)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvaRomInfo, DrvaRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x600, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen42abl = {
	"1942abl", "1942", nullptr, nullptr, "1984",
	"1942 (Revision A, bootleg)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	...
	nullptr, DrvablRomInfo, DrvablRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvablInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x600, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen42b = {
	"1942b", "1942", nullptr, nullptr, "1984",
	"1942 (First Version)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
...
struct BurnDriver BurnDrvNineteen42w = {
	"1942w", "1942", nullptr, nullptr, "1985",
	"1942 (Williams Electronics license)\0", nullptr, "Capcom (Williams Electronics license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvwRomInfo, DrvwRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x600, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen42h = {
	"1942h", "1942", nullptr, nullptr, "1991",
	"Supercharger 1942\0", nullptr, "hack (Two Bit Score)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvhRomInfo, DrvhRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x600, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen42c64 = {
	"1942c64", "1942", nullptr, nullptr, "2015",
	"1942 (C64 Music)\0", nullptr, "hack by Minwah", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, Drvc64RomInfo, Drvc64RomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x600, 224, 256, 3, 4
};
