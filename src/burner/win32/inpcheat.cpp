...
HWND hInpCheatDlg = nullptr; // Handle to the Cheat Dialog

static HWND hInpCheatList = nullptr;

static bool bOK;

static int nCurrentCheat;
static int* nPrevCheatSettings = nullptr;

static int InpCheatListBegin()
{
	LVCOLUMN LvCol;
	if (hInpCheatList == nullptr)
	{
		return 1;
	}
...
static int InpCheatInit()
{
	...
	nPrevCheatSettings = static_cast<int*>(malloc(nCurrentCheat * sizeof(int)));
...
static int InpCheatExit()
{
	if (nPrevCheatSettings)
	{
		...
		nPrevCheatSettings = nullptr;
	}

	hInpCheatList = nullptr;
	hInpCheatDlg = nullptr;
...
static void InpCheatSelect()
{
	...
	int nSel = SendMessage(hInpCheatList, LVM_GETNEXTITEM, static_cast<WPARAM>(-1), LVNI_SELECTED);
	if (nSel >= 0)
	{
		LVITEM LvItem;
		memset(&LvItem, 0, sizeof(LvItem));
		...
		SendMessage(GetDlgItem(hInpCheatDlg, IDC_INPCX1_VALUE), CB_SETCURSEL,
		            static_cast<WPARAM>(pCurrentCheat->nCurrent), 0);
	}
}
