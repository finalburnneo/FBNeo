...
void k007121_draw(INT32 chip, UINT16* dest, UINT8* gfx, UINT8* ctable, UINT8* source, INT32 base_color, INT32 xoffs,
                  INT32 yoffs, INT32 bank_base, INT32 pri_mask, INT32 color_offset)
{
	...
	for (INT32 i = 0; i < num; i++)
	{
		...
		static constexpr INT32 x_offset[4] = {0x0, 0x1, 0x4, 0x5};
		static constexpr INT32 y_offset[4] = {0x0, 0x2, 0x8, 0xa};
		...
		case 0x02: width = 2;
			...
			number &= (~1);
			break;
		case 0x00: width = height = 2;
			number &= (~3);
			break;
		case 0x08: width = height = 4;
			number &= (~3);
			break;
		default: width = 1;
			height = 1;
		...
		for (y = 0; y < height; y++)
		{
			for (x = 0; x < width; x++)
			{
				...
				if (pri_mask != -1)
				{
					if (ctable != nullptr)
					{
						RenderPrioMaskTranstabSpriteOffset(dest, gfx, code, (color * 16), 0, destx, desty, flipx, flipy,
						                                   8, 8, ctable, color_offset, pri_mask);
					}
					else
					{
						if (flipy)
						{
							if (flipx)
							{
								Render8x8Tile_Prio_Mask_FlipXY_Clip(dest, code, destx, desty, color, 4, 0, color_offset,
								                                    pri_mask, gfx);
							}
							else
							{
								Render8x8Tile_Prio_Mask_FlipY_Clip(dest, code, destx, desty, color, 4, 0, color_offset,
								                                   pri_mask, gfx);
							}
						}
						else
						{
							if (flipx)
							{
								Render8x8Tile_Prio_Mask_FlipX_Clip(dest, code, destx, desty, color, 4, 0, color_offset,
								                                   pri_mask, gfx);
							}
							else
							{
								Render8x8Tile_Prio_Mask_Clip(dest, code, destx, desty, color, 4, 0, color_offset,
								                             pri_mask, gfx);
							}
						}
					}
				...
				else
				{
					if (ctable != nullptr)
					{
						RenderTileTranstabOffset(dest, gfx, code, (color * 16), 0, destx, desty, flipx, flipy, 8, 8,
						                         ctable, color_offset);
					}
					else
					{
						if (flipy)
						{
							if (flipx)
							{
								Render8x8Tile_Mask_FlipXY_Clip(dest, code, destx, desty, color, 4, 0, color_offset,
								                               gfx);
							}
							else
							{
