...
static UINT8* DrvSpriteRamBuffer = nullptr;
static UINT8* DrvPaletteRam = nullptr;
static UINT8* DrvScroll1Ram = nullptr;
static UINT8* DrvScroll2Ram = nullptr;
static UINT8* DrvChars = nullptr;
static UINT8* DrvBgTiles = nullptr;
static UINT8* DrvFgTiles = nullptr;
...
static INT32 DrvInit() // Madgear, Ledstorm
{
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00002, 8, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40002, 9, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 10, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 11, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00003, 12, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40003, 13, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 14, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40001, 15, 4);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x400, DrvTempRom, DrvSprites);

	// Load the samples
	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(MSM6295ROM + 0x20000, 17, 1);
	if (nRet != 0) return 1;
...
static INT32 Leds2011Init()
{
	...
	// Load and decode the sprites
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 8, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 9, 2);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x400, DrvTempRom, DrvSprites);

	// Load the samples
	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(MSM6295ROM + 0x20000, 11, 1);
	if (nRet != 0) return 1;
...
static INT32 LastduelInit()
{
	...
	// Load and decode the bg tiles
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 6, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 7, 2);
	if (nRet != 0) return 1;
	GfxDecode(0x0800, 4, 16, 16, BgTilePlaneOffsets, TileXOffsets, TileYOffsets, 0x400, DrvTempRom, DrvBgTiles);
	...
	// Load and decode the sprites
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 9, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 10, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00002, 11, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00003, 12, 4);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x400, DrvTempRom, DrvSprites);
...
static INT32 LastduelbInit()
{
	...
	// Load Z80 Program Rom
	nRet = BurnLoadRom(DrvZ80Rom, 6, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 8, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20001, 9, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 10, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 11, 2);
	if (nRet != 0) return 1;
	GfxDecode(0x0800, 4, 16, 16, BgTilePlaneOffsets, TileXOffsets, TileYOffsets, 0x400, DrvTempRom, DrvBgTiles);

	// Load and decode the fg tiles
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 12, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 13, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20001, 14, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 15, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40001, 16, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 17, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60001, 18, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 19, 2);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, BgTilePlaneOffsets, TileXOffsets, TileYOffsets, 0x400, DrvTempRom, DrvFgTiles);

	// Load and decode the sprites
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 20, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 21, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00001, 22, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40001, 23, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00002, 24, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40002, 25, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00003, 26, 4);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40003, 27, 4);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x400, DrvTempRom, DrvSprites);
...
static void DrvTileDraw(UINT8* gfx, INT32 Code, INT32 Colour, INT32 x, INT32 y, INT32 xFlip, INT32 yFlip,
                        INT32 transparent, INT32 tcolor)
{
	if (transparent)
	{
...
static void DrvRenderBgLayer(INT32 nTransparent)
{
	...
	auto VideoRam = (UINT16*)DrvScroll2Ram;
...
{
	INT32 Offset;
	auto SpriteRam = (UINT16*)DrvSpriteRamBuffer;
...
static void DrvRenderCharLayer()
{
	...
	auto VideoRam = (UINT16*)DrvVideoRam;
...
static void LastduelRenderFgLayer(INT32 BackLayer)
{
	...
	auto VideoRam = (UINT16*)DrvScroll1Ram;
...
struct BurnDriver BurnDrvLastduel = {
	"lastduel", nullptr, nullptr, nullptr, "1988",
	"Last Duel (US New Ver.)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, LastduelRomInfo, LastduelRomName, nullptr, nullptr, nullptr, nullptr, LastduelInputInfo, LastduelDIPInfo,
	LastduelInit, DrvExit, LastduelFrame, LastduelDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvLastduelo = {
	"lastduelo", "lastduel", nullptr, nullptr, "1988",
	"Last Duel (US Old Ver.)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, LastdueloRomInfo, LastdueloRomName, nullptr, nullptr, nullptr, nullptr, LastduelInputInfo, LastduelDIPInfo,
	LastduelInit, DrvExit, LastduelFrame, LastduelDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvLastduelj = {
	"lastduelj", "lastduel", nullptr, nullptr, "1988",
	"Last Duel (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, LastdueljRomInfo, LastdueljRomName, nullptr, nullptr, nullptr, nullptr, LastduelInputInfo, LastduelDIPInfo,
	LastduelInit, DrvExit, LastduelFrame, LastduelDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvLsstduelb = {
	"lastduelb", "lastduel", nullptr, nullptr, "1988",
	"Last Duel (bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	...
	nullptr, LastduelbRomInfo, LastduelbRomName, nullptr, nullptr, nullptr, nullptr, LastduelInputInfo, LastduelDIPInfo,
	LastduelbInit, DrvExit, LastduelFrame, LastduelDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvMadgear = {
	"madgear", nullptr, nullptr, nullptr, "1989",
	"Mad Gear (US)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
...
struct BurnDriver BurnDrvMadgearj = {
	"madgearj", "madgear", nullptr, nullptr, "1989",
	"Mad Gear (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_RACING,
	0,
...
struct BurnDriver BurnDrvMadgearu = {
	"ledstorm", "madgear", nullptr, nullptr, "1988",
	"Led Storm (US)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_RACING,
	0,
...
struct BurnDriver BurnDrvLeds2011 = {
	"leds2011", nullptr, nullptr, nullptr, "1988",
	"Led Storm Rally 2011 (World)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_RACING, 0,
	nullptr, Leds2011RomInfo, Leds2011RomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	Leds2011Init, MadgearExit, DrvFrame, DrvDraw, MadgearScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvLeds2011u = {
	"leds2011u", "leds2011", nullptr, nullptr, "1988",
	"Led Storm Rally 2011 (US)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_RACING,
	0,
	nullptr, Leds2011uRomInfo, Leds2011uRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	Leds2011Init, MadgearExit, DrvFrame, DrvDraw, MadgearScan,
	nullptr, 0x800, 240, 384, 3, 4
};
