...
static UINT8* K051960Ram = nullptr;
...
using K051960_Callback = void(*)(INT32* Code, INT32* Colour, INT32* Priority, INT32* Shadow);
		...
		if (xZoom == 0x10000 && yZoom == 0x10000)
		{
			...
			for (y = 0; y < h; y++)
			{
				...
				for (x = 0; x < w; x++)
				{
					...
					if (Shadow)
					{
						konami_render_zoom_shadow_sprite(K051960RomExp, c, nBpp, Colour, sx, sy, xFlip, yFlip, 16, 16,
						                                 0x10000, 0x10000, (max_priority == -1) ? Pri : 0xffffffff, 0);
					}
					else
					{
						if (max_priority == -1)
		...
		else
		{
			...
			for (y = 0; y < h; y++)
			{
				...
				for (x = 0; x < w; x++)
				{
					...
					{
						konami_render_zoom_shadow_sprite(K051960RomExp, c, nBpp, Colour, sx, sy, xFlip, yFlip, 16, 16,
						                                 zw << 12, zh << 12, (max_priority == -1) ? Pri : 0xffffffff,
						                                 0);
					}
					else
					{
						if (max_priority == -1)
						{
							konami_draw_16x16_priozoom_sprite(K051960RomExp, c, nBpp, Colour, 0, sx, sy, xFlip, yFlip,
							                                  16, 16, zw << 12, zh << 12, Pri);
						}
						else
						{
							konami_draw_16x16_zoom_tile(K051960RomExp, c, nBpp, Colour, 0, sx, sy, xFlip, yFlip, 16, 16,
							                            zw << 12, zh << 12);
						}
					}
				}
...
void K051960Exit()
{
	...
	K051960Callback = nullptr;
	K051960RomMask = 0;
	K051960Rom = nullptr;
	K051960RomExpMask = 0;
	K051960RomExp = nullptr;
	memset(K051960SpriteRomBank, 0, 3);
	K051960ReadRoms = 0;
	K051960RomOffset = 0;
