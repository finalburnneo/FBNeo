...
UINT16 st0020GfxramReadWord(UINT32 offset)
{
	...
	auto ram = (UINT16*)(st0020GfxRAM + (offset & 0xfffff) + bank);
...
{
	auto st0020_blitram = (UINT16*)st0020BlitRAM;
	auto st0020_gfxram = (UINT16*)st0020GfxRAM;

	UINT32 src = (BURN_ENDIAN_SWAP_INT16(st0020_blitram[0xc0/2]) + (BURN_ENDIAN_SWAP_INT16(st0020_blitram[0xc2/2]) <<
		16)) << 1;
	UINT32 dst = (BURN_ENDIAN_SWAP_INT16(st0020_blitram[0xc4/2]) + (BURN_ENDIAN_SWAP_INT16(st0020_blitram[0xc6/2]) <<
		16)) << 4;
...
{
	auto spriteram16_2 = (UINT16*)st0020SprRAM;
	UINT16* s1 = spriteram16_2;
	UINT16* end1 = spriteram16_2 + 0x02000 / 2;

	constexpr INT32 ramoffs[2][4] = {{0, 1, 2, 3}, {2, 3, 1, 0}};
	...
	for (; s1 < end1; s1 += 8 / 2)
	{
		...
		for (; num > 0; num--, s2 += 16 / 2)
		{
			...
			for (x = xstart; x != xend; x += xinc)
			{
				...
				{
					RenderZoomedTile(pTransDraw, st0020GfxRAM, code % (0x400000 / 0x80), color * 64, 0,
					                 (sx + x * xdim) / 0x10000, (sy + y * ydim) / 0x10000, flipx, flipy, 16, 8, xscale,
					                 yscale);
				}
			}
		}
