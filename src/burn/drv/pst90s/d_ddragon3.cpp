...
static UINT8 *DrvMSM6295ROM       = NULL;
...
static INT32 MemIndex()
{
	...
	DrvMSM6295ROM          = Next; Next += 0x80000;
...
static void set_m6295_bank(INT32 bank)
{
	MSM6295SetBank(0, DrvMSM6295ROM + (0x40000 * (bank & 1)), 0x00000, 0x3ffff);
}
...
static INT32 DrvInit()
{
	...
	// Load Sample Roms
	nRet = BurnLoadRom(DrvMSM6295ROM + 0x00000, 15, 1); if (nRet != 0) return 1;
...
static INT32 DrvpInit()
{
	...
	// Load Sample Roms
	nRet = BurnLoadRom(DrvMSM6295ROM + 0x00000, 31, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvMSM6295ROM + 0x40000, 32, 1); if (nRet != 0) return 1;
...
static INT32 DrvbInit()
{
	...
	// Load Sample Roms
	nRet = BurnLoadRom(DrvMSM6295ROM + 0x00000, 28, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvMSM6295ROM + 0x40000, 29, 1); if (nRet != 0) return 1;
...
static INT32 CtribeInit()
{
	...
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "ctribeb")) {
		...
		memcpy(DrvMSM6295ROM + 0x20000, DrvTempRom + 0x00000, 0x20000);
		memcpy(DrvMSM6295ROM + 0x00000, DrvTempRom + 0x20000, 0x20000);
	} else {
		if (!strcmp(BurnDrvGetTextA(DRV_NAME), "ctribeb2")) {
			nRet = BurnLoadRom(DrvMSM6295ROM + 0x00000, 32, 1); if (nRet != 0) return 1;
			nRet = BurnLoadRom(DrvMSM6295ROM + 0x20000, 33, 1); if (nRet != 0) return 1;
		} else {
			nRet = BurnLoadRom(DrvMSM6295ROM + 0x00000, 16, 1); if (nRet != 0) return 1;
			nRet = BurnLoadRom(DrvMSM6295ROM + 0x20000, 17, 1); if (nRet != 0) return 1;
		}
	}
...
static INT32 CtribeDraw()
{
	...
	if (DrvVReg & 0x08) {
		if (nBurnLayer & 1) DrvRenderFgLayer(1);
		if (nSpriteEnable & 1) DrvRenderSprites();
		if (nBurnLayer & 2) DrvRenderBgLayer(0);
	} else {
		if (nBurnLayer & 1) DrvRenderBgLayer(1);
		if (nBurnLayer & 2) DrvRenderFgLayer(0);
		if (nSpriteEnable & 1) DrvRenderSprites();
	}
...
struct BurnDriver BurnDrvDdrago3p = {
	...
	DrvpInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	NULL, 0x600, 320, 240, 4, 3
};
...
struct BurnDriver BurnDrvCtribe = {
	...
	CtribeInit, DrvExit, DrvFrame, CtribeDraw, DrvScan,
	NULL, 0x600, 320, 240, 4, 3
};
