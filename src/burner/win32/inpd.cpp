...
static int ListItemActivate()
{
	...
	if (nSel >= (int)(nGameInpCount + nMacroCount))
	{
		// out of range
		return 1;
	}
...
static int InitAnalogOptions(int nGi, int nPci)
{
	...
	if (nPci >= 1 && nPci <= 3)
	{
		...
		SendMessage(hInpdAnalog, CB_ADDSTRING, 0,
		            (LPARAM)(LPARAM)FBNLoadStringEx(hAppInst, IDS_INPUT_ANALOG_ABS, true));
	}
	else
	{
		if (nAnalog > 0)
