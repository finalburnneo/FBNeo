...
static UINT8* DrvZ80Code = nullptr;
...
static UINT8* DrvAttrRam = nullptr;
...
{
	// Default Values
	{0x2c, 0xff, 0xff, 0xef, nullptr},
	{0x2d, 0xff, 0xff, 0x8f, nullptr},
...
static constexpr eeprom_interface MitchellEEPROMIntf =
{
	6,
	16,
	"0110",
...
static INT32 DrvDoReset()
{
	...
	if (DrvHasEEPROM)
	{
		ZetMapArea(0x8000, 0xbfff, 2, DrvZ80Code + 0x10000 + (DrvRomBank * 0x4000),
		           DrvZ80Rom + 0x10000 + (DrvRomBank * 0x4000));
	}
	else
	{
...
static void __fastcall MstworldZ80PortWrite(UINT16 a, UINT8 d)
{
	...
	default:
		{
			// block block seems to write to random ports quite often
			//bprintf(PRINT_NORMAL, _T("Z80 #1 Port Write => %02X, %02X\n"), a, d);
		}
	}
}
...
extern void kabuki_decode(UINT8* src, UINT8* dest_op, UINT8* dest_data, INT32 base_addr, INT32 length, INT32 swap_key1,
                          INT32 swap_key2, INT32 addr_key, INT32 xor_key);
...
static void mitchell_decode(INT32 swap_key1, INT32 swap_key2, INT32 addr_key, INT32 xor_key)
{
	...
	for (i = 0; i < numbanks; i++)
		kabuki_decode(rom + i * 0x4000, decrypt + i * 0x4000, rom + i * 0x4000, 0x8000, 0x4000, swap_key1, swap_key2,
		              addr_key, xor_key);
}
...
static INT32 MgakuenInit()
{
	...
	DrvTempRom = BurnMalloc(0x200000);

	nRet = BurnLoadRom(DrvZ80Rom + 0x000000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x010000, 1, 1);
	if (nRet != 0) return 1;

	memset(DrvTempRom, 0xff, 0x200000);
	nRet = BurnLoadRom(DrvTempRom + 0x000000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x080000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x100000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x180000, 5, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x10000, 4, 8, 8, MahjongCharPlaneOffsets, MahjongCharXOffsets, MahjongCharYOffsets, 0x100, DrvTempRom,
	          DrvChars);

	memset(DrvTempRom, 0xff, 0x200000);
	nRet = BurnLoadRom(DrvTempRom + 0x000000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x020000, 7, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x00800, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);

	BurnFree(DrvTempRom);

	nRet = BurnLoadRom(DrvSoundRom + 0x000000, 8, 1);
	if (nRet != 0) return 1;
...
static INT32 Mgakuen2Init()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom + 0x010000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x030000, 2, 1);
	if (nRet != 0) return 1;

	memset(DrvTempRom, 0xff, 0x200000);
	nRet = BurnLoadRom(DrvTempRom + 0x000000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x080000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x100000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x180000, 6, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x10000, 4, 8, 8, MahjongCharPlaneOffsets, MahjongCharXOffsets, MahjongCharYOffsets, 0x100, DrvTempRom,
	          DrvChars);

	memset(DrvTempRom, 0xff, 0x200000);
	nRet = BurnLoadRom(DrvTempRom + 0x000000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x020000, 8, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x00800, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);

	BurnFree(DrvTempRom);

	nRet = BurnLoadRom(DrvSoundRom + 0x000000, 9, 1);
	if (nRet != 0) return 1;
...
static INT32 PkladiesInit()
{
	...
	memset(DrvTempRom, 0xff, 0x200000);
	nRet = BurnLoadRom(DrvTempRom + 0x000000, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x000001, 3, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x100000, 4, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x100001, 5, 2);
	if (nRet != 0) return 1;
	GfxDecode(0x10000, 4, 8, 8, MahjongCharPlaneOffsets, MahjongCharXOffsets, MahjongCharYOffsets, 0x100, DrvTempRom,
	          DrvChars);
	...
	nRet = BurnLoadRom(DrvSoundRom + 0x000000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvSoundRom + 0x020000, 9, 1);
	if (nRet != 0) return 1;
...
static INT32 DokabenInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x10000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x30000, 2, 1);
	if (nRet != 0) return 1;

	memset(DrvTempRom, 0xff, 0x100000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x80000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0xa0000, 6, 1);
	if (nRet != 0) return 1;
...
static INT32 PangInit()
{
	...
	memset(DrvTempRom, 0xff, 0x100000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x80000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0xa0000, 5, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x8000, 4, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
	...
	nRet = BurnLoadRom(DrvSoundRom + 0x00000, 8, 1);
	if (nRet != 0) return 1;
...
static INT32 PangbInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Code + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Code + 0x10000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x10000, 2, 1);
	if (nRet != 0) return 1;
...
static INT32 CworldInit()
{
	...
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x80000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0xa0000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0xc0000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0xe0000, 10, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x8000, 4, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x80, DrvTempRom, DrvChars);
...
static INT32 MstworldInit()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom2 + 0x00000, 1, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 3, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 5, 1);
	if (nRet != 0) return 1;
	for (INT32 i = 0; i < 0x80000; i++) DrvTempRom[i] ^= 0xff;
	GfxDecode(0x4000, 4, 8, 8, MstworldCharPlaneOffsets, MstworldCharXOffsets, MstworldCharYOffsets, 0x40, DrvTempRom,
	          DrvChars);
	...
	GfxDecode(0x0800, 4, 16, 16, MstworldSpritePlaneOffsets, MstworldSpriteXOffsets, MstworldSpriteYOffsets, 0x200,
	          DrvTempRom, DrvSprites);
	...
	static constexpr INT32 tablebank[] = {
		0, 0,
		1, 1,
		-1, -1,
		-1, -1,
	...
	MSM6295Init(0, 990000 / 132, false);
	MSM6295SetRoute(0, 0.50, BURN_SND_ROUTE_BOTH);
...
struct BurnDriver BurnDrvMgakuen = {
	"mgakuen", nullptr, nullptr, nullptr, "1988",
	"Mahjong Gakuen\0", nullptr, "Yuga", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, MgakuenRomInfo, MgakuenRomName, nullptr, nullptr, nullptr, nullptr, MgakuenInputInfo, MgakuenDIPInfo,
	MgakuenInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x400, 384, 240, 4, 3
};

struct BurnDriver BurnDrvSeventoitsu = {
	"7toitsu", "mgakuen", nullptr, nullptr, "1988",
	"Chi-Toitsu\0", nullptr, "Yuga", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, SeventoitsuRomInfo, SeventoitsuRomName, nullptr, nullptr, nullptr, nullptr, MgakuenInputInfo,
	MgakuenDIPInfo,
	MgakuenInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x400, 384, 240, 4, 3
};
...
struct BurnDriver BurnDrvMgakuen2 = {
	"mgakuen2", nullptr, nullptr, nullptr, "1989",
	"Mahjong Gakuen 2 Gakuen-chou no Fukushuu\0", nullptr, "Face", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, Mgakuen2RomInfo, Mgakuen2RomName, nullptr, nullptr, nullptr, nullptr, MarukinInputInfo, nullptr,
	Mgakuen2Init, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPkladies = {
	"pkladies", nullptr, nullptr, nullptr, "1989",
	"Poker Ladies\0", nullptr, "Mitchell", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, PkladiesRomInfo, PkladiesRomName, nullptr, nullptr, nullptr, nullptr, PkladiesInputInfo, nullptr,
	PkladiesInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPkladiesl = {
	"pkladiesl", "pkladies", nullptr, nullptr, "1989",
	"Poker Ladies (Leprechaun ver. 510)\0", nullptr, "Leprechaun", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, PkladieslRomInfo, PkladieslRomName, nullptr, nullptr, nullptr, nullptr, PkladiesInputInfo, nullptr,
	PkladiesInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPkladiesla = {
	"pkladiesla", "pkladies", nullptr, nullptr, "1989",
	"Poker Ladies (Leprechaun ver. 401)\0", nullptr, "Leprechaun", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_MAHJONG, 0,
	nullptr, PkladieslaRomInfo, PkladieslaRomName, nullptr, nullptr, nullptr, nullptr, PkladiesInputInfo, nullptr,
	PkladiesInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvDokaben2 = {
	"dokaben2", nullptr, nullptr, nullptr, "1989",
	"Dokaben 2 (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSMISC, 0,
	nullptr, Dokaben2RomInfo, Dokaben2RomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	DokabenInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvDokaben = {
	"dokaben", nullptr, nullptr, nullptr, "1989",
	"Dokaben (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSMISC, 0,
	nullptr, DokabenRomInfo, DokabenRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	DokabenInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPang = {
	"pang", nullptr, nullptr, nullptr, "1989",
	"Pang (World)\0", nullptr, "Mitchell", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, PangRomInfo, PangRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvBbros = {
	"bbros", "pang", nullptr, nullptr, "1989",
	"Buster Bros. (USA)\0", nullptr, "Mitchell (Capcom license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, BbrosRomInfo, BbrosRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPompingw = {
	"pompingw", "pang", nullptr, nullptr, "1989",
	"Pomping World (Japan)\0", nullptr, "Mitchell", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, PompingwRomInfo, PompingwRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPangb = {
	"pangb", "pang", nullptr, nullptr, "1989",
	"Pang (bootleg, set 1)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, PangbRomInfo, PangbRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangbInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPangbold = {
	"pangbold", "pang", nullptr, nullptr, "1989",
	"Pang (bootleg, set 2)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, PangboldRomInfo, PangboldRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangboldInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPangbold2 = {
	"pangbold2", "pang", nullptr, nullptr, "1990",
	"Pang (bootleg, set 5)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, Pangbold2RomInfo, Pangbold2RomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangboldInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPangbold3 = {
	"pangbold3", "pang", nullptr, nullptr, "1989",
	"Pang (bootleg, set 6)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, Pangbold3RomInfo, Pangbold3RomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	PangboldInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvPangb2 = {
	"pangb2", "pang", nullptr, nullptr, "1989",
	"Pang (bootleg, set 4)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_PUZZLE, 0,
	nullptr, Pangb2RomInfo, Pangb2RomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	Pangb2Init, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvCworld = {
	"cworld", nullptr, nullptr, nullptr, "1990",
	"Capcom World (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_QUIZ, 0,
	nullptr, CworldRomInfo, CworldRomName, nullptr, nullptr, nullptr, nullptr, Qtono1InputInfo, nullptr,
	CworldInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvHatena = {
	"hatena", nullptr, nullptr, nullptr, "1990",
	"Adventure Quiz 2 - Hatena? no Daibouken (Japan 900228)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_QUIZ, 0,
	nullptr, HatenaRomInfo, HatenaRomName, nullptr, nullptr, nullptr, nullptr, Qtono1InputInfo, nullptr,
	HatenaInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvSpang = {
	"spang", nullptr, nullptr, nullptr, "1990",
	"Super Pang (World 900914)\0", nullptr, "Mitchell", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_PUZZLE, 0,
	nullptr, SpangRomInfo, SpangRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	SpangInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvSpangj = {
	"spangj", "spang", nullptr, nullptr, "1990",
	"Super Pang (Japan 901023)\0", nullptr, "Mitchell", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_PUZZLE, 0,
	nullptr, SpangjRomInfo, SpangjRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	SpangjInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvSbbros = {
	"sbbros", "spang", nullptr, nullptr, "1990",
	"Super Buster Bros. (USA 901001)\0", nullptr, "Mitchell (Capcom license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_PUZZLE, 0,
	nullptr, SbbrosRomInfo, SbbrosRomName, nullptr, nullptr, nullptr, nullptr, PangInputInfo, nullptr,
	SbbrosInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvMstworld = {
	"mstworld", nullptr, nullptr, nullptr, "1994",
	"Monsters World (bootleg of Super Pang)\0", nullptr, "bootleg (TCH)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_PUZZLE, 0,
	nullptr, MstworldRomInfo, MstworldRomName, nullptr, nullptr, nullptr, nullptr, MstworldInputInfo, MstworldDIPInfo,
	MstworldInit, CommonExit, MstworldFrame, DrvDraw, MstworldScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvMarukin = {
	"marukin", nullptr, nullptr, nullptr, "1990",
	"Super Marukin-Ban (Japan 911128)\0", nullptr, "Yuga", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_MAHJONG, 0,
	nullptr, MarukinRomInfo, MarukinRomName, nullptr, nullptr, nullptr, nullptr, MarukinInputInfo, nullptr,
	MarukinInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvMarukina = {
	"marukina", "marukin", nullptr, nullptr, "1990",
	"Super Marukin-Ban (Japan 901017)\0", nullptr, "Yuga", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_MAHJONG, 0,
	nullptr, MarukinaRomInfo, MarukinaRomName, nullptr, nullptr, nullptr, nullptr, MarukinInputInfo, nullptr,
	MarukinInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvQtono1 = {
	"qtono1", nullptr, nullptr, nullptr, "1991",
	"Quiz Tonosama no Yabou (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_QUIZ, 0,
	nullptr, Qtono1RomInfo, Qtono1RomName, nullptr, nullptr, nullptr, nullptr, Qtono1InputInfo, nullptr,
	Qtono1Init, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvQsangoku = {
	"qsangoku", nullptr, nullptr, nullptr, "1991",
	"Quiz Sangokushi (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_QUIZ, 0,
	nullptr, QsangokuRomInfo, QsangokuRomName, nullptr, nullptr, nullptr, nullptr, Qtono1InputInfo, nullptr,
	QsangokuInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 384, 240, 4, 3
};

struct BurnDriver BurnDrvBlock = {
	"block", nullptr, nullptr, nullptr, "1991",
	"Block Block (World 911219 Joystick)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S, GBF_BREAKOUT, 0,
	nullptr, BlockRomInfo, BlockRomName, nullptr, nullptr, nullptr, nullptr, BlockjoyInputInfo, nullptr,
	BlockjoyInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvBlockr1 = {
	"blockr1", "block", nullptr, nullptr, "1991",
	"Block Block (World 911106 Joystick)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S,
	GBF_BREAKOUT, 0,
	nullptr, Blockr1RomInfo, Blockr1RomName, nullptr, nullptr, nullptr, nullptr, BlockjoyInputInfo, nullptr,
	BlockjoyInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvBlockr2 = {
	"blockr2", "block", nullptr, nullptr, "1991",
	"Block Block (World 910910)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S,
	GBF_BREAKOUT, 0,
	nullptr, Blockr2RomInfo, Blockr2RomName, nullptr, nullptr, nullptr, nullptr, BlockInputInfo, nullptr,
	BlockInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvBlockj = {
	"blockj", "block", nullptr, nullptr, "1991",
	"Block Block (Japan 910910)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_POST90S,
	GBF_BREAKOUT, 0,
	nullptr, BlockjRomInfo, BlockjRomName, nullptr, nullptr, nullptr, nullptr, BlockInputInfo, nullptr,
	BlockInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	nullptr, 0x800, 240, 384, 3, 4
};

struct BurnDriver BurnDrvBlockbl = {
	"blockbl", "block", nullptr, nullptr, "1991",
	"Block Block (bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_POST90S, GBF_BREAKOUT, 0,
	nullptr, BlockblRomInfo, BlockblRomName, nullptr, nullptr, nullptr, nullptr, BlockInputInfo, nullptr,
