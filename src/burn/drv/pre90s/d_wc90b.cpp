...
static UINT8* Wc90b1Z80Rom1 = nullptr;
static UINT8* Wc90b1Z80Rom2 = nullptr;
static UINT8* Wc90b1Z80Rom3 = nullptr;
static UINT8* Wc90b1Z80Ram1 = nullptr;
static UINT8* Wc90b1Z80Ram2 = nullptr;
static UINT8* Wc90b1Z80Ram3 = nullptr;
static UINT8* Wc90b1FgVideoRam = nullptr;
static UINT8* Wc90b1BgVideoRam = nullptr;
static UINT8* Wc90b1TextVideoRam = nullptr;
static UINT8* Wc90b1SpriteRam = nullptr;
static UINT8* Wc90b1PaletteRam = nullptr;
static UINT8* Wc90b1SharedRam = nullptr;
static UINT32* Wc90b1Palette = nullptr;
static UINT8* Wc90b1CharTiles = nullptr;
static UINT8* Wc90b1Tiles = nullptr;
static UINT8* Wc90b1Sprites = nullptr;
static UINT8* Wc90b1TempGfx = nullptr;
...
static INT32 MemIndex()
{
	...
	Wc90b1Z80Ram3 = Next;
	Next += 0x00800;
	Wc90b1FgVideoRam = Next;
	Next += 0x01000;
	Wc90b1BgVideoRam = Next;
	Next += 0x01000;
	Wc90b1TextVideoRam = Next;
	Next += 0x01000;
	Wc90b1SpriteRam = Next;
	Next += 0x00800;
	Wc90b1PaletteRam = Next;
	Next += 0x00800;
	Wc90b1SharedRam = Next;
	Next += 0x00400;

	RamEnd = Next;

	Wc90b1CharTiles = Next;
	Next += (2048 * 8 * 8);
	Wc90b1Tiles = Next;
	Next += (4096 * 16 * 16);
	Wc90b1Sprites = Next;
	Next += (4096 * 16 * 16);
	Wc90b1Palette = (UINT32*)Next;
...
{
	return static_cast<INT64>(static_cast<double>(ZetTotalCycles())) * nSoundRate / 5000000;
}
...
static INT32 TileXOffsets[16] = {
	0, 1, 2, 3, 4, 5, 6, 7, 0x8000, 0x8001, 0x8002, 0x8003, 0x8004, 0x8005, 0x8006, 0x8007
};
static INT32 TileYOffsets[16] = {
	0, 8, 16, 24, 32, 40, 48, 56, 0x4000, 0x4008, 0x4010, 0x4018, 0x4020, 0x4028, 0x4030, 0x4038
};
...
static INT32 Wc90b1Init()
{
	...
	Wc90b1TempGfx = BurnMalloc(0x80000);
	if (Wc90b1TempGfx == nullptr) return 1;

	// Load Z80 #1 Program Roms
	nRet = BurnLoadRom(Wc90b1Z80Rom1 + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1Z80Rom1 + 0x10000, 1, 1);
	if (nRet != 0) return 1;

	// Load Z80 #2 Program Roms
	nRet = BurnLoadRom(Wc90b1Z80Rom2 + 0x00000, 2, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1Z80Rom2 + 0x10000, 3, 1);
	if (nRet != 0) return 1;

	// Load Z80 #3 Program Rom
	nRet = BurnLoadRom(Wc90b1Z80Rom3 + 0x00000, 4, 1);
	if (nRet != 0) return 1;

	// Load and Decode Char Tile Roms
	memset(Wc90b1TempGfx, 0, 0x80000);
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x04000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x08000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x0c000, 8, 1);
	if (nRet != 0) return 1;
	...
	memset(Wc90b1TempGfx, 0, 0x80000);
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x00000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x20000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x40000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x60000, 12, 1);
	if (nRet != 0) return 1;
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x00000,
	          Wc90b1Tiles + (0 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x02000,
	          Wc90b1Tiles + (1 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x04000,
	          Wc90b1Tiles + (2 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x06000,
	          Wc90b1Tiles + (3 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x08000,
	          Wc90b1Tiles + (4 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x0a000,
	          Wc90b1Tiles + (5 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x0c000,
	          Wc90b1Tiles + (6 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x0e000,
	          Wc90b1Tiles + (7 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x10000,
	          Wc90b1Tiles + (8 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x12000,
	          Wc90b1Tiles + (9 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x14000,
	          Wc90b1Tiles + (10 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x16000,
	          Wc90b1Tiles + (11 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x18000,
	          Wc90b1Tiles + (12 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x1a000,
	          Wc90b1Tiles + (13 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x1c000,
	          Wc90b1Tiles + (14 * 256 * 16 * 16));
	GfxDecode(256, 4, 16, 16, TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x40, Wc90b1TempGfx + 0x1e000,
	          Wc90b1Tiles + (15 * 256 * 16 * 16));

	// Load and Decode Sprite Roms
	memset(Wc90b1TempGfx, 0, 0x80000);
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x00000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x10000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x20000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x30000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x40000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x50000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x60000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Wc90b1TempGfx + 0x70000, 20, 1);
	if (nRet != 0) return 1;
	...
	BurnYM2203Init(1, 1250000, nullptr, 0);
...
static INT32 Wc90b1Frame()
{
	...
	INT32 nVBlankIRQFire = static_cast<INT32>(static_cast<double>(242) / 260 * nInterleave);

	if (Wc90b1Reset) Wc90b1DoReset();

	Wc90b1MakeInputs();

	nCyclesTotal[0] = static_cast<INT32>(static_cast<INT64>(7159090) * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[1] = static_cast<INT32>(static_cast<INT64>(7159090) * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[2] = static_cast<INT32>(static_cast<double>(5000000 / 60));
...
struct BurnDriver BurnDrvWc90b1 = {
	"twcup90b1", "twcup90", nullptr, nullptr, "1989",
	"Euro League (Italian hack of Tecmo World Cup '90, set 1)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90b1RomInfo, Wc90b1RomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};

struct BurnDriver BurnDrvWc90b2 = {
	"twcup90b2", "twcup90", nullptr, nullptr, "1989",
	"Worldcup '90 (hack)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90b2RomInfo, Wc90b2RomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};

struct BurnDriver BurnDrvWc90b3 = {
	"twcup90b3", "twcup90", nullptr, nullptr, "1989",
	"Euro League (Italian hack of Tecmo World Cup '90, set 3)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90b3RomInfo, Wc90b3RomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};

struct BurnDriver BurnDrvWc90b4 = {
	"twcup90b4", "twcup90", nullptr, nullptr, "1989",
	"World Cup '90 (hack, alt)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90b4RomInfo, Wc90b4RomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};

struct BurnDriver BurnDrvWc90ba = {
	"twcup90ba", "twcup90", nullptr, nullptr, "1989",
	"Euro League (Italian hack of Tecmo World Cup '90, set 2)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90baRomInfo, Wc90baRomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};

struct BurnDriver BurnDrvWc90bb = {
	"twcup90bb", "twcup90", nullptr, nullptr, "1989",
	"World Cup '90 (European hack, different title)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_MISC_PRE90S, GBF_SPORTSFOOTBALL, 0,
	nullptr, Wc90bbRomInfo, Wc90bbRomName, nullptr, nullptr, nullptr, nullptr, Wc90b1InputInfo, Wc90b1DIPInfo,
	Wc90b1Init, Wc90b1Exit, Wc90b1Frame, Wc90b1Draw, Wc90b1Scan,
	nullptr, 0x400, 256, 224, 4, 3
};
