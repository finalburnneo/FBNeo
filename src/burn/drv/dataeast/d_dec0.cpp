...
static UINT8* MemEnd = nullptr;
static UINT8* RamStart = nullptr;
static UINT8* RamEnd = nullptr;
static UINT8* Drv68KRom = nullptr;
static UINT8* Drv68KRam = nullptr;
static UINT8* DrvMCURom = nullptr;
static UINT8* DrvM6502Rom = nullptr;
static UINT8* DrvM6502Ram = nullptr;
static UINT8* DrvH6280Rom = nullptr;
static UINT8* DrvH6280Ram = nullptr;
static UINT8* DrvCharRam = nullptr;
static UINT8* DrvCharCtrl0Ram = nullptr;
static UINT8* DrvCharCtrl1Ram = nullptr;
static UINT8* DrvCharColScrollRam = nullptr;
static UINT8* DrvCharRowScrollRam = nullptr;
static UINT8* DrvVideo1Ram = nullptr;
static UINT8* DrvVideo1Ctrl0Ram = nullptr;
static UINT8* DrvVideo1Ctrl1Ram = nullptr;
static UINT8* DrvVideo1ColScrollRam = nullptr;
static UINT8* DrvVideo1RowScrollRam = nullptr;
static UINT8* DrvVideo2Ram = nullptr;
static UINT8* DrvVideo2Ctrl0Ram = nullptr;
static UINT8* DrvVideo2Ctrl1Ram = nullptr;
static UINT8* DrvVideo2ColScrollRam = nullptr;
static UINT8* DrvVideo2RowScrollRam = nullptr;
static UINT8* DrvPaletteRam = nullptr;
static UINT8* DrvPalette2Ram = nullptr;
static UINT8* DrvSpriteRam = nullptr;
static UINT8* DrvSpriteDMABufferRam = nullptr;
static UINT8* DrvSharedRam = nullptr;
static UINT8* DrvChars = nullptr;
static UINT8* DrvTiles1 = nullptr;
static UINT8* DrvTiles2 = nullptr;
static UINT8* DrvSprites = nullptr;
static UINT8* DrvTempRom = nullptr;
static UINT32* DrvPalette = nullptr;
static UINT16* pCharLayerDraw = nullptr;
static UINT16* pTile1LayerDraw = nullptr;
static UINT16* pTile2LayerDraw = nullptr;
...
static struct BurnDIPInfo BouldashDIPList[] =
{
	...
	// Default Values
	{0x00, 0xff, 0xff, 0x7f, nullptr},
	{0x01, 0xff, 0xff, 0x7f, nullptr},
...
static struct BurnRomInfo Drgninjab2RomDesc[] = {
	...
	{"a15.7b", 0x08000, 0x82007af2, BRF_ESS | BRF_PRG},
	//  4	6502 Program (ADPCM sounds are wrong with this, see above, 1st half identical to orginal, 2nd half 99.225%)
...
static INT32 MemIndex()
{
	...
	DrvCharRam = Next;
	Next += 0x04000;
	DrvCharCtrl0Ram = Next;
	Next += 0x00008;
	DrvCharCtrl1Ram = Next;
	Next += 0x00008;
	DrvCharColScrollRam = Next;
	Next += 0x00100;
	DrvCharRowScrollRam = Next;
	Next += 0x00400;
	DrvVideo1Ram = Next;
	Next += 0x04000;
	DrvVideo1Ctrl0Ram = Next;
	Next += 0x00008;
	DrvVideo1Ctrl1Ram = Next;
	Next += 0x00008;
	DrvVideo1ColScrollRam = Next;
	Next += 0x00100;
	DrvVideo1RowScrollRam = Next;
	Next += 0x00400;
	DrvVideo2Ram = Next;
	Next += 0x04000;
	DrvVideo2Ctrl0Ram = Next;
	Next += 0x00008;
	DrvVideo2Ctrl1Ram = Next;
	Next += 0x00008;
	DrvVideo2ColScrollRam = Next;
	Next += 0x00100;
	DrvVideo2RowScrollRam = Next;
	Next += 0x00400;
	DrvPaletteRam = Next;
	Next += 0x00800;
	DrvPalette2Ram = Next;
	...
	pCharLayerDraw = (UINT16*)Next;
	Next += (1024 * 256 * sizeof(UINT16));
	pTile1LayerDraw = (UINT16*)Next;
	Next += (1024 * 256 * sizeof(UINT16));
	pTile2LayerDraw = (UINT16*)Next;
	Next += (1024 * 256 * sizeof(UINT16));
...
static void BaddudesI8751Write(UINT16 Data)
{
	...
	switch (Data & 0xffff)
	{
	case 0x714: i8751RetVal = 0x700;
		break;
	case 0x73b: i8751RetVal = 0x701;
		break;
	case 0x72c: i8751RetVal = 0x702;
		break;
	case 0x73f: i8751RetVal = 0x703;
		break;
	case 0x755: i8751RetVal = 0x704;
		break;
	case 0x722: i8751RetVal = 0x705;
		break;
	case 0x72b: i8751RetVal = 0x706;
		break;
	case 0x724: i8751RetVal = 0x707;
		break;
	case 0x728: i8751RetVal = 0x708;
		break;
	case 0x735: i8751RetVal = 0x709;
		break;
	case 0x71d: i8751RetVal = 0x70a;
		break;
	case 0x721: i8751RetVal = 0x70b;
		break;
	case 0x73e: i8751RetVal = 0x70c;
		break;
	case 0x761: i8751RetVal = 0x70d;
		break;
	case 0x753: i8751RetVal = 0x70e;
		break;
	case 0x75b: i8751RetVal = 0x70f;
		break;
	}
}
...
static UINT8 Joy2Rotate(UINT8* joy)
{
	// ugly code, but the effect is awesome. -dink
...
static int dialRotation(INT32 playernum)
{
	...
	if (playernum == 0)
	{
		player[0] = DrvFakeInput[0];
		player[1] = DrvFakeInput[1];
	}
	if (playernum == 1)
	{
		player[0] = DrvFakeInput[2];
		player[1] = DrvFakeInput[3];
	}
...
static UINT8* rotate_gunpos[2] = {nullptr, nullptr};
...
static void RotateDoTick()
{
	...
	for (INT32 i = 0; i < 2; i++)
	{
		if (rotate_gunpos[i] && (nRotateTarget[i] != -1) && (nRotateTarget[i] != (*rotate_gunpos[i] & 0xff)))
		{
			...
			bprintf(0, _T("p%X target %X mempos %X nRotate %X.\n"), i, nRotateTarget[0], *rotate_gunpos[0] & 0xff,
			        nRotate[0]);
			nRotateTry[i]++;
			if (nRotateTry[i] > 10) nRotateTarget[i] = -1;
			// don't get stuck in a loop if something goes horribly wrong here.
		}
		else
		{
			nRotateTarget[i] = -1;
...
static void SuperJoy2Rotate()
{
	...
	for (INT32 i = 0; i < 2; i++)
	{
		...
		else if (DrvFakeInput[4 + i])
		{
			//  rotate-button had been pressed
			...
			DrvInput[i] = (DrvInput[i] & ~0xf) | (nRotateHoldInput[i] & 0xf);
			// for midnight resistance! be able to duck + change direction of gun.
			nRotateTry[i] = 0;
		}
		else
		{
			// cache joystick UDLR if the rotate button isn't pressed.
...
static void DrvMCUSync()
{
	INT32 todo = (static_cast<double>(SekTotalCycles()) * (8000000 / 12) / 10000000) - nCyclesDone[2];
...
static void __fastcall Dec068KWriteWord(UINT32 a, UINT16 d)
{
	...
	case 0x240006:
		{
			auto Control0 = (UINT16*)DrvCharCtrl0Ram;
	...
	case 0x240016:
		{
			auto Control1 = (UINT16*)DrvCharCtrl1Ram;
	...
	case 0x246006:
		{
			auto Control0 = (UINT16*)DrvVideo1Ctrl0Ram;
	...
	case 0x246016:
		{
			auto Control1 = (UINT16*)DrvVideo1Ctrl1Ram;
	...
	case 0x24c006:
		{
			auto Control0 = (UINT16*)DrvVideo2Ctrl0Ram;
	...
	case 0x24c016:
		{
			auto Control1 = (UINT16*)DrvVideo2Ctrl1Ram;
...
static void __fastcall Midres68KWriteWord(UINT32 a, UINT16 d)
{
	...
	case 0x1a0000:
		{
			// midres ending sequence writes here after the credits, it brings the music volume down quite a bit making the drums quite prominent during hiscore name entry.
			DrvSoundLatch = d & 0xff;
			h6280SetIRQLine(H6280_INPUT_LINE_NMI, CPU_IRQSTATUS_AUTO);
			return;
		}
...
static INT32 Dec0MachineInit()
{
	...
	Mem = nullptr;
	MemIndex();
	nLen = MemEnd - static_cast<UINT8*>(nullptr);
	if ((Mem = BurnMalloc(nLen)) == nullptr) return 1;
	memset(Mem, 0, nLen);
	MemIndex();

	DrvTempRom = BurnMalloc(0x80000);
...
static INT32 BaddudesInit()
{
	...
	nRet = BurnLoadRom(Drv68KRom + 0x00001, 0, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x00000, 1, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x40001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x40000, 3, 2);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvM6502Rom, 4, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 6, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 4, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x40, DrvTempRom, DrvChars);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 10, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 4, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles1);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 12, 1);
	if (nRet != 0) return 1;
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 20, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 21, 1);
	if (nRet != 0) return 1;
...
static INT32 BirdtryInit()
{
	...
	nRet = BurnLoadRom(Drv68KRom + 0x00000, 1, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x20001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x20000, 3, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x40001, 4, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x40000, 5, 2);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvM6502Rom, 6, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom + 0x00000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 9, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 4, 8, 8, CharPlaneOffsets, CharXOffsets, CharYOffsets, 0x40, DrvTempRom, DrvChars);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 17, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles1);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 24, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 25, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 26, 1);
	if (nRet != 0) return 1;

	BurnFree(DrvTempRom);

	realMCU = 1;
	nRet = BurnLoadRom(DrvMCURom + 0x00000, 7, 1);
	if (nRet != 0) return 1;
	DrvMCUInit();
...
static INT32 Drgninjab2Init()
{
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 14, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x400, 4, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles2);
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 22, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 23, 1);
	if (nRet != 0) return 1;
...
static INT32 HbarrelInit()
{
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 16, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles1);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 20, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 4, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles2);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 24, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 25, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 26, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 27, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 28, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 29, 1);
	if (nRet != 0) return 1;

	realMCU = 1;
	nRet = BurnLoadRom(DrvMCURom + 0x00000, 30, 1);
	if (nRet != 0) return 1;
	DrvMCUInit();
...
static INT32 HippodrmInit()
{
	...
	nRet = BurnLoadRom(DrvH6280Rom, 5, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom + 0x00000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 7, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 8, 8, RobocopCharPlaneOffsets, CharXOffsets, CharYOffsets, 0x40, DrvTempRom, DrvChars);
	...
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 11, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 15, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x400, 4, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles2);

	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x70000, 23, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 24, 1);
	if (nRet != 0) return 1;
...
static INT32 RobocopInit()
{
	...
	nRet = BurnLoadRom(DrvH6280Rom + 0x01e00, 5, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 11, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 4, 16, 16, Tile1PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles1);
...
static INT32 RobocopbInit()
{
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 6, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 8, 8, RobocopCharPlaneOffsets, CharXOffsets, CharYOffsets, 0x40, DrvTempRom, DrvChars);
	...
	memset(DrvTempRom, 0, 0x80000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 12, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 14, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x400, 4, 16, 16, Tile2PlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles2);
...
static INT32 SlyspyDrvInit()
{
	...
	MSM6295Init(0, 1000000 / 132, true);
...
static INT32 SlyspyLoadRoms()
{
	...
	nRet = BurnLoadRom(DrvH6280Rom, 4, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom + 0x10000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 6, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 14, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);

	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 15, 1);
	if (nRet != 0) return 1;
...
static INT32 BouldashLoadRoms()
{
	...
	nRet = BurnLoadRom(DrvH6280Rom, 6, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvTempRom + 0x20000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 8, 1);
	if (nRet != 0) return 1;
	memcpy(DrvTempRom + 0x08000, DrvTempRom + 0x20000, 0x8000);
	memcpy(DrvTempRom + 0x00000, DrvTempRom + 0x28000, 0x8000);
	memcpy(DrvTempRom + 0x18000, DrvTempRom + 0x30000, 0x8000);
	...
	nRet = BurnLoadRom(MSM6295ROM + 0x00000, 17, 1);
	if (nRet != 0) return 1;
...
static INT32 MidresInit()
{
	...
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 6, 1);
	if (nRet != 0) return 1;
	memcpy(DrvTempRom + 0x08000, DrvTempRom + 0x20000, 0x8000);
	memcpy(DrvTempRom + 0x00000, DrvTempRom + 0x28000, 0x8000);
	memcpy(DrvTempRom + 0x18000, DrvTempRom + 0x30000, 0x8000);
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 10, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvTiles1);
	...
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x60000, 16, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x1000, 4, 16, 16, SpritePlaneOffsets, TileXOffsets, TileYOffsets, 0x100, DrvTempRom, DrvSprites);
...
{
	auto PaletteRam = (UINT16*)DrvPaletteRam;
	auto Palette2Ram = (UINT16*)DrvPalette2Ram;
...
static void Dec0Render8x8Tile_Mask(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY, INT32 nTilePalette,
                                   INT32 nColourDepth, INT32 nMaskColour, INT32 nPaletteOffset, UINT8* pTile,
                                   INT32 TilemapWidth)
{
...
static void Dec0Render8x8Tile_Mask_FlipXY(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY,
                                          INT32 nTilePalette, INT32 nColourDepth, INT32 nMaskColour,
                                          INT32 nPaletteOffset, UINT8* pTile, INT32 TilemapWidth)
{
	UINT32 nPalette = nTilePalette << nColourDepth;
	pTileData = pTile + (nTileNumber << 6);
...
static void Dec0Render16x16Tile(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY, INT32 nTilePalette,
                                INT32 nColourDepth, INT32 nPaletteOffset, UINT8* pTile, INT32 TilemapWidth)
{
	UINT32 nPalette = nTilePalette << nColourDepth;
...
static void Dec0Render16x16Tile_FlipXY(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY,
                                       INT32 nTilePalette, INT32 nColourDepth, INT32 nPaletteOffset, UINT8* pTile,
                                       INT32 TilemapWidth)
{
	UINT32 nPalette = nTilePalette << nColourDepth;
	pTileData = pTile + (nTileNumber << 8);
...
static void Dec0Render16x16Tile_Mask(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY,
                                     INT32 nTilePalette, INT32 nColourDepth, INT32 nMaskColour, INT32 nPaletteOffset,
                                     UINT8* pTile, INT32 TilemapWidth)
{
	UINT32 nPalette = nTilePalette << nColourDepth;
	pTileData = pTile + (nTileNumber << 8);
...
static void Dec0Render16x16Tile_Mask_FlipXY(UINT16* pDestDraw, INT32 nTileNumber, INT32 StartX, INT32 StartY,
                                            INT32 nTilePalette, INT32 nColourDepth, INT32 nMaskColour,
                                            INT32 nPaletteOffset, UINT8* pTile, INT32 TilemapWidth)
{
	UINT32 nPalette = nTilePalette << nColourDepth;
...
static void DrvRenderCustomTilemap(UINT16* pSrc, UINT16* pControl0, UINT16* pControl1, UINT16* RowScrollRam,
                                   UINT16* ColScrollRam, INT32 TilemapWidth, INT32 TilemapHeight, INT32 Opaque,
                                   INT32 DrawLayer)
{
	...
	for (y = 0; y < nScreenHeight; y++)
	{
		if (RowScrollEnabled)
		{
			xSrc = xScroll + BURN_ENDIAN_SWAP_INT16(
				RowScrollRam[(ySrc >> (BURN_ENDIAN_SWAP_INT16(pControl1[3]) & 0x0f)) & (0x1ff >> (BURN_ENDIAN_SWAP_INT16
					(pControl1[3]) & 0x0f))]);
		}
		else
		{
		...
		{
			if (ColScrollEnabled) ColOffset = BURN_ENDIAN_SWAP_INT16(
				ColScrollRam[((xSrc >> 3) >> (BURN_ENDIAN_SWAP_INT16(pControl1[2]) & 0x0f)) & (0x3f >> (
					BURN_ENDIAN_SWAP_INT16(pControl1[2]) & 0x0f))]);
...
static void DrvRenderTile1Layer(INT32 Opaque, INT32 DrawLayer)
{
	...
	auto Control0 = (UINT16*)DrvVideo1Ctrl0Ram;
	auto VideoRam = (UINT16*)DrvVideo1Ram;
			...
			if (DrawLayer == TILEMAP_BOTH_LAYERS || DrawLayer == Layer)
			{
				...
				if (Opaque)
				{
					if (DrvFlipScreen)
					{
						...
						Dec0Render16x16Tile_FlipXY(pTile1LayerDraw, Code, x, y, Colour, 4, 512, DrvTiles1,
						                           DrvTile1TilemapWidth);
					}
					else
					{
						Dec0Render16x16Tile(pTile1LayerDraw, Code, x, y, Colour, 4, 512, DrvTiles1,
						                    DrvTile1TilemapWidth);
					}
				}
				else
				{
					if (DrvFlipScreen)
					{
						...
						x &= (DrvTile1TilemapWidth - 1);
						y &= (DrvTile1TilemapHeight - 1);
						Dec0Render16x16Tile_Mask_FlipXY(pTile1LayerDraw, Code, x, y, Colour, 4, 0, 512, DrvTiles1,
						                                DrvTile1TilemapWidth);
					}
					else
					{
						Dec0Render16x16Tile_Mask(pTile1LayerDraw, Code, x, y, Colour, 4, 0, 512, DrvTiles1,
						                         DrvTile1TilemapWidth);
					}
				}
			}
	...
	DrvRenderCustomTilemap(pTile1LayerDraw, (UINT16*)DrvVideo1Ctrl0Ram, (UINT16*)DrvVideo1Ctrl1Ram,
	                       (UINT16*)DrvVideo1RowScrollRam, (UINT16*)DrvVideo1ColScrollRam, DrvTile1TilemapWidth,
	                       DrvTile1TilemapHeight, Opaque, DrawLayer);
}
...
static void DrvRenderTile2Layer(INT32 Opaque, INT32 DrawLayer)
{
	INT32 mx, my, Code, Attr, Colour, x, y, TileIndex, Layer;
	auto Control0 = (UINT16*)DrvVideo2Ctrl0Ram;
	auto VideoRam = (UINT16*)DrvVideo2Ram;
			...
			if (DrawLayer == TILEMAP_BOTH_LAYERS || DrawLayer == Layer)
			{
				...
				if (Opaque)
				{
					if (DrvFlipScreen)
					{
						...
						Dec0Render16x16Tile_FlipXY(pTile2LayerDraw, Code, x, y, Colour, 4, 768, DrvTiles2,
						                           DrvTile2TilemapWidth);
					}
					else
					{
						Dec0Render16x16Tile(pTile2LayerDraw, Code, x, y, Colour, 4, 768, DrvTiles2,
						                    DrvTile2TilemapWidth);
					}
				}
				else
				{
					if (DrvFlipScreen)
					{
						...
						x &= (DrvTile2TilemapWidth - 1);
						y &= (DrvTile2TilemapHeight - 1);
						Dec0Render16x16Tile_Mask_FlipXY(pTile2LayerDraw, Code, x, y, Colour, 4, 0, 768, DrvTiles2,
						                                DrvTile2TilemapWidth);
					}
					else
					{
						Dec0Render16x16Tile_Mask(pTile2LayerDraw, Code, x, y, Colour, 4, 0, 768, DrvTiles2,
						                         DrvTile2TilemapWidth);
					}
				}
			}
	...
	DrvRenderCustomTilemap(pTile2LayerDraw, (UINT16*)DrvVideo2Ctrl0Ram, (UINT16*)DrvVideo2Ctrl1Ram,
	                       (UINT16*)DrvVideo2RowScrollRam, (UINT16*)DrvVideo2ColScrollRam, DrvTile2TilemapWidth,
	                       DrvTile2TilemapHeight, Opaque, DrawLayer);
}
...
static void DrvRenderCharLayer()
{
	...
	auto Control0 = (UINT16*)DrvCharCtrl0Ram;
	auto CharRam = (UINT16*)DrvCharRam;
	...
	for (my = 0; my < (DrvCharTilemapHeight / 8); my++)
	{
		for (mx = 0; mx < (DrvCharTilemapWidth / 8); mx++)
		{
			...
			if (DrvFlipScreen)
			{
				...
				Dec0Render8x8Tile_Mask_FlipXY(pCharLayerDraw, Code, x, y, Colour, 4, 0, DrvCharPalOffset, DrvChars,
				                              DrvCharTilemapWidth);
			}
			else
			{
				Dec0Render8x8Tile_Mask(pCharLayerDraw, Code, x, y, Colour, 4, 0, DrvCharPalOffset, DrvChars,
				                       DrvCharTilemapWidth);
			}
		}
	}

	DrvRenderCustomTilemap(pCharLayerDraw, (UINT16*)DrvCharCtrl0Ram, (UINT16*)DrvCharCtrl1Ram,
	                       (UINT16*)DrvCharRowScrollRam, (UINT16*)DrvCharColScrollRam, DrvCharTilemapWidth,
	                       DrvCharTilemapHeight, 0, TILEMAP_BOTH_LAYERS);
}

static void DrvRenderSprites(INT32 PriorityMask, INT32 PriorityVal)
{
	auto spriteram = (UINT16*)DrvSpriteDMABufferRam;
	constexpr INT32 size = 0x800 / 2;
		...
		for (INT32 x = 0; x < w; x++)
		{
			if (offs < size)
			{
				...
				for (INT32 y = 0; y < h; y++)
				{
					...
					if (draw)
					{
						Draw16x16MaskTile(pTransDraw, (code - y * incy) & 0xfff, sx + (mult * x), (sy + (mult * y)) - 8,
						                  flipx, flipy, color & 0xf, 4, 0, DrvSpritePalOffset, DrvSprites);
					}
				}
			}
			offs += 4;
...
static INT32 DrvFrame()
{
	...
	nCyclesTotal[0] = static_cast<INT32>(static_cast<double>(10000000) / 57.41);
	nCyclesTotal[1] = static_cast<INT32>(static_cast<double>(1500000) / 57.41);
	nCyclesTotal[2] = static_cast<INT32>(static_cast<double>(8000000) / 57.41 / 12);
	nCyclesDone[0] = nExtraCycles[0];
	nCyclesDone[1] = nExtraCycles[1];
	nCyclesDone[2] = nExtraCycles[2];
...
static INT32 RobocopFrame()
{
	...
	nCyclesTotal[0] = static_cast<INT32>(static_cast<double>(10000000) / 57.41);
	nCyclesTotal[1] = static_cast<INT32>(static_cast<double>(1500000) / 57.41);
	nCyclesTotal[2] = static_cast<INT32>(static_cast<double>(1342329) / 57.41);
...
static INT32 Dec1Frame()
{
	...
	if (nPrevBurnCPUSpeedAdjust != nBurnCPUSpeedAdjust)
	{
		...
		nCyclesTotal[0] = static_cast<INT32>(static_cast<INT64>(10000000.0) * 100 * nBurnCPUSpeedAdjust / (0x100 *
			nBurnFPS));
		INT32 adj_mhz = static_cast<INT32>(10000000.0 * nBurnCPUSpeedAdjust / 0x100);
		bprintf(0, _T("adjusted mhz / cycles per frame:  %d  /  %d\n"), adj_mhz, nCyclesTotal[0]);
		nPrevBurnCPUSpeedAdjust = nBurnCPUSpeedAdjust;
	}

	nCyclesTotal[1] = static_cast<INT32>(static_cast<double>(2000000) / 57.44);
	if (slyspy_mode)
		nCyclesTotal[1] = static_cast<INT32>(static_cast<double>(3000000) / 57.44);
...
struct BurnDriver BurnDrvBaddudes = {
	"baddudes", nullptr, nullptr, nullptr, "1988",
	"Bad Dudes vs. Dragonninja (US revision 1)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, BaddudesRomInfo, BaddudesRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, BaddudesDIPInfo,
	BaddudesInit, BaddudesExit, DrvFrame, BaddudesDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvBirdtry = {
	"birdtry", nullptr, nullptr, nullptr, "1988",
	"Birdie Try (Japan revision 2, revision 1 MCU)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SPORTSMISC, 0,
	nullptr, birdtryRomInfo, birdtryRomName, nullptr, nullptr, nullptr, nullptr, BirdtryInputInfo, BirdtryDIPInfo,
	BirdtryInit, BaddudesExit, DrvFrame, BirdtryDraw, BaddudesScan,
	nullptr, 0x400, 240, 256, 3, 4
};

struct BurnDriver BurnDrvBirdtrya = {
	"birdtrya", "birdtry", nullptr, nullptr, "1988",
	"Birdie Try (Japan revision 2)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST,
	GBF_SPORTSMISC, 0,
	nullptr, birdtryaRomInfo, birdtryaRomName, nullptr, nullptr, nullptr, nullptr, BirdtryInputInfo, BirdtryDIPInfo,
	BirdtryInit, BaddudesExit, DrvFrame, BirdtryDraw, BaddudesScan,
	nullptr, 0x400, 240, 256, 3, 4
};

struct BurnDriver BurnDrvBirdtryb = {
	"birdtryb", "birdtry", nullptr, nullptr, "1988",
	"Birdie Try (Japan revision S)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST,
	GBF_SPORTSMISC, 0,
	nullptr, birdtrybRomInfo, birdtrybRomName, nullptr, nullptr, nullptr, nullptr, BirdtryInputInfo, BirdtryDIPInfo,
	BirdtryInit, BaddudesExit, DrvFrame, BirdtryDraw, BaddudesScan,
	nullptr, 0x400, 240, 256, 3, 4
};

struct BurnDriver BurnDrvDrgninja = {
	"drgninja", "baddudes", nullptr, nullptr, "1988",
	"Dragonninja (Japan revision 1)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, DrgninjaRomInfo, DrgninjaRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, BaddudesDIPInfo,
	BaddudesInit, BaddudesExit, DrvFrame, BaddudesDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvDrgninjab = {
	"drgninjab", "baddudes", nullptr, nullptr, "1988",
	"Dragonninja (bootleg)\0", nullptr, "bootleg", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, DrgninjabRomInfo, DrgninjabRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, BaddudesDIPInfo,
	BaddudesInit, BaddudesExit, DrvFrame, BaddudesDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvDrgninjab2 = {
	"drgninjab2", "baddudes", nullptr, nullptr, "1988",
	"Dragonninja (bootleg with 68705)\0", nullptr, "bootleg", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, Drgninjab2RomInfo, Drgninjab2RomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, BaddudesDIPInfo,
	Drgninjab2Init, BaddudesExit, DrvFrame, BaddudesDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvBouldash = {
	"bouldash", nullptr, nullptr, nullptr, "1990",
	"Boulder Dash / Boulder Dash Part 2 (World)\0", nullptr, "Data East Corporation (licensed from First Star)", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_MAZE, 0,
	nullptr, BouldashRomInfo, BouldashRomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, BouldashDIPInfo,
	BouldashInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvBouldashj = {
	"bouldashj", "bouldash", nullptr, nullptr, "1990",
	"Boulder Dash / Boulder Dash Part 2 (Japan)\0", nullptr, "Data East Corporation (licensed from First Star)", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_MAZE, 0,
	nullptr, BouldashjRomInfo, BouldashjRomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, BouldashDIPInfo,
	BouldashInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvHbarrel = {
	"hbarrel", nullptr, nullptr, nullptr, "1987",
	"Heavy Barrel (World)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, HbarrelRomInfo, HbarrelRomName, nullptr, nullptr, nullptr, nullptr, HbarrelInputInfo, HbarrelDIPInfo,
	HbarrelInit, BaddudesExit, DrvFrame, HbarrelDraw, BaddudesScan,
	nullptr, 0x400, 240, 256, 3, 4
};

struct BurnDriver BurnDrvHbarrelu = {
	"hbarrelu", "hbarrel", nullptr, nullptr, "1987",
	"Heavy Barrel (US)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST,
	GBF_RUNGUN, 0,
	nullptr, HbarreluRomInfo, HbarreluRomName, nullptr, nullptr, nullptr, nullptr, HbarrelInputInfo, HbarrelDIPInfo,
	HbarrelInit, BaddudesExit, DrvFrame, HbarrelDraw, BaddudesScan,
	nullptr, 0x400, 240, 256, 3, 4
};

struct BurnDriver BurnDrvHippodrm = {
	"hippodrm", nullptr, nullptr, nullptr, "1989",
	"Hippodrome (US)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, HippodrmRomInfo, HippodrmRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, HippodrmDIPInfo,
	HippodrmInit, RobocopExit, RobocopFrame, HippodrmDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvFfantasy = {
	"ffantasy", "hippodrm", nullptr, nullptr, "1989",
	"Fighting Fantasy (Japan revision 3)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, FfantasyRomInfo, FfantasyRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, FfantasyDIPInfo,
	HippodrmInit, RobocopExit, RobocopFrame, HippodrmDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvFfantasyj = {
	"ffantasyj", "hippodrm", nullptr, nullptr, "1989",
	"Fighting Fantasy (Japan revision 2)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, FfantasyjRomInfo, FfantasyjRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, FfantasyDIPInfo,
	HippodrmInit, RobocopExit, RobocopFrame, HippodrmDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvFfantasya = {
	"ffantasya", "hippodrm", nullptr, nullptr, "1989",
	"Fighting Fantasy (Japan)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, FfantasyaRomInfo, FfantasyaRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, FfantasyDIPInfo,
	HippodrmInit, RobocopExit, RobocopFrame, HippodrmDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvFfantasyb = {
	"ffantasyb", "hippodrm", nullptr, nullptr, "1989",
	"Fighting Fantasy (Japan revision ?)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, FfantasybRomInfo, FfantasybRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, FfantasyDIPInfo,
	HippodrmInit, RobocopExit, RobocopFrame, HippodrmDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvMidres = {
	"midres", nullptr, nullptr, nullptr, "1989",
	"Midnight Resistance (World, set 1)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, MidresRomInfo, MidresRomName, nullptr, nullptr, nullptr, nullptr, MidresInputInfo, MidresDIPInfo,
	MidresInit, SlyspyExit, Dec1Frame, MidresDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvMidres2 = {
	"midres2", "midres", nullptr, nullptr, "1989",
	"Midnight Resistance (World, set 2)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, Midres2RomInfo, Midres2RomName, nullptr, nullptr, nullptr, nullptr, MidresInputInfo, MidresDIPInfo,
	MidresInit, SlyspyExit, Dec1Frame, MidresDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvMidresu = {
	"midresu", "midres", nullptr, nullptr, "1989",
	"Midnight Resistance (US)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, MidresuRomInfo, MidresuRomName, nullptr, nullptr, nullptr, nullptr, MidresInputInfo, MidresDIPInfo,
	MidresInit, SlyspyExit, Dec1Frame, MidresDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvMidresj = {
	"midresj", "midres", nullptr, nullptr, "1989",
	"Midnight Resistance (Japan)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, MidresjRomInfo, MidresjRomName, nullptr, nullptr, nullptr, nullptr, MidresInputInfo, MidresDIPInfo,
	MidresInit, SlyspyExit, Dec1Frame, MidresDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocop = {
	"robocop", nullptr, nullptr, nullptr, "1988",
	"Robocop (World revision 4)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, RobocopRomInfo, RobocopRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopInit, RobocopExit, RobocopFrame, RobocopDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopw = {
	"robocopw", "robocop", nullptr, nullptr, "1988",
	"Robocop (World revision 3)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, RobocopwRomInfo, RobocopwRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopInit, RobocopExit, RobocopFrame, RobocopDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopj = {
	"robocopj", "robocop", nullptr, nullptr, "1988",
	"Robocop (Japan)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, RobocopjRomInfo, RobocopjRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopInit, RobocopExit, RobocopFrame, RobocopDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopu = {
	"robocopu", "robocop", nullptr, nullptr, "1988",
	"Robocop (US revision 1)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, RobocopuRomInfo, RobocopuRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopInit, RobocopExit, RobocopFrame, RobocopDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopu0 = {
	"robocopu0", "robocop", nullptr, nullptr, "1988",
	"Robocop (US revision 0)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, Robocopu0RomInfo, Robocopu0RomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopInit, RobocopExit, RobocopFrame, RobocopDraw, RobocopScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopb = {
	"robocopb", "robocop", nullptr, nullptr, "1988",
	"Robocop (World bootleg)\0", nullptr, "bootleg", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, RobocopbRomInfo, RobocopbRomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopbInit, BaddudesExit, DrvFrame, RobocopDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvRobocopb2 = {
	"robocopb2", "robocop", nullptr, nullptr, "1989",
	"Robocop (Red Corporation World bootleg)\0", nullptr, "bootleg", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, Robocopb2RomInfo, Robocopb2RomName, nullptr, nullptr, nullptr, nullptr, Dec0InputInfo, RobocopDIPInfo,
	RobocopbInit, BaddudesExit, DrvFrame, RobocopDraw, BaddudesScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvSecretag = {
	"secretag", nullptr, nullptr, nullptr, "1989",
	"Secret Agent (World revision 3)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, SecretagRomInfo, SecretagRomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, SlyspyDIPInfo,
	SlyspyInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvSecretagj = {
	"secretagj", "secretag", nullptr, nullptr, "1989",
	"Secret Agent (Japan revision 2)\0", nullptr, "Data East Corporation", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, SecretagjRomInfo, SecretagjRomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, SlyspyDIPInfo,
	SlyspyInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvSlyspy = {
	"slyspy", "secretag", nullptr, nullptr, "1989",
	"Sly Spy (US revision 4)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, SlyspyRomInfo, SlyspyRomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, SlyspyDIPInfo,
	SlyspyInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvSlyspy2 = {
	"slyspy2", "secretag", nullptr, nullptr, "1989",
	"Sly Spy (US revision 2)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, Slyspy2RomInfo, Slyspy2RomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, SlyspyDIPInfo,
	SlyspyInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};

struct BurnDriver BurnDrvSlyspy3 = {
	"slyspy3", "secretag", nullptr, nullptr, "1989",
	"Sly Spy (US revision 3)\0", nullptr, "Data East USA", "DEC0",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, Slyspy3RomInfo, Slyspy3RomName, nullptr, nullptr, nullptr, nullptr, Dec1InputInfo, SlyspyDIPInfo,
	SlyspyInit, SlyspyExit, Dec1Frame, SlyspyDraw, SlyspyScan,
	nullptr, 0x400, 256, 240, 4, 3
};
