...
INT32 GamcMisc(struct GameInp* pgi, char* szi, INT32 nPlayer)
{
	...
	if (strncmp(szi, "mouse", 5) == 0)
	{
		...
		{
			char* szb = szi + 13;
			INT32 nButton = strtol(szb, nullptr, 0);
			if (nButton >= 1)
			{
				nButton--;
			}
...
static void SetSliderKey(struct GameInp* pgi, INT32 k0, INT32 k1, INT32 nSlide)
{
	...
	pgi->Input.Slider.SliderAxis.nSlider[0] = static_cast<UINT8>(k0);
	pgi->Input.Slider.SliderAxis.nSlider[1] = static_cast<UINT8>(k1);
...
{
	char* szSearch = nullptr;
	INT32 k0 = 0, k1 = 0;
	szSearch = szPlay[nPlayer & 3];
	if (_strnicmp(szSearch, szi, 3) != 0)
	{
		// Not our player
		return 1;
	}
...
INT32 GamcAnalogJoy(struct GameInp* pgi, char* szi, INT32 nPlayer, INT32 nJoy, INT32 nSlide)
{
	...
	case 2: // Sliding
		...
		pgi->Input.Slider.JoyAxis.nJoy = static_cast<UINT8>(nJoy);
		break;
	...
	case 3: // Absolute, axis-neg
		...
		pgi->Input.JoyAxis.nJoy = static_cast<UINT8>(nJoy);
		break;
