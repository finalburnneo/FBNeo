...
int setCooperativeLevel(bool exclusive, bool foreGround)
{
	if (keyboardProperties[0].lpdid)
	{
		...
		if (foreGround)
		{
			keyboardProperties[0].lpdid->SetCooperativeLevel(
				hDinpWnd,
				DISCL_NONEXCLUSIVE | DISCL_FOREGROUND | (nVidFullscreen ? DISCL_NOWINKEY : 0));
		}
		else
		{
			keyboardProperties[0].lpdid->SetCooperativeLevel(
				hDinpWnd,
				DISCL_NONEXCLUSIVE | DISCL_BACKGROUND | (nVidFullscreen ? DISCL_NOWINKEY : 0));
		}
...
{
	if (keyboard->readStatus)
	{
		// Already read this frame - ready to go
		return 0;
	}
	if (!keyboard->lpdid)
	{
...
{
	if (gamepad->readStatus == 1)
	{
		// Already read this frame - ready to go
		return 0;
	}
	if (gamepad->readStatus == 2)
	{
...
int gamepadState(gamepadData* gamepad, unsigned int subCode)
{
	...
	if (subCode < 0x10 + (gamepad->dwPOVs << 2))
	{
		// POV hat controls
	...
	if (subCode < 0x80 + gamepad->dwButtons)
	{
		// gamepad buttons
...
int checkMouseState(mouseData* mouse, unsigned int subCode)
{
	...
	if (subCode < 0x80 + mouse->dwButtons)
	{
		// mouse buttons
...
int getState(int code)
{
	...
	if (code < 256)
	{
		...
		{
			// Check keyboard has been read - return not pressed on error
			return 0;
		}
	...
	if (code < 0x8000)
	{
		...
		if (i >= gamepadCount)
		{
			// This gamepad number isn't connected
			return 0;
		}
		if (readGamepad(&gamepadProperties[i]))
		{
			// Error polling the gamepad
			return 0;
		}
	...
	if (code < 0xC000)
	{
		...
		if (i >= mouseCount)
		{
			// This mouse number isn't connected
			return 0;
		}
		if (readMouse(&mouseProperties[i]))
		{
			// Error Reading the mouse
			return 0;
		}
...
int readGamepadAxis(int i, int axis)
{
	...
	{
		// This gamepad isn't connected
		return 0;
	}
	if (readGamepad(gamepad))
	{
		// Error polling the gamepad
		return 0;
	}
...
int readMouseAxis(int i, int axis)
{
	...
	if (i < 0 || i >= mouseCount)
	{
		// This mouse isn't connected
		return 0;
	}
	if (readMouse(mouse) != 0)
	{
		// Error polling the mouse
		return 0;
	}
...
int find(bool createBaseline)
{
	...
	for (int i = 0; i < gamepadCount; i++)
	{
		...
		if (readGamepad(gamepad))
		{
			// There was an error polling the gamepad
			continue;
		}
		...
		for (unsigned int j = 0x10; j < 0x10 + (gamepad->dwPOVs << 2); j++)
		{
			// POV hats
			if (gamepadState(gamepad, j))
			{
		...
		for (unsigned int j = 0x80; j < 0x80 + gamepad->dwButtons; j++)
		{
			// Buttons
			if (gamepadState(gamepad, j))
			{
	...
	case 0x0000:
		{
			...
			if (controlName)
			{
				...
				if (SUCCEEDED(
					keyboardProperties[i].lpdid->GetObjectInfo(&didoi, DIDFT_MAKEINSTANCE(code & 0xFF) | DIDFT_PSHBUTTON
						, DIPH_BYID)))
				{
...
struct InputInOut InputInOutDInput = {
	init, exit, setCooperativeLevel, newFrame, getState, readGamepadAxis, readMouseAxis, find, getControlName, NULL,
