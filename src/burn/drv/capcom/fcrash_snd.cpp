...
static UINT8* FcrashZ80Ram = nullptr;
...
{
	INT32 nCyclesToDo = (static_cast<INT64>(SekTotalCycles()) * nCpsZ80Cycles / nCpsCycles) - ZetTotalCycles();
	INT32 nEnd = FcrashSoundPos + static_cast<INT64>(FcrashMSM5205Interleave) * nCyclesToDo / nCpsZ80Cycles;
...
{
	return static_cast<INT64>((double)ZetTotalCycles() * nSoundRate / (24000000 / 6));
}
...
INT32 FcrashSoundInit()
{
	...
	BurnYM2203Init(2, 24000000 / 6, nullptr, 0);
...
void FcrashSoundFrameStart()
{
	...
	FcrashSoundPos = 0;
	FcrashCyclesPerSegment = static_cast<INT64>(nCpsZ80Cycles) / FcrashMSM5205Interleave;
