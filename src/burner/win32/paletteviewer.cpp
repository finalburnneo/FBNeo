...
static void UpdateLabels()
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		SendMessage(GetDlgItem(hPaletteViewerDlg, IDC_GFX_VIEWER_VERT_1 + i), WM_SETTEXT, (WPARAM)0,
		            (LPARAM)szItemText);
	}
}
...
static INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		for (int y = 0; y < 16; y++)
		{
			for (int x = 0; x < 16; x++)
			{
				PaletteControl[(y * 16) + x] = CreateWindowEx(0, _T("STATIC"), NULL, WS_CHILD | WS_VISIBLE | SS_NOTIFY,
				                                              (x * 21) + 38, (y * 21) + 21, 20, 20, hPaletteViewerDlg,
				                                              NULL, NULL, NULL);
			}
		}
		...
		if (Notify == STN_CLICKED)
		{
			...
			for (int i = 0; i < 256; i++)
			{
				if ((HWND)lParam == PaletteControl[i])
				{
					...
					SendMessage(GetDlgItem(hPaletteViewerDlg, IDC_GFX_VIEWER_SEL_COL), WM_SETTEXT, (WPARAM)0,
					            (LPARAM)szText);
					return 0;
				}
			}
