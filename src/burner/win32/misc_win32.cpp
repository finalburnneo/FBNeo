...
int AppDirectory()
{
	...
	// strip .exe
	if ((pc1 = _tcschr(szAppExeName, _T('.'))) != nullptr)
	{
		*pc1 = 0;
	}
...
static void MyRegCreateKeys(int nDepth, TCHAR* pNames[], HKEY* pResult)
{
	...
	{
		pResult[i] = nullptr;
		RegCreateKeyEx((i ? pResult[i - 1] : HKEY_CLASSES_ROOT), pNames[i], 0, _T(""), 0, KEY_WRITE, nullptr,
		               &pResult[i], nullptr);
	}
}
...
void RegisterExtensions(bool bCreateKeys)
{
	...
	if (bCreateKeys)
	{
		...
		RegSetValueEx(myKeys[0], nullptr, 0, REG_SZ, (BYTE*)myKeyValue, (_tcslen(myKeyValue) + 1) * sizeof(TCHAR));
		...
		RegSetValueEx(myKeys[3], nullptr, 0, REG_SZ, (BYTE*)myKeyValue, (_tcslen(myKeyValue) + 1) * sizeof(TCHAR));
		...
		RegSetValueEx(myKeys[1], nullptr, 0, REG_SZ, (BYTE*)myKeyValue, (_tcslen(myKeyValue) + 1) * sizeof(TCHAR));
		MyRegCloseKeys(2, myKeys);
	}
	else
	{
...
int WndInMid(HWND hMid, HWND hBase)
{
	...
	// Center the window
	SetWindowPos(hMid, nullptr, bx, by, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
