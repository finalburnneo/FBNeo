...
static TCHAR* pszTextBuffer = nullptr;
...
static constexpr bool bPrintDriverInfo = false;
...
static int AddLine(TCHAR* pszFormat, ...)
{
	...
	auto pszNewBuffer = static_cast<TCHAR*>(realloc(pszTextBuffer, (nLen + nTextBufferSize + 1) * sizeof(TCHAR)));
	if (pszNewBuffer)
	{
...
static int PrintInterfaceInfo(InterfaceInfo* pInfo)
{
	...
	{
		AddLine(_T("    %s%s"), (i == 0) ? _T("Interface settings: ") : _T("                    "),
		        pInfo->ppszInterfaceSettings[i]);
	}
	for (int i = 0; pInfo->ppszModuleSettings[i]; i++)
	{
		AddLine(_T("    %s%s"), (i == 0) ? _T("Module settings:    ") : _T("                    "),
		        pInfo->ppszModuleSettings[i]);
	}
...
#undef EXCEPTION_LIST_ENTRY
	...
	AddLine(_T("Exception 0x%08X (%s) thrown.\r\nEIP: 0x%p"), pExceptionPointers->ExceptionRecord->ExceptionCode,
	        ExceptionString[i].szString, pExceptionPointers->ExceptionRecord->ExceptionAddress);
	if (pExceptionPointers->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION)
	{
		AddLine(_T(" (attempting to %s address 0x%p)"), pExceptionPointers->ExceptionRecord->ExceptionInformation[0]
			                                                ? _T("write")
			                                                : _T("read"),
...
int PrintOSInfo()
{
	...
	AddText(_T("OS:  Microsoft "));
	{
		...
		using GETPRODUCTINFO = BOOL(WINAPI*)(DWORD, DWORD, DWORD, DWORD, PDWORD);

		GETPRODUCTINFO pGPI = nullptr;
		DWORD dwType = 0;
		...
		if (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT)
		{
			...
			if (osvi.dwMajorVersion == 10 && osvi.dwMinorVersion == 0 && osvi.wProductType != VER_NT_WORKSTATION && osvi
				.dwBuildNumber <= 14393)
			{
			...
			if (osvi.dwMajorVersion == 10 && osvi.dwMinorVersion == 0 && osvi.wProductType != VER_NT_WORKSTATION && osvi
				.dwBuildNumber > 14393 && osvi.dwBuildNumber <= 17763)
			{
...
int PrintGlobalMemoryInfo()
{
	...
	AddLine(_T("Physical RAM: %.2f MB (%.2f GB) total, %.2f MB (%.2f GB) avail"), fTotalPhys, fTotalPhys / 1024,
	        fAvailPhys, fAvailPhys / 1024);
	AddLine(_T("PageFile RAM: %.2f MB (%.2f GB) total, %.2f MB (%.2f GB) avail"), fTotalPageFile, fTotalPageFile / 1024,
	        fAvailPageFile, fAvailPageFile / 1024);

	// Information on FB Neo memory usage
	BOOL (WINAPI*pGetProcessMemoryInfo)(HANDLE, PPROCESS_MEMORY_COUNTERS, DWORD) = nullptr;
	HINSTANCE hPsapiDLL;

	hPsapiDLL = LoadLibrary(_T("psapi.dll"));
	if (hPsapiDLL)
	{
		pGetProcessMemoryInfo = (BOOL (WINAPI*)(HANDLE, PPROCESS_MEMORY_COUNTERS, DWORD))GetProcAddress(
			hPsapiDLL, "GetProcessMemoryInfo");
		if (pGetProcessMemoryInfo)
		{
			...
			if (pGetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc)))
			{
				...
				AddLine(_T("%s%.2f MB in use (%.2f MB peak, %.2f MB virtual)"), szLine, fWorkingSetSize,
				        fPeakWorkingSetSize, fPagefileUsage);
			}
		}
...
HKEY FindMonitor(TCHAR* szHardwareID, TCHAR* szDriver)
{
	...
	DWORD nNameSize = sizeof(szName);
	TCHAR szClass[1024] = _T("");
	DWORD nClassSize = sizeof(szClass);
	FILETIME ftLastWriteTime;
	HKEY hKey = nullptr;
	HKEY hMonitorKey = nullptr;
	...
	for (int i = 0; !bFound && RegEnumKeyEx(hKey, i, szName, &nNameSize, nullptr, szClass, &nClassSize,
	                                        &ftLastWriteTime) != ERROR_NO_MORE_ITEMS; i++)
	{
		TCHAR szSubName[1024] = _T("");
		DWORD nSubNameSize = sizeof(szSubName);
		TCHAR szSubClass[1024] = _T("");
		DWORD nSubClassSize = sizeof(szSubClass);
		...
		for (int j = 0; !bFound && RegEnumKeyEx(hSubKey, j, szSubName, &nSubNameSize, nullptr, szSubClass,
		                                        &nSubClassSize, &ftSubLastWriteTime) != ERROR_NO_MORE_ITEMS; j++)
		{
			HKEY hMonitorInfoKey = nullptr;
			TCHAR szKeyValue[1024];
			DWORD nSize = sizeof(szKeyValue);
			DWORD nType;
			...
			nType = REG_SZ;
			nSize = sizeof(szKeyValue);
			RegQueryValueEx(hMonitorKey, _T("HardwareID"), nullptr, &nType, (BYTE*)szKeyValue, &nSize);
			...
			nType = REG_SZ;
			nSize = sizeof(szKeyValue);
			RegQueryValueEx(hMonitorKey, _T("Driver"), nullptr, &nType, (BYTE*)szKeyValue, &nSize);
...
int ProcessEDID(HKEY hMonitorKey)
{
	HKEY hMonitorInfoKey = nullptr;
	BYTE EDIDData[1024];
	DWORD nEDIDSize = sizeof(EDIDData);
	DWORD nType;

	if (hMonitorKey == nullptr)
	{
		return 1;
	}

	RegOpenKeyEx(hMonitorKey, _T("Device Parameters"), 0, KEY_READ, &hMonitorInfoKey);
	if (hMonitorInfoKey == nullptr)
	{
		return 1;
	}
	...
	nType = REG_BINARY;
	nEDIDSize = sizeof(EDIDData);
	if (RegQueryValueEx(hMonitorInfoKey, _T("BAD_EDID"), nullptr, &nType, EDIDData, &nEDIDSize) == 0)
	{
		AddLine(_T("        No EDID data present for this device"));
	}
	nType = REG_BINARY;
	nEDIDSize = sizeof(EDIDData);
	if (RegQueryValueEx(hMonitorInfoKey, _T("EDID"), nullptr, &nType, EDIDData, &nEDIDSize) == 0)
	{
		// Print some basic information about this display
		AddLine(_T("        Display size ~%dx%dcm, Gamma %1.2lf"), EDIDData[0x15], EDIDData[0x16],
		        (static_cast<double>(EDIDData[0x17]) + 100.0) / 100.0);
		...
		if (EDIDData[0x18] & 2)
		{
			...
			double dRefresh = 1.0 / (static_cast<double>(nActiveH + nBlankH) * (nActiveV + nBlankV) / nPixelClock);

			AddLine(_T("        Preferred mode %dx%d, %1.3lf Hz (%dx%dmm, %1.3lf MHz)"), nActiveH, nActiveV, dRefresh,
...

		{
			...
			for (int i = 0; i < 4; i++)
			{
				if (EDIDData[0x36 + i * 0x12 + 0] == 0x00 && EDIDData[0x36 + i * 0x12 + 1] == 0x00 && EDIDData[0x36 + i
					* 0x12 + 2] == 0x00 && EDIDData[0x36 + i * 0x12 + 3] == 0xFD)
				{
			...
			{
				AddLine(
					_T("        Max. bandwidth %d MHz, H sync %d-%d KHz, V sync %d-%d Hz"),
					EDIDData[nLimitsOffset + 0x09] * 10, EDIDData[nLimitsOffset + 0x07], EDIDData[nLimitsOffset + 0x08],
...
{
	BOOL (WINAPI*pEnumDisplayDevices)(LPCTSTR, DWORD, PDISPLAY_DEVICE, DWORD) = nullptr;
	...
	{
#if defined (UNICODE)
		pEnumDisplayDevices = (BOOL (WINAPI*)(LPCTSTR, DWORD, PDISPLAY_DEVICE, DWORD))GetProcAddress(
			hUser32DLL, "EnumDisplayDevicesW");
#else
		...
		if (pEnumDisplayDevices)
		{
			...
			for (int i = 0; pEnumDisplayDevices(nullptr, i, &ddAdapter, 0); i++)
			{
				...
				if (!(ddAdapter.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER))
				{
					for (int j = 0; pEnumDisplayDevices(ddAdapter.DeviceName, j, &ddDisplay, 0); j++)
					{
						...
						TCHAR szMfg[1024] = _T("");
						DWORD nMfgSize = sizeof(szMfg);
						TCHAR szDeviceDesc[1024] = _T("");
						DWORD nDeviceDescSize = sizeof(szDeviceDesc);
						...
						if (ddDisplay.StateFlags & DISPLAY_DEVICE_ACTIVE)
						{
							...
							{
								TCHAR szDriver[1024];
								DWORD nSize = sizeof(szDriver);
								DWORD nType = REG_SZ;
								RegOpenKeyEx(HKEY_LOCAL_MACHINE, ddDisplay.DeviceKey + 18, 0, KEY_READ, &hKey);

								// Find the registry key for this display in the hardware area of the registry
								RegQueryValueEx(hKey, _T("MatchingDeviceId"), nullptr, &nType, (BYTE*)szDriver, &nSize);
								hMonitorKey = FindMonitor(szDriver, ddDisplay.DeviceKey + 57);

								RegQueryValueEx(hMonitorKey, _T("DeviceDesc"), nullptr, &nType, (BYTE*)szDeviceDesc,
								                &nDeviceDescSize);
								RegQueryValueEx(hMonitorKey, _T("Mfg"), nullptr, &nType, (BYTE*)szMfg, &nMfgSize);
							}
							...
							{
								AddLine(_T("    %s %s on %s%s"), szMfg, szDeviceDesc, ddAdapter.DeviceString,
								        pszStatus);
							}
							else
							{
								AddLine(_T("    %s on %s%s"), ddDisplay.DeviceString, ddAdapter.DeviceString,
								        pszStatus);
							}
...
int PrintFBAInfo()
{
	...
	AddLine(
		_T("Built on ") _T(MAKE_STRING(BUILD_DATE)) _T(", ") _T(MAKE_STRING(BUILD_TIME)) _T(", using ") _T(
			MAKE_STRING(BUILD_COMP)) _T("."));
	...
	if (bDrvOkay)
	{
		...
		AddLine(_T("%s"), szName);
		AddLine(_T("    Vertical refresh is %.2lf Hz."), static_cast<double>(nBurnFPS) / 100);
		AddLine(
			_T("    CPU running at %i%% of normal frequency."),
			static_cast<int>(static_cast<double>(nBurnCPUSpeedAdjust) * 100 / 256 + 0.5));
		AddLine(_T(""));
	}
	else
	{
	...
	if ((pInfo = ProfileGetInfo()) != nullptr)
	{
	...
	else
	{
		...
		else
		{
			...
			if (pExceptionPointers)
			{
				...
				do
				{
					if (me32.modBaseAddr <= pExceptionPointers->ExceptionRecord->ExceptionAddress && (me32.modBaseAddr +
						me32.modBaseSize) > pExceptionPointers->ExceptionRecord->ExceptionAddress)
					{
						bFound = true;
						break;
					}
				...
				if (bFound)
				{
					...
					AddLine(_T("    %20s (base address 0x%p, size %6i KB)"), me32.szModule, me32.modBaseAddr,
					        me32.modBaseSize / 1024);
					AddLine(_T(""));
				}
				else
				{
...
int PrintDeviceInfo()
{
	...
	{
		SC_HANDLE (WINAPI*pOpenSCManager)(LPCTSTR, LPCTSTR, DWORD) = nullptr;
		SC_HANDLE (WINAPI*pOpenService)(SC_HANDLE, LPCTSTR, DWORD) = nullptr;
		BOOL (WINAPI*pQueryServiceConfig)(SC_HANDLE, LPQUERY_SERVICE_CONFIG, DWORD, LPDWORD) = nullptr;
		BOOL (WINAPI*pCloseServiceHandle)(SC_HANDLE) = nullptr;
		...
		{
#if defined (_UNICODE)
			pOpenSCManager = (SC_HANDLE (WINAPI*)(LPCTSTR, LPCTSTR, DWORD))GetProcAddress(
				hAdvapi32DLL, "OpenSCManagerW");
			pOpenService = (SC_HANDLE (WINAPI*)(SC_HANDLE, LPCTSTR, DWORD))GetProcAddress(hAdvapi32DLL, "OpenServiceW");
			pQueryServiceConfig = (BOOL (WINAPI*)(SC_HANDLE, LPQUERY_SERVICE_CONFIG, DWORD, LPDWORD))GetProcAddress(
				hAdvapi32DLL, "QueryServiceConfigW");
#else
		...
		{
			TCHAR* list[] = {_T("display"), _T("media"), _T("hid"), _T("hidclass"), _T("mouse"), _T("system"), nullptr};
			TCHAR szClass[1024] = _T("");

			// Get the installer class of a device
			SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_CLASS, nullptr, (BYTE*)szClass, 1024, nullptr);
			...
			for (int j = 0; list[j]; j++)
			{
				if (!_tcsicmp(list[j], szClass))
				{
					...
					// Get the device name
					SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_DEVICEDESC, nullptr, (BYTE*)szName, 1024,
					                                 nullptr);

					// Check if there are any lower filter drivers
					SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_LOWERFILTERS, nullptr, (BYTE*)szService,
					                                 1024, nullptr);
					if (szService[0] == _T('\0'))
					{
						// If we haven't got any lower filter drivers, just use the service key
						SetupDiGetDeviceRegistryProperty(hDevInfo, &did, SPDRP_SERVICE, nullptr, (BYTE*)szService, 1024,
						                                 nullptr);
					}
					...
					if (hKey != INVALID_HANDLE_VALUE)
					{
						...
						RegQueryValueEx(hKey, _T("DriverVersion"), nullptr, &nType, (BYTE*)szDriverVersion, &nSize);
						RegQueryValueEx(hKey, _T("DriverDate"), nullptr, &nType, (BYTE*)szDriverDate, &nSize);
						RegCloseKey(hKey);
					}
					...
					if (szService[0] && pOpenSCManager && pOpenService && pQueryServiceConfig && pCloseServiceHandle)
					{
						...
						QUERY_SERVICE_CONFIG* pSC = nullptr;
						DWORD nSize;

						hSCManager = OpenSCManager(nullptr, nullptr, GENERIC_READ);
						hService = OpenService(hSCManager, szService, GENERIC_READ);

						QueryServiceConfig(hService, nullptr, 0, &nSize);
						pSC = static_cast<QUERY_SERVICE_CONFIG*>(malloc(nSize));
					...
					if (j < 2 || szImagePath[0])
					{
						...
						{
							AddLine(_T("        version %s (%s)"), szDriverVersion,
							        szDriverDate[0] ? szDriverDate : _T("no date"));
						}
					}
	...
	case WM_INITDIALOG:
		{
			...
			if (pExceptionPointers)
			{
				...
				hCodeFont = CreateFont(22, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, ANTIALIASED_QUALITY, FF_MODERN,
				                       _T("Lucida Console"));
				SendDlgItemMessage(hDlg, IDC_SYSINFO_CODE, WM_SETFONT, (WPARAM)hCodeFont, 0);

				_stprintf(szText, _T("Guru Meditation #%08X.%08X"), pExceptionPointers->ExceptionRecord->ExceptionCode,
				          (INT_PTR)pExceptionPointers->ExceptionRecord->ExceptionAddress);
			...
			SendDlgItemMessage(hDlg, IDC_SYSINFO_EDIT, EM_SETMARGINS, EC_LEFTMARGIN, 3);
			...
			if (HIWORD(wParam) == BN_CLICKED)
			{
				...
				if (LOWORD(wParam) == IDC_SYSINFO_LOG_SAVE)
				{
					...
					if (fp)
					{
						...
						szText = static_cast<TCHAR*>(malloc((nSize + 1) * sizeof(TCHAR)));
						...
						if (nSize && szText)
						{
							SendDlgItemMessage(hDlg, IDC_SYSINFO_EDIT, WM_GETTEXT,
							                   static_cast<WPARAM>(nSize + 1) * sizeof(TCHAR), (LPARAM)szText);
...
LONG CALLBACK ExceptionFilter(_EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	if (nRecursion++)
	{
		if (nRecursion <= 2)
		{
			MessageBox(hScrnWnd, _T(APP_TITLE) _T(" will now be terminated."), _T(APP_TITLE) _T(" Fatal exception"),
