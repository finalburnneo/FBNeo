...
static UINT8* DrvZ80Rom = nullptr;
static UINT8* DrvZ80Ram = nullptr;
static UINT8* DrvProms = nullptr;
static UINT8* DrvColourProm = nullptr;
static UINT8* DrvVideoRam = nullptr;
static UINT8* DrvSpriteRam = nullptr;
static UINT8* DrvSharedRam = nullptr;
static UINT8* DrvGfxData[8] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
...
static INT32 SstingryInit()
{
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x08001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x08000, 3, 2);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvZ80Rom + 0x00000, 4, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom + 0x04000, 5, 1);
	if (nRet != 0) return 1;

	memset(DrvTempRom, 0, 0x60000);
	nRet = BurnLoadRom(DrvTempRom + 0x00000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 12, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x400, 3, 8, 8, Sstingry1PlaneOffsets, Sstingry1XOffsets, Sstingry1Offsets, 0x80, DrvTempRom + 0x00000,
	          DrvGfxData[0]);
	GfxDecode(0x400, 3, 8, 8, Sstingry2PlaneOffsets, Sstingry2XOffsets, Sstingry2Offsets, 0x80, DrvTempRom + 0x00000,
	          DrvGfxData[1]);
	GfxDecode(0x400, 3, 8, 8, Sstingry1PlaneOffsets, Sstingry1XOffsets, Sstingry1Offsets, 0x80, DrvTempRom + 0x10000,
	          DrvGfxData[2]);
	GfxDecode(0x400, 3, 8, 8, Sstingry3PlaneOffsets, Sstingry3XOffsets, Sstingry3Offsets, 0x80, DrvTempRom + 0x10000,
	          DrvGfxData[3]);
	BurnFree(DrvTempRom);

	nRet = BurnLoadRom(DrvProms + 0x00000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00100, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00200, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00300, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00400, 17, 1);
	if (nRet != 0) return 1;
	...
	BurnYM2203Init(3, 3000000, nullptr, 0);
...
static INT32 KyrosInit()
{
	...
	nRet = BurnLoadRom(Drv68KRom + 0x00000, 1, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x10001, 2, 2);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(Drv68KRom + 0x10000, 3, 2);
	if (nRet != 0) return 1;
	memset(DrvTempRom, 0, 0x60000);
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 13, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 14, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x38000, 15, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x40000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x48000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x50000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x58000, 19, 1);
	if (nRet != 0) return 1;
	GfxDecode(0x800, 3, 8, 8, Kyros1PlaneOffsets, Kyros1XOffsets, Kyros1Offsets, 0x80, DrvTempRom + 0x00000,
	          DrvGfxData[0]);
	GfxDecode(0x800, 3, 8, 8, Kyros2PlaneOffsets, Kyros2XOffsets, Kyros2Offsets, 0x80, DrvTempRom + 0x00000,
	          DrvGfxData[1]);
	GfxDecode(0x800, 3, 8, 8, Kyros1PlaneOffsets, Kyros1XOffsets, Kyros1Offsets, 0x80, DrvTempRom + 0x18000,
	          DrvGfxData[2]);
	GfxDecode(0x800, 3, 8, 8, Kyros2PlaneOffsets, Kyros2XOffsets, Kyros2Offsets, 0x80, DrvTempRom + 0x18000,
	          DrvGfxData[3]);
	GfxDecode(0x800, 3, 8, 8, Kyros1PlaneOffsets, Kyros1XOffsets, Kyros1Offsets, 0x80, DrvTempRom + 0x30000,
	          DrvGfxData[4]);
	GfxDecode(0x800, 3, 8, 8, Kyros2PlaneOffsets, Kyros2XOffsets, Kyros2Offsets, 0x80, DrvTempRom + 0x30000,
	          DrvGfxData[5]);
	GfxDecode(0x800, 3, 8, 8, Kyros1PlaneOffsets, Kyros1XOffsets, Kyros1Offsets, 0x80, DrvTempRom + 0x48000,
	          DrvGfxData[6]);
	GfxDecode(0x800, 3, 8, 8, Kyros2PlaneOffsets, Kyros2XOffsets, Kyros2Offsets, 0x80, DrvTempRom + 0x48000,
	          DrvGfxData[7]);
	BurnFree(DrvTempRom);

	nRet = BurnLoadRom(DrvProms + 0x00000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00100, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00200, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00300, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvProms + 0x00400, 24, 1);
	if (nRet != 0) return 1;

	nRet = BurnLoadRom(DrvColourProm + 0x00000, 25, 1);
	if (nRet != 0) return 1;
	...
	BurnYM2203Init(3, 2000000, nullptr, 0);
...
struct BurnDriver BurnDrvSstingry = {
	"sstingry", nullptr, nullptr, nullptr, "1986",
	"Super Stingray (Japan)\0", nullptr, "Alpha Denshi Co.", "Alpha 68k",
	...
	nullptr, SstingryRomInfo, SstingryRomName, nullptr, nullptr, nullptr, nullptr, SstingryInputInfo, SstingryDIPInfo,
	SstingryInit, DrvExit, DrvFrame, SstingryDraw, DrvScan,
	nullptr, 0x101, 224, 256, 3, 4
};

struct BurnDriver BurnDrvKyros = {
	"kyros", nullptr, nullptr, nullptr, "1987",
	"Kyros\0", nullptr, "Alpha Denshi Co. (World Games Inc. license)", "Alpha 68k",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_VERSHOOT, 0,
	nullptr, KyrosRomInfo, KyrosRomName, nullptr, nullptr, nullptr, nullptr, SstingryInputInfo, KyrosDIPInfo,
	KyrosInit, DrvExit, DrvFrame, KyrosDraw, DrvScan,
	nullptr, 0x101, 224, 256, 3, 4
};

struct BurnDriver BurnDrvKyrosj = {
	"kyrosj", "kyros", nullptr, nullptr, "1986",
	"Kyros no Yakata (Japan)\0", nullptr, "Alpha Denshi Co.", "Alpha 68k",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_VERSHOOT, 0,
	nullptr, KyrosjRomInfo, KyrosjRomName, nullptr, nullptr, nullptr, nullptr, SstingryInputInfo, KyrosDIPInfo,
	KyrosInit, DrvExit, DrvFrame, KyrosDraw, DrvScan,
	nullptr, 0x101, 224, 256, 3, 4
};
