...
INT32 M6800CoreInit(INT32 num, INT32 type)
{
	...
	{
		M6800CPUContext = static_cast<M6800Ext*>(malloc(MAX_CPU * sizeof(M6800Ext)));
		if (M6800CPUContext == nullptr)
		{
			return 1;
		}
...
void M6800SetIRQLine(INT32 vector, INT32 status)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6800) bprintf(
		PRINT_ERROR, _T("M6800SetIRQLine called with invalid CPU Type\n"));
#endif
...
void HD63701SetIRQLine(INT32 vector, INT32 status)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_HD63701) bprintf(
		PRINT_ERROR, _T("HD63701SetIRQLine called with invalid CPU Type\n"));
#endif
...
void M6803SetIRQLine(INT32 vector, INT32 status)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6803) bprintf(
		PRINT_ERROR, _T("M6803SetIRQLine called with invalid CPU Type\n"));
#endif
...
void M6801SetIRQLine(INT32 vector, INT32 status)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6801) bprintf(
		PRINT_ERROR, _T("M6800SetIRQLine called with invalid CPU Type\n"));
#endif
...
void NSC8105SetIRQLine(INT32 vector, INT32 status)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6801) bprintf(
		PRINT_ERROR, _T("NSC8105SetIRQLine called with invalid CPU Type\n"));
#endif
...
INT32 M6800Run(INT32 cycles)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6800) bprintf(
		PRINT_ERROR, _T("M6800Run called with invalid CPU Type\n"));
#endif
...
INT32 HD63701Run(INT32 cycles)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_HD63701) bprintf(
		PRINT_ERROR, _T("HD63701Run called with invalid CPU Type\n"));
#endif
...
INT32 M6803Run(INT32 cycles)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6803 && M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_M6801
	) bprintf(PRINT_ERROR, _T("M6803Run called with invalid CPU Type\n"));
#endif
...
INT32 NSC8105Run(INT32 cycles)
{
	...
	if (M6800CPUContext[nActiveCPU].nCpuType != CPU_TYPE_NSC8105) bprintf(
		PRINT_ERROR, _T("NSC8105Run called with invalid CPU Type\n"));
#endif
...
INT32 M6800Scan(INT32 nAction)
{
	...
	if (nAction & ACB_DRIVER_DATA)
	{
		...
		{
			// this will only work for 1 cpu (that's all this interface currently supports)
			struct BurnArea ba;
