...
static struct GX_OBJ
{
	INT32 order, offs, code, color;
}* gx_objpool;
...
void konamigx_mixer_init(INT32 objdma)
{
	...
	gx_shdzbuf = BurnMalloc(512 * 256 * 2);
	gx_objzbuf = BurnMalloc(512 * 256 * 2);
...
static void gx_draw_basic_tilemaps(INT32 mixerflags, INT32 code)
{
	...
	if (disp & (1 << code))
	{
		if (j == GXMIX_BLEND_NONE)
		{
			temp1 = 0xff;
			temp2 = temp3 = 0;
		}
		else if (j == GXMIX_BLEND_FORCE)
		{
			temp1 = 0x00;
		...
		*/
		if (temp1 != 0xff && temp2 /*&& temp3==3*/)
		{
			...
			if (temp4 <= 0 && !konamigx_mystwarr_kludge) return;
			// alpha level so high that layer is completely invisible. mystwarr needs this disabled for tile-based alpha tile counting.
...
static void gx_draw_basic_extended_tilemaps_1(INT32 mixerflags, INT32 code, INT32 sub1, INT32 sub1flags,
                                              INT32 rushingheroes_hack, INT32 offs)
{
	INT32 temp1, temp2, temp3, temp4;
	INT32 i = code << 1;
	INT32 j = mixerflags >> i & 3;
	...
	if ((disp & K55_INP_SUB1) || (rushingheroes_hack))
	{
		...
		if (offs == -2)
		{
			...
			if (nSpriteEnable & 4 && K053936_external_bitmap)
				K053936GP_0_zoom_draw(K053936_external_bitmap, l, k, alpha, pixeldouble_output, m_k053936_0_ctrl_16,
				                      m_k053936_0_linectrl_16, m_k053936_0_ctrl, m_k053936_0_linectrl);
		}
		else
		{
			if (nSpriteEnable & 2)
...
static void gx_draw_basic_extended_tilemaps_2(INT32 mixerflags, INT32 code, INT32 sub2, INT32 sub2flags,
                                              INT32 extra_bitmap, INT32 offs)
{
	INT32 temp1, temp2, temp3, temp4;
	INT32 i = code << 1;
	INT32 j = mixerflags >> i & 3;
...
extern void k053247_draw_single_sprite_gxcore(UINT8*, UINT8* gx_shdzbuf, INT32 code, UINT16* gx_spriteram, INT32 offs,
                                              ...
                                              INT32 primask, INT32 shadow, UINT8* drawmode_table,
                                              UINT8* shadowmode_table, INT32 shdmask);


static void konamigx_mixer_draw(INT32 sub1, INT32 sub1flags, INT32 sub2, INT32 sub2flags, INT32 mixerflags,
                                INT32 extra_bitmap, INT32 rushingheroes_hack, struct GX_OBJ* objpool, INT32* objbuf,
                                INT32 nobj)
{
	// traverse draw list
	INT32 disp = K055555ReadRegister(K55_INPUT_ENABLES);
	...
	for (INT32 count = 0; count < nobj; count++)
	{
		...
		if (offs >= 0)
		{
			...
			if (nSpriteEnable & 1)
				k053247_draw_single_sprite_gxcore(gx_objzbuf, gx_shdzbuf, code, gx_spriteram, offs, color, alpha,
				                                  drawmode, zcode, pri, 0, 0, nullptr, nullptr, 0);
		}
		else
		{
			switch (offs)
...
void konamigx_mixer(INT32 sub1 /*extra tilemap 1*/, INT32 sub1flags, INT32 sub2 /*extra tilemap 2*/, INT32 sub2flags,
                    INT32 mixerflags, INT32 extra_bitmap /*extra tilemap 3*/, INT32 rushingheroes_hack)
{
	...
	for (INT32 j = 0; j < 5; j++)
	{
		...
		for (INT32 i = j + 1; i < 6; i++)
		{
			...
			if (static_cast<UINT32>(temp1) <= static_cast<UINT32>(temp2))
			{
				layerpri[i] = temp1;
	...
	for (INT32 i = 5; i >= 0; i--)
	{
		...
		*/
		case 4:
			...
			if (sub1flags & 0xf)
			{
				if (sub1flags & GXSUB_K053250) offs = -4;
				else if (sub1) offs = -2;
			}
			break;
		case 5:
			...
			if (sub2flags & 0xf)
			{
				if (sub2flags & GXSUB_K053250) offs = -5;
				else if (sub2) offs = -3;
			}
	...
	for (INT32 j = 0; j < l; j++)
	{
		...
		for (INT32 i = j + 1; i < k; i++)
		{
			...
			if (static_cast<UINT32>(temp2) <= static_cast<UINT32>(temp4))
			{
				temp2 = temp4;
				objbuf[i] = temp1;
	...
	konamigx_mixer_draw(sub1, sub1flags, sub2, sub2flags, mixerflags, extra_bitmap, rushingheroes_hack, objpool, objbuf,
	                    nobj);
}
