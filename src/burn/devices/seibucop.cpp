...
{
	if (data & 0x0001)
		cpu_write_long(cop_regs[0] + 16 + offset * 4,
		               cpu_read_long(cop_regs[0] + 16 + offset * 4) + cpu_read_long(cop_regs[0] + 0x28 + offset * 4));
...
static void LEGACY_execute_130e_cupsoc(INT32, UINT16 data)
{
	...
	{
		cop_angle = static_cast<int>(atan(static_cast<double>(dy) / static_cast<double>(dx)) * 128.0 / M_PI);
		if (dx < 0)
			cop_angle += 0x80;
...
static void execute_2288(INT32, UINT16 data)
{
	...
	{
		cop_angle = static_cast<int>(atan(static_cast<double>(dx) / static_cast<double>(dy)) * 128 / M_PI);
		if (dy < 0)
			cop_angle += 0x80;
	}
...
static void execute_3b30(INT32, UINT16 data)
{
	...
	dx = dx >> 16;
	dy = dy >> 16;
	cop_dist = static_cast<UINT16>(sqrt(dx * dx + dy * dy));
...
static void execute_6200(INT32, UINT16)
{
	...
	if (!host_endian)
		...
		// angle is a byte, but grainbow (cave mid-boss) is only happy with write-word, could be more endian weirdness, or it always writes a word?
...
static void execute_8100(INT32, UINT16)
{
	...
	if (raw_angle == 0xc0)
		amp *= 2;
	res = static_cast<int>(amp * sin(angle)) << cop_scale;
...
static void execute_8900(INT32, UINT16)
{
	...
	if (raw_angle == 0x80)
		amp *= 2;
	res = static_cast<int>(amp * cos(angle)) << cop_scale;
...
{
	UINT32 hitadr2 = cpu_read_word(hitadr) | (cop_hit_baseadr << 16);
	// DON'T use cop_read_word here, doesn't need endian fixing?!
	INT32 num_axis = 2;
	INT32 extraxor = 0;
	...
	for (i = 0; i < num_axis; i++)
	{
		size[i] = cop_collision_info[slot].size[i];
		dx[i] = cop_collision_info[slot].dx[i];
	}
	...
	for (i = 0; i < num_axis; i++)
	{
		...
		if (cop_collision_info[0].max[i] > cop_collision_info[1].min[i] && cop_collision_info[0].min[i] <
			cop_collision_info[1].max[i])
			res &= ~(1 << i);

		if (cop_collision_info[1].max[i] > cop_collision_info[0].min[i] && cop_collision_info[1].min[i] <
			cop_collision_info[0].max[i])
			res &= ~(1 << i);
...
static INT32 check_command_matches(INT32 command, UINT16 seq0, UINT16 seq1, UINT16 seq2, UINT16 seq3, UINT16 seq4,
                                   UINT16 seq5, UINT16 seq6, UINT16 seq7, UINT16 _funcval_, UINT16 _funcmask_)
{
	command *= 8;

	if (cop_program[command + 0] == seq0 && cop_program[command + 1] == seq1 && cop_program[command + 2] == seq2 &&
		cop_program[command + 3] == seq3 &&
		cop_program[command + 4] == seq4 && cop_program[command + 5] == seq5 && cop_program[command + 6] == seq6 &&
		cop_program[command + 7] == seq7 &&
...
void cop_cmd_write(INT32 offset, UINT16 data)
{
	...
	case 0x0205:
		{
			// 0205 0006 ffeb 0000 - 0188 0282 0082 0b8e 098e 0000 0000 0000
	...
	case 0x2288:
		{
			// 2208 0005 f5df 0020 - 0f8a 0b8a 0388 0b9a 0b9a 0a9a 0000 0000
	...
	case 0x2a05:
		{
			// 2a05 0006 ebeb 0028 - 09af 0a82 0082 0a8f 018e 0000 0000 0000
	...
	case 0x3bb0:
		{
			// 3bb0 0004 007f 0038 - 0f9c 0b9c 0b9c 0b9c 0b9c 0b9c 0b9c 099c
			execute_3b30(offset, data);
	...
	case 0x42c2:
		{
			// 42c2 0005 fcdd 0040 - 0f9a 0b9a 0b9c 0b9c 0b9c 029c 0000 0000
	...
	case 0x4aa0:
		{
			// 4aa0 0005 fcdd 0048 - 0f9a 0b9a 0b9c 0b9c 0b9c 099b 0000 0000
	...
	case 0x8100:
		{
			// 8100 0007 fdfb 0080 - 0b9a 0b88 0888 0000 0000 0000 0000 0000
			execute_8100(offset, data); // SIN
			break;
		}
	...
	case 0x8900:
		{
			// 8900 0007 fdfb 0088 - 0b9a 0b8a 088a 0000 0000 0000 0000 0000
			execute_8900(offset, data); // COS
			break;
		}
...
static void dma_zsorting(UINT16 sort_size)
{
	...
	for (INT32 i = 2; i < sort_size; i += 2)
	{
		...
		for (INT32 j = i - 2; j < sort_size; j += 2)
		{
			...
			{
			case 2: xchg_flag = (vali > valj);
				break;
			case 1: xchg_flag = (vali < valj);
				break;
			default: xchg_flag = 0;
				break;
			}
