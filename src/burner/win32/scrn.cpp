...
static int WINAPI gameCallback(char* game, int player, int numplayers)
{
	...
	TCHAR szTemp1[256];
	TCHAR szTemp2[256];
	VidSAddChatMsg(FBNLoadStringEx(hAppInst, IDS_NETPLAY_START, true), 0xFFFFFF, BurnDrvGetText(DRV_FULLNAME),
	               0xFFBFBF);
...
{
	if (bVidOkay && bRunPause && bDrvOkay && (hSelDlg == NULL))
	{
...
static void OnCommand(HWND /*hDlg*/, int id, HWND /*hwndCtl*/, UINT codeNotify)
{
	...
	case MENU_STARTNET:
		if (Init_Network())
		{
			MessageBox(hScrnWnd, FBNLoadStringEx(hAppInst, IDS_ERR_NO_NETPLAYDLL, true),
			           FBNLoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
			break;
		}
#ifdef BUILD_A68K
	...
	case MENU_INPUT_AUTOFIRE_RATE_1: nAutoFireRate = 22;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_2: nAutoFireRate = 12;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_3: nAutoFireRate = 8;
		break;
	case MENU_INPUT_AUTOFIRE_RATE_4: nAutoFireRate = 4;
		break;
	...
	case MENU_CHEATSEARCH_NOCHANGE:
		{
			...
			if (nValues <= CHEATSEARCH_SHOWRESULTS)
			{
				...
				{
					_stprintf(tmpmsg, FBNLoadStringEx(hAppInst, IDS_CHEAT_SEARCH_RESULTS, true),
	...
	if ((bVidCorrectAspect || bVidFullStretch) && !(nVidSelect == 2 && (nVidBlitterOpt[2] & 0x0100) == 0))
	{
		...
		do
		{
			if (nBmapWidth < nBmapHeight && bVidScanRotate)
			{
				...
				if (hh > ww * nVidScrnAspectX * nGameAspectY * nScrnHeight / (nScrnWidth * nVidScrnAspectY *
					nGameAspectX))
				{
					hh = ww * nVidScrnAspectX * nGameAspectY * nScrnHeight / (nScrnWidth * nVidScrnAspectY *
						nGameAspectX);
				}
			}
			else
			{
				...
				if (ww > hh * nVidScrnAspectY * nGameAspectX * nScrnWidth / (nScrnHeight * nVidScrnAspectX *
					nGameAspectY))
				{
					ww = hh * nVidScrnAspectY * nGameAspectX * nScrnWidth / (nScrnHeight * nVidScrnAspectX *
						nGameAspectY);
				}
			}
		}
...
int ScrnTitle()
{
	...
	else
	{
		_stprintf(szText, _T(APP_TITLE) _T(" v%.20s") _T(SEPERATOR_1) _T("[%s]"), szAppBurnVer,
		          FBNLoadStringEx(hAppInst, IDS_SCRN_NOGAME, true));
	}
