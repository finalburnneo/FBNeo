...
HWND hInpsDlg = nullptr; // Handle to the Input Set Dialog
...
static struct GameInp* pgi = nullptr; // Current GameInp
...
static int InpsInit()
{
	...
	{
		// Normal input
		if (bii.szName == nullptr || bii.szName[0] == _T('\0'))
		{
...
static int SetInput(int nCode)
{
		...
		if ((bii.nType & BIT_GROUP_ANALOG) && (nCode & 0xFF) < 0x10)
		{
			...
			if (nCode == 0)
			{
				...
				if (bClearLock)
				{
					...
					pgi->Input.Switch.nCode = static_cast<unsigned short>(nCode);
				}
			}
		}
		else
	...
	else
	{
		...
		pgi->Macro.Switch.nCode = static_cast<unsigned short>(nCode); // Assign switch
...
static int InpsPushUpdate()
{
	...
	if (pgi == nullptr || nInpsInput >= nGameInpCount)
	{
		return 0;
	}
...
int InpsUpdate()
{
	...
	nButtonState = SendDlgItemMessage(hInpsDlg, IDC_INPS_CLEARLOCK, BM_GETSTATE, 0, 0);
	...
	nButtonState = SendDlgItemMessage(hInpsDlg, IDC_INPS_MAPDIG, BM_GETSTATE, 0, 0);
	// Lock Input = If checked: clear an input, and don't let a default value or default preset re-fill it in.
...
static INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		// Initialise slider
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETRANGE, 0, MAKELONG(0x700, 0x2000));
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETLINESIZE, 0, 0x000);
		// need to be able to map arrow keys without moving the slider
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETPAGESIZE, 0, 0x400);
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETTIC, 0, 0x2000);
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETTIC, 0, 0x1000);
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETTIC, 0, 0x0d00);
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETTIC, 0, 0x0a00);
		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETTIC, 0, 0x0700);

		SendDlgItemMessage(hDlg, IDC_INPS_DIGSLIDER, TBM_SETPOS, true, 0x800);
