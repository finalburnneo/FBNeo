...
void K053247Init(UINT8* gfxrom, UINT8* gfxromexp, INT32 gfxlen,
                 void (*Callback)(INT32* code, INT32* color, INT32* priority), INT32 flags)
{
	K053247Ram = BurnMalloc(0x1000);
...
#define NUM_SPRITES 256
	...
	INT32 offx = static_cast<INT16>((K053246Regs[0] << 8) | K053246Regs[1]);
	INT32 offy = static_cast<INT16>((K053246Regs[2] << 8) | K053246Regs[3]);

	auto SprRam = (UINT16*)K053247Ram;
	...
	{
		for (; offs < 0x800; offs += 8)
			if ((BURN_ENDIAN_SWAP_INT16(SprRam[offs]) & 0x8000) && ((BURN_ENDIAN_SWAP_INT16(SprRam[offs]) & 0xff) !=
				zcode)) sortedlist[count++] = offs;
	}
	...
	for (INT32 i = count; i >= 0; i--)
	{
		...
		oy = static_cast<INT16>(SprRam[offs + 2]);
		ox = static_cast<INT16>(SprRam[offs + 3]);
		...
		for (y = 0; y < h; y++)
		{
			...
			for (x = 0; x < w; x++)
			{
				...
				{
					if (mirrory && h == 1)
						konami_render_zoom_shadow_sprite(gfxbase, c, nBpp, color, sx >> 12, sy >> 12, fx, !fy, 16, 16,
						                                 zw, zh, primask, highlight);

					konami_render_zoom_shadow_sprite(gfxbase, c, nBpp, color, sx >> 12, sy >> 12, fx, fy, 16, 16, zw,
					                                 zh, primask, highlight);
					continue;
				}
				...
				if (mirrory && h == 1)
				{
					...
					{
						konami_draw_16x16_priozoom_sprite(gfxbase, c, nBpp, color, 0, sx >> 12, sy >> 12, fx, !fy, 16,
						                                  16, zw, zh, primask);
					}
				}
				...
				{
					konami_draw_16x16_priozoom_sprite(gfxbase, c, nBpp, color, 0, sx >> 12, sy >> 12, fx, fy, 16, 16,
					                                  zw, zh, primask);
				}
			} // end of X loop
		} // end of Y loop
...
void zdrawgfxzoom32GP(UINT32 code, UINT32 color, INT32 flipx, INT32 flipy, INT32 sx, INT32 sy,
                      INT32 scalex, INT32 scaley, INT32 alpha, INT32 drawmode, INT32 zcode, INT32 pri,
...
#define FPENT  0
	...
	{
		if (!flipx) src_fbx = FPENT;
		else
		{
			src_fbx = src_fw - FPENT - 1;
			src_fdx = -src_fdx;
		}
		if (!flipy) src_fby = FPENT;
		else
		{
	...
	z8 = static_cast<UINT8>(zcode);
	p8 = static_cast<UINT8>(pri);
	ozbuf_ptr += eax;
	szbuf_ptr += eax << 1;
...
void k053247_draw_single_sprite_gxcore(UINT8* gx_objzbuf, UINT8* gx_shdzbuf, INT32 code, UINT16* gx_spriteram,
                                       INT32 offs,
