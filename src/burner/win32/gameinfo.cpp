...
static int GameInfoInit()
{
	...
	_stprintf(szText, _T("%s") _T(SEPERATOR_1) _T("%s"), FBNLoadStringEx(hAppInst, IDS_GAMEINFO_DIALOGTITLE, true),
	          szFullName);
	...
	{
		_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_SEL_BOOTLEG, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_HACK)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_SEL_HACK, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_HOMEBREW)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_SEL_HOMEBREW, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	if (BurnDrvGetFlags() & BDF_DEMO)
	{
		_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_SEL_DEMO, true),
		          bUseInfo ? _T(", ") : _T(""));
		bUseInfo = true;
	}
	TCHAR szPlayersMax[100];
	_stprintf(szPlayersMax, FBNLoadStringEx(hAppInst, IDS_NUM_PLAYERS_MAX, true));
	_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_NUM_PLAYERS, true),
	          bUseInfo ? _T(", ") : _T(""), BurnDrvGetMaxPlayers(),
	          (BurnDrvGetMaxPlayers() != 1) ? szPlayersMax : _T(""));
	bUseInfo = true;
	if (BurnDrvGetText(DRV_BOARDROM))
	{
		_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_BOARD_ROMS_FROM, true),
	...
	_stprintf(szItemText, FBNLoadStringEx(hAppInst, IDS_HARDWARE_DESC, true),
	...
	if (Msg == WM_COMMAND)
	{
		...
		if (Id == IDRESCANSET && Notify == BN_CLICKED)
		{
			...
			if (gameAv[nGiDriverSelected] > 0)
			{
				...
				if (TabPage != 0)
				{
					MessageBox(hGameInfoDlg, FBNLoadStringEx(hAppInst, IDS_ERR_LOAD_OK, true),
					           FBNLoadStringEx(hAppInst, IDS_ERR_INFORMATION, true), MB_OK);
				}
				if (TabPage == 0)
				{
