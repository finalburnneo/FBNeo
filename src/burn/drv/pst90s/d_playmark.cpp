...
static void msm6295_set_bank(INT32 bank)
{
	...
	MSM6295SetBank(0, DrvMSM6295Src + (0x20000 * DrvOkiBank), 0x20000, 0x3ffff);
}
...
static INT32 DrvDoReset()
{
	...
	MSM6295SetBank(0, DrvMSM6295Src, 0x00000, 0x1ffff);
	msm6295_set_bank(1);
	MSM6295Reset(0);
...
static INT32 MemIndex()
{
	...
	DrvMSM6295Src        = Next; Next += 0x100000;
...
static UINT8 __fastcall ExcelsrReadByte(UINT32 a)
{
	switch (a) {
		default: {
			bprintf(PRINT_NORMAL, _T("Read byte -> %06X\n"), a);
...
static void __fastcall ExcelsrWriteByte(UINT32 a, UINT8 d)
{
	switch (a) {
		case 0x70001f: {
			DrvSoundCommand = d;
...
static UINT16 __fastcall ExcelsrReadWord(UINT32 a)
{
	switch (a) {
		case 0x700010: {
			return 0xffff - DrvInput[0];
...
static INT32 DrvInit()
{
	...
	nRet = BurnLoadRom(DrvMSM6295Src, 11, 1); if (nRet != 0) return 1;
...
static INT32 ExcelsrInit()
{
	...
	nRet = BurnLoadRom(DrvMSM6295Src, 15, 1); if (nRet != 0) return 1;
...
static void DrvRenderFgLayer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			Draw16x16Tile(pTransDraw, Code, x, y, 0, 0, Colour, 4, 0, DrvTiles);
...
static void HotmindRenderTileLayer(INT32 Layer)
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			if (Layer == 1) {
				Draw16x16MaskTile(pTransDraw, Code, x, y, 0, 0, Colour, 4, 0, 0, DrvTiles);
			}
...
static void DrvRenderCharLayer(INT32 Columns, INT32 CharSize)
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < Columns; mx++) {
			...
			//if (y < -8) y += 256;  // borks excelsr intro

			y -= 16;

			if (CharSize == 8) {
				Draw8x8MaskTile(pTransDraw, Code, x, y, 0, 0, Colour, 4, 0, 0x80, DrvChars);
			}

			if (CharSize == 16) {
				Draw16x16MaskTile(pTransDraw, Code, x, y, 0, 0, Colour, 4, 0, 0x80, DrvChars);
			}
...
static void HotmindRenderCharLayer()
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			...
			Draw8x8MaskTile(pTransDraw, Code, x, y, 0, 0, Colour, 4, 0, 0x100, DrvChars);
...
static void DrvRenderSprites(INT32 CodeShift, INT32 SpriteRamSize, INT32 SpriteSize, INT32 PriDraw, INT32 xOffset, INT32 yOffset)
{
	...
	for (Offs = 0; Offs < SpriteRamSize; Offs += 4) {
		...
		if (Pri == PriDraw || PriDraw == -1) {
			...
			if (SpriteSize == 16) {
				Draw16x16MaskTile(pTransDraw, Code, sx, sy, xFlip, 0, Colour, 4, 0, 0x200, DrvSprites);
			}

			if (SpriteSize == 32) {
				Draw32x32MaskTile(pTransDraw, Code, sx, sy, xFlip, 0, Colour, 4, 0, 0x200, DrvSprites);
			}
		}
	}
}
...
{
	BurnTransferClear();
	if (nBurnLayer & 1) DrvRenderFgLayer();
	if (nSpriteEnable & 1) DrvRenderSprites(2, 0xd00, 16, 2, 0, 0);
	if (nBurnLayer & 2 && DrvBgEnable) DrvRenderBitmap();
	if (nSpriteEnable & 2) DrvRenderSprites(2, 0xd00, 16, 1, 0, 0);
	if (nBurnLayer & 4) DrvRenderCharLayer(32, 16);
	if (nSpriteEnable & 4) DrvRenderSprites(2, 0xd00, 16, 0, 0, 0);
	BurnTransferCopy(DrvPalette);
...
struct BurnDriver BurnDrvBigtwin = {
	...
	DrvInit, DrvExit, DrvFrame, DrvRender, DrvScan,
...
struct BurnDriver BurnDrvExcelsr = {
	...
	ExcelsrInit, DrvExit, DrvFrame, ExcelsrRender, DrvScan,
	NULL, 0x400, 320, 240, 4, 3
};
...
struct BurnDriver BurnDrvHotmind = {
	...
	HotmindInit, DrvExit, DrvFrame, HotmindRender, DrvScan,
