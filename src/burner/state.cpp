...
{
	auto szHeader = "FS1 "; // Chunk identifier
	...
	Def = static_cast<UINT8*>(malloc(nDefLen));
	if (Def == nullptr)
	{
		return -1;
	}
...
{
	constexpr char szHeader[] = "FB1 "; // File identifier
	char szReadHeader[4] = "";
	INT32 nRet = 0;
	...
	if (nReplayStatus)
	{
		constexpr char szMovieExtra[] = "MOV ";
		constexpr char szDecodeChunk[] = "INPB";
		constexpr char szInputChunk[] = "INP ";
		...
		do
		{
			...
			if (memcmp(szReadHeader, szMovieExtra, 4))
			{
				nRet = -1;
				break;
			}
			...
			fread(szReadHeader, 1, 4, fp);
			if (memcmp(szReadHeader, szDecodeChunk, 4))
			{
				nRet = -1;
				break;
			}
			...
			if ((buf = static_cast<UINT8*>(malloc(nChunkSize))) == nullptr)
			{
				nRet = -1;
				break;
			}
			...
			fread(szReadHeader, 1, 4, fp);
			if (memcmp(szReadHeader, szInputChunk, 4))
			{
				nRet = -1;
				break;
			}
			...
			fread(buf, 1, nChunkSize, fp);
			if (UnfreezeInput(buf, nChunkSize))
			{
				nRet = -1;
				break;
			}
...
INT32 BurnStateSave(TCHAR* szName, INT32 bAll)
{
	...
	*/
	...
	if (nReplayStatus)
	{
		UINT8* huff_buf = nullptr;
		INT32 huff_size;
		UINT8* input_buf = nullptr;
		INT32 input_size;
		INT32 ret = -1;
