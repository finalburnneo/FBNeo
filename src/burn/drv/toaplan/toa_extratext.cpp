		...
		if (y < 233) {
			...
			if (i == 8) {
				...
				for (x = 0, pTile = pCurrentRow - nOffset * nBurnColumn; x < 41; x++, pTile += nTileLeft) {
					...
					if (1 /*nTile && nTile != 0x20*/) { // nTile is usually Black/Opaque, for mosaic tile effects in FixEight.  0x20 needs to be drawn, too, otherwise certain tiles of the HUD will become transparent during gameplay.
