...
{
	{0x19, 0xff, 0xff, 0xff, nullptr},
	{0x1a, 0xff, 0xff, 0x7f, nullptr},
...
static void __fastcall wizdfire_main_write_word(UINT32 address, UINT16 data)
{
	...
	if ((address >= 0xff4000 && address <= 0xff7fff) || // wizdfire
		(address >= 0xfe4000 && address <= 0xfe7fff))
	{
		// nitrobal
		deco146_104_prot_ww(0, address, data);
	}
...
static INT32 MemIndex()
{
	...
	tempdraw[0] = (UINT16*)Next;
	Next += 320 * 240 * sizeof(UINT16);
	tempdraw[1] = (UINT16*)Next;
	Next += 320 * 240 * sizeof(UINT16);
...
static INT32 DrvSpriteDecode()
{
	...
	INT32 XOffs[16] = {
		7, 6, 5, 4, 3, 2, 1, 0, 32 * 8 + 7, 32 * 8 + 6, 32 * 8 + 5, 32 * 8 + 4, 32 * 8 + 3, 32 * 8 + 2, 32 * 8 + 1,
		32 * 8 + 0
	};
	INT32 YOffs[16] = {
		15 * 16, 14 * 16, 13 * 16, 12 * 16, 11 * 16, 10 * 16, 9 * 16, 8 * 16, 7 * 16, 6 * 16, 5 * 16, 4 * 16, 3 * 16,
		2 * 16, 1 * 16, 0 * 16
	};

	auto tmp = BurnMalloc(0x600000);
	if (tmp == nullptr)
	{
		return 1;
	}
...
static void rohga_draw_sprites(UINT8* ram, INT32 is_schmeisr)
{
	...
	for (INT32 offs = 0x400 - 4; offs >= 0; offs -= 4)
	{
		...
		while (multi >= 0)
		{
			deco16_draw_prio_sprite(pTransDraw, DrvGfxROM3, (sprite - multi * inc) & 0x7fff, colour + 0x400, x,
...
static void wizdfire_draw_sprites(UINT8* ram, UINT8* gfx, INT32 coloff, INT32 mode, INT32 bank, INT32 alpha_on)
{
	...
	for (INT32 offs = 0; offs < 0x400; offs += 4)
	{
		...
		while (multi >= 0)
		{
			if (alpha_on)
			{
				deco16_draw_alphaprio_sprite(DrvPalette, gfx, sprite - multi * inc, (colour << 4) + coloff, x,
				                             y + mult * multi, fx, fy, prio | 0xf0, 1, alpha,
				                             1 /* special mode for wizdfire */);
			}
			else
			{
				if (prio >= 0x20) // "over alpha" solid sprites.  see mix_alpha_tilemap()
					deco16_draw_prio_sprite_dumb(pTransDraw, gfx, sprite - multi * inc, (colour << 4) + coloff, x,
					                             y + mult * multi, fx, fy, 0xf0, -1);
				else
					deco16_draw_prio_sprite(pTransDraw, gfx, sprite - multi * inc, (colour << 4) + coloff, x,
					                        y + mult * multi, fx, fy, -1);
			}
...
static void nitrobal_draw_sprites(UINT8* ram, INT32 gfxbank, INT32 alpha_on)
{
	...
	while (offs != end)
	{
		...
		if (gfxbank == 3)
		{
			...
			case 0xc0: tilemap_pri = 8;
				break;
			case 0x80: tilemap_pri = 64;
				break; // was 32, fixes horseshoe "ring" under bouncers in first level
			case 0x20: tilemap_pri = 32;
				break;
			case 0x40: tilemap_pri = 8;
				break;
			case 0xa0: tilemap_pri = (deco16_priority) ? 8 : 32;
				break; // was 32
			case 0x00: tilemap_pri = 72;
				break; // was 128, this and above fixes the archways over the player in alien world
			default: tilemap_pri = 128;
				break;
			}
		...
		for (x = 0; x < w; x++)
		{
			for (y = 0; y < h; y++)
			{
				if (!alpha_on)
				{
					deco16_draw_prio_sprite_nitrobal(pTransDraw, gfx, sprite + y + h * x, (colour << 4) + coloff,
					                                 sx + x_mult * (w - x), sy + y_mult * (h - y), fx, fy, tilemap_pri,
					                                 sprite_pri);
				}
				else
				{
					deco16_draw_alphaprio_sprite(DrvPalette, gfx, sprite + y + h * x, (colour << 4) + coloff,
					                             sx + x_mult * (w - x), sy + y_mult * (h - y), fx, fy, tilemap_pri,
					                             sprite_pri, alpha);
				}
			}
...
static INT32 WizdfireDraw()
{
	...
	if (nBurnLayer & 4) deco16_draw_layer(2, ((deco16_priority & 0x1f) == 0x1f && nBurnBpp == 4)
		                                         ? tempdraw[0]
		                                         : pTransDraw, 0); // tilemap draw alpha 0x80...
...
static INT32 NitrobalDraw()
{
	...
	if (nBurnBpp != 4)
	{
		// regular (non-alpha) draw for anything !32bpp
...
struct BurnDriver BurnDrvRohga = {
	"rohga", nullptr, nullptr, nullptr, "1991",
	"Rohga Armor Force (Asia/Europe v5.0)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, rohgaRomInfo, rohgaRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
...
struct BurnDriver BurnDrvRohga1 = {
	"rohga1", "rohga", nullptr, nullptr, "1991",
	"Rohga Armor Force (Asia/Europe v3.0 set 1)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, rohga1RomInfo, rohga1RomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
	RohgaInit, DrvExit, DrvFrame, RohgaDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvRohga2 = {
	"rohga2", "rohga", nullptr, nullptr, "1991",
	"Rohga Armor Force (Asia/Europe v3.0 set 2)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, rohga2RomInfo, rohga2RomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
	RohgaInit, DrvExit, DrvFrame, RohgaDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvRohgah = {
	"rohgah", "rohga", nullptr, nullptr, "1991",
	"Rohga Armor Force (Hong Kong v3.0)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, rohgahRomInfo, rohgahRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
	RohgaInit, DrvExit, DrvFrame, RohgaDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvRohgau = {
	"rohgau", "rohga", nullptr, nullptr, "1991",
	"Rohga Armor Force (US v1.0)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, rohgauRomInfo, rohgauRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
	RohgaInit, DrvExit, DrvFrame, RohgaDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvWolffang = {
	"wolffang", "rohga", nullptr, nullptr, "1991",
	"Wolf Fang -Kuhga 2001- (Japan)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT | GBF_PLATFORM, 0,
	nullptr, wolffangRomInfo, wolffangRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, RohgaDIPInfo,
	RohgaInit, DrvExit, DrvFrame, RohgaDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvWizdfire = {
	"wizdfire", nullptr, nullptr, nullptr, "1992",
	"Wizard Fire (Over Sea v2.1)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, wizdfireRomInfo, wizdfireRomName, nullptr, nullptr, nullptr, nullptr, WizdfireInputInfo, WizdfireDIPInfo,
...
struct BurnDriver BurnDrvWizdfireu = {
	"wizdfireu", "wizdfire", nullptr, nullptr, "1992",
	"Wizard Fire (US v1.1)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, wizdfireuRomInfo, wizdfireuRomName, nullptr, nullptr, nullptr, nullptr, WizdfireInputInfo, WizdfireDIPInfo,
	WizdfireEnglishInit, DrvExit, DrvFrame, WizdfireDraw, DrvScan, &DrvRecalc, 0x800,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvDarkseal2 = {
	"darkseal2", "wizdfire", nullptr, nullptr, "1992",
	"Dark Seal 2 (Japan v2.1)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, darkseal2RomInfo, darkseal2RomName, nullptr, nullptr, nullptr, nullptr, WizdfireInputInfo, WizdfireDIPInfo,
...
struct BurnDriver BurnDrvSchmeisr = {
	"schmeisr", nullptr, nullptr, nullptr, "1993",
	"Schmeiser Robo (Japan)\0", nullptr, "Hot-B", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_HISCORE_SUPPORTED, 2, HARDWARE_PREFIX_DATAEAST, GBF_VSFIGHT, 0,
	nullptr, schmeisrRomInfo, schmeisrRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, SchmeisrDIPInfo,
...
struct BurnDriver BurnDrvHangzo = {
	"hangzo", nullptr, nullptr, nullptr, "1992",
	"Hangzo (Japan, prototype)\0", nullptr, "Hot-B", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_PROTOTYPE, 2, HARDWARE_PREFIX_DATAEAST, GBF_SCRFIGHT, 0,
	nullptr, hangzoRomInfo, hangzoRomName, nullptr, nullptr, nullptr, nullptr, RohgaInputInfo, HangzoDIPInfo,
...
struct BurnDriver BurnDrvNitrobal = {
	"nitrobal", nullptr, nullptr, nullptr, "1992",
	"Nitro Ball (World, set 1)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 3, HARDWARE_PREFIX_DATAEAST, GBF_RUNGUN, 0,
	nullptr, nitrobalRomInfo, nitrobalRomName, nullptr, nullptr, nullptr, nullptr, NitrobalInputInfo, NitrobalDIPInfo,
...
static struct BurnRomInfo nitrobalaRomDesc[] = {
	...
	{"6d", 0x040000, 0x59e620cc, 1 | BRF_PRG | BRF_ESS},
	//  4 // 7ee3 - 7f06 are 0xff instead of 0x00 in the nitrobal set, why?
...
struct BurnDriver BurnDrvNitrobala = {
	"nitrobala", "nitrobal", nullptr, nullptr, "1992",
	"Nitro Ball (World, set 2)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 3, HARDWARE_PREFIX_DATAEAST,
	GBF_RUNGUN, 0,
	nullptr, nitrobalaRomInfo, nitrobalaRomName, nullptr, nullptr, nullptr, nullptr, NitrobalInputInfo, NitrobalDIPInfo,
	NitrobalInit, DrvExit, DrvFrame, NitrobalDraw, DrvScan, &DrvRecalc, 0x800,
	240, 320, 3, 4
};
...
struct BurnDriver BurnDrvNitrobalb = {
	"nitrobalb", "nitrobal", nullptr, nullptr, "1992",
	"Nitro Ball (World, set 3)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 3, HARDWARE_PREFIX_DATAEAST,
	GBF_RUNGUN, 0,
	nullptr, nitrobalbRomInfo, nitrobalbRomName, nullptr, nullptr, nullptr, nullptr, NitrobalInputInfo, NitrobalDIPInfo,
	NitrobalInit, DrvExit, DrvFrame, NitrobalDraw, DrvScan, &DrvRecalc, 0x800,
	240, 320, 3, 4
};
...
struct BurnDriver BurnDrvGunball = {
	"gunball", "nitrobal", nullptr, nullptr, "1992",
	"Gun Ball (Japan)\0", nullptr, "Data East Corporation", "DECO IC16",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 3, HARDWARE_PREFIX_DATAEAST,
	GBF_RUNGUN, 0,
	nullptr, gunballRomInfo, gunballRomName, nullptr, nullptr, nullptr, nullptr, NitrobalInputInfo, NitrobalDIPInfo,
	NitrobalInit, DrvExit, DrvFrame, NitrobalDraw, DrvScan, &DrvRecalc, 0x800,
	240, 320, 3, 4
};
