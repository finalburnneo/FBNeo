...
INT32 ConfigGameLoad(bool bOverWrite)
{
	...
	while (_fgetts(szLine, 256, h))
	{
		...
		if (szValue)
		{
			nFileVersion = _tcstol(szValue, nullptr, 0);
		}
		...
		if (bOverWrite)
		{
			...
			if (szValue)
			{
				nAnalogSpeed = _tcstol(szValue, nullptr, 0);
			}
			szValue = LabelCheck(szLine, _T("cpu"));
			if (szValue)
			{
				nBurnCPUSpeedAdjust = _tcstol(szValue, nullptr, 0);
			}
		}
...
INT32 ConfigGameSave(bool bSave)
{
	...
	_ftprintf(h, _T("// ") _T(APP_TITLE) _T(" v%s --- Config File for %s (%s)\n\n"), szAppBurnVer,
	          BurnDrvGetText(DRV_NAME), ANSIToTCHAR(BurnDrvGetTextA(DRV_FULLNAME), nullptr, 0));
