...
static void DrawSprite(INT32 offs)
{
	...
	if (BIT(DrvSprRAM[offs+1], 4))
	{
		...
		if (flipy)
		{
			if (flipx)
			{
				Render16x16Tile_Mask_FlipXY_Clip(pTransDraw, code, sx, sy, color, 2, 0, 0,
				                                 DrvGfxROM1 + (((DrvSprRAM[offs + 1] & 0x20) >> 5) * 16384));
			}
			else
			{
				Render16x16Tile_Mask_FlipY_Clip(pTransDraw, code, sx, sy, color, 2, 0, 0,
				                                DrvGfxROM1 + (((DrvSprRAM[offs + 1] & 0x20) >> 5) * 16384));
			}
		}
		else
		{
			if (flipx)
			{
				Render16x16Tile_Mask_FlipX_Clip(pTransDraw, code, sx, sy, color, 2, 0, 0,
				                                DrvGfxROM1 + (((DrvSprRAM[offs + 1] & 0x20) >> 5) * 16384));
			}
			else
			{
				Render16x16Tile_Mask_Clip(pTransDraw, code, sx, sy, color, 2, 0, 0,
				                          DrvGfxROM1 + (((DrvSprRAM[offs + 1] & 0x20) >> 5) * 16384));
			}
		}
	}
...
static void DrvGfxDecode()
{
	...
	INT32 YOffs1[16] = {
		STEP8(0, 8), 128 + 8 * 0, 128 + 8 * 1, 128 + 8 * 2, 128 + 8 * 3, 128 + 8 * 4, 128 + 8 * 5, 128 + 8 * 6,
		128 + 8 * 7
	};
	...
	GfxDecode(0x0040, 2, 16, 16, Planes1, XOffs1, YOffs1, 0x100, tmp + 0x1000, DrvGfxROM1);
	// modulo 0x040 to verify !!!
	GfxDecode(0x0040, 2, 16, 16, Planes1, XOffs1, YOffs1, 0x100, tmp + 0x1800, DrvGfxROM1 + (0x800 * 8));
	// modulo 0x100 to verify !!!
...
struct BurnDriver BurnDrvMouser = {
	"mouser", nullptr, nullptr, nullptr, "1983",
	"Mouser\0", nullptr, "UPL", "Miscellaneous",
	...
	nullptr, mouserRomInfo, mouserRomName, nullptr, nullptr, nullptr, nullptr, MouserInputInfo, MouserDIPInfo,
...
struct BurnDriver BurnDrvMouserc = {
	"mouserc", "mouser", nullptr, nullptr, "1983",
	"Mouser (Cosmos)\0", nullptr, "UPL (Cosmos license)", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED | BDF_HISCORE_SUPPORTED, 2,
	HARDWARE_MISC_PRE90S, GBF_PLATFORM, 0,
	nullptr, mousercRomInfo, mousercRomName, nullptr, nullptr, nullptr, nullptr, MouserInputInfo, MouserDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc, 0x40,
	224, 256, 3, 4
};
