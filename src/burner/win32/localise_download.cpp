...
static INT32 LocaliseDownloadInit()
{
	...
	if (!connect)
	{
		MessageBox(hLocalDownDlg, FBNLoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_CONNECT, true),
		           FBNLoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
		return 1;
	}

	HINTERNET OpenAddress = InternetOpenUrl(
		connect, _T("https://finalburnneo.github.io/FBNeo-translations/localisationinfo/"), NULL, 0,
		INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, 0);
	if (!OpenAddress)
	{
		MessageBox(hLocalDownDlg, FBNLoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_OPEN_URL, true),
		           FBNLoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
...
static int LocaliseDownloadOkay()
{
	...
	HINTERNET OpenAddress = InternetOpenUrl(connect, url, NULL, 0,
	                                        INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, 0);
	if (!OpenAddress)
	{
		MessageBox(hLocalDownDlg, FBNLoadStringEx(hAppInst, IDS_ERR_LOCAL_FAIL_OPEN_URL, true),
