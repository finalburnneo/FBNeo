...
int StatedAuto(int bSave)
{
	...
	if (bSave == 0)
	{
		nRet = BurnStateLoad(szName, bDrvSaveAll, nullptr); // Load ram
		if (nRet && bDrvSaveAll)
		{
			nRet = BurnStateLoad(szName, 0, nullptr); // Couldn't get all - okay just try the nvram
		}
	}
	else
	{
