...
static INT32 DrvFrame()
{
	...
	INT32 nInterleave = 256; // WRONG # of interrupts/frame MSM5205 cpu needs
	INT32 nCyclesTotal[4] = { cpu_cycles[0] / 60, cpu_cycles[1] / 60, cpu_cycles[2] / 60, cpu_cycles[3] / 60 };
	...
	for (INT32 i = 0; i < nInterleave; i++)
	{
		...
		if (MSM5205InUse) {
			ZetOpen(2);
			CPU_RUN(2, Zet);
			if ((i&0x3) == 0) ZetSetIRQLine(0, CPU_IRQSTATUS_HOLD);
