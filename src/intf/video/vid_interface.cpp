...
// & 1: do not rotate the graphics for vertical games,  & 2: Reverse flipping for vertical games
...
void (*pVidTransCallback)(void) = NULL;
// Callback for video driver, after BurnDrvFrame() / BurnDrvRedraw() (see win32/vid_d3d.cpp:vidFrame() for example)
		...
		if (bVidRecalcPalette)
		{
			for (INT32 r = 0; r < 256; r += 8)
			{
				for (INT32 g = 0; g < 256; g += 8)
				{
					...
					{
						pVidTransPalette[(r << 7) | (g << 2) | (b >> 3)] = VidHighCol(
							r | (r >> 5), g | (g >> 5), b | (b >> 5), 0);
					}
				}
			}
...
InterfaceInfo* VidGetInfo()
{
	...
	if (bVidOkay)
	{
		...
		if (nVidFullscreen == 0)
		{
			...
			_sntprintf(szString, MAX_PATH, _T("Running in windowed mode, %ix%i, %ibpp"), rect.right - rect.left,
			           rect.bottom - rect.top, nVidScrnDepth);
		}
		else
		{
			_sntprintf(szString, MAX_PATH, _T("Running fullscreen, %ix%i, %ibpp"), nVidScrnWidth, nVidScrnHeight,
			           nVidScrnDepth);
		}
#elif defined (BUILD_SDL)
		...
		_sntprintf(szString, MAX_PATH, _T("Source image %ix%i, %ibpp"), nVidImageWidth, nVidImageHeight,
		           nVidImageDepth);
		IntInfoAddStringInterface(&VidInfo, szString);
