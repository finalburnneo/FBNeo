...
struct NGCDGAME games[] =
{
	...
	{
		_T("ssideki3"), _T("Super Sidekicks 3 - The Next Glory / Tokuten Oh 3 - Eikoue No Chousen"), _T("1995"),
		_T("SNK"), 0x0081
	}, //
	...
	{
		_T("ssrpg"), _T("Samurai Shodown RPG / Shinsetsu Samurai Spirits - Bushidohretsuden"), _T("1997"), _T("SNK"),
		0x0085
	}, //
	...
	{
		_T("rbff2"), _T("Real Bout Fatal Fury 2 - The Newcomers / Real Bout Garou Densetsu 2 - The Newcomers"),
		_T("1998"), _T("SNK"), 0x0240
	}, //
	...
	{
		_T("fatfury3"), _T("Fatal Fury 3 - Road to the Final Victory / Garou Densetsu 3 - Harukanaru Tatakai"),
		_T("1995"), _T("SNK"), 0x069c
	}, //
...
static void SetNeoCDTitle(TCHAR* pszTitle)
{
	...
	_stprintf(szText, _T(APP_TITLE) _T(" v%.20s") _T(SEPERATOR_1) _T("%s") _T(SEPERATOR_1) _T("%s"), szAppBurnVer,
	          BurnDrvGetText(DRV_FULLNAME), pszTitle);
...
{
	if (fp == nullptr || Dest == nullptr) return;

	fseek(fp, lOffset + ((nSectorLength == 2352) ? 16 : 0), SEEK_SET);
	fread(Dest, lLength, lSize, fp);
}

static void NeoCDList_iso9660_CheckDirRecord(void (*pfEntryCallBack)(INT32, TCHAR*), TCHAR* pszFile, FILE* fp,
                                             int nSector)
{
	...
	UINT8 Flags;
	UINT8 LEN_FI;
	auto ExtentLoc = static_cast<UINT8*>(malloc(8 + 32));
	auto Data = static_cast<UINT8*>(malloc(0x10b + 32));
	auto File = static_cast<char*>(malloc(32 + 32));
...
{
	if (!game || !IsNeoGeoCD()) return nullptr;
