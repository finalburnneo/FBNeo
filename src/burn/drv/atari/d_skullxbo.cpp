...
static void __fastcall skullxbo_main_write_word(UINT32 address, UINT16 data)
{
	...
	if ((address & 0xffe000) == 0xff8000)
	{
		...
		*((UINT16*)(DrvPfRAM1 + (address & 0x1ffe))) = BURN_ENDIAN_SWAP_INT16(
			(BURN_ENDIAN_SWAP_INT16(*((UINT16*)(DrvPfRAM1 + (address & 0x1ffe)))) & 0xff00) | (playfield_latch));
		return;
	}
...
static void __fastcall skullxbo_main_write_byte(UINT32 address, UINT8 data)
{
	if ((address & 0xfff000) == 0xffd000)
	{
		...
		if (address & 1)
			AtariMoWrite(0, (address & 0xfff) >> 1,
			             BURN_ENDIAN_SWAP_INT16(*((UINT16*)(DrvMobRAM + (address & 0x0ffe)))));
		return;
	}
...
static INT32 DrvInit()
{
	...
	AtariJSAInit(DrvM6502ROM, &update_interrupts, DrvSndROM, nullptr);
...
static void skullxbo_scanline_update()
{
	...
	auto base = (UINT16*)(DrvAlphaRAM + offs);
...
static INT32 DrvFrame()
{
	...
	for (INT32 i = 0; i < nInterleave; i++)
	{
		...
		if (cpu_halted)
			nCyclesDone[0] += SekIdle(static_cast<INT32>(static_cast<double>(sek_line) * 0.9)); //0.7368));
		else
			nCyclesDone[0] += SekRun(static_cast<INT32>(static_cast<double>(sek_line) * 0.9)); //0.7368));
		...
		if (scanline >= 0 && scanline == do_scanline_irq)
		{
			// 6th
...
struct BurnDriver BurnDrvSkullxbo = {
	"skullxbo", nullptr, nullptr, nullptr, "1989",
	"Skull & Crossbones (rev 5)\0", nullptr, "Atari Games", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING, 2, HARDWARE_MISC_PRE90S, GBF_MISC, 0,
	nullptr, skullxboRomInfo, skullxboRomName, nullptr, nullptr, nullptr, nullptr, SkullxboInputInfo, SkullxboDIPInfo,
