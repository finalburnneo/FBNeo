...
static void myDrawText(HDC hDC, RECT* pRect, TCHAR* szText, int nSizeDelta, int nColour, UINT uFormat)
{
	...
	myFont = CreateFont(myMetric.tmHeight + nSizeDelta, 0, 0, 0, FW_BLACK, 0, 0, 0, 0, 0, 0, ANTIALIASED_QUALITY, 0,
	                    szName);
	hFont = static_cast<HFONT>(SelectObject(hDC, myFont));
...
{
	HRSRC hrsrc = FindResource(nullptr, MAKEINTRESOURCE(ID_LICENSE), MAKEINTRESOURCE(256));
	HGLOBAL hglobal = LoadResource(nullptr, hrsrc);
	auto pszLicense = static_cast<char*>(LockResource(hglobal));

	SendDlgItemMessageA(hDlg, nControlID, WM_SETTEXT, 0, (LPARAM)pszLicense);
}
...
static INT_PTR CALLBACK FirstProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Msg == WM_INITDIALOG)
	{
		...
		SendDlgItemMessage(hDlg, IDC_LICENSE, WM_SETTEXT, 0, (LPARAM)szWarningString);

		ShowWindow(GetDlgItem(hDlg, IDC_ACCEPTLICENSE), SW_HIDE);

		SetForegroundWindow(hDlg);
		WndInMid(hDlg, nullptr);
