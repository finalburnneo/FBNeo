...
static UINT8* DrvZ80Rom1 = nullptr;
static UINT8* DrvZ80Rom2 = nullptr;
static UINT8* DrvZ80Ram1 = nullptr;
static UINT8* DrvZ80Ram2 = nullptr;
static UINT8* DrvVideoRam = nullptr;
static UINT8* DrvPaletteRam = nullptr;
static UINT8* DrvSpriteRam = nullptr;
static UINT8* DrvPromRed = nullptr;
static UINT8* DrvPromGreen = nullptr;
static UINT8* DrvPromBlue = nullptr;
static UINT8* DrvPromCharLookup = nullptr;
static UINT8* DrvPromBg2Lookup = nullptr;
static UINT8* DrvPromBg2PalBank = nullptr;
static UINT8* DrvPromBgLookup = nullptr;
static UINT8* DrvPromBgPalBank = nullptr;
static UINT8* DrvPromSpriteLookup = nullptr;
static UINT8* DrvPromSpritePalBank = nullptr;
static UINT8* DrvBgTilemap = nullptr;
static UINT8* DrvBg2Tilemap = nullptr;
static UINT8* DrvChars = nullptr;
static UINT8* DrvBg2Tiles = nullptr;
static UINT8* DrvBgTiles = nullptr;
...
static INT32 TileXOffsets[32] = {
	STEP4(0, 1), STEP4(8, 1), STEP4(512, 1), STEP4(520, 1), STEP4(1024, 1), STEP4(1032, 1), STEP4(1536, 1),
	STEP4(1544, 1)
};
...
static INT32 MemIndex()
{
	...
	DrvPromBlue = Next;
	Next += 0x00100;
	DrvPromCharLookup = Next;
	Next += 0x00100;
	DrvPromBg2Lookup = Next;
	Next += 0x00100;
	DrvPromBg2PalBank = Next;
	Next += 0x00100;
	DrvPromBgLookup = Next;
	Next += 0x00100;
	DrvPromBgPalBank = Next;
	Next += 0x00100;
	DrvPromSpriteLookup = Next;
	Next += 0x00100;
	DrvPromSpritePalBank = Next;
	Next += 0x00100;
	DrvBgTilemap = Next;
	Next += 0x08000;
	DrvBg2Tilemap = Next;
...
static INT32 CommonInit(INT32 (*load)())
{
	...
	BurnYM2203Init(2, 1500000, nullptr, 0);
...
static INT32 DrvLoad()
{
	...
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x00000, 0, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x10000, 1, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvZ80Rom1 + 0x20000, 2, 1);
	if (nRet != 0) return 1;

	// Load Z80 #2 Program Roms
	nRet = BurnLoadRom(DrvZ80Rom2 + 0x00000, 3, 1);
	if (nRet != 0) return 1;

	// Load and decode the chars
	nRet = BurnLoadRom(DrvTempRom, 4, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 14, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 8, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 9, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 10, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 11, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x38000, 12, 1);
	if (nRet != 0) return 1;
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x10000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x18000, 18, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x20000, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x38000, 22, 1);
	if (nRet != 0) return 1;
	GfxDecode(2048, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);

	// Load the Tilemaps
	nRet = BurnLoadRom(DrvBgTilemap, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvBg2Tilemap, 24, 1);
	if (nRet != 0) return 1;

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromRed, 25, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromGreen, 26, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBlue, 27, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromCharLookup, 28, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgLookup, 29, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgPalBank, 30, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2Lookup, 31, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2PalBank, 32, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpriteLookup, 33, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpritePalBank, 34, 1);
	if (nRet != 0) return 1;
...
static INT32 DrvbLoad()
{
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x08000, 10, 1);
	if (nRet != 0) return 1;
	GfxDecode(128, 4, 32, 32, Bg2TilePlaneOffsets, TileXOffsets, TileYOffsets, 0x800, DrvTempRom, DrvBg2Tiles);
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 16, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x30000, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x38000, 18, 1);
	if (nRet != 0) return 1;
	GfxDecode(2048, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);

	// Load the Tilemaps
	nRet = BurnLoadRom(DrvBgTilemap, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvBg2Tilemap, 20, 1);
	if (nRet != 0) return 1;

	// Load the PROMs
	nRet = BurnLoadRom(DrvPromRed, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromGreen, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBlue, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromCharLookup, 24, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgLookup, 25, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgPalBank, 26, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2Lookup, 27, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2PalBank, 28, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpriteLookup, 29, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpritePalBank, 30, 1);
	if (nRet != 0) return 1;
...
static INT32 DrvbjLoad()
{
	...
	memset(DrvTempRom, 0, 0x40000);
	auto pTemp = BurnMalloc(0x40000);
	nRet = BurnLoadRom(pTemp + 0x00000, 5, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(pTemp + 0x10000, 6, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(pTemp + 0x20000, 7, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(pTemp + 0x30000, 8, 1);
	if (nRet != 0) return 1;
	...
	nRet = BurnLoadRom(DrvBgTilemap, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom, 20, 1);
	if (nRet != 0) return 1;
...
static INT32 Drvb2Load()
{
	...
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x28000, 15, 1);
	if (nRet != 0) return 1;
	...
	// Load the Tilemaps
	nRet = BurnLoadRom(DrvBgTilemap, 17, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom, 18, 1);
	if (nRet != 0) return 1;
	memcpy(DrvBg2Tilemap + 0x6000, DrvTempRom + 0x0000, 0x2000);
	memcpy(DrvBg2Tilemap + 0x4000, DrvTempRom + 0x2000, 0x2000);
	memcpy(DrvBg2Tilemap + 0x2000, DrvTempRom + 0x4000, 0x2000);
	...
	// Load the PROMs
	nRet = BurnLoadRom(DrvPromRed, 19, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromGreen, 20, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBlue, 21, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromCharLookup, 22, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgLookup, 23, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBgPalBank, 24, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2Lookup, 25, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromBg2PalBank, 26, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpriteLookup, 27, 1);
	if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvPromSpritePalBank, 28, 1);
	if (nRet != 0) return 1;
...
static void DrvCalcPalette()
{
	...
	{
		DrvPalette[i] = Palette[((DrvPromSpritePalBank[i - 0x280] & 0x07) << 4) | (DrvPromSpriteLookup[i - 0x280] &
			0x0f) | 0x80];
	}
}
...
struct BurnDriver BurnDrvNineteen43 = {
	"1943", nullptr, nullptr, nullptr, "1987",
	"1943: The Battle of Midway (Euro)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, DrvRomInfo, DrvRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43u = {
	"1943u", "1943", nullptr, nullptr, "1987",
	"1943: The Battle of Midway (US, Rev C)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvuRomInfo, DrvuRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43ua = {
	"1943ua", "1943", nullptr, nullptr, "1987",
	"1943: The Battle of Midway (US)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvuaRomInfo, DrvuaRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43j = {
	"1943j", "1943", nullptr, nullptr, "1987",
	"1943: Midway Kaisen (Japan, Rev B)\0", nullptr, "Capcom", "Miscellaneous",
	L"1943: \u30DF\u30C3\u30C9\u30A6\u30A7\u30A4\u6D77\u6226 (Japan, Rev B)\0", nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvjRomInfo, DrvjRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43ja = {
	"1943ja", "1943", nullptr, nullptr, "1987",
	"1943: Midway Kaisen (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	L"1943: \u30DF\u30C3\u30C9\u30A6\u30A7\u30A4\u6D77\u6226 (Japan)\0", nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvjaRomInfo, DrvjaRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43jah = {
	"1943jah", "1943", nullptr, nullptr, "1987",
	"1943: Midway Kaisen (Japan, no protection hack)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvjahRomInfo, DrvjahRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43jaha = {
	"1943jaha", "1943", nullptr, nullptr, "1987",
	"1943: Midway Kaisen (Japan, no protection hack, alt)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC,
	GBF_VERSHOOT, 0,
	nullptr, DrvjahaRomInfo, DrvjahaRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43b = {
	"1943b", "1943", nullptr, nullptr, "1987",
	"1943: Battle of Midway (bootleg set 1, hack of Japan set)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, DrvbRomInfo, DrvbRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvbInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43b2 = {
	"1943b2", "1943", nullptr, nullptr, "1987",
	"1943: Battle of Midway (bootleg set 2, hack of Japan set)\0", nullptr, "bootleg", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, Drvb2RomInfo, Drvb2RomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	Drvb2Init, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43bj = {
	"1943bj", "1943", nullptr, nullptr, "1987",
	"1943: Midway Kaisen (bootleg)\0", nullptr, "bootleg", "Miscellaneous",
	L"1943: \u30DF\u30C3\u30C9\u30A6\u30A7\u30A4\u6D77\u6226 (bootleg)\0", nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG | BDF_HISCORE_SUPPORTED, 2,
	HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, DrvbjRomInfo, DrvbjRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvbjInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43kai = {
	"1943kai", nullptr, nullptr, nullptr, "1987",
	"1943 Kai: Midway Kaisen (Japan)\0", nullptr, "Capcom", "Miscellaneous",
	L"1943 \u6539: \u30DF\u30C3\u30C9\u30A6\u30A7\u30A4\u6D77\u6226 (Japan)\0", nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, DrvkaiRomInfo, DrvkaiRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};

struct BurnDriver BurnDrvNineteen43mii = {
	"1943mii", nullptr, nullptr, nullptr, "1988",
	"1943: The Battle of Midway Mark II (US)\0", nullptr, "Capcom", "Miscellaneous",
	nullptr, nullptr, nullptr, nullptr,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_HISCORE_SUPPORTED, 2, HARWARE_CAPCOM_MISC, GBF_VERSHOOT, 0,
	nullptr, DrvmiiRomInfo, DrvmiiRomName, nullptr, nullptr, nullptr, nullptr, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 0x380, 224, 256, 3, 4
};
