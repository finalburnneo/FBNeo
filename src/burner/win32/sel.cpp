...
static int MASKALL = MASKCAPMISC | MASKCAVE | MASKCPS | MASKCPS2 | MASKCPS3 | MASKDATAEAST | MASKGALAXIAN | MASKIREM |
	...
	MASKSG1000 | MASKCOLECO | MASKMSX | MASKSPECTRUM | MASKMIDWAY; // | MASKSNES
...
static int SelListMake()
{
	...
	_stprintf(szRomsAvailableInfo, FBNLoadStringEx(hAppInst, IDS_SEL_SETSTATUS, true), nTmpDrvCount,
...
static void SelOkay()
{
	...
	if (!CheckWorkingStatus(nSelect))
	{
		...
		if (MessageBox(hSelDlg, FBNLoadStringEx(hAppInst, IDS_ERR_NON_WORKING, true), szWarningText,
...
void LoadDrvIcons()
{
...

	{
		...
		hConsDrvIcon[ICON_MEGADRIVE] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                                LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%spce_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_PCEFAM] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                             LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%ssg1000_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SG1000] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                             LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%scolecovision_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_COLECO] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                             LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%ssms_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SMS] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                          LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%sgamegear_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_GG] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                         LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%smsx_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_MSX] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                          LR_LOADFROMFILE);

		_stprintf(szIcon, _T("%spectrum_icon.ico"), szAppIconsPath);
		hConsDrvIcon[ICON_SPECTRUM] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                               LR_LOADFROMFILE);
	}
	...
	for (unsigned int nDrvIndex = 0; nDrvIndex < nBurnDrvCount; nDrvIndex++)
	{
		...
		if (bIconsOnlyParents && BurnDrvGetText(DRV_PARENT) != NULL && (BurnDrvGetFlags() & BDF_CLONE))
		{
			// Skip clones
			continue;
		}
		...
		hDrvIcon[nDrvIndex] = (HICON)LoadImage(hAppInst, szIcon, IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                       LR_LOADFROMFILE);
...
static INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG)
	{
		...
		hNotWorking = (HICON)LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTWORKING), IMAGE_ICON, nIconsSizeXY,
		                               nIconsSizeXY, LR_DEFAULTCOLOR);
		hNotFoundEss = (HICON)LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTFOUND_ESS), IMAGE_ICON, nIconsSizeXY,
		                                nIconsSizeXY, LR_DEFAULTCOLOR);
		hNotFoundNonEss = (HICON)LoadImage(hAppInst, MAKEINTRESOURCE(IDI_TV_NOTFOUND_NON), IMAGE_ICON, nIconsSizeXY,
		                                   nIconsSizeXY, LR_DEFAULTCOLOR);

		hDrvIconMiss = (HICON)LoadImage(hAppInst, MAKEINTRESOURCE(IDI_APP), IMAGE_ICON, nIconsSizeXY, nIconsSizeXY,
		                                LR_DEFAULTCOLOR);
	...
	if (Msg == UM_CHECKSTATECHANGE)
	{
		...
		if (hItemChanged == hHardware)
		{
			...
			else
			{
				...
				nLoadMenuShowX &= (0xFFFFFFFF - MASKALL);
				//0xf8000000; make this dynamic for future hardware additions -dink
			}
		}
	...
	if (Msg == WM_COMMAND)
	{
		...
		if (HIWORD(wParam) == BN_CLICKED)
		{
			...
			case IDGAMEINFO:
				...
				{
					MessageBox(hSelDlg, FBNLoadStringEx(hAppInst, IDS_ERR_NO_DRIVER_SELECTED, true),
...

					{
...

						{
							...
							else
							{
								...
								{
									if (!(nLoadMenuShowY & AVAILABLE) && !(gameAv[((NODEINFO*)TvItem.lParam)->nBurnDrvNo
									] & 2))
									{
						...
						if (bEnableIcons && bIconsLoaded)
						{
							if (hDrvIcon[nBurnDrvActive])
							{
								DrawIconEx(lplvcd->nmcd.hdc, rect.left, rect.top, hDrvIcon[nBurnDrvActive],
								           nIconsSizeXY, nIconsSizeXY, 0, NULL, DI_NORMAL);
							}
				...
				case 0:
					{
						...
						if ((BurnDrvGetFlags() & BDF_CLONE) && BurnDrvGetTextA(DRV_PARENT))
						{
							...
							_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_CLONE_OF, true),
							...
							if (nBurnDrvActive < nBurnDrvCount)
							{
								if (BurnDrvGetText(DRV_PARENT))
								{
									_stprintf(szItemText + _tcslen(szItemText),
									          FBNLoadStringEx(hAppInst, IDS_ROMS_FROM_1, true),
									          BurnDrvGetText(DRV_PARENT));
								}
							}
						...
						else
						{
							...
							{
								_stprintf(szItemText + _tcslen(szItemText),
								          FBNLoadStringEx(hAppInst, IDS_ROMS_FROM_2, true),
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_SAMPLES_FROM, true), bBracket ? _T(", ") : _T(" ("),
				...
				case 1:
					{
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_SEL_PROTOTYPE, true));
							bUseInfo = true;
						}
						if (BurnDrvGetFlags() & BDF_BOOTLEG)
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_SEL_BOOTLEG, true), bUseInfo ? _T(", ") : _T(""));
							bUseInfo = true;
						}
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_SEL_HOMEBREW, true), bUseInfo ? _T(", ") : _T(""));
							bUseInfo = true;
						}
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_BOARD_ROMS_FROM, true),
				...
				case 4:
					{
						...
						{
							_stprintf(szItemText + _tcslen(szItemText),
							          FBNLoadStringEx(hAppInst, IDS_HISCORES_SUPPORTED, true),
...
static void UpdateInfoROMInfo()
{
	...
	if ((BurnDrvGetFlags() & BDF_CLONE) && BurnDrvGetTextA(DRV_PARENT))
	{
		...
		if (nBurnDrvActive < nBurnDrvCount)
		{
			if (BurnDrvGetText(DRV_PARENT))
			{
				_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_ROMS_FROM_1, true),
				          BurnDrvGetText(DRV_PARENT));
			}
		}
		nBurnDrvActive = nOldDrvSelect;
	...
	else
	{
		if (BurnDrvGetTextA(DRV_PARENT))
		{
			_stprintf(szItemText + _tcslen(szItemText), FBNLoadStringEx(hAppInst, IDS_ROMS_FROM_2, true),
			          bBracket ? _T(", ") : _T(" ("), BurnDrvGetText(DRV_PARENT));
			bBracket = true;
		}
	}
	...
	case WM_COMMAND:
		...
		if (HIWORD(wParam) == BN_CLICKED)
		{
			if (LOWORD(wParam) == IDOK)
			{
				...
				{
					MessageBox(hSelDlg, FBNLoadStringEx(hAppInst, IDS_ERR_NO_DRIVER_SEL_SLOT1, true),
					           FBNLoadStringEx(hAppInst, IDS_ERR_ERROR, true), MB_OK);
					break;
				}
				MVSpreviewEndDialog();
