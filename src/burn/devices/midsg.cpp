...
static void soundsgood_porta_w(UINT16, UINT8 data)
{
	...
	INT32 tval = (sg_ram[0x80 / 2] | sg_ram[0x82 / 2] | sg_ram[0x90 / 2] | sg_ram[0xa0 / 2] | sg_ram[0xb0 / 2] | sg_ram[
		0xc2 / 2]) & ml.mask;
...
{
	return static_cast<INT32>(static_cast<float>(nBurnSoundLen * (SekTotalCycles() / (8000000 / (nBurnFPS /
		100.0000)))));
}
