...
// RomData drivers are cloned from a base driver at runtime; see burner/romdata_core.cpp.
bool IsRomDataDrv();
char* RomDataDrvGetDrvName();
struct BurnRomInfo* RomDataDrvGetRomInfo(UINT32* pRomCount);
const char* RomDataDrvGetExtName();
...
struct BurnDriver* BurnGetActiveDriver();
INT32 BurnReserveExtlDrivers(UINT32 nAdditional);
UINT32 LinkExtlDrivers(struct BurnDriver* drv, UINT32* pallCount);
INT32 UnlinkExtlDriver(struct BurnDriver* drv);
extern void (*BurnLibExitHook)(void);	// called first in BurnLibExit(); runtime driver modules install their teardown here
